...
function authenticate_with_creqit(socket, next) {
	...
	socket.creqit_request = (path, args = {}, opts = {}) => {
	...
	socket
		.creqit_request("/api/method/creqit.realtime.get_user_info")
		.then((res) => res.json())
		.then(({ message }) => {
...
function get_site_name(socket) {
	...
	} else if (socket.request.headers["x-creqit-site-name"]) {
		socket.site_name = get_hostname(socket.request.headers["x-creqit-site-name"]);
	} else if (
		conf.default_site &&
...
module.exports = authenticate_with_creqit;
