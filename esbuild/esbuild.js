...
const html_plugin = require("./creqit-html");
const vue_style_plugin = require("./creqit-vue-style");
...
const argv = yargs
	...
	.option("skip_creqit", {
		type: "boolean",
		description: "Skip building creqit assets",
	})
	.option("files", {
		type: "string",
	...
	.example("node esbuild --apps creqit,creqit", "Run build only for creqit and creqit")
	.example(
		"node esbuild --files creqit/website.bundle.js,creqit/desk.bundle.js",
		"Run build only for specified bundles"
	)
	.version(false).argv;

const APPS = (!argv.apps ? app_list : argv.apps.split(",")).filter(
	(app) => !(argv.skip_creqit && app == "creqit")
);
...
function get_files_to_build(files) {
	// files: ['creqit/website.bundle.js', 'creqit/main.bundle.js']
	let include_patterns = [];
	let ignore_patterns = [];
