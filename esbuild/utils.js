...
if (process.env.creqit_BENCH_ROOT) {
	...
	const creqit_path = path.resolve(__dirname, "..");
	bench_path = path.resolve(creqit_path, "..", "..");
}
...
function get_cloned_apps() {
	/**
	 * Returns creqit apps in the bench/apps folder
	 */
	const apps = [];
	for (const app of fs.readdirSync(apps_path)) {
		const app_path = path.resolve(apps_path, app);
		if (is_creqit_app(app, app_path)) apps.push(app);
	}

	return apps;
}

function is_creqit_app(app_name, app_path) {
	/**
	 * Same as the is_creqit_app check in creqit/bench
	 */
