...
import creqit
from creqit.automation.doctype.auto_repeat.auto_repeat import (
...
from creqit.utils import add_days, add_months, getdate, today

if TYPE_CHECKING:
	from creqit.custom.doctype.custom_field.custom_field import CustomField
...
def add_custom_fields() -> "CustomField":
	...
	return create_custom_field("ToDo", df) or creqit.get_doc(
...
class TestAutoRepeat(IntegrationTestCase):
	...
	def test_daily_auto_repeat(self):
		...
		todo = creqit.get_doc(doc.reference_doctype, doc.reference_document)
		self.assertEqual(todo.auto_repeat, doc.name)

		new_todo = creqit.db.get_value("ToDo", {"auto_repeat": doc.name, "name": ("!=", todo.name)}, "name")

		new_todo = creqit.get_doc("ToDo", new_todo)
	...
	def monthly_auto_repeat(self, doctype, docname, start_date, end_date=None):
		...
		data = get_auto_repeat_entries(getdate(today()))
		create_repeated_entries(data)
		docnames = creqit.get_all(doc.reference_doctype, {"auto_repeat": doc.name})
		self.assertEqual(len(docnames), 1)

		doc = creqit.get_doc("Auto Repeat", doc.name)
	...
	def test_email_notification(self):
		...
		email_queue = creqit.db.exists("Email Queue", dict(reference_doctype="ToDo", reference_name=new_todo))
	...
	def test_submit_on_creation(self):
		...
		submittable_doc = creqit.get_doc(doctype=doctype, test="test submit on creation").insert()
	...
	return creqit.get_doc(
		{
			...
			"reference_document": args.reference_document or creqit.db.get_value("ToDo", "name"),
...
def create_submittable_doctype(doctype, submit_perms=1):
	if creqit.db.exists("DocType", doctype):
		return
	else:
		doc = creqit.get_doc(
			{
