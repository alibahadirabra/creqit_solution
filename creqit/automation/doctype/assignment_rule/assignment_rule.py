...
from creqit.cache_manager import clear_doctype_map, get_doctype_map
from creqit.desk.form import assign_to
from creqit.model import log_types
from creqit.model.document import Document
from creqit.utils.data import comma_and
...
class AssignmentRule(Document):
	...
	if TYPE_CHECKING:
		from creqit.automation.doctype.assignment_rule_day.assignment_rule_day import AssignmentRuleDay
		from creqit.automation.doctype.assignment_rule_user.assignment_rule_user import (
			AssignmentRuleUser,
		)
	...
	def validate_document_types(self):
		if self.document_type == "ToDo":
			creqit.throw(_("Assignment Rule is not allowed on {0} document type").format(creqit.bold("ToDo")))
		...
		if user:
			assign_to.add(
				dict(
					...
					name=doc.get("name"),
					description=creqit.render_template(self.description, doc),
	...
	def get_user_based_on_field(self, doc):
		...
		if creqit.db.exists("User", val):
			return val

	def safe_eval(self, fieldname, doc):
		try:
			if self.get(fieldname):
				return creqit.safe_eval(self.get(fieldname), None, doc)
		except Exception as e:
			# when assignment fails, don't block the document as it may be
			# a part of the email pulling
			creqit.msgprint(creqit._("Auto assignment failed: {0}").format(str(e)), indicator="orange")
	...
	def is_rule_not_applicable_today(self):
		today = creqit.flags.assignment_day or creqit.utils.get_weekday()
		assignment_days = self.get_assignment_days()
...
def reopen_closed_assignment(doc):
	...
	for todo in todo_list:
		todo_doc = creqit.get_doc("ToDo", todo)
...
def apply(doc=None, method=None, doctype=None, name=None):
	...
	assignment_rule_docs: list[AssignmentRule] = [
		creqit.get_cached_doc("Assignment Rule", d.get("name")) for d in assignment_rules
	]
			...
			if not new_apply:
				...
				if to_close_todos:
					...
					for todo in todos_to_close:
						_todo = creqit.get_doc("ToDo", todo)
						_todo.status = "Closed"
...
def update_due_date(doc, state=None):
	...
	for rule in assignment_rules:
		rule_doc = creqit.get_cached_doc("Assignment Rule", rule.get("name"))
		...
		if field_updated:
			assignment_todos = creqit.get_all(
				"ToDo",
				filters={
...
def get_assignment_rules() -> list[str]:
	return creqit.get_all("Assignment Rule", filters={"disabled": 0}, pluck="document_type")
