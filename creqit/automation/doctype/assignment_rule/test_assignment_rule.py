...
class TestAutoAssign(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		creqit.db.delete("Assignment Rule")
	...
	def test_load_balancing(self):
		...
		for user in ("test@example.com", "test1@example.com", "test2@example.com"):
			self.assertEqual(
				len(creqit.get_all("ToDo", dict(allocated_to=user, reference_type=TEST_DOCTYPE))), 10
			)
		...
		for d in creqit.get_all(
			...
			creqit.db.delete("ToDo", {"name": d.name})
	...
	def check_assignment_rule_scheduling(self):
		...
		creqit.flags.assignment_day = "Monday"
		note = _make_test_record(public=1)
		...
		creqit.flags.assignment_day = "Friday"
		note = _make_test_record(public=1)
	...
	def test_assignment_rule_condition(self):
		...
		assignment_rule = creqit.get_doc(
		...
		expiry_date = creqit.utils.add_days(creqit.utils.nowdate(), 2)
		note1 = _make_test_record(expiry_date=expiry_date)
		note2 = _make_test_record(expiry_date=expiry_date)

		note1_todo = creqit.get_all(
			"ToDo", filters=dict(reference_type=TEST_DOCTYPE, reference_name=note1.name, status="Open")
		)[0]

		note1_todo_doc = creqit.get_doc("ToDo", note1_todo.name)
		self.assertEqual(creqit.utils.get_date_str(note1_todo_doc.date), expiry_date)

		# due date should be updated if the reference doc's date is updated.
		note1.expiry_date = creqit.utils.add_days(expiry_date, 2)
		note1.save()
		note1_todo_doc.reload()
		self.assertEqual(creqit.utils.get_date_str(note1_todo_doc.date), note1.expiry_date)

		# saving one note's expiry should not update other note todo's due date
		note2_todo = creqit.get_all(
			"ToDo",
			filters=dict(reference_type=TEST_DOCTYPE, reference_name=note2.name, status="Open"),
			fields=["name", "date"],
		)[0]
		self.assertNotEqual(creqit.utils.get_date_str(note2_todo.date), note1.expiry_date)
		self.assertEqual(creqit.utils.get_date_str(note2_todo.date), expiry_date)
		assignment_rule.delete()
		creqit.db.commit()  # undo changes commited by DDL
	...
	def test_submittable_assignment(self):
		...
		dt = creqit.get_doc("DocType", submittable_doctype)
		dt.is_submittable = 1
		dt.save()

		# create a rule for the submittable doctype
		assignment_rule = creqit.new_doc("Assignment Rule")
		...
		doc = creqit.new_doc(submittable_doctype)
		doc.save()
		doc.submit()
...
def clear_assignments():
	creqit.db.delete("ToDo", {"reference_type": TEST_DOCTYPE})


def get_assignment_rule(days, assign=None):
	creqit.delete_doc_if_exists("Assignment Rule", f"For {TEST_DOCTYPE} 1")
	...
	creqit.delete_doc_if_exists("Assignment Rule", f"For {TEST_DOCTYPE} 2")
...
def _make_test_record(
	...
	**kwargs,
	...
	doc = creqit.new_doc(TEST_DOCTYPE)
