...
from creqit.build import scrub_html_template
from creqit.model.meta import Meta
from creqit.model.utils import render_include
from creqit.modules import get_module_path, load_doctype_module, scrub
from creqit.utils import get_bench_path, get_html_format
...
def get_meta(doctype, cached=True) -> "FormMeta":
	...
	cached = cached and not creqit.conf.developer_mode
	if cached:
		meta = creqit.cache.hget("doctype_form_meta", doctype)
		if not meta:
			# Cache miss - explicitly get meta from DB to avoid
			meta = FormMeta(doctype, cached=False)
			creqit.cache.hset("doctype_form_meta", doctype, meta)
	else:
...
class FormMeta(Meta):
	def __init__(self, doctype, *, cached=True):
		self.__dict__.update(creqit.get_meta(doctype, cached=cached).__dict__)
		self.load_assets()
	...
	def add_code(self):
		...
		system_country = creqit.get_system_settings("country")
...
"""
	...
	def add_search_fields(self):
		...
		for df in self.get("fields", {"fieldtype": "Link", "options": ["!=", "[Select]"]}):
			if df.options:
				try:
					search_fields = creqit.get_meta(df.options).search_fields
	...
	def _show_missing_doctype_msg(self, df):
		...
		msg = _("Field {0} is referring to non-existing doctype {1}.").format(
			creqit.bold(df.fieldname), creqit.bold(df.options)
		)
		...
		creqit.throw(msg, title=_("Missing DocType"))

	def add_linked_document_type(self):
		for df in self.get("fields", {"fieldtype": "Link"}):
			if df.options:
				try:
					df.linked_document_type = creqit.get_meta(df.options).document_type
				except creqit.DoesNotExistError:
					self._show_missing_doctype_msg(df)

	def load_print_formats(self):
		print_formats = creqit.db.sql(
	...
	def load_workflows(self):
		...
		if workflow_name and creqit.db.exists("Workflow", workflow_name):
			workflow = creqit.get_doc("Workflow", workflow_name)
			workflow_docs.append(workflow)

			workflow_docs.extend(creqit.get_doc("Workflow State", d.state) for d in workflow.get("states"))
		...
		if not self.custom:
			...
			if hasattr(module, "form_grid_templates"):
				for key, path in module.form_grid_templates.items():
					templates[key] = get_html_format(creqit.get_app_path(app, path))
	...
	def load_workspaces(self):
		Shortcut = creqit.qb.DocType("Workspace Shortcut")
		Workspace = creqit.qb.DocType("Workspace")
		shortcut = (
			creqit.qb.from_(Shortcut)
		...
		if shortcut:
			...
			Link = creqit.qb.DocType("Workspace Link")
			link = (
				creqit.qb.from_(Link)
				.select(Link.parent)
				.inner_join(Workspace)
...
def get_code_files_via_hooks(hook, name):
	...
	for app_name in creqit.get_installed_apps():
		code_hook = creqit.get_hooks(hook, default={}, app_name=app_name)
		if not code_hook:
			continue
		...
		for file in files:
			path = creqit.get_app_path(app_name, *file.strip("/").split("/"))
