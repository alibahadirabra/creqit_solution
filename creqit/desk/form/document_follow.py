...
from creqit.utils import get_url_to_form


@creqit.whitelist()
def update_follow(doctype: str, doc_name: str, following: bool):
	if following:
		return follow_document(doctype, doc_name, creqit.session.user) and True or False
	else:
		return unfollow_document(doctype, doc_name, creqit.session.user)
...
def follow_document(doctype, doc_name, user):
	...
	if not creqit.get_meta(doctype).track_changes:
		creqit.toast(_("Can't follow since changes are not tracked."))
		return False

	if user == "Administrator":
		creqit.toast(_("Administrator can't follow"))
		return False

	if not creqit.db.get_value("User", user, "document_follow_notify", ignore=True, cache=True):
		creqit.toast(_("Document follow is not enabled for this user."))
		return False

	if not is_document_followed(doctype, doc_name, user):
		doc = creqit.new_doc("Document Follow")
		doc.update({"ref_doctype": doctype, "ref_docname": doc_name, "user": user})
		doc.save()
		creqit.toast(_("Following document {0}").format(doc_name))
		return doc
...
def unfollow_document(doctype, doc_name, user):
	...
	if doc:
		creqit.delete_doc("Document Follow", doc[0].name)
		creqit.toast(_("Un-following document {0}").format(doc_name))
		return False
	return False
...
def get_user_list(frequency):
	...
	User = DocType("User")
	return (
		creqit.qb.from_(DocumentFollow)
		.join(User)
...
def get_version(doctype, doc_name, frequency, user):
	...
	if version:
		for v in version:
			change = creqit.parse_json(v.data)
			time = creqit.utils.format_datetime(v.modified, "hh:mm a")
			timeline_items = []
			if change.changed:
...
def get_comments(doctype, doc_name, frequency, user):
	...
	for comment in comments:
		...
		time = creqit.utils.format_datetime(comment.modified, "hh:mm a")
		timeline.append(
			{
...
def _get_filters(frequency, user):
	...
	if frequency == "Weekly":
		filters += [
			["modified", ">", creqit.utils.add_days(creqit.utils.nowdate(), -7)],
			["modified", "<", creqit.utils.nowdate()],
		]

	elif frequency == "Daily":
		filters += [
			["modified", ">", creqit.utils.add_days(creqit.utils.nowdate(), -1)],
			["modified", "<", creqit.utils.nowdate()],
		]

	elif frequency == "Hourly":
		filters += [
			["modified", ">", creqit.utils.add_to_date(creqit.utils.now_datetime(), hours=-1)],
			["modified", "<", creqit.utils.now_datetime()],
		]
