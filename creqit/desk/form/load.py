...
from creqit.desk.form.document_follow import is_document_followed
from creqit.model.utils.user_settings import get_user_settings
from creqit.permissions import get_doc_permissions, has_permission
...
def getdoc(doctype, name, user=None):
	...
	if not doc.has_permission("read"):
		creqit.flags.error_message = _("Insufficient Permission for {0}").format(
			creqit.bold(_(doctype) + " " + name)
		)
		raise creqit.PermissionError(("read", doctype, name))
	...
	doc.add_seen()
	set_link_titles(doc)
	if creqit.response.docs is None:
		creqit.local.response = _dict({"docs": []})
...
def getdoctype(doctype, with_parent=False, cached_timestamp=None):
	...
	if with_parent and (parent_dt := creqit.model.meta.get_parent_dt(doctype)):
		docs = get_meta_bundle(parent_dt)
		creqit.response["parent_dt"] = parent_dt

	if not docs:
		docs = get_meta_bundle(doctype)

	creqit.response["user_settings"] = get_user_settings(parent_dt or doctype)

	if cached_timestamp and docs[0].modified == cached_timestamp:
		return "use_cache"

	creqit.response.docs.extend(docs)


def get_meta_bundle(doctype):
	bundle = [creqit.desk.form.meta.get_meta(doctype)]
	bundle.extend(
		creqit.desk.form.meta.get_meta(df.options)
		for df in bundle[0].fields
		if df.fieldtype in creqit.model.table_fields
	)
	return bundle
...
def get_docinfo(doc=None, doctype=None, name=None):
	from creqit.share import _get_users as get_docshares
	...
	docinfo = creqit._dict(user_info={})
	...
	docinfo.update(
		{
			...
			"is_document_followed": is_document_followed(doc.doctype, doc.name, creqit.session.user),
	...
	creqit.response["docinfo"] = docinfo
...
def add_comments(doc, docinfo):
	...
	for c in comments:
		match c.comment_type:
			case "Comment":
				c.content = creqit.utils.markdown(c.content)
...
def get_error_log_exists(doc: "Document") -> bool:
	if has_permission("Error Log", print_logs=False):
		return creqit.db.exists("Error Log", {"reference_doctype": doc.doctype, "reference_name": doc.name})
	return False


def get_webhook_request_log_exists(doc: "Document") -> bool:
	if has_permission("Webhook Request Log", print_logs=False):
		return creqit.db.exists(
...
def get_comments(doctype: str, name: str, comment_type: str | list[str] = "Comment") -> list[creqit._dict]:
...
def get_additional_timeline_content(doctype, docname):
	contents = []
	hooks = creqit.get_hooks().get("additional_timeline_content", {})
	...
	for method in methods_for_all_doctype + methods_for_current_doctype:
		contents.extend(creqit.get_attr(method)(doctype, docname) or [])
...
def get_title_values_for_link_and_dynamic_link_fields(doc, link_fields=None):
	...
	for field in link_fields:
		...
		link_title = creqit.db.get_value(doctype, doc_fieldvalue, meta.title_field, cache=True, order_by=None)
...
def send_link_titles(link_titles):
	"""Append link titles dict in `creqit.local.response`."""
	if "_link_titles" not in creqit.local.response:
		creqit.local.response["_link_titles"] = {}

	creqit.local.response["_link_titles"].update(link_titles)
...
def update_user_info(docinfo):
	...
	creqit.utils.add_user_info(users, docinfo.user_info)
...
def get_user_info_for_viewers(users):
	...
	for user in json.loads(users):
		creqit.utils.add_user_info(user, user_info)
