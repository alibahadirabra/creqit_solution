import BaseTimeline from "../../../public/js/creqit/form/footer/base_timeline";
creqit.provide("creqit.energy_points");
...
class UserProfile {
	...
	show() {
		...
		this.user_id = route[1] || creqit.session.user;
		creqit.dom.freeze(__("Loading user profile") + "...");
		creqit.db.exists("User", this.user_id).then((exists) => {
			creqit.dom.unfreeze();
			if (exists) {
				this.make_user_profile();
			} else {
				creqit.msgprint(__("User does not exist"));
			}
		});
	}

	make_user_profile() {
		this.user = creqit.user_info(this.user_id);
		this.page.set_title(this.user.fullname);
		this.setup_user_search();
		this.main_section.empty().append(creqit.render_template("user_profile"));
	...
	show_user_search_dialog() {
		let dialog = new creqit.ui.Dialog({
			...
			primary_action: ({ user }) => {
				dialog.hide();
				creqit.set_route("user-profile", user);
			},
		});
		dialog.show();
	}
	...
	render_heatmap() {
		this.heatmap = new creqit.Chart(".performance-heatmap", {
			type: "heatmap",
	...
	update_heatmap_data(date_from) {
		creqit
			.xcall("creqit.desk.page.user_profile.user_profile.get_energy_points_heatmap_data", {
				user: this.user_id,
				date: date_from || creqit.datetime.year_start(),
			})
			.then((r) => {
	...
	render_line_chart() {
		...
		this.line_chart = new creqit.Chart(".performance-line-chart", {
			type: "line",
			height: 200,
			data: {
				labels: [],
	...
	update_line_chart_data() {
		...
		creqit
			.xcall("creqit.desk.doctype.dashboard_chart.dashboard_chart.get", {
	...
	render_percentage_chart(field, title) {
		creqit
			.xcall(
				"creqit.desk.page.user_profile.user_profile.get_energy_points_percentage_chart_data",
				{
					user: this.user_id,
					field: field,
				}
			...
			.then((chart) => {
				if (chart.labels.length) {
					this.percentage_chart = new creqit.Chart(".performance-percentage-chart", {
						type: "percentage",
						data: {
							labels: chart.labels,
	...
	create_line_chart_filters() {
		...
		creqit.dashboard_utils.render_chart_filters(
			filters,
			"chart-filter",
			".line-chart-options",
			1
	...
	create_heatmap_chart_filters() {
		let filters = [
			{
				label: creqit.dashboard_utils.get_year(creqit.datetime.now_date()),
				options: creqit.dashboard_utils.get_years_since_creation(
					creqit.boot.user.creation
				),
				action: (selected_item) => {
					this.update_heatmap_data(creqit.datetime.obj_to_str(selected_item));
				},
			},
		];
		creqit.dashboard_utils.render_chart_filters(filters, "chart-filter", ".heatmap-options");
	}

	edit_profile() {
		let edit_profile_dialog = new creqit.ui.Dialog({
			...
			primary_action: (values) => {
				...
				creqit
					.xcall("creqit.desk.page.user_profile.user_profile.update_profile_info", {
						profile_info: values,
					})
					.then((user) => {
	...
	render_user_details() {
		this.sidebar.empty().append(
			creqit.render_template("user_profile_sidebar", {
				user_image: this.user.image,
	...
	setup_user_profile_links() {
		if (this.user_id !== creqit.session.user) {
	...
	get_user_rank() {
		return creqit
			.xcall("creqit.desk.page.user_profile.user_profile.get_user_rank", {
				user: this.user_id,
			})
			.then((r) => {
	...
	get_user_points() {
		return creqit
			.xcall(
				"creqit.social.doctype.energy_point_log.energy_point_log.get_user_energy_and_review_points",
				{
					user: this.user_id,
				}
			)
	...
	render_points_and_rank() {
		...
		const _get_stat_dom = (value, label, icon) => {
			return `<div class="user-stats-item mt-4">
				${creqit.utils.icon(icon, "lg", "no-stroke")}
				<div>
	...
	go_to_user_settings() {
		creqit.set_route("Form", "User", this.user_id);
	}
...
class UserProfileTimeline extends BaseTimeline {
	...
	get_user_activity_data() {
		return creqit.xcall("creqit.desk.page.user_profile.user_profile.get_energy_points_list", {
	...
	get_activity_timeline_item(data) {
		...
		return {
			...
			is_card: true,
			content: creqit.energy_points.format_history_log(data),
		};
	}
...
creqit.ui.UserProfile = UserProfile;
