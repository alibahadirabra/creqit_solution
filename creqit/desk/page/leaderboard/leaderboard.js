creqit.pages["leaderboard"].on_page_load = (wrapper) => {
	creqit.leaderboard = new Leaderboard(wrapper);

	$(wrapper).bind("show", () => {
		// Get which leaderboard to show
		let doctype = creqit.get_route()[1];
		creqit.leaderboard.show_leaderboard(doctype);
	});
};
...
class Leaderboard {
	...
	get_leaderboard_config() {
		...
		creqit
			.xcall("creqit.desk.page.leaderboard.leaderboard.get_leaderboard_config")
			.then((config) => {
				...
				const _initial_doctype = creqit.get_route()[1] || this.doctypes[0];
	...
	setup_leaderboard_fields() {
		this.company_select = this.page.add_field({
			...
			fieldtype: "Link",
			options: "Company",
			default: creqit.defaults.get_default("company"),
			reqd: 1,
			change: (e) => {
				this.make_request();
			},
		...
		this.type_select = this.page.add_select(
			...
			this.options.selected_filter.map((d) => {
				return { label: __(creqit.model.unscrub(d)), value: d };
			})
		);
	...
	create_date_range_field() {
		let timespan_field = $(this.parent).find(
			`.creqit-control[data-original-title="${__("Timespan")}"]`
		);
		this.date_range_field = $(`<div class="from-date-field"></div>`)
			.insertAfter(timespan_field)
			.hide();

		let date_field = creqit.ui.form.make_control({
			df: {
				fieldtype: "DateRange",
				fieldname: "selected_date_range",
				placeholder: __("Date Range"),
				default: [creqit.datetime.month_start(), creqit.datetime.now_date()],
	...
	render_selected_doctype() {
		this.$sidebar_list.on("click", "li", (e) => {
			...
			this.company_select.set_value(
				creqit.defaults.get_default("company") || this.company_select.get_value()
			);
			...
			creqit.set_route("leaderboard", this.options.selected_doctype);
			this.make_request();
		});
	}
	...
	make_request() {
		creqit.model.with_doctype(this.options.selected_doctype, () => {
	...
	get_leaderboard(notify) {
		...
		if (!company && !this.leaderboard_config[this.options.selected_doctype].company_disabled) {
			notify(this, null);
			creqit.show_alert(__("Please select Company"));
			return;
		}
		creqit
			...
			.then((r) => {
				...
				creqit.utils.make_chart(".leaderboard-graph", custom_options);
	...
	get_leaderboard_data(me, res) {
		if (res && res.message.length) {
			...
			creqit.utils.setup_search($(me.parent), ".list-item-container", ".list-id");
		} else {
			me.$graph_area.hide();
	...
	render_list_header() {
		const _selected_filter = this.options.selected_filter.map((i) => creqit.model.unscrub(i));
		const fields = ["rank", "name", this.options.selected_filter_item];
		const filters = fields
			.map((filter) => {
				const col = __(creqit.model.unscrub(filter));
	...
	render_message() {
		...
		return `<div class="leaderboard-empty-state ${display_class}">
  			<div class="no-result text-center">
  				<img src="/assets/creqit/images/ui-states/search-empty-state.svg"
  					alt="Empty State"
  					class="null-state"
  				>
	...
	get_item_html(item, index) {
		...
		const link = `/app/${creqit.router.slug(this.options.selected_doctype)}/${item.name}`;
	...
	get_sidebar_item(item, icon) {
		let icon_html = icon ? creqit.utils.icon(icon, "md") : "";
	...
	get_date_range() {
		...
		let current_date = creqit.datetime.now_date();
		let date_range_map = {
			"this week": [creqit.datetime.week_start(), creqit.datetime.week_end()],
			"this month": [creqit.datetime.month_start(), creqit.datetime.month_end()],
			"this quarter": [creqit.datetime.quarter_start(), creqit.datetime.quarter_end()],
			"this year": [creqit.datetime.year_start(), creqit.datetime.year_end()],
			"last week": [creqit.datetime.add_days(current_date, -7), current_date],
			"last month": [creqit.datetime.add_months(current_date, -1), current_date],
			"last quarter": [creqit.datetime.add_months(current_date, -3), current_date],
			"last year": [creqit.datetime.add_months(current_date, -12), current_date],
			"all time": null,
			"select date range": this.selected_date_range || [
				creqit.datetime.month_start(),
				current_date,
			],
		};
