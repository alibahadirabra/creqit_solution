...
class NotificationLog(Document):
	...
	def after_insert(self):
		creqit.publish_realtime("notification", after_commit=True, user=self.for_user)
	...
	def clear_old_logs(days=180):
		...
		table = creqit.qb.DocType("Notification Log")
		creqit.db.delete(table, filters=(table.creation < (Now() - Interval(days=days))))


def get_permission_query_conditions(for_user):
	if not for_user:
		for_user = creqit.session.user

	if for_user == "Administrator":
		return

	return f"""(`tabNotification Log`.for_user = {creqit.db.escape(for_user)})"""


def get_title(doctype, docname, title_field=None):
	if not title_field:
		title_field = creqit.get_meta(doctype).get_title_field()
	return docname if title_field == "name" else creqit.db.get_value(doctype, docname, title_field)
...
def enqueue_create_notification(users: list[str] | str, doc: dict):
	...
	creqit.enqueue(
		"creqit.desk.doctype.notification_log.notification_log.make_notification_logs",
		doc=doc,
		users=users,
		now=creqit.flags.in_test,
		enqueue_after_commit=not creqit.flags.in_test,
	)
...
def _get_user_ids(user_emails):
	user_names = creqit.db.get_values(
...
def send_notification_email(doc: NotificationLog):
	...
	from creqit.utils import get_url_to_form, strip_html

	user = creqit.db.get_value("User", doc.for_user, fieldname=["email", "language"], as_dict=True)
	if not user:
		return
...
def get_notification_logs(limit=20):
	notification_logs = creqit.db.get_list(
...
def mark_all_as_read():
	unread_docs_list = creqit.get_all(
		"Notification Log", filters={"read": 0, "for_user": creqit.session.user}
	)
	unread_docnames = [doc.name for doc in unread_docs_list]
	if unread_docnames:
		filters = {"name": ["in", unread_docnames]}
		creqit.db.set_value("Notification Log", filters, "read", 1, update_modified=False)
...
def mark_as_read(docname: str):
	...
	if docname:
		creqit.db.set_value("Notification Log", str(docname), "read", 1, update_modified=False)


@creqit.whitelist()
def trigger_indicator_hide():
	creqit.publish_realtime("indicator_hide", user=creqit.session.user)


def set_notifications_as_unseen(user):
	try:
		creqit.db.set_value("Notification Settings", user, "seen", 0, update_modified=False)
