...
def after_doctype_insert():
	creqit.db.add_unique("Desktop Icon", ("module_name", "owner", "standard"))
...
def get_desktop_icons(user=None):
	...
	user_icons = creqit.cache.hget("desktop_icons", user)
	...
	if not user_icons:
		...
		blocked_doctypes = creqit.get_all(
			"DocType",
		...
		standard_icons = creqit.get_all("Desktop Icon", fields=fields, filters={"standard": 1})
		...
		user_icons = creqit.get_all("Desktop Icon", fields=fields, filters={"standard": 0, "owner": user})
		...
		user_blocked_modules = creqit.get_doc("User", user).get_blocked_modules()
		for icon in user_icons:
		...
		creqit.cache.hset("desktop_icons", user, user_icons)
...
def add_user_icon(_doctype, _report=None, label=None, link=None, type="link", standard=0):
	...
	icon_name = creqit.db.exists(
		"Desktop Icon", {"standard": standard, "link": link, "owner": creqit.session.user}
	)

	if icon_name:
		if creqit.db.get_value("Desktop Icon", icon_name, "hidden"):
			# if it is hidden, unhide it
			creqit.db.set_value("Desktop Icon", icon_name, "hidden", 0)
			clear_desktop_icons_cache()

	else:
		idx = (
			creqit.db.sql("select max(idx) from `tabDesktop Icon` where owner=%s", creqit.session.user)[0][0]
			or creqit.db.sql("select count(*) from `tabDesktop Icon` where standard=1")[0][0]
		)

		if not creqit.db.get_value("Report", _report):
			_report = None
			userdefined_icon = creqit.db.get_value(
		...
		try:
			...
			creqit.throw(_("Desktop Icon already exists"))
	...
	for i, module_name in enumerate(new_order):
		if module_name not in ("Explore",):
			if user:
				...
				name = creqit.db.get_value("Desktop Icon", {"standard": 1, "module_name": module_name})
				if name:
					icon = creqit.get_doc("Desktop Icon", name)
				else:
...
def set_desktop_icons(visible_list, ignore_duplicate=True):
	...
	if not creqit.defaults.get_defaults().get("setup_complete", 0):
		creqit.db.delete("Desktop Icon", {"standard": 0})

	# set standard as blocked and hidden if setting first active domain
	if not creqit.flags.keep_desktop_icons:
		creqit.db.sql("update `tabDesktop Icon` set blocked=0, hidden=1 where standard=1")

	# set as visible if present, or add icon
	for module_name in list(visible_list):
		name = creqit.db.get_value("Desktop Icon", {"module_name": module_name})
		if name:
			creqit.db.set_value("Desktop Icon", name, "hidden", 0)
		else:
			if creqit.db.exists("DocType", module_name):
				try:
					add_user_icon(module_name, standard=1)
				except creqit.UniqueValidationError as e:
...
def set_hidden(module_name, user=None, hidden=1):
	...
	if user:
		...
		if hidden and icon.custom:
			creqit.delete_doc(icon.doctype, icon.name, ignore_permissions=True)
			return

		# hidden by user
		icon.db_set("hidden", hidden)
	else:
		icon = creqit.get_doc("Desktop Icon", {"standard": 1, "module_name": module_name})
...
def get_all_icons():
	return [
		d.module_name for d in creqit.get_all("Desktop Icon", filters={"standard": 1}, fields=["module_name"])
	]


def clear_desktop_icons_cache(user=None):
	creqit.cache.hdel("desktop_icons", user or creqit.session.user)
	creqit.cache.hdel("bootinfo", user or creqit.session.user)
...
def get_user_copy(module_name, user=None):
	...
	desktop_icon_name = creqit.db.get_value(
		"Desktop Icon", {"module_name": module_name, "owner": user, "standard": 0}
	)

	if desktop_icon_name:
		return creqit.get_doc("Desktop Icon", desktop_icon_name)
	else:
...
def make_user_copy(module_name, user):
	...
	standard_name = creqit.db.get_value("Desktop Icon", {"module_name": module_name, "standard": 1})

	if not standard_name:
		creqit.throw(_("{0} not found").format(module_name), creqit.DoesNotExistError)

	original = creqit.get_doc("Desktop Icon", standard_name)

	desktop_icon = creqit.get_doc(
...
def sync_from_app(app):
	...
	try:
		modules = creqit.get_attr(app + ".config.desktop.get_data")() or {}
	except ImportError:
		return []
	...
	for i, m in enumerate(modules_list):
		...
		if desktop_icon_name:
			desktop_icon = creqit.get_doc("Desktop Icon", desktop_icon_name)
		else:
			# new icon
			desktop_icon = creqit.get_doc(
		...
		try:
			desktop_icon.save()
		except creqit.exceptions.UniqueValidationError:
			pass
...
def update_icons(hidden_list, user=None):
	...
	creqit.msgprint(creqit._("Updated"), indicator="green", title=_("Success"), alert=True)


def get_context(context):
	context.icons = get_user_icons(creqit.session.user)
	context.user = creqit.session.user

	if "System Manager" in creqit.get_roles():
		context.users = creqit.get_all(
			"User",
...
def get_module_icons(user=None):
	...
	if not user:
		icons = creqit.get_all("Desktop Icon", fields="*", filters={"standard": 1}, order_by="idx")
	else:
		creqit.cache.hdel("desktop_icons", user)
		icons = get_user_icons(user)

	for icon in icons:
		icon.value = creqit.db.escape(_(icon.label or icon.module_name))
...
def get_user_icons(user):
	...
	from creqit.boot import get_allowed_pages
