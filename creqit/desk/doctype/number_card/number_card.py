...
from creqit.query_builder import Criterion
from creqit.query_builder.utils import DocType
from creqit.utils import cint, flt
...
class NumberCard(Document):
	...
	def autoname(self):
		...
		if creqit.db.exists("Number Card", self.name):
	...
	def validate(self):
		if self.type == "Document Type":
			if not (self.document_type and self.function):
				creqit.throw(_("Document Type and Function are required to create a number card"))

			if self.function != "Count" and not self.aggregate_function_based_on:
				creqit.throw(_("Aggregate Field is required to create a number card"))

			if creqit.get_meta(self.document_type).istable and not self.parent_document_type:
				creqit.throw(_("Parent Document Type is required to create a number card"))

		elif self.type == "Report":
			if not (self.report_name and self.report_field and self.function):
				creqit.throw(_("Report Name, Report Field and Fucntion are required to create a number card"))

		elif self.type == "Custom":
			if not self.method:
				creqit.throw(_("Method is required to create a number card"))
...
def has_permission(doc, ptype, user):
	...
	if doc.type == "Report":
		...
		allowed_doctypes = tuple(creqit.permissions.get_doctypes_with_read())
...
def get_result(doc, filters, to_date=None):
	...
	if not filters:
		...
		filters = creqit.parse_json(filters)
...
def get_percentage_difference(doc, filters, result):
	doc = creqit.parse_json(doc)
	result = creqit.parse_json(result)

	doc = creqit.get_doc("Number Card", doc.name)
...
def calculate_previous_result(doc, filters):
	from creqit.utils import add_to_date

	current_date = creqit.utils.now()
...
def create_number_card(args):
	...
	doc = creqit.new_doc("Number Card")
...
def get_cards_for_user(doctype, txt, searchfield, start, page_len, filters):
	...
	condition_query = creqit.qb.get_query(
		doctype,
		filters=filters,
	...
	return (
		...
		.where((numberCard.owner == creqit.session.user) | (numberCard.is_public == 1))
...
def create_report_number_card(args):
	...
	args.name = card.name
	if args.dashboard:
		add_card_to_dashboard(creqit.as_json(args))
...
def add_card_to_dashboard(args):
	...
	dashboard_link = creqit.new_doc("Number Card Link")
	dashboard_link.card = args.name

	if args.set_standard and dashboard.is_standard:
		card = creqit.get_doc("Number Card", dashboard_link.card)
		card.is_standard = 1
