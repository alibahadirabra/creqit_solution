...
creqit.ui.form.on("System Health Report", {
	onload(frm) {
		let poll_attempts = 0;
		const interval = setInterval(() => {
			creqit
				.xcall(
					"creqit.desk.doctype.system_health_report.system_health_report.get_job_status",
	...
	refresh(frm) {
		...
		creqit.realtime.on("pong", () => {
			frm.set_value("socketio_ping_check", "Pass");
			frm.set_value(
				"socketio_transport_mode",
				creqit.realtime.socket.io?.engine?.transport?.name
			);
		});
		creqit.realtime.emit("ping");
		frm.disable_save();
	...
	setup_highlight(frm) {
		...
		const update_fields = () => {
			if (!creqit.get_route().includes(frm.doc.name)) {
				clearInterval(interval);
			}
