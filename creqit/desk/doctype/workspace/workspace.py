...
from creqit.desk.desktop import get_workspace_sidebar_items, save_new_widget
from creqit.desk.utils import validate_route_conflict
from creqit.model.document import Document
from creqit.model.rename_doc import rename_doc
from creqit.modules.export_file import delete_folder, export_to_files
...
class Workspace(Document):
	...
	if TYPE_CHECKING:
		...
		from creqit.desk.doctype.workspace_chart.workspace_chart import WorkspaceChart
		from creqit.desk.doctype.workspace_custom_block.workspace_custom_block import WorkspaceCustomBlock
		from creqit.desk.doctype.workspace_link.workspace_link import WorkspaceLink
		from creqit.desk.doctype.workspace_number_card.workspace_number_card import WorkspaceNumberCard
		from creqit.desk.doctype.workspace_quick_list.workspace_quick_list import WorkspaceQuickList
		from creqit.desk.doctype.workspace_shortcut.workspace_shortcut import WorkspaceShortcut
	...
	def validate(self):
		...
		if self.public and not is_workspace_manager() and not disable_saving_as_public():
			creqit.throw(_("You need to be Workspace Manager to edit this document"))
		...
		try:
			...
			creqit.throw(_("Content data shoud be a list"))

		for d in self.get("links"):
			if d.link_type == "Report" and d.is_query_report != 1:
				d.report_ref_doctype = creqit.get_value("Report", d.link_to, "ref_doctype")
	...
	def clear_cache(self):
		super().clear_cache()
		if self.for_user:
			creqit.cache.hdel("bootinfo", self.for_user)
		else:
	...
	def on_update(self):
		...
		if creqit.conf.developer_mode and self.public:
			if self.module:
		...
		for link in self.links:
			...
			if link.type == "Card Break":
				if card_links and (
					...
					or current_card.get("only_for") == creqit.get_system_settings("country")
				):
				...
				current_card = link
				card_links = []
			elif not link.get("only_for") or link.get("only_for") == creqit.get_system_settings("country"):
...
def get_report_type(report):
	report_type = creqit.get_value("Report", report, "report_type")
...
def new_page(new_page):
	...
	if page.get("public") and not is_workspace_manager():
		...
		not page.get("public") and page.get("for_user") != creqit.session.user and not is_workspace_manager()
	):
		creqit.throw(_("Cannot create private workspace of other users"), creqit.PermissionError)

	elif not creqit.has_permission(doctype="Workspace", ptype="create"):
		creqit.flags.error_message = _("User {0} does not have the permission to create a Workspace.").format(
			creqit.bold(creqit.session.user)
		)
		raise creqit.PermissionError

	doc = creqit.new_doc("Workspace")
...
def save_page(name, public, new_widgets, blocks):
	public = creqit.parse_json(public)

	doc = creqit.get_doc("Workspace", name)
	doc.content = blocks
...
def update_page(name, title, icon, indicator_color, parent, public):
	...
	if not doc.get("public") and doc.get("for_user") != creqit.session.user and not is_workspace_manager():
		creqit.throw(
	...
	if doc:
		child_docs = creqit.get_all("Workspace", filters={"parent_page": doc.title, "public": doc.public})
		doc.title = title
		doc.icon = icon
		doc.indicator_color = indicator_color
		doc.parent_page = parent
		if doc.public != public:
			doc.sequence_id = creqit.db.count("Workspace", {"public": public}, cache=True)
			doc.public = public
		doc.for_user = "" if public else doc.for_user or creqit.session.user
		...
		if child_docs:
			for child in child_docs:
				child_doc = creqit.get_doc("Workspace", child.name)
				child_doc.parent_page = doc.title
				if child_doc.public != public:
					child_doc.public = public
				child_doc.for_user = "" if public else child_doc.for_user or creqit.session.user
...
def last_sequence_id(doc):
	doc_exists = creqit.db.exists({"doctype": "Workspace", "public": doc.public, "for_user": doc.for_user})
...
def get_page_list(fields, filters):
	return creqit.get_all("Workspace", fields=fields, filters=filters, order_by="sequence_id asc")


def is_workspace_manager():
	return "Workspace Manager" in creqit.get_roles()
