...
creqit.ui.form.on("Workspace", {
	setup: function () {
		creqit.meta.get_field("Workspace Link", "only_for").no_default = true;
	},
	...
	refresh: function (frm) {
		...
		let url = `/app/${
			frm.doc.public
				? creqit.router.slug(frm.doc.title)
				: "private/" + creqit.router.slug(frm.doc.title)
		}`;
		frm.sidebar
		...
		if (
			(frm.doc.for_user && frm.doc.for_user !== creqit.session.user) ||
			(frm.doc.public && !creqit.user.has_role("Workspace Manager"))
		) {
