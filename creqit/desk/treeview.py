...
def get_all_nodes(doctype, label, parent, tree_method, **filters):
	...
	tree_method = creqit.get_attr(tree_method)

	creqit.is_whitelisted(tree_method)
...
def get_children(doctype, parent="", include_disabled=False, **filters):
	if isinstance(include_disabled, str):
		include_disabled = creqit.sbool(include_disabled)
...
def _get_children(doctype, parent="", ignore_permissions=False, include_disabled=False):
	parent_field = "parent_" + creqit.scrub(doctype)
	filters = [[f"ifnull(`{parent_field}`,'')", "=", parent], ["docstatus", "<", 2]]
	if creqit.db.has_column(doctype, "disabled") and not include_disabled:
...
@creqit.whitelist()
def add_node():
	args = make_tree_args(**creqit.form_dict)
	doc = creqit.get_doc(args)
