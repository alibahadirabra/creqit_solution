...
def get_notifications():
	...
	if creqit.flags.in_install or not creqit.get_system_settings("setup_complete"):
		return out
	...
	for name in groups:
		count = creqit.cache.hget("notification_count:" + name, creqit.session.user)
...
def get_notifications_for_doctypes(config, notification_count):
	...
	can_read = creqit.get_user().get_can_read()
	open_count_doctype = {}
			...
			if d in notification_count:
				...
				try:
					if isinstance(condition, dict):
						result = creqit.get_list(
							d, fields=["count(*) as count"], filters=condition, ignore_ifnull=True
						)[0].count
					else:
						result = creqit.get_attr(condition)()

				except creqit.PermissionError:
					creqit.clear_messages()
					pass
				# creqit.msgprint("Permission Error in notifications for {0}".format(d))
					...
					creqit.cache.hset("notification_count:" + d, creqit.session.user, result)
...
def clear_notifications(user=None):
	...
	if user:
		creqit.cache.hdel_names([f"notification_count:{name}" for name in groups], user)
	else:
		creqit.cache.delete_value([f"notification_count:{name}" for name in groups])


def clear_notification_config(user):
	creqit.cache.hdel("notification_config", user)


def delete_notification_count_for(doctype):
	creqit.cache.delete_key("notification_count:" + doctype)
...
def get_notification_info():
	...
	conditions = {}
	module_doctypes = {}
	doctype_info = dict(creqit.db.sql("""select name, module from tabDocType"""))
...
def get_notification_config():
	user = creqit.session.user or "Guest"
	...
	return creqit.cache.hget("notification_config", user, _get)
...
def get_open_count(doctype: str, name: str, items=None):
	...
	if creqit.flags.in_migrate or creqit.flags.in_install:
		return {"count": []}
	...
	if not meta.custom:
		module = creqit.get_meta_module(doctype)
...
def notify_mentions(ref_doctype, ref_name, content):
	if ref_doctype and ref_name and content:
		...
		sender_fullname = get_fullname(creqit.session.user)
		...
		notification_message = _("""{0} mentioned you in a comment in {1} {2}""").format(
			creqit.bold(sender_fullname), creqit.bold(ref_doctype), get_title_html(title)
		)
...
def extract_mentions(txt):
	...
	for mention in soup.find_all(class_="mention"):
		if mention.get("data-is-group") == "true":
			try:
				user_group = creqit.get_cached_doc("User Group", mention["data-id"])
