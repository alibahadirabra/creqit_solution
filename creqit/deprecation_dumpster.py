...
This file is the final resting place (or should we say, "retirement home"?) for all the deprecated functions and methods of the creqit framework. It's like a code nursing home, but with more monkey-patching and less bingo.
...
2. The creqit version in which it will be removed (its "graduation" to the great codebase in the sky)
...
class creqitDeprecationWarning(Warning):
	...
...
try:
	...
	def _deprecated(message: str, category=creqitDeprecationWarning, stacklevel=1) -> Callable[[T], T]:
...
def deprecated(original: str, marked: str, graduation: str, msg: str, stacklevel: int = 1):
	...
	Arguments:
	        - original: creqit.utils.make_esc  (fully qualified)
	...
	def decorator(func):
		...
		if os.path.basename(caller_filepath) != "deprecation_dumpster.py":
			raise RuntimeError(
				...
				+ colorize("creqit/deprecation_dumpster.py\n", Color.CYAN)
				+ colorize("Move the entire function there and import it back via adding\n ", Color.YELLOW)
				+ colorize(f"from creqit.deprecation_dumpster import {func.__name__}\n", Color.CYAN)
				+ colorize("to file\n ", Color.YELLOW)
...
def _old_deprecated(func):
	return deprecated(
		"creqit.deprecations.deprecated",
		"2024-09-13",
		"v17",
		"Make use of the creqit/deprecation_dumpster.py file, instead. 🎉🗑️",
	)(_deprecated("")(func))


def _old_deprecation_warning(msg):
	@deprecated(
		"creqit.deprecations.deprecation_warning",
		"2024-09-13",
		"v17",
		"Use creqit.deprecation_dumpster.deprecation_warning, instead. 🎉🗑️",
	)
...
@deprecated("creqit.utils.make_esc", "unknown", "v17", "Not used anymore.")
...
@deprecated(
	"creqit.db.is_column_missing",
	"unknown",
	"v17",
	"Renamed to creqit.db.is_missing_column.",
)
def is_column_missing(e):
	import creqit

	return creqit.db.is_missing_column(e)


@deprecated(
	"creqit.desk.doctype.bulk_update.bulk_update",
	"unknown",
	"v17",
	"Unknown.",
)
...
def get_js(items):
	...
	for src in items:
		...
		if ".." in src or src[0] != "assets":
			creqit.throw(_("Invalid file path: {0}").format("/".join(src)))

		contentpath = os.path.join(creqit.local.sites_path, *src)
		with open(contentpath) as srcfile:
			code = creqit.utils.cstr(srcfile.read())
...
@deprecated(
	"creqit.utils.print_format.read_multi_pdf",
	"unknown",
	"v17",
	"Unknown.",
)
...
@deprecated("creqit.gzip_compress", "unknown", "v17", "Use py3 methods directly (this was compat for py2).")
...
@deprecated("creqit.gzip_decompress", "unknown", "v17", "Use py3 methods directly (this was compat for py2).")
...
@deprecated(
	"creqit.email.doctype.email_queue.email_queue.send_mail",
	"unknown",
	"v17",
	"Unknown.",
)
...
@deprecated(
	"creqit.geo.country_info.get_translated_dict",
	"unknown",
	"v17",
	"Use creqit.geo.country_info.get_translated_countries, instead.",
)
...
@deprecated("creqit.tests_runner.get_modules", "2024-20-08", "v17", "use creqit.tests.utils.get_modules")
def test_runner_get_modules(doctype):
	from creqit.tests.utils import get_modules

	return get_modules(doctype)


@deprecated(
	"creqit.tests_runner.make_test_records", "2024-20-08", "v17", "use creqit.tests.utils.make_test_records"
)
...
@deprecated(
	"creqit.tests_runner.make_test_objects", "2024-20-08", "v17", "use creqit.tests.utils.make_test_objects"
)
...
@deprecated(
	"creqit.tests_runner.make_test_records_for_doctype",
	"2024-20-08",
	"v17",
	"use creqit.tests.utils.make_test_records_for_doctype",
)
...
@deprecated(
	"creqit.tests_runner.print_mandatory_fields",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_print_mandatory_fields(*args, **kwargs):
	from creqit.tests.utils.generators import print_mandatory_fields

	return print_mandatory_fields(*args, **kwargs)


@deprecated(
	"creqit.tests_runner.get_test_record_log",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_get_test_record_log(doctype):
	from creqit.tests.utils.generators import TestRecordManager

	return TestRecordManager().get(doctype)


@deprecated(
	"creqit.tests_runner.add_to_test_record_log",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
...
@deprecated(
	"creqit.tests_runner.main",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_main(*args, **kwargs):
	from creqit.commands.testing import main

	return main(*args, **kwargs)


@deprecated(
	"creqit.tests_runner.xmlrunner_wrapper",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
...
@deprecated(
	"creqit.tests.upate_system_settings",
	"2024-20-08",
	"v17",
...
def tests_update_system_settings(args, commit=False):
	...
	if commit:
		# moved here
		creqit.db.commit()  # nosemgrep


@deprecated(
	"creqit.tests.get_system_setting",
	"2024-20-08",
	"v17",
	"use `creqit.db.get_single_value('System Settings', key)`",
)
def tests_get_system_setting(key):
	import creqit

	return creqit.db.get_single_value("System Settings", key)


@deprecated(
	"creqit.tests.utils.change_settings",
	"2024-20-08",
	"v17",
	"use `creqit.tests.change_settings` or the cls.change_settings",
)
def tests_change_settings(*args, **kwargs):
	from creqit.tests.classes.context_managers import change_settings

	return change_settings(*args, **kwargs)


@deprecated(
	"creqit.tests.utils.patch_hooks",
	"2024-20-08",
	"v17",
	"use `creqit.tests.patch_hooks` or the cls.patch_hooks",
)
def tests_patch_hooks(*args, **kwargs):
	from creqit.tests.classes.context_managers import patch_hooks

	return patch_hooks(*args, **kwargs)


@deprecated(
	"creqit.tests.utils.debug_on",
	"2024-20-08",
	"v17",
	"use `creqit.tests.debug_on` or the cls.debug_on",
)
def tests_debug_on(*args, **kwargs):
	from creqit.tests.classes.context_managers import debug_on

	return debug_on(*args, **kwargs)


@deprecated(
	"creqit.tests.utils.timeout",
	"2024-20-08",
	"v17",
	"use `creqit.tests.timeout` or the cls.timeout",
)
def tests_timeout(*args, **kwargs):
	from creqit.tests.classes.context_managers import timeout

	return timeout(*args, **kwargs)


def get_tests_creqitTestCase():
	class CompatcreqitTestCase:
		def __new__(cls, *args, **kwargs):
			from creqit.tests import IntegrationTestCase

			class _CompatcreqitTestCase(IntegrationTestCase):
				def __init__(self, *args, **kwargs):
					deprecation_warning(
						"2024-20-08",
						"v17",
						"Import `creqit.tests.UnitTestCase` or `creqit.tests.IntegrationTestCase` respectively instead of `creqit.tests.utils.creqitTestCase`",
					)
					super().__init__(*args, **kwargs)

			return _CompatcreqitTestCase(*args, **kwargs)

	return CompatcreqitTestCase
			...
			class _CompatIntegrationTestCase(IntegrationTestCase):
				def __init__(self, *args, **kwargs):
					deprecation_warning(
						"2024-20-08",
						"v17",
						"Import `creqit.tests.IntegrationTestCase` instead of `creqit.tests.utils.IntegrationTestCase`",
					)
					super().__init__(*args, **kwargs)
			...
			class _CompatUnitTestCase(UnitTestCase):
				def __init__(self, *args, **kwargs):
					deprecation_warning(
						"2024-20-08",
						"v17",
						"Import `creqit.tests.UnitTestCase` instead of `creqit.tests.utils.UnitTestCase`",
					)
					super().__init__(*args, **kwargs)
...
@deprecated(
	"creqit.model.trace.traced_field_context",
	"2024-20-08",
	"v17",
	"use `cls.trace_fields`",
)
def model_trace_traced_field_context(*args, **kwargs):
	from creqit.tests.classes.context_managers import trace_fields

	return trace_fields(*args, **kwargs)


@deprecated(
	"creqit.tests.utils.get_dependencies",
	"2024-20-09",
	"v17",
	"refactor to use creqit.tests.utils.get_missing_records_doctypes",
)
def tests_utils_get_dependencies(doctype):
	"""Get the dependencies for the specified doctype"""
	import creqit
	from creqit.tests.utils.generators import get_modules
	...
	for df in meta.get_table_fields():
		link_fields.extend(creqit.get_meta(df.options).get_link_fields())
...
@deprecated(
	"creqit.tests_runner.get_dependencies",
	"2024-20-08",
	"v17",
	"refactor to use creqit.tests.utils.get_missing_record_doctypes",
)
...
@deprecated(
	"creqit.get_test_records",
	"2024-20-09",
	"v17",
	"""Please access the global test records pool via cls.globalTestRecords['Some Doc'] -> list.
If not an IntegrationTestCase, use creqit.tests.utils.load_test_records_for (check return type).
""",
)
def creqit_get_test_records(doctype):
	import creqit
	from creqit.tests.utils.generators import load_test_records_for

	creqit.flags.deprecation_dumpster_invoked = True
