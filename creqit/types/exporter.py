...
class TypeExporter:
	def __init__(self, doc):
		...
		self.imports = {"from creqit.types import DF"}
		self.indent = "\t"
	...
	def _validate_code(code) -> bool:
		"""Make sure whatever code creqit adds dynamically is valid python."""
		try:
			ast.parse(code)
			return True
		except Exception:
			creqit.msgprint(creqit._("Failed to export python type hints"), alert=True)
			return False
