...
from creqit.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data
from creqit.tests import IntegrationTestCase


def get_custom_linked_doctype():
	if bool(creqit.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def create_linked_address(link_list):
	if creqit.flags.test_address_created:
		return

	address = creqit.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	creqit.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if creqit.flags.test_contact_created:
		return
	...
	creqit.flags.test_contact_created = True
