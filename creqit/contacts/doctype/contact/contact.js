...
creqit.ui.form.on("Contact", {
	...
	refresh: function (frm) {
		...
		if (
			...
			!frm.is_new() &&
			frm.perm[0].write &&
			creqit.boot.user.can_create.includes("User")
		) {
			frm.add_custom_button(__("Invite as User"), function () {
				return creqit.call({
					method: "creqit.contacts.doctype.contact.contact.invite_user",
					args: {
		...
		if (frm.doc.links && frm.doc.links.length > 0) {
			...
			if (filtered_links.length > 0) {
				creqit.call({
					method: "creqit.contacts.doctype.contact.contact.address_query",
		...
		if (!frm.is_dirty()) {
			frm.page.add_menu_item(__("Download vCard"), function () {
				window.open(
					`/api/method/creqit.contacts.doctype.contact.contact.download_vcard?contact=${encodeURIComponent(
						frm.doc.name
					)}`,
					"_blank"
				);
...
creqit.ui.form.on("Dynamic Link", {
	link_name: function (frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		if (child.link_name) {
			creqit.model.with_doctype(child.link_doctype, function () {
				var title_field = creqit.get_meta(child.link_doctype).title_field || "name";
				creqit.model.get_value(
					child.link_doctype,
					child.link_name,
					title_field,
					function (r) {
						creqit.model.set_value(cdt, cdn, "link_title", r[title_field]);
					}
				);
			});
		}
