...
import creqit
from creqit.core.doctype.data_export.exporter import DataExporter
...
class TestDataExporter(IntegrationTestCase):
	...
	def create_test_data(self, force=False):
		...
		"""
		if force:
			creqit.delete_doc(self.doctype_name, self.doc_name)

		if not creqit.db.exists(self.doctype_name, self.doc_name):
			self.doc = creqit.get_doc(
			...
			self.doc = creqit.get_doc(self.doctype_name, self.doc_name)

	def test_export_content(self):
		exp = DataExporter(doctype=self.doctype_name, file_type="CSV")
		exp.build_response()

		self.assertEqual(creqit.response["type"], "csv")
		self.assertEqual(creqit.response["doctype"], self.doctype_name)
		self.assertTrue(creqit.response["result"])
		self.assertRegex(creqit.response["result"], r"Child Title 1.*?,50")
		self.assertRegex(creqit.response["result"], r"Child Title 2.*?,51")
		...
		for type in ["csv", "Excel"]:
			with self.subTest(type=type):
				...
				if type == "csv":
					...
					self.assertEqual(creqit.response["type"], "binary")
					self.assertEqual(
						creqit.response["filename"], self.doctype_name + ".xlsx"
					)  # 'Test DocType for Export Tool.xlsx')
					self.assertTrue(creqit.response["filecontent"])
