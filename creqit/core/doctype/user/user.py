...
import creqit.share
from creqit import STANDARD_USERS, _, msgprint, throw
from creqit.apps import get_default_path
from creqit.auth import MAX_PASSWORD_SIZE
from creqit.core.doctype.user_type.user_type import user_linked_with_permission_on_doctype
...
from creqit.utils.data import sha256_hash
from creqit.utils.password import check_password, get_password_reset_limit
from creqit.utils.password import update_password as _update_password
from creqit.utils.user import get_system_managers
from creqit.website.utils import get_home_page, is_signup_disabled
...
class User(Document):
	...
	if TYPE_CHECKING:
		...
		from creqit.core.doctype.defaultvalue.defaultvalue import DefaultValue
		from creqit.core.doctype.has_role.has_role import HasRole
		from creqit.core.doctype.user_email.user_email import UserEmail
		from creqit.core.doctype.user_role_profile.user_role_profile import UserRoleProfile
		from creqit.core.doctype.user_social_login.user_social_login import UserSocialLogin
	...
	def after_insert(self):
		...
		creqit.cache.delete_key("users_for_mentions")
		creqit.cache.delete_key("enabled_users")
	...
	def validate(self):
		...
		if (self.name not in ["Administrator", "Guest"]) and (not self.get_social_login_userid("creqit")):
			self.set_social_login_userid("creqit", creqit.generate_hash(length=39))
	...
	def populate_role_profile_roles(self):
		...
		for role_profile in self.role_profiles:
			role_profile = creqit.get_cached_doc("Role Profile", role_profile.role_profile)
	...
	from creqit.deprecation_dumpster import validate_roles
	...
	def validate_allowed_modules(self):
		if self.module_profile:
			module_profile = creqit.get_doc("Module Profile", self.module_profile)
	...
	def validate_user_image(self):
		if self.user_image and len(self.user_image) > 2000:
			creqit.throw(_("Not a valid User Image."))

	def on_update(self):
		# clear new password
		self.share_with_self()
		clear_notifications(user=self.name)
		creqit.clear_cache(user=self.name)
		now = creqit.flags.in_test or creqit.flags.in_install
		self.send_password_notification(self.__new_password)
		creqit.enqueue(
			"creqit.core.doctype.user.user.create_contact",
			user=self,
		...
		if self.name not in STANDARD_USERS and not self.user_image:
			creqit.enqueue(
				"creqit.core.doctype.user.user.update_gravatar",
				name=self.name,
				now=now,
				enqueue_after_commit=True,
			)
		...
		if self.time_zone:
			creqit.defaults.set_default("time_zone", self.time_zone, self.name)

		if self.has_value_changed("language"):
			locale_keys = ("date_format", "time_format", "number_format", "first_day_of_the_week")
			if self.language:
				language = creqit.get_doc("Language", self.language)
				for key in locale_keys:
					value = language.get(key)
					if value:
						creqit.defaults.set_default(key, value, self.name)
			else:
				for key in locale_keys:
					creqit.defaults.clear_default(key, parent=self.name)
	...
	def has_website_permission(self, ptype, user, verbose=False):
		...
		return self.name == creqit.session.user
	...
	def check_enable_disable(self):
		...
		if not cint(self.enabled) and self.name in STANDARD_USERS:
			creqit.throw(_("User {0} cannot be disabled").format(self.name))

		# clear sessions if disabled
		if not cint(self.enabled) and getattr(creqit.local, "login_manager", None):
			creqit.local.login_manager.logout(user=self.name)
	...
	def set_system_user(self):
		...
		if self.user_type and not creqit.get_cached_value("User Type", self.user_type, "is_standard"):
	...
	def set_roles_and_modules_based_on_user_type(self):
		user_type_doc = creqit.get_cached_doc("User Type", self.user_type)
	...
	def validate_share(self, docshare):
		...
		# 		if docshare.share != 1:
		# 			creqit.throw(_("Sorry! User should have complete access to their own record."))
		# 	else:
		# 		creqit.throw(_("Sorry! Sharing with Website User is prohibited."))
	...
	def send_password_notification(self, new_password):
		...
		except creqit.OutgoingEmailError:
	...
	def password_reset_mail(self, link):
		reset_password_template = creqit.db.get_system_setting("reset_password_template")
	...
	def send_welcome_mail_to_user(self):
		...
		subject = None
		method = creqit.get_hooks("welcome_email")
		if method:
			subject = creqit.get_attr(method[-1])()
		if not subject:
			site_name = creqit.db.get_default("site_name") or creqit.get_conf().get("site_name")
			if site_name:
				subject = _("Welcome to {0}").format(site_name)
			else:
				subject = _("Complete Registration")

		welcome_email_template = creqit.db.get_system_setting("welcome_email_template")
	...
	def send_login_mail(self, subject, template, add_args, now=None, custom_template=None):
		...
		from creqit.utils import get_url
		from creqit.utils.user import get_user_fullname

		created_by = get_user_fullname(creqit.session["user"])
		if created_by == "Guest":
		...
		sender = (
			creqit.session.user not in STANDARD_USERS and get_formatted_email(creqit.session.user) or None
		)

		if custom_template:
			from creqit.email.doctype.email_template.email_template import get_email_template
	...
	def on_trash(self):
		...
		self.enabled = 0
		if getattr(creqit.local, "login_manager", None):
			creqit.local.login_manager.logout(user=self.name)

		# delete todos
		creqit.db.delete("ToDo", {"allocated_to": self.name})
		todo_table = DocType("ToDo")
		(
			creqit.qb.update(todo_table)
			.set(todo_table.assigned_by, None)
			.where(todo_table.assigned_by == self.name)
		).run()

		# delete events
		creqit.db.delete("Event", {"owner": self.name, "event_type": "Private"})

		# delete shares
		creqit.db.delete("DocShare", {"user": self.name})
		# delete messages
		...
		creqit.qb.update(table).where(table.user == self.name).set(table.user, None).run()

		# delete notification settings
		creqit.delete_doc("Notification Settings", self.name, ignore_permissions=True)
		...
		creqit.db.delete("User Permission", {"user": self.name})

		# Delete OAuth data
		creqit.db.delete("OAuth Authorization Code", {"user": self.name})
		creqit.db.delete("Token Cache", {"user": self.name})

		# Delete EPS data
		creqit.db.delete("Energy Point Log", {"user": self.name})

		# Remove user link from Workflow Action
		creqit.db.set_value("Workflow Action", {"user": self.name}, "user", None)

		# Delete user's List Filters
		creqit.db.delete("List Filter", {"for_user": self.name})

		# Remove user from Note's Seen By table
		seen_notes = creqit.get_all("Note", filters=[["Note Seen By", "user", "=", self.name]], pluck="name")
		for note_id in seen_notes:
			note = creqit.get_doc("Note", note_id)
	...
	def before_rename(self, old_name, new_name, merge=False):
		...
		if merge:
			creqit.delete_doc("Notification Settings", old_name, ignore_permissions=True)

		creqit.clear_cache(user=old_name)
	...
	def after_rename(self, old_name, new_name, merge=False):
		tables = creqit.db.get_tables()
		for tab in tables:
			desc = creqit.db.get_table_columns_description(tab)
		...
		if creqit.db.exists("Notification Settings", old_name):
			creqit.rename_doc("Notification Settings", old_name, new_name, force=True, show_alert=False)

		# set email
		creqit.db.set_value("User", new_name, "email", new_name)
	...
	def remove_disabled_roles(self):
		disabled_roles = [d.name for d in creqit.get_all("Role", filters={"disabled": 1})]
	...
	def validate_username(self):
		if not self.username and self.is_new() and self.first_name:
			self.username = creqit.scrub(self.first_name)
		...
		if self.username_exists():
			if self.user_type == "System User":
				creqit.msgprint(_("Username {0} already exists").format(self.username))
	...
	def suggest_username(self):
		...
		# @firstname
		username = _check_suggestion(creqit.scrub(self.first_name))

		if not username:
			# @firstname_last_name
			username = _check_suggestion(creqit.scrub("{} {}".format(self.first_name, self.last_name or "")))

		if username:
			creqit.msgprint(_("Suggested Username: {0}").format(username))

		return username

	def username_exists(self, username=None):
		return creqit.db.get_value("User", {"username": username or self.username, "name": ("!=", self.name)})
	...
	def validate_user_email_inbox(self):
		...
		if len(email_accounts) != len(set(email_accounts)):
			creqit.throw(_("Email Account added multiple times"))
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(creqit.get_system_settings("allow_login_using_mobile_number"))
		login_with_username = cint(creqit.get_system_settings("allow_login_using_user_name"))
		...
		users = creqit.get_all("User", fields=["name", "enabled"], or_filters=or_filters, limit=1)
		if not users:
			return
	...
	def check_roles_added(self):
		...
		creqit.msgprint(
			_("Newly created user {0} has no roles enabled.").format(creqit.bold(self.name)),
			title=_("No Roles Specified"),
			indicator="orange",
			primary_action={
				"label": _("Add Roles"),
				"client_action": "creqit.set_route",
...
def get_all_roles():
	...
	roles = creqit.get_all(
		"Role",
		filters={
			"name": ("not in", creqit.permissions.AUTOMATIC_ROLES),
			"disabled": 0,
		},
...
def get_roles(arg=None):
	...
	return creqit.get_roles(creqit.form_dict["uid"])


@creqit.whitelist()
def get_perm_info(role):
	"""get permission info"""
	from creqit.permissions import get_all_perms

	return get_all_perms(role)


@creqit.whitelist(allow_guest=True, methods=["POST"])
def update_password(
	...
	if len(new_password) > MAX_PASSWORD_SIZE:
		creqit.throw(_("Password size exceeded the maximum allowed size."))
	...
	if res.get("message"):
		creqit.local.response.http_status_code = 410
		return res["message"]
	else:
		user = res["user"]

	logout_all_sessions = cint(logout_all_sessions) or creqit.get_system_settings("logout_on_password_reset")
	_update_password(user, new_password, logout_all_sessions=cint(logout_all_sessions))

	user_doc, redirect_url = reset_user_data(user)

	# get redirect url from cache
	redirect_to = creqit.cache.hget("redirect_after_login", user)
	if redirect_to:
		redirect_url = redirect_to
		creqit.cache.hdel("redirect_after_login", user)

	creqit.local.login_manager.login_as(user)

	creqit.db.set_value("User", user, "last_password_reset_date", today())
	creqit.db.set_value("User", user, "reset_password_key", "")
...
def test_password_strength(new_password: str, key=None, old_password=None, user_data: tuple | None = None):
	from creqit.utils.password_strength import test_password_strength as _test_password_strength
	...
	enable_password_policy = creqit.get_system_settings("enable_password_policy")

	if not enable_password_policy:
		return {}

	if not user_data:
		user_data = creqit.db.get_value(
			"User", creqit.session.user, ["first_name", "middle_name", "last_name", "email", "birth_date"]
		)

	if new_password:
		result = _test_password_strength(new_password, user_inputs=user_data)
		password_policy_validation_passed = False
		minimum_password_score = cint(creqit.get_system_settings("minimum_password_score"))
...
def has_email_account(email: str):
	return creqit.get_list("Email Account", filters={"email_id": email})


@creqit.whitelist(allow_guest=False)
...
def ask_pass_update():
	...
	from creqit.utils import set_default

	password_list = creqit.get_all(
	...
	if key:
		...
		if result.user:
			reset_password_link_expiry = cint(
				creqit.get_system_settings("reset_password_link_expiry_duration")
			)
			if (
		...
		# verify old password
		creqit.local.login_manager.check_password(creqit.session.user, old_password)
...
def reset_user_data(user):
	user_doc = creqit.get_doc("User", user)
...
@creqit.whitelist(methods=["POST"])
def verify_password(password):
	creqit.local.login_manager.check_password(creqit.session.user, password)


@creqit.whitelist(allow_guest=True)
def sign_up(email: str, full_name: str, redirect_to: str) -> tuple[int, str]:
	if is_signup_disabled():
		creqit.throw(_("Sign Up is disabled"), title=_("Not Allowed"))

	user = creqit.db.get("User", {"email": email})
	if user:
		if user.enabled:
		...
		if creqit.db.get_creation_count("User", 60) > 300:
		...
		default_role = creqit.db.get_single_value("Portal Settings", "default_role")
		if default_role:
			user.add_roles(default_role)

		if redirect_to:
			creqit.cache.hset("redirect_after_login", user.name, redirect_to)
...
def reset_password(user: str) -> str:
	try:
		user: User = creqit.get_doc("User", user)
		...
		creqit.local.response["http_status_code"] = 404
...
def user_query(doctype, txt, searchfield, start, page_len, filters):
	from creqit.desk.reportview import get_filters_cond, get_match_cond

	doctype = "User"
	conditions = []

	user_type_condition = "and user_type != 'Website User'"
	if filters and filters.get("ignore_user_type") and creqit.session.data.user_type == "System User":
		user_type_condition = ""
    ...
    """.format(
			...
			standard_users=", ".join(creqit.db.escape(u) for u in STANDARD_USERS),
			key=searchfield,
...
def get_website_users():
	...
	return creqit.db.count("User", filters={"enabled": True, "user_type": "Website User"})
...
def get_permission_query_conditions(user):
	if user == "Administrator":
		...
		return """(`tabUser`.name not in ({standard_users}))""".format(
			standard_users=", ".join(creqit.db.escape(user) for user in STANDARD_USERS)
		)
...
def notify_admin_access_to_system_manager(login_manager=None):
	if (
		...
		and creqit.local.conf.notify_admin_access_to_system_manager
	):
		site = '<a href="{0}" target="_blank">{0}</a>'.format(creqit.local.request.host_url)
		date_and_time = "<b>{}</b>".format(format_datetime(now_datetime(), format_string="medium"))
		ip_address = creqit.local.request_ip
...
def handle_password_test_fail(feedback: dict):
	...
	creqit.throw(msg=" ".join([warning, *suggestions]), title=_("Invalid Password"))


def update_gravatar(name):
	gravatar = has_gravatar(name)
	if gravatar:
		creqit.db.set_value("User", name, "user_image", gravatar)
...
def throttle_user_creation():
	...
	if creqit.db.get_creation_count("User", 60) > creqit.local.conf.get("throttle_user_limit", 60):
		creqit.throw(_("Throttled"))


@creqit.whitelist()
def get_module_profile(module_profile: str):
	module_profile = creqit.get_doc("Module Profile", {"module_profile_name": module_profile})
...
def create_contact(user, ignore_links=False, ignore_mandatory=False):
	...
	if not contact_name:
		...
		except creqit.DuplicateEntryError:
			pass
	else:
		try:
			contact = creqit.get_doc("Contact", contact_name)
		...
		except creqit.TimestampMismatchError:
			raise creqit.RetryBackgroundJobError
...
def generate_keys(user: str):
	...
	user_details: User = creqit.get_doc("User", user)
	api_secret = creqit.generate_hash(length=15)
	# if api key is not set generate api key
	if not user_details.api_key:
		api_key = creqit.generate_hash(length=15)
...
def switch_theme(theme):
	if theme in ["Dark", "Light", "Automatic"]:
		creqit.db.set_value("User", creqit.session.user, "desk_theme", theme)


def get_enabled_users():
	def _get_enabled_users():
		enabled_users = creqit.get_all("User", filters={"enabled": "1"}, pluck="name")
		return enabled_users

	return creqit.cache.get_value("enabled_users", _get_enabled_users)
...
def impersonate(user: str, reason: str):
	...
	impersonator = creqit.session.user
	creqit.get_doc(
		{
