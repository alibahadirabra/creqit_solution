creqit.ui.form.on("User", {
	setup: function (frm) {
		frm.set_query("default_workspace", () => {
			return {
				filters: {
					for_user: ["in", [null, creqit.session.user]],
	...
	before_load: function (frm) {
		let update_tz_options = function () {
			frm.fields_dict.time_zone.set_data(creqit.all_timezones);
		};

		if (!creqit.all_timezones) {
			creqit.call({
				method: "creqit.core.doctype.user.user.get_timezones",
				callback: function (r) {
					creqit.all_timezones = r.message.timezones;
					update_tz_options();
				},
			});
		} else {
	...
	module_profile: function (frm) {
		if (frm.doc.module_profile) {
			creqit.call({
				method: "creqit.core.doctype.user.user.get_module_profile",
				args: {
	...
	onload: function (frm) {
		...
		if (
			...
			if (!frm.roles_editor) {
				...
				frm.roles_editor = new creqit.RoleEditor(
					role_area,
					frm,
	...
	refresh: function (frm) {
		...
		creqit.xcall("creqit.apps.get_apps").then((r) => {
			let apps = r?.map((r) => r.name) || [];
			frm.set_df_property("default_app", "options", [" ", ...apps]);
		});

		if (frm.is_new()) {
			frm.set_value("time_zone", creqit.sys_defaults.time_zone);
		}
		...
		if (!frm.is_new()) {
			if (has_access_to_edit_user()) {
				...
				frm.add_custom_button(
					...
					() =>
						creqit.set_route("query-report", "Permitted Documents For User", {
							user: frm.doc.name,
						}),
					__("Permissions")
				);
				...
				frm.add_custom_button(
					...
					() =>
						creqit.set_route("query-report", "User Doctype Permissions", {
							user: frm.doc.name,
						}),
					__("Permissions")
				);
			...
			frm.add_custom_button(
				...
				function () {
					creqit.call({
						method: "creqit.core.doctype.user.user.reset_password",
						args: {
							user: frm.doc.name,
						},
					});
			...
			if (creqit.user.has_role("System Manager")) {
				creqit.db.get_single_value("LDAP Settings", "enabled").then((value) => {
								...
								const d = new creqit.ui.Dialog({
									...
									primary_action: (values) => {
										...
										if (values.new_password !== values.confirm_password) {
											creqit.throw(__("Passwords do not match!"));
										}
										creqit.call(
											"creqit.integrations.doctype.ldap_settings.ldap_settings.reset_password",
											{
												user: frm.doc.email,
			...
			if (
				cint(creqit.boot.sysdefaults.enable_two_factor_auth) &&
				(creqit.session.user == doc.name || creqit.user.has_role("System Manager"))
			) {
				frm.add_custom_button(
					__("Reset OTP Secret"),
					function () {
						creqit.call({
							method: "creqit.twofactor.reset_otp_secret",
							args: {
								user: frm.doc.name,
							},
						});
			...
			if (creqit.session.user == doc.name) {
				// update display settings
				if (doc.user_image) {
					creqit.boot.user_info[creqit.session.user].image = creqit.utils.get_file_link(
						doc.user_image
					);
				}
			}
		}
		if (frm.doc.user_emails && creqit.model.can_create("Email Account")) {
			var found = 0;
		...
		if (creqit.route_flags.unsaved === 1) {
			delete creqit.route_flags.unsaved;
	...
	create_user_email: function (frm) {
		creqit.call({
			method: "creqit.core.doctype.user.user.has_email_account",
			args: {
				email: frm.doc.email,
			},
			callback: function (r) {
				if (!Array.isArray(r.message) || !r.message.length) {
					...
					creqit.model.with_doctype("Email Account", function (doc) {
						doc = creqit.model.get_new_doc("Email Account");
						creqit.route_flags.linked_user = frm.doc.name;
						creqit.route_flags.delete_user_from_locals = true;
						creqit.set_route("Form", "Email Account", doc.name);
					});
				} else {
					creqit.route_flags.create_user_account = frm.doc.name;
					creqit.set_route("Form", "Email Account", r.message[0]["name"]);
				}
			},
		});
	},
	generate_keys: function (frm) {
		creqit.call({
			method: "creqit.core.doctype.user.user.generate_keys",
			args: {
				user: frm.doc.name,
			},
			callback: function (r) {
				if (r.message) {
					creqit.msgprint(__("Save API Secret: {0}", [r.message.api_secret]));
					frm.reload_doc();
				}
			},
		});
	...
	after_save: function (frm) {
		...
		if (doc.name === creqit.session.user && attr_tuples.some(has_effectively_changed)) {
			creqit.msgprint(__("Refreshing..."));
			window.location.reload();
		}
	},
	setup_impersonation: function (frm) {
		if (creqit.session.user === "Administrator" && frm.doc.name != "Administrator") {
			frm.add_custom_button(__("Impersonate"), () => {
				...
				creqit.prompt(
					...
					(values) => {
						creqit
							.xcall("creqit.core.doctype.user.user.impersonate", {
								user: frm.doc.name,
...
creqit.ui.form.on("User Email", {
	email_account(frm, cdt, cdn) {
		let child_row = locals[cdt][cdn];
		creqit.model.get_value(
			"Email Account",
...
function has_access_to_edit_user() {
	return has_common(creqit.user_roles, get_roles_for_editing_user());
}
