...
from creqit.core.doctype.user_permission.user_permission import (
	add_user_permissions,
	remove_applicable,
)
from creqit.permissions import add_permission, has_user_permission
from creqit.tests import IntegrationTestCase, UnitTestCase
from creqit.website.doctype.blog_post.test_blog_post import make_test_blog
...
class TestUserPermission(IntegrationTestCase):
	def setUp(self):
		...
		creqit.db.delete("User Permission", {"user": ("in", test_users)})
		creqit.delete_doc_if_exists("DocType", "Person")
		creqit.db.sql_ddl("DROP TABLE IF EXISTS `tabPerson`")
		creqit.delete_doc_if_exists("DocType", "Doc A")
		creqit.db.sql_ddl("DROP TABLE IF EXISTS `tabDoc A`")
	...
	def test_default_user_permission_validation(self):
		...
		self.assertRaises(creqit.ValidationError, add_user_permissions, param)
	...
	def test_default_user_permission_corectness(self):
		...
		add_user_permissions(param)
		creqit.db.delete("User Permission", filters={"for_value": test_blog.name})
		creqit.delete_doc("Blog Post", test_blog.name)
	...
	def test_default_user_permission(self):
		...
		for category in ["general", "public"]:
			if not creqit.db.exists("Blog Category", category):
				creqit.get_doc({"doctype": "Blog Category", "title": category}).insert()
		...
		creqit.set_user("test_user_perm1@example.com")
	...
	def test_for_applicable_on_update_from_apply_to_all(self):
		...
		removed_apply_to_all = creqit.db.exists("User Permission", get_exists_param(user))
		is_created_applicable_first = creqit.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		is_created_applicable_second = creqit.db.exists(
	...
	def test_for_apply_to_all_on_update_from_applicable(self):
		...
		is_created = add_user_permissions(param)
		is_created_apply_to_all = creqit.db.exists("User Permission", get_exists_param(user))
		removed_applicable_first = creqit.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		removed_applicable_second = creqit.db.exists(
			"User Permission", get_exists_param(user, applicable="Contact")
		)
	...
	def test_user_perm_for_nested_doctype(self):
		...
		if not creqit.db.exists("DocType", "Person"):
			doc = new_doctype(
				"Person",
		...
		parent_record = creqit.get_doc({"doctype": "Person", "person_name": "Parent", "is_group": 1}).insert()

		child_record = creqit.get_doc(
			{
				"doctype": "Person",
				"person_name": "Child",
				"is_group": 0,
		...
		self.assertTrue(has_user_permission(creqit.get_doc("Person", parent_record.name), user.name))
		self.assertTrue(has_user_permission(creqit.get_doc("Person", child_record.name), user.name))

		#  give access of Parent DocType to Blogger role
		add_permission("Person", "Blogger")
		creqit.set_user(user.name)
		visible_names = creqit.get_list(
			doctype="Person",
			pluck="person_name",
		)

		user_permission = creqit.get_doc(
		...
		# hides child records
		self.assertTrue(has_user_permission(creqit.get_doc("Person", parent_record.name), user.name))
		self.assertFalse(has_user_permission(creqit.get_doc("Person", child_record.name), user.name))

		visible_names_after_hide_descendants = creqit.get_list(
			"Person",
			pluck="person_name",
		)
	...
	def test_user_perm_on_new_doc_with_field_default(self):
		...
		creqit.set_user("new_doc_test@example.com")

		new_doc = creqit.new_doc("Doc A")
	...
	def test_user_perm_on_new_doc_with_user_default(self):
		"""Test User Perm impact on creqit.new_doc. with *user* default value"""
		...
		if not creqit.db.exists("Session Default", {"ref_doctype": "DocType"}):
		...
		creqit.set_user("user_default_test@example.com")
...
def create_user(email, *roles):
	...
	if creqit.db.exists("User", email):
		return creqit.get_doc("User", email)
