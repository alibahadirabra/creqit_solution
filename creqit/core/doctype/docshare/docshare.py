...
from creqit.utils import cint, get_fullname
...
class DocShare(Document):
	...
	def get_doc(self):
		if not getattr(self, "_doc", None):
			self._doc = creqit.get_doc(self.share_doctype, self.share_name)
		return self._doc
	...
	def validate_user(self):
		if self.everyone:
			...
			creqit.throw(_("User is mandatory for Share"), creqit.MandatoryError)

	def check_share_permission(self):
		if not self.flags.ignore_share_permission and not creqit.has_permission(
			self.share_doctype, "share", self.get_doc()
		):
			creqit.throw(_('You need to have "Share" permission'), creqit.PermissionError)

	def check_is_submittable(self):
		if self.submit and not cint(creqit.db.get_value("DocType", self.share_doctype, "is_submittable")):
			creqit.throw(
				_("Cannot share {0} with submit permission as the doctype {1} is not submittable").format(
					creqit.bold(self.share_name), creqit.bold(self.share_doctype)
				)
			)
...
def on_doctype_update():
	...
	creqit.db.add_index("DocShare", ["user", "share_doctype"])
	creqit.db.add_index("DocShare", ["share_doctype", "share_name"])
