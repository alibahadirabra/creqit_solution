...
import creqit
from creqit.desk.query_report import generate_report_result, get_report_doc
...
class TestPreparedReport(IntegrationTestCase):
	...
	def tearDownClass(cls):
		for r in creqit.get_all("Prepared Report", pluck="name"):
			creqit.delete_doc("Prepared Report", r, force=True, delete_permanently=True)

		creqit.db.commit()

	@timeout(seconds=20)
	def wait_for_status(self, report, status):
		creqit.db.commit()  # Flush changes first
		while True:
			creqit.db.rollback()  # read new data
			report.reload()
	...
	def test_queueing(self):
		...
		doc = creqit.get_last_doc("Prepared Report")
	...
	def test_start_status_and_kill_jobs(self):
		with test_report(report_type="Query Report", query="select sleep(10)") as report:
			...
			doc.delete()
			time.sleep(1)
			job = creqit.get_doc("RQ Job", job_id)
...
def test_report(**args):
	try:
		...
		if not report.report_name:
			report.report_name = creqit.generate_hash()
