...
import creqit
from creqit.core.doctype.user_permission.test_user_permission import create_user
from creqit.custom.doctype.customize_form.customize_form import reset_customization
from creqit.desk.query_report import add_total_row, run, save_report
from creqit.desk.reportview import delete_report
from creqit.desk.reportview import save_report as _save_report
...
class TestReport(IntegrationTestCase):
	...
	def test_report_builder(self):
		if creqit.db.exists("Report", "User Activity Report"):
			creqit.delete_doc("Report", "User Activity Report")

		with open(os.path.join(os.path.dirname(__file__), "user_activity_report.json")) as f:
			creqit.get_doc(json.loads(f.read())).insert()

		report = creqit.get_doc("Report", "User Activity Report")
	...
	def test_query_report(self):
		report = creqit.get_doc("Report", "Permitted Documents For User")
	...
	def test_save_or_delete_report(self):
		...
		try:
			...
			creqit.set_user("test_report_owner@example.com")
			self.assertRaises(creqit.PermissionError, delete_report, report.name)
	...
	def test_custom_report(self):
		...
		custom_report = creqit.get_doc("Report", custom_report_name)
		columns, result = custom_report.run_query_report(user=creqit.session.user)

		self.assertListEqual(["email"], [column.get("fieldname") for column in columns])
		admin_dict = creqit.core.utils.find(result, lambda d: d["name"] == "Administrator")
	...
	def test_report_permissions(self):
		creqit.set_user("test@example.com")
		creqit.db.delete("Has Role", {"parent": creqit.session.user, "role": "Test Has Role"})
		creqit.db.commit()
		if not creqit.db.exists("Role", "Test Has Role"):
			creqit.get_doc({"doctype": "Role", "role_name": "Test Has Role"}).insert(ignore_permissions=True)

		if not creqit.db.exists("Report", "Test Report"):
			report = creqit.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = creqit.get_doc("Report", "Test Report")
	...
	def test_report_custom_permissions(self):
		creqit.set_user("test@example.com")
		creqit.db.delete("Custom Role", {"report": "Test Custom Role Report"})
		creqit.db.commit()  # nosemgrep
		if not creqit.db.exists("Report", "Test Custom Role Report"):
			report = creqit.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = creqit.get_doc("Report", "Test Custom Role Report")
	...
	def test_format_method(self):
		if creqit.db.exists("Report", "User Activity Report Without Sort"):
			creqit.delete_doc("Report", "User Activity Report Without Sort")
		with open(os.path.join(os.path.dirname(__file__), "user_activity_report_without_sort.json")) as f:
			creqit.get_doc(json.loads(f.read())).insert()

		report = creqit.get_doc("Report", "User Activity Report Without Sort")
		columns, data = report.get_data()
	...
	def test_non_standard_script_report(self):
		...
		if not creqit.db.exists("Report", report_name):
			report = creqit.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
...
totals = {}
for user in creqit.get_all('User', fields = ['name', 'user_type', 'creation']):
...
"""
	...
	def test_script_report_with_columns(self):
		...
		if creqit.db.exists("Report", report_name):
			creqit.delete_doc("Report", report_name)
...
"""
	...
	def test_toggle_disabled(self):
		...
		reports = creqit.get_all(doctype="Report", limit=1)
		...
		doc = creqit.get_doc("Report", report_name)
		with self.assertRaises(creqit.exceptions.ValidationError):
	...
	def test_cte_in_query_report(self):
		...
		if creqit.db.db_type == "mariadb":
			...
			self.assertRaises(creqit.PermissionError, report.execute_query_report, filters={})
