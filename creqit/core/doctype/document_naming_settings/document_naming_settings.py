...
from creqit.core.doctype.doctype.doctype import validate_series
from creqit.model.document import Document
from creqit.model.naming import NamingSeries
from creqit.permissions import get_doctypes_with_read


class NamingSeriesNotSetError(creqit.ValidationError):
	pass
...
class DocumentNamingSettings(Document):
	...
	if TYPE_CHECKING:
		from creqit.core.doctype.amended_document_naming_settings.amended_document_naming_settings import (
	...
	def _get_transactions(self) -> list[str]:
		...
		standard = creqit.get_all("DocField", {"fieldname": "naming_series"}, "parent", pluck="parent")
		custom = creqit.get_all("Custom Field", {"fieldname": "naming_series"}, "dt", pluck="dt")
	...
	def _get_prefixes(self, doctypes) -> list[str]:
		...
		for d in doctypes:
			try:
				options = creqit.get_meta(d).get_naming_series_options()
				series_templates.update(options)
			except creqit.DoesNotExistError:
				creqit.msgprint(_("Unable to find DocType {0}").format(d))
				continue

		custom_templates = creqit.get_all(
			"DocType",
			fields=["autoname"],
			filters={
	...
	def _evaluate_and_clean_templates(self, series_templates: set[str]) -> list[str]:
		...
		prefixes_from_db = creqit.qb.from_(series).select(series.name).run(pluck=True)
		...
		for series_template in series_templates:
			try:
				...
				creqit.log_error(f"Invalid naming series {series_template}")
	...
	def validate_set_series(self):
		if self.transaction_type and not self.naming_series_options:
			creqit.throw(_("Please set the series to be used."))
	...
	def check_duplicate(self):
		...
		for doctype in all_doctypes_with_naming_series:
			for series in creqit.get_meta(doctype).get_naming_series_options():
				existing_series[stripped_series(series)] = doctype

		dt = creqit.get_doc("DocType", self.transaction_type)

		options = self.get_options_list(self.naming_series_options)
		for series in options:
			if stripped_series(series) in existing_series:
				creqit.throw(_("Series {0} already used in {1}").format(series, existing_series[series]))
	...
	def get_options(self, doctype=None):
		...
		if creqit.get_meta(doctype or self.transaction_type).get_field("naming_series"):
			return creqit.get_meta(doctype or self.transaction_type).get_field("naming_series").options
	...
	def update_amendment_rule(self):
		...
		existing_overrides = creqit.db.get_all(
		...
		for override in existing_overrides:
			creqit.delete_doc("Amended Document Naming Settings", override)

		for row in self.amend_naming_override:
			row.save()

		creqit.msgprint(_("Amendment naming rules updated."), indicator="green", alert=True)
	...
	def update_series_start(self):
		...
		if self.prefix is None:
			creqit.throw(_("Please select prefix first"))
	...
	def create_version_log_for_change(self, series, old, new):
		...
		version.data = creqit.as_json({"changed": [["current", old, new]]})
	...
	def _fetch_last_doc_if_available(self):
		...
		try:
			return creqit.get_last_doc(self.transaction_type)
		except Exception:
			return None
