...
from creqit.core.doctype.data_import.exporter import Exporter
from creqit.core.doctype.data_import.importer import Importer
...
from creqit.utils.csvutils import validate_google_sheets_url
...
class DataImport(Document):
	...
	def set_delimiters_flag(self):
		if self.import_file:
			creqit.flags.delimiter_options = self.delimiter_options or ","

	def validate_doctype(self):
		if self.reference_doctype in BLOCKED_DOCTYPES:
			creqit.throw(_("Importing {0} is not allowed.").format(self.reference_doctype))
	...
	def start_import(self):
		...
		run_now = creqit.flags.in_test or creqit.conf.developer_mode
		if is_scheduler_inactive() and not run_now:
			creqit.throw(_("Scheduler is inactive. Cannot import data."), title=_("Scheduler Inactive"))
...
def get_preview_from_template(data_import, import_file=None, google_sheets_url=None):
	return creqit.get_doc("Data Import", data_import).get_preview_from_template(
...
def form_start_import(data_import: str):
	return creqit.get_doc("Data Import", data_import).start_import()


def start_import(data_import):
	"""This method runs in background job"""
	data_import = creqit.get_doc("Data Import", data_import)
	try:
	...
	creqit.publish_realtime("data_import_refresh", {"data_import": data_import.name})
...
def download_template(doctype, export_fields=None, export_records=None, export_filters=None, file_type="CSV"):
	...
	export_fields = creqit.parse_json(export_fields)
	export_filters = creqit.parse_json(export_filters)
...
def download_errored_template(data_import_name):
	data_import = creqit.get_doc("Data Import", data_import_name)
...
def get_import_status(data_import_name):
	...
	total_payload_count = creqit.db.get_value("Data Import", data_import_name, "payload_count")
...
def get_import_logs(data_import: str):
	doc = creqit.get_doc("Data Import", data_import)
...
def import_file(doctype, file_path, import_type, submit_after_import=False, console=False):
	...
	data_import = creqit.new_doc("Data Import")
...
def export_json(doctype, path, filters=None, or_filters=None, name=None, order_by="creation asc"):
	...
	out = []
	if name:
		out.append(creqit.get_doc(doctype, name).as_dict())
	elif creqit.db.get_value("DocType", doctype, "issingle"):
		out.append(creqit.get_doc(doctype).as_dict())
	else:
		for doc in creqit.get_all(
			doctype,
			fields=["name"],
			filters=filters,
			...
			out.append(creqit.get_doc(doctype, doc.name).as_dict())
	post_process(out)
	...
	with open(path, "w") as outfile:
		outfile.write(creqit.as_json(out, ensure_ascii=False))


def export_csv(doctype, path):
	from creqit.core.doctype.data_export.exporter import export_data

	with open(path, "wb") as csvfile:
		export_data(doctype=doctype, all_doctypes=True, template=True, with_data=True)
		csvfile.write(creqit.response.result.encode("utf-8"))
