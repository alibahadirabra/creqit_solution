...
from creqit.utils import cstr, get_site_url
...
class TestAccessLog(IntegrationTestCase):
	def setUp(self):
		...
		generate_keys(creqit.session.user)
		creqit.db.commit()
		generated_secret = creqit.utils.password.get_decrypted_password(
			"User", creqit.session.user, fieldname="api_secret"
		)
		...
		self.file_name = creqit.utils.random_string(10) + ".txt"
		self.test_content = creqit.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = creqit.get_last_doc("Access Log")
	...
	def test_private_file_download(self):
		...
		new_private_file = creqit.get_doc(
			{
				"doctype": self.test_doctype,
		...
		private_file_link = get_site_url(creqit.local.site) + new_private_file.file_url
