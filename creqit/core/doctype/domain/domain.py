...
class Domain(Document):
	...
	def setup_domain(self):
		...
		if not cint(creqit.defaults.get_defaults().setup_complete):
		...
		if self.data.on_setup:
			...
			creqit.get_attr(self.data.on_setup)()
	...
	def remove_domain(self):
		...
		if self.data.restricted_roles:
			for role_name in self.data.restricted_roles:
				if creqit.db.exists("Role", role_name):
					role = creqit.get_doc("Role", role_name)
					role.disabled = 1
					role.save()
		...
		if self.data.custom_fields:
			for doctype in self.data.custom_fields:
				...
				for custom_field_detail in custom_fields:
					custom_field_name = creqit.db.get_value(
	...
	def setup_roles(self):
		...
		if self.data.restricted_roles:
			...
			for role_name in self.data.restricted_roles:
				...
				if not creqit.db.get_value("Role", role_name):
					creqit.get_doc(doctype="Role", role_name=role_name).insert()
					continue
	...
	def setup_data(self, domain=None):
		...
		self.data = creqit.get_domain_data(self.name)

	def get_domain_data(self, module):
		return creqit.get_attr(creqit.get_hooks("domains")[self.name] + ".data")
	...
	def setup_properties(self):
		if self.data.properties:
			for args in self.data.properties:
				creqit.make_property_setter(args)

	def set_values(self):
		"""set values based on `data.set_value`"""
		if self.data.set_value:
			for args in self.data.set_value:
				creqit.reload_doctype(args[0])
				doc = creqit.get_doc(args[0], args[1] or args[0])
	...
	def setup_sidebar_items(self):
		...
		if self.data.allow_sidebar_items:
			# disable all
			creqit.db.sql("update `tabPortal Menu Item` set enabled=0")
		...
		if self.data.remove_sidebar_items:
			# disable all
			creqit.db.sql("update `tabPortal Menu Item` set enabled=1")
