creqit.ui.form.on("Communication", {
	onload: function (frm) {
		if (frm.doc.content) {
			frm.doc.content = creqit.dom.remove_script_and_style(frm.doc.content);
		}
			...
			fields: [
				{
					...
					get_query: function () {
						return { query: "creqit.email.get_communication_doctype" };
					},
				},
				{
	...
	show_move_dialog: function (frm) {
		var d = new creqit.ui.Dialog({
			...
			primary_action(values) {
				d.hide();
				creqit.call({
					method: "creqit.email.inbox.move_email",
					args: {
	...
	mark_as_read_unread: function (frm) {
		...
		return creqit.call({
			method: "creqit.email.inbox.create_email_flag_queue",
			args: {
				names: [frm.doc.name],
				action: action,
				flag: flag,
	...
	mark_as_closed_open: function (frm) {
		...
		return creqit.call({
			method: "creqit.email.inbox.mark_as_closed_open",
			args: {
				communication: frm.doc.name,
				status: status,
			},
	...
	get_mail_args: function (frm) {
		var sender_email_id = "";
		$.each(creqit.boot.email_accounts, function (idx, account) {
	...
	add_to_contact: function (frm) {
		...
		creqit.new_doc("Contact");
	},

	mark_as_spam: function (frm) {
		creqit.call({
			method: "creqit.email.inbox.mark_as_spam",
			args: {
				communication: frm.doc.name,
			...
			freeze: true,
			callback: function (r) {
				creqit.msgprint(__("Email has been marked as spam"));
			},
		});
	},
	...
	move_to_trash: function (frm) {
		creqit.call({
			method: "creqit.email.inbox.mark_as_trash",
			args: {
				communication: frm.doc.name,
			},
			freeze: true,
			callback: function (r) {
				creqit.msgprint(__("Email has been moved to trash"));
			},
		});
	},
});
