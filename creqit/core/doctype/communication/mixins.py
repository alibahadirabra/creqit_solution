...
from creqit.utils import cstr, get_formatted_email, get_url, parse_addr
...
class CommunicationEmailMixin:
	...
	def mail_cc(self, is_inbound_mail_communcation=False, include_sender=False):
		...
		* Remove standard users from email list
		...
		if include_sender:
			...
			if creqit.session.user not in creqit.STANDARD_USERS:
				sender = creqit.db.get_value("User", creqit.session.user, "email")
			cc.append(sender)
		...
		self._final_cc = [m for m in cc if m and m not in creqit.STANDARD_USERS]
		return self._final_cc
	...
	def mail_bcc(self, is_inbound_mail_communcation=False):
		...
		* remove standard users.
		...
		self._final_bcc = [m for m in bcc if m not in creqit.STANDARD_USERS]
		return self._final_bcc
	...
	def get_content(self, print_format=None):
		if print_format and creqit.get_system_settings("attach_view_link"):
	...
	def get_attach_link(self, print_format):
		...
		return creqit.get_template("templates/emails/print_link.html").render(
			{
				"url": get_url(),
		...
		if not hasattr(self, "_outgoing_email_account"):
			if self.email_account:
				...
				if self.sent_or_received == "Sent" and self._outgoing_email_account:
					if creqit.db.exists("Email Account", self._outgoing_email_account.name):
	...
	def mail_attachments(self, print_format=None, print_html=None, print_language=None):
		...
		if print_format or print_html:
			d = {
				...
				"doctype": self.reference_doctype,
				"name": self.reference_name,
				"lang": print_language or creqit.local.lang,
			}
	...
	def filter_thread_notification_disbled_users(emails):
		...
		return creqit.get_all("User", pluck="email", filters={"email": ["in", emails], "thread_notify": 0})
	...
	def filter_disabled_users(emails):
		...
		return creqit.get_all("User", pluck="email", filters={"email": ["in", emails], "enabled": 0})
	...
	def send_email(
		...
		if input_dict := self.sendmail_input_dict(
			...
			creqit.sendmail(now=now, **input_dict)
