...
creqit.ui.form.on("Recorder", {
	...
	refresh: function (frm) {
		...
		frm.add_custom_button(__("Suggest Optimizations"), () => {
			creqit.xcall("creqit.core.doctype.recorder.recorder.optimize", {
				recorder_id: frm.doc.name,
			});
		});

		creqit.realtime.on("recorder-analysis-complete", () => {
			frm.reload_doc();
		...
		index_grid.add_custom_button(__("Add Indexes"), function () {
			...
			if (!indexes_to_add.length) {
				creqit.toast(__("You need to select indexes you want to add first."));
				return;
			}
			creqit.xcall("creqit.core.doctype.recorder.recorder.add_indexes", {
...
creqit.ui.form.on("Recorder Query", "form_render", function (frm, cdt, cdn) {
	let row = creqit.get_doc(cdt, cdn);
