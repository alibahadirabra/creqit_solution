creqit.listview_settings["Recorder"] = {
	...
	onload(listview) {
		...
		listview.page.add_button(__("Clear"), () => {
			creqit.xcall("creqit.recorder.delete").then(listview.refresh);
		});
		...
		listview.page.add_menu_item(__("Import"), () => {
			new creqit.ui.FileUploader({
				...
				on_success: (file) => {
					...
					creqit.call({
						method: "creqit.recorder.import_data",
						args: {
							file: file.file_url,
						},
						callback: function () {
		...
		listview.page.add_menu_item(__("Export"), () => {
			creqit.call({
				method: "creqit.recorder.export_data",
				callback: function (r) {
	...
	fetch_recorder_status(listview) {
		return creqit.xcall("creqit.recorder.status").then((status) => {
	...
	stop_recorder(listview) {
		let me = this;
		creqit.xcall("creqit.recorder.stop", {}).then(() => {
			listview.refresh();
	...
	start_recorder(listview) {
		...
		creqit.prompt(
			[
				...
				},
				{
					...
					description: `This will be used for filtering paths which will be recorded.
						...
						e.g. <code>/api/method/creqit</code>. Leave it empty to record every request.`,
				},
				{
					fieldtype: "Column Break",
			...
			(values) => {
				creqit.xcall("creqit.recorder.start", values).then(() => {
					listview.refresh();
