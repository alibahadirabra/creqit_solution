creqit.ui.form.on("System Settings", {
	refresh: function (frm) {
		creqit.call({
			method: "creqit.core.doctype.system_settings.system_settings.load",
			callback: function (data) {
				creqit.all_timezones = data.message.timezones;
				frm.set_df_property("time_zone", "options", creqit.all_timezones);

				$.each(data.message.defaults, function (key, val) {
					frm.set_value(key, val, null, true);
					creqit.sys_defaults[key] = val;
				});
				if (frm.re_setup_moment) {
					creqit.app.setup_moment();
	...
	after_save: function (frm) {
		...
		const attr_tuples = [
			[frm.doc.language, creqit.boot.sysdefaults.language, creqit.boot.user.language],
			[frm.doc.rounding_method, creqit.boot.sysdefaults.rounding_method], // no user override.
		];
	...
	rounding_method: function (frm) {
		if (frm.doc.rounding_method == creqit.boot.sysdefaults.rounding_method) return;
		let msg = __(
		...
		creqit.confirm(
			...
			() => {},
			() => {
				frm.set_value("rounding_method", creqit.boot.sysdefaults.rounding_method);
			}
		);
	},
