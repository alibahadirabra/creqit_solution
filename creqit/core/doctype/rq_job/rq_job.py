...
from creqit.utils.background_jobs import get_queues, get_redis_conn
...
class RQJob(Document):
	...
	def stop_job(self):
		try:
			...
			creqit.msgprint(_("Job is not running."), title=_("Invalid Operation"))
...
def serialize_job(job: Job) -> creqit._dict:
	...
	if job_name == "creqit.utils.background_jobs.run_doc_method":
	...
	return creqit._dict(
		...
		exc_info=exc_info,
		arguments=creqit.as_json(job.kwargs),
		timeout=job.timeout,
...
def for_current_site(job: Job) -> bool:
	return job.kwargs.get("site") == creqit.local.site
...
@creqit.whitelist()
def stop_job(job_id):
	creqit.get_doc("RQ Job", job_id).stop_job()
