...
from creqit.permissions import add_permission, add_user_permission
from creqit.utils import get_link_to_form
from creqit.utils.modules import get_modules_from_app
...
class UserType(Document):
	...
	if TYPE_CHECKING:
		from creqit.core.doctype.user_document_type.user_document_type import UserDocumentType
		from creqit.core.doctype.user_select_document_type.user_select_document_type import (
			UserSelectDocumentType,
		)
		from creqit.core.doctype.user_type_module.user_type_module import UserTypeModule
	...
	def clear_cache(self):
		...
		if not self.is_standard:
			creqit.cache.delete_value("non_standard_user_types")
	...
	def on_trash(self):
		if self.is_standard:
			creqit.throw(_("Standard user type {0} can not be deleted.").format(creqit.bold(self.name)))

	def set_modules(self):
		if not self.user_doctypes:
			return

		modules = creqit.get_all(
			"DocType",
	...
	def validate_document_type_limit(self):
		limit = creqit.conf.get("user_type_doctype_limit", {}).get(creqit.scrub(self.name))

		if not limit and creqit.session.user != "Administrator":
			creqit.throw(
				_("User does not have permission to create the new {0}").format(creqit.bold(_("User Type"))),
	...
	def validate_role(self):
		if not self.role:
			creqit.throw(_("The field {0} is mandatory").format(creqit.bold(_("Role"))))

		if not creqit.db.get_value("Role", self.role, "is_custom"):
			creqit.throw(
				_("The role {0} should be a custom role.").format(
					creqit.bold(get_link_to_form("Role", self.role))
				)
			)

	def update_users(self):
		for row in creqit.get_all("User", filters={"user_type": self.name}):
			user = creqit.get_cached_doc("User", row.name)
	...
	def add_select_perm_doctypes(self):
		if creqit.flags.ignore_select_perm:
			return
	...
	def remove_permission_for_deleted_doctypes(self):
		...
		for perm in creqit.get_all(
			"Custom DocPerm", filters={"role": self.role, "parent": ["not in", doctypes]}
		):
			creqit.delete_doc("Custom DocPerm", perm.name)


def add_role_permissions(doctype, role):
	name = creqit.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=0))
...
def get_user_linked_doctypes(doctype, txt, searchfield, start, page_len, filters):
	modules = [d.get("module_name") for d in get_modules_from_app("creqit")]
	...
	custom_doctypes = creqit.get_all(
...
def user_linked_with_permission_on_doctype(doc, user):
	...
	if not doc.user_id_field:
		creqit.throw(_("User Id Field is mandatory in the user type {0}").format(creqit.bold(doc.name)))

	if creqit.db.get_value(doc.apply_user_permission_on, {doc.user_id_field: user}, "name"):
		return True
	else:
		label = creqit.get_meta(doc.apply_user_permission_on).get_field(doc.user_id_field).label

		creqit.msgprint(
			_(
				"To set the role {0} in the user {1}, kindly set the {2} field as {3} in one of the {4} record."
			).format(
				creqit.bold(doc.role),
				creqit.bold(user),
				creqit.bold(label),
				creqit.bold(user),
				creqit.bold(doc.apply_user_permission_on),
			)
		)
...
def apply_permissions_for_non_standard_user_type(doc, method=None):
	...
	if not creqit.db.table_exists("User Type") or creqit.flags.in_migrate:
		return

	user_types = creqit.cache.get_value(
	...
	for user_type, data in user_types.items():
		...
		if doc.get(data[1]) and (
			...
			if not perm_data:
				...
				user_perm = creqit.get_doc("User Permission", perm_data[0])
