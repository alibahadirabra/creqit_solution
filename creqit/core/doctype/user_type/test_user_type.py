...
class TestUserType(IntegrationTestCase):
	...
	def test_add_select_perm_doctypes(self):
		...
		doc = creqit.get_meta("Contact")
		link_fields = doc.get_link_fields()
		select_doctypes = creqit.get_all(
	...
	def test_print_share_email_default(self):
		...
		perm = creqit.get_all("Custom DocPerm", filters={"role": "_Test User Type"}, fields=["*"])[0]
...
def create_user_type(user_type):
	if creqit.db.exists("User Type", user_type):
		creqit.delete_doc("User Type", user_type)

	user_type_limit = {creqit.scrub(user_type): 1}
...
def create_role():
	if not creqit.db.exists("Role", "_Test User Type"):
		creqit.get_doc(
