creqit.listview_settings["DocType"] = {
	...
	new_doctype_dialog(args) {
		...
		let non_developer = creqit.session.user !== "Administrator" || !creqit.boot.developer_mode;
		let fields = [
			{
		...
		let new_d = new creqit.ui.Dialog({
			...
			primary_action(values) {
				...
				creqit.db
					...
					.then((doc) => {
						creqit.set_route("Form", "DocType", doc.name);
					});
			},
			secondary_action_label: __("Cancel"),
			secondary_action() {
				new_d.hide();
				if (creqit.get_route()[0] === "Form") {
