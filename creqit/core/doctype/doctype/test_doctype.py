...
import creqit
from creqit.cache_manager import clear_doctype_cache
...
from creqit.core.doctype.rq_job.test_rq_job import wait_for_completion
from creqit.custom.doctype.custom_field.custom_field import create_custom_fields
from creqit.desk.form.load import getdoc
from creqit.model.delete_doc import delete_controllers
from creqit.model.sync import remove_orphan_doctypes
...
class TestDocType(IntegrationTestCase):
	...
	def test_validate_name(self):
		self.assertRaises(creqit.NameError, new_doctype("_Some DocType").insert)
		self.assertRaises(creqit.NameError, new_doctype("8Some DocType").insert)
		self.assertRaises(creqit.NameError, new_doctype("Some (DocType)").insert)
		self.assertRaises(
			creqit.NameError,
		...
		for name in ("Some DocType", "Some_DocType", "Some-DocType"):
			if creqit.db.exists("DocType", name):
				creqit.delete_doc("DocType", name)

			doc = new_doctype(name).insert()
			doc.delete()

	def test_making_sequence_on_change(self):
		creqit.delete_doc_if_exists("DocType", self._testMethodName)
		...
		self.assertEqual(
			...
			"varchar" if creqit.db.db_type == "mariadb" else "character varying",
		)

	def test_doctype_unique_constraint_dropped(self):
		if creqit.db.exists("DocType", "With_Unique"):
			creqit.delete_doc("DocType", "With_Unique")

		dt = new_doctype("With_Unique", unique=1)
		dt.insert()

		doc1 = creqit.new_doc("With_Unique")
		doc2 = creqit.new_doc("With_Unique")
		...
		doc1.insert()
		self.assertRaises(creqit.UniqueValidationError, doc2.insert)
		creqit.db.rollback()
	...
	def test_validate_search_fields(self):
		...
		self.assertRaises(creqit.ValidationError, doc.save)
	...
	def test_sync_field_order(self):
		...
		creqit.delete_doc_if_exists("DocType", "Test Field Order DocType")
		if os.path.isfile(path):
			os.remove(path)

		try:
			creqit.flags.allow_doctype_export = 1
			test_doctype.save()

			# assert that field_order list is being created with the default order
			test_doctype_json = creqit.get_file_json(path)
			...
			with open(path, "w+") as txtfile:
				txtfile.write(creqit.as_json(test_doctype_json))
			...
			creqit.reload_doctype(test_doctype.name, force=True)
			test_doctype.reload()
			...
			creqit.flags.allow_doctype_export = 0
	...
	def test_cancel_link_doctype(self):
		...
		from creqit.desk.form.linked_with import cancel_all_linked_docs, get_submitted_linked_docs
		...
		# create doctype data
		data_link_doc = creqit.new_doc("Test Linked Doctype")
		data_link_doc.some_fieldname = "Data1"
		data_link_doc.insert()
		data_link_doc.save()
		data_link_doc.submit()

		data_doc = creqit.new_doc("Test Doctype")
	...
	def test_ignore_cancelation_of_linked_doctype_during_cancel(self):
		...
		data_doc = creqit.new_doc("Test Doctype 2")
		data_doc.some_fieldname = "Data1"
		...
		self.assertRaises(creqit.LinkExistsError, data_link_doc_1.cancel)
	...
	def test_create_virtual_doctype(self):
		...
		virtual_doc.reload()
		doc = creqit.get_doc("DocType", "Test Virtual Doctype")

		self.assertDictEqual(doc.as_dict(), virtual_doc.as_dict())
		self.assertEqual(doc.is_virtual, 1)
		self.assertFalse(creqit.db.table_exists("Test Virtual Doctype"))

	def test_create_virtual_doctype_as_child_table(self):
		"""Test virtual DocType as Child Table below a normal DocType."""
		creqit.delete_doc_if_exists("DocType", "Test Parent Virtual DocType", force=1)
		creqit.delete_doc_if_exists("DocType", "Test Virtual DocType as Child Table", force=1)
		...
		doc = creqit.get_doc("DocType", "Test Virtual DocType as Child Table")

		self.assertEqual(doc.is_virtual, 1)
		self.assertEqual(doc.istable, 1)
		self.assertFalse(creqit.db.table_exists("Test Virtual DocType as Child Table"))
		...
		self.assertRaises(creqit.exceptions.ValidationError, parent_doc.insert)
		parent_doc.is_virtual = 1
		parent_doc.insert(ignore_permissions=True)
		self.assertFalse(creqit.db.table_exists("Test Parent Virtual DocType"))
	...
	def test_autoincremented_doctype_transition(self):
		creqit.delete_doc_if_exists("DocType", "testy_autoinc_dt")
		dt = new_doctype("testy_autoinc_dt", autoname="autoincrement").insert(ignore_permissions=True)
		dt.autoname = "hash"

		dt.save(ignore_permissions=True)

		dt_data = creqit.get_doc({"doctype": dt.name, "some_fieldname": "test data"}).insert(
		...
		try:
			dt.save(ignore_permissions=True)
		except creqit.ValidationError as e:
			self.assertEqual(
				e.args[0],
	...
	def test_json_field(self):
		...
		json_doc.insert()
		json_doc.save()
		doc = creqit.get_doc("DocType", "Test Json Doctype")
		...
		test_json = creqit.get_doc("Test Json Doctype", doc.name)
	...
	def test_no_delete_doc(self):
		self.assertRaises(creqit.ValidationError, creqit.delete_doc, "DocType", "Address")
	...
	def test_custom_field_deletion(self):
		...
		creqit.delete_doc("DocType", child)
		self.assertFalse(creqit.get_meta(doctype).get_field(field))
	...
	def test_delete_doctype_with_customization(self):
		...
		# Create 1 record
		original_doc = creqit.get_doc(doctype=doctype, custom_field_name="wat").insert()
		self.assertEqual(original_doc.some_fieldname, "DELETETHIS")

		# delete doctype
		creqit.delete_doc("DocType", doctype)
		...
		restored_doc = creqit.get_last_doc(doctype)
		...
		self.assertEqual(creqit.get_meta(doctype).get_field(field).default, "DELETETHIS")
		creqit.delete_doc("DocType", doctype)
	...
	def test_delete_orphaned_doctypes(self):
		...
		job = creqit.enqueue(remove_orphan_doctypes)
		wait_for_completion(job)

		creqit.db.rollback()
		self.assertFalse(creqit.db.exists("DocType", doctype.name))
	...
	def test_not_in_list_view_for_not_allowed_mandatory_field(self):
		...
		creqit.delete_doc("DocType", doctype.name)
	...
	def test_no_recursive_fetch(self):
		...
		self.assertRaises(creqit.ValidationError, recursive_dt.insert)
