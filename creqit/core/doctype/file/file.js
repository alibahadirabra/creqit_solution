creqit.ui.form.on("File", {
	...
	preview_file: function (frm) {
		...
		if (creqit.utils.is_image_file(frm.doc.file_url)) {
			$preview = $(`<div class="img_preview">
				<img
					class="img-responsive"
					src="${creqit.utils.escape_html(frm.doc.file_url)}"
					onerror="${frm.toggle_display("preview", false)}"
				/>
			</div>`);
		} else if (creqit.utils.is_video_file(frm.doc.file_url)) {
			$preview = $(`<div class="img_preview">
				<video width="480" height="320" controls>
					<source src="${creqit.utils.escape_html(frm.doc.file_url)}">
			...
			$preview = $(`<div class="img_preview">
				<object style="background:#323639;" width="100%">
					<embed
						...
						width="100%"
						height="1190"
						src="${creqit.utils.escape_html(frm.doc.file_url)}" type="application/pdf"
					>
				</object>
			</div>`);
			...
			$preview = $(`<div class="img_preview">
				<audio width="480" height="60" controls>
					<source src="${creqit.utils.escape_html(frm.doc.file_url)}" type="audio/mpeg">
	...
	optimize: function (frm) {
		creqit.show_alert(__("Optimizing image..."));
		frm.call("optimize_file").then(() => {
			creqit.show_alert(__("Image optimized"));
		});
	},

	unzip: function (frm) {
		creqit.call({
			method: "creqit.core.api.file.unzip_file",
			args: {
				name: frm.doc.name,
			},
			callback: function () {
