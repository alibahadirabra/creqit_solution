...
import creqit
from creqit.templates.includes.comments.comments import add_comment
from creqit.tests import IntegrationTestCase, UnitTestCase
from creqit.tests.test_model_utils import set_user
...
class TestComment(IntegrationTestCase):
	def test_comment_creation(self):
		test_doc = creqit.get_doc(doctype="ToDo", description="test")
		test_doc.insert()
	...
	def test_public_comment(self):
		...
		creqit.db.delete("Comment", {"reference_doctype": "Blog Post"})
		add_comment_args = {
	...
	def test_user_not_logged_in(self):
		some_system_user = creqit.db.get_value("User", {"name": ("not in", creqit.STANDARD_USERS)})
