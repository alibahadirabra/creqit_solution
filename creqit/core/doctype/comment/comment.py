...
from creqit.desk.notifications import notify_mentions
from creqit.exceptions import ImplicitCommitError
...
class Comment(Document):
	...
	def validate(self):
		if not self.comment_email:
			self.comment_email = creqit.session.user
		self.content = creqit.utils.sanitize_html(self.content, always_sanitize=True)
...
def on_doctype_update():
	creqit.db.add_index("Comment", ["reference_doctype", "reference_name"])
...
def get_comments_from_parent(doc):
	...
	try:
		if is_virtual_doctype(doc.reference_doctype):
			...
			_comments = creqit.db.get_value(doc.reference_doctype, doc.reference_name, "_comments") or "[]"

	except Exception as e:
		if creqit.db.is_missing_table_or_column(e):
			_comments = "[]"
...
def update_comments_in_parent(reference_doctype, reference_name, _comments):
	...
	if (
		...
		or creqit.db.get_value("DocType", reference_doctype, "issingle")
	...
	try:
		...
		if creqit.db.is_missing_column(e) and getattr(creqit.local, "request", None):
			pass
		elif creqit.db.is_data_too_long(e):
			raise creqit.DataTooLongException
		else:
			raise
	else:
		...
		# Clear route cache
		if route := creqit.get_cached_value(reference_doctype, reference_name, "route"):
			clear_cache(route)
