...
class Dashboard {
	constructor(wrapper) {
		...
		//div içerisindeki style den  margin: var(--margin-sm); kodu kaldıırlmıştır. <<creqit.v1.sevval
	...
	show() {
		...
		if (this.route.length > 1) {
			...
			// last opened
			if (creqit.last_dashboard) {
				creqit.set_re_route("dashboard-view", creqit.last_dashboard);
			} else {
				// default dashboard
				creqit.db.get_list("Dashboard", { filters: { is_default: 1 } }).then((data) => {
					if (data && data.length) {
						creqit.set_re_route("dashboard-view", data[0].name);
					} else {
						// no default, get the latest one
						creqit.db.get_list("Dashboard", { limit: 1 }).then((data) => {
							if (data && data.length) {
								creqit.set_re_route("dashboard-view", data[0].name);
							} else {
								// create a new dashboard!
								creqit.new_doc("Dashboard");
							}
						});
					}
				});
	...
	show_dashboard(current_dashboard_name) {
		...
		this.charts = {};
		creqit.last_dashboard = current_dashboard_name;
	}

	set_breadcrumbs() {
		creqit.breadcrumbs.add("Desk", "Dashboard");
	}

	refresh() {
		creqit.run_serially([() => this.render_cards(), () => this.render_charts()]);
	}

	render_charts() {
		return this.get_permitted_items(
			"creqit.desk.doctype.dashboard.dashboard.get_permitted_charts"
		).then((charts) => {
			if (!charts.length) {
				creqit.msgprint(
			...
			creqit.dashboard_utils.get_dashboard_settings().then((settings) => {
				...
				this.chart_group = new creqit.widget.WidgetGroup({
					title: null,
	...
	render_cards() {
		return this.get_permitted_items(
			"creqit.desk.doctype.dashboard.dashboard.get_permitted_cards"
		).then((cards) => {
			if (!cards.length) {
				return;
			}
			...
			this.number_card_group = new creqit.widget.WidgetGroup({
				container: this.container,
				type: "number_card",
				columns: 3,
				options: {
	...
	set_dropdown() {
		...
		this.page.add_menu_item(__("Edit"), () => {
			creqit.set_route("Form", "Dashboard", creqit.dashboard.dashboard_name);
		});
		...
		creqit.db.get_list("Dashboard").then((dashboards) => {
			dashboards.map((dashboard) => {
				let name = dashboard.name;
				if (name != this.dashboard_name) {
					this.page.add_menu_item(
						name,
						() => creqit.set_route("dashboard-view", name),
						1
					);
				}
			});
