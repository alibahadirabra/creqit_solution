creqit.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = creqit.ui.make_app_page({
		parent: wrapper,
	...
	$(creqit.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new creqit.PermissionEngine(wrapper);
};

creqit.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

creqit.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page.add_field({
			...
			options: "DocType",
			change: function () {
				creqit.set_route("permission-manager", this.get_value());
			},
		});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return creqit.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (creqit.get_route()[1]) {
			this.doctype_select.set_value(creqit.get_route()[1]);
		} else if (creqit.route_options) {
			if (creqit.route_options.doctype) {
				this.doctype_select.set_value(creqit.route_options.doctype);
			}
			if (creqit.route_options.role) {
				this.role_select.set_value(creqit.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = creqit.confirm(__("Reset Permissions for {0}?", [__(doctype)]), () => {
			return creqit
				.call({
					module: "creqit.core",
					page: "permission_manager",
		...
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(creqit.unscrub(r)));
				});
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${creqit.utils.icon(
				"delete"
			)}</button>`
		)
			...
			.on("click", () => {
				return creqit.call({
					...
					callback: (r) => {
						if (r.exc) {
							creqit.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			creqit.route_options = { allow: this.get_doctype() || "" };
			...
			() => {
				...
				d.set_primary_action(__("Add"), () => {
					...
					creqit.call({
						...
						callback: (r) => {
							if (r.exc) {
								creqit.msgprint(__("Did not add"));
							} else {
								this.refresh();
							}
						},
	...
	get_link_fields(doctype) {
		return creqit.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",
