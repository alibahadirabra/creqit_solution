...
import creqit
from creqit.defaults import _clear_cache
from creqit.utils import cint, is_git_url
...
def _is_scheduler_enabled(site) -> bool:
	...
	try:
		creqit.init(site)
		creqit.connect()
		enable_scheduler = cint(creqit.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
		creqit.db.close()
...
def _new_site(
	...
	"""Install a new creqit site"""

	from creqit.utils import scheduler
	...
	if not db_name:
		db_name = f"_{creqit.generate_hash(length=16)}"
	...
	if rollback_callback:
		rollback_callback.add(lambda: shutil.rmtree(creqit.get_site_path()))
	...
	with filelock("bench_new_site", timeout=1):
		...
		apps_to_install = ["creqit"] + (creqit.conf.get("install_apps") or []) + (list(install_apps or []))
	...
	scheduler_status = "disabled" if creqit.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from creqit.database import bootstrap_database, drop_user_and_database, setup_database
	...
	creqit.flags.in_install_db = True

	creqit.flags.root_login = root_login
	creqit.flags.root_password = root_password
	...
	creqit.conf.admin_password = creqit.conf.admin_password or admin_password

	remove_missing_apps()

	creqit.db.create_auth_table()
	creqit.db.create_global_search_table()
	creqit.db.create_user_settings_table()

	creqit.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	find_org()
	ref -> https://github.com/creqit/bench/blob/develop/bench/utils/__init__.py#L390
	...
	from creqit.exceptions import InvalidRemoteException

	for org in ["creqit", "creqit"]:
...
def fetch_details_from_tag(_tag: str) -> tuple[str, str, str]:
	...
	ref -> https://github.com/creqit/bench/blob/develop/bench/utils/__init__.py#L403
...
def parse_app_name(name: str) -> str:
	...
	ref -> https://github.com/creqit/bench/blob/develop/bench/app.py#L114
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	...
	from creqit.model.sync import sync_for
	from creqit.modules.utils import sync_customizations
	from creqit.utils.fixtures import sync_fixtures

	creqit.flags.in_install = name
	creqit.flags.ignore_in_install = False

	creqit.clear_cache()
	app_hooks = creqit.get_hooks(app_name=name)
	installed_apps = creqit.get_installed_apps()
	...
	if name not in creqit.get_all_apps():
	...
	for before_install in app_hooks.before_install or []:
		out = creqit.get_attr(before_install)()
		if out is False:
			return

	for fn in creqit.get_hooks("before_app_install"):
		creqit.get_attr(fn)(name)
	...
	creqit.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		creqit.get_attr(after_install)()

	for fn in creqit.get_hooks("after_app_install"):
		creqit.get_attr(fn)(name)
	...
	for after_sync in app_hooks.after_sync or []:
		creqit.get_attr(after_sync)()  #

	creqit.flags.in_install = False
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		creqit.db.set_global("installed_apps", json.dumps(installed_apps))
		creqit.db.commit()
		if creqit.flags.in_install:
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = creqit.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in creqit.get_installed_apps():
	...
	for app in creqit.get_installed_apps():
		if app != app_name:
			hooks = creqit.get_hooks(app_name=app)
	...
	if not (dry_run or no_backup):
		from creqit.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	creqit.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		creqit.get_attr(before_uninstall)()

	for fn in creqit.get_hooks("before_app_uninstall"):
		creqit.get_attr(fn)(app_name)

	modules = creqit.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	for after_uninstall in app_hooks.after_uninstall or []:
		creqit.get_attr(after_uninstall)()

	for fn in creqit.get_hooks("after_app_uninstall"):
		creqit.get_attr(fn)(app_name)

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	creqit.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in creqit.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					creqit.delete_doc("DocType", doctype.name, ignore_on_trash=True, force=True)
				else:
		...
		if not dry_run:
			creqit.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(module_name: str, doctype_linkfield_map: dict[str, str], dry_run: bool) -> None:
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in creqit.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				creqit.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = creqit.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if creqit.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			creqit.delete_doc("DocType", doctype, ignore_on_trash=True, force=True)
			creqit.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from creqit.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		creqit.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	creqit.db.commit()


def init_singles():
	singles = creqit.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if creqit.db.get_singles_dict(single):
			continue

		try:
			doc = creqit.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, creqit.DoesNotExistError):
...
def make_conf(
	...
	sites_path = creqit.local.sites_path
	creqit.destroy()
	creqit.init(site, sites_path=sites_path)
...
def make_site_config(
	...
	creqit.create_folder(os.path.join(creqit.local.site_path))
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(creqit.local, "conf"):
		creqit.local.conf[key] = value


def get_site_config_path():
	return os.path.join(creqit.local.site_path, "site_config.json")
...
def make_site_dirs():
	for dir_path in [
		...
		path = creqit.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = creqit.get_module_list(app)
	for module in modules:
		d = creqit.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	apps = ("creqit_subscription", "shopping_cart")
	installed_apps = json.loads(creqit.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_files(site_name, file_path):
	...
	from creqit.utils import get_bench_relative_path

	file_path = get_bench_relative_path(file_path)

	# Need to do creqit.init to maintain the site locals
	creqit.init(site_name)
	abs_site_path = os.path.abspath(creqit.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	...
	current_version = Version(creqit.__version__)
	...
	if verbose and is_downgrade:
		print(f"Your site is currently on creqit {current_version} and your backup is {backup_version}.")

	return is_downgrade


def get_old_backup_version(sql_file_path: str) -> Version | None:
	"""Return the creqit version used to create the specified database dump.

	This methods supports older versions of creqit wich used a different format.
	"""
	header = get_db_dump_header(sql_file_path).split("\n")
	if match := re.search(r"creqit (\d+\.\d+\.\d+)", header[0]):
...
def get_backup_version(sql_file_path: str) -> Version | None:
	...
	header = get_db_dump_header(sql_file_path).split("\n")
	metadata = ""
	if "begin creqit metadata" in header[0]:
		for line in header[1:]:
			if "end creqit metadata" in line:
				break
			metadata += line.replace("--", "").strip() + "\n"
		parser = configparser.ConfigParser()
		parser.read_string(metadata)
		return Version(parser["creqit"]["version"])
...
def partial_restore(sql_file_path, verbose=False):
	if creqit.conf.db_type == "mariadb":
		from creqit.database.mariadb.setup_db import import_db_from_sql
	elif creqit.conf.db_type == "postgres":
		import warnings

		from creqit.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path: str, _raise: bool = True) -> None:
	...
	if os.path.getsize(path):
		if (executable := which(executable_name)) is None:
			creqit.throw(
				...
				exc=creqit.ExecutableNotFound,
			)
		try:
			creqit.utils.execute_in_shell(f"{executable} -m1 __Auth {path}", check_exit_code=True)
			return
		except Exception:
	...
	if _raise:
		raise creqit.InvalidDatabaseFile
