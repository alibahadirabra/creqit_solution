...
def get_apps():
	...
	for app in apps:
		if app == "creqit":
			continue
		app_details = creqit.get_hooks("add_to_apps_screen", app_name=app)
		if not len(app_details):
			continue
		for app_detail in app_details:
			has_permission_path = app_detail.get("has_permission")
			if has_permission_path and not creqit.get_attr(has_permission_path)():
				continue
			app_list.append(
				{
					"name": app,
...
def get_default_path():
	apps = get_apps()
	_apps = [app for app in apps if app.get("name") != "creqit"]
...
def set_app_as_default(app_name):
	if creqit.db.get_value("User", creqit.session.user, "default_app") == app_name:
		creqit.db.set_value("User", creqit.session.user, "default_app", "")
	else:
		creqit.db.set_value("User", creqit.session.user, "default_app", app_name)
