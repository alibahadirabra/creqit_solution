...
"""
creqit - Low Code Open Source Framework in Python and JS

creqit, pronounced fra-pay, is a full stack, batteries-included, web
framework written in Python and Javascript with MariaDB as the database.
It is the framework which powers creqit. It is pretty generic and can
be used to build database driven apps.

Read the documentation: https://creqitframework.com/docs
"""
...
import creqit
from creqit.query_builder import (
	get_query,
	get_query_builder,
	patch_query_aggregation,
	patch_query_execute,
)
from creqit.utils.caching import request_cache
from creqit.utils.data import cint, cstr, sbool
...
_tune_gc = bool(sbool(os.environ.get("creqit_TUNE_GC", True)))
...
def _(msg: str, lang: str | None = None, context: str | None = None) -> str:
	...
	"""
	from creqit.translate import get_all_translations
...
def set_user_lang(user: str, user_language: str | None = None) -> None:
	"""Guess and set user language for the session. `creqit.local.lang`"""
	from creqit.translate import get_user_lang
...
if TYPE_CHECKING:  # pragma: no cover
	...
	from creqit.database.mariadb.database import MariaDBDatabase
	from creqit.database.postgres.database import PostgresDatabase
	from creqit.email.doctype.email_queue.email_queue import EmailQueue
	from creqit.model.document import Document
	from creqit.query_builder.builder import MariaDB, Postgres
	from creqit.utils.redis_wrapper import RedisWrapper
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force=False) -> None:
	"""Initialize creqit for the current site. Reset thread locals `creqit.local`"""
	...
	setup_module_map(include_all_apps=not (creqit.request or creqit.job or creqit.flags.in_migrate))
...
def connect(site: str | None = None, db_name: str | None = None, set_admin_as_user: bool = True) -> None:
	...
	:param site: (Deprecated) If site is given, calls `creqit.init`.
	:param db_name: (Deprecated) Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from creqit.database import get_db
	...
	if site:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling creqit.connect with the site argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke creqit.init(site) prior to calling creqit.connect(), if initializing the site is necessary.",
		)
		init(site)
	if db_name:
		from creqit.deprecation_dumpster import deprecation_warning
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling creqit.connect with the db_name argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke creqit.init(site) with the right config prior to calling creqit.connect(), if necessary.",
		)
...
def get_site_config(sites_path: str | None = None, site_path: str | None = None) -> dict[str, Any]:
	...
	if site_path:
		...
		if os.path.exists(site_config):
			...
			if common_config.developer_mode:
				from creqit.utils import get_sites
	...
	config["redis_queue"] = (
		os.environ.get("creqit_REDIS_QUEUE") or config.get("redis_queue") or "redis://127.0.0.1:11311"
	)
	config["redis_cache"] = (
		os.environ.get("creqit_REDIS_CACHE") or config.get("redis_cache") or "redis://127.0.0.1:13311"
	)
	config["db_type"] = os.environ.get("creqit_DB_TYPE") or config.get("db_type") or "mariadb"
	config["db_socket"] = os.environ.get("creqit_DB_SOCKET") or config.get("db_socket")
	config["db_host"] = os.environ.get("creqit_DB_HOST") or config.get("db_host") or "127.0.0.1"
	config["db_port"] = int(
		os.environ.get("creqit_DB_PORT") or config.get("db_port") or db_default_ports(config["db_type"])
	)

	# Set the user as database name if not set in config
	config["db_user"] = os.environ.get("creqit_DB_USER") or config.get("db_user") or config.get("db_name")

	# vice versa for dbname if not defined
	config["db_name"] = os.environ.get("creqit_DB_NAME") or config.get("db_name") or config["db_user"]

	# read password
	config["db_password"] = os.environ.get("creqit_DB_PASSWORD") or config.get("db_password")
...
def setup_redis_cache_connection():
	"""Defines `creqit.cache` as `RedisWrapper` instance"""
	global cache

	if not cache:
		from creqit.utils.redis_wrapper import setup_cache
...
def get_traceback(with_context: bool = False) -> str:
	...
	from creqit.utils import get_traceback
	...
	*,
	...
	def _raise_exception():
		if raise_exception:
			...
			if out.__creqit_exc_id:
				exc.__creqit_exc_id = out.__creqit_exc_id
			raise exc
	...
	if raise_exception:
		...
		out.__creqit_exc_id = generate_hash()
...
def toast(message: str, indicator: Literal["blue", "green", "orange", "red", "yellow"] | None = None):
	creqit.msgprint(message, indicator=indicator, alert=True)
...
def throw(
	...
	:param msg: Message.
	:param exc: Exception class. Default `creqit.ValidationError`
	:param title: [optional] Message title. Default: "Message".
	:param is_minimizable: [optional] Allow users to minimize the modal
	:param wide: [optional] Show wide modal
	:param as_list: [optional] If `msg` is a list, render as un-ordered list.
...
def create_folder(path, with_init=False):
	...
	from creqit.utils import touch_file
...
def get_roles(username=None) -> list[str]:
	...
	return creqit.permissions.get_roles(username or local.session.user)
...
def sendmail(
	...
	if as_markdown:
		from creqit.utils import md_to_html
	...
	from creqit.email.doctype.email_queue.email_queue import QueueBuilder
...
def whitelist(allow_guest=False, xss_safe=False, methods=None):
	...
	def innerfn(fn):
		from creqit.utils.typing_validations import validate_argument_types
...
def is_whitelisted(method):
	from creqit.utils import sanitize_html
...
def read_only():
	def innfn(fn):
		...
		def wrapper_fn(*args, **kwargs):
			# creqit.read_only could be called from nested functions, in such cases don't swap the
			# connection again.
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `creqit.PermissionError` if the user does not have any of the permitted roles.
...
def clear_cache(user: str | None = None, doctype: str | None = None):
	...
	import creqit.utils.caching
	from creqit.website.router import clear_routing_cache

	if doctype:
		creqit.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		creqit.cache_manager.clear_user_cache(user)
	else:  # everything
		# Delete ALL keys associated with this site.
		keys_to_delete = set(creqit.cache.get_keys(""))
		for key in creqit.get_hooks("persistent_cache_keys"):
			keys_to_delete.difference_update(creqit.cache.get_keys(key))
		creqit.cache.delete_value(list(keys_to_delete), make_keys=False)
	...
	creqit.utils.caching._SITE_CACHE.clear()
	local.role_permissions = {}
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	from creqit.permissions import get_role_permissions
...
def has_permission(
	...
	*,
	...
	Raise `creqit.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = creqit.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
	...
	if throw and not out:
		...
		creqit.flags.error_message = _("No permission for {0}").format(document_label)
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `creqit.PermissionError` if not permitted.
...
def get_precision(
	...
	from creqit.model.meta import get_field_precision
...
def new_doc(
	...
	**kwargs,
	...
	from creqit.model.create_new import get_new_doc
...
def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `creqit.client.set_value`"""
	import creqit.client

	return creqit.client.set_value(doctype, docname, fieldname, value)


def get_cached_doc(*args, **kwargs) -> "Document":
	"""Identical to `creqit.get_doc`, but return from cache if available."""
...
def get_doc(**kwargs: dict) -> "_NewDocument":
	...
	Not recommended. Use `creqit.new_doc` instead."""
	pass
...
def get_doc(*args, **kwargs):
	"""Return a `creqit.model.document.Document` object of the given type and name.
	...
	Examples:
	        ...
	        todo = creqit.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = creqit.get_doc("ToDo", "TD0001")

	"""
	import creqit.model.document

	doc = creqit.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `creqit.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)


def get_meta(doctype, cached=True):
	"""Get `creqit.model.meta.Meta` instance of given doctype name."""
	import creqit.model.meta

	return creqit.model.meta.get_meta(doctype, cached=cached)


def get_meta_module(doctype):
	import creqit.modules

	return creqit.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `creqit.model.delete_doc.delete_doc`.
	...
	import creqit.model.delete_doc

	return creqit.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return creqit.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(
	...
	*,
	...
	Calls `creqit.model.rename_doc.rename_doc`
	"""
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = creqit.utils.strip(content)
...
def import_doc(path):
	...
	from creqit.core.doctype.data_import.data_import import import_doc
...
def copy_doc(doc: "Document", ignore_no_copy: bool = True) -> "Document":
	...
	from creqit.model.base_document import BaseDocument
...
def redirect(url):
	...
	from creqit.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        creqit.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import creqit.desk.reportview

	return creqit.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `creqit.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        creqit.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        creqit.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])
	"""
	import creqit.model.db_query

	return creqit.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `creqit.model.db_query`. Will **not** check for permissions.
	Parameters are same as `creqit.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        creqit.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        creqit.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])
	"""
...
def as_json(obj: dict | list, indent=1, separators=None, ensure_ascii=True) -> str:
	...
	from creqit.utils.response import json_handler
...
from creqit.deprecation_dumpster import creqit_get_test_records as get_test_records
...
def format_value(*args, **kwargs):
	...
	import creqit.utils.formatters

	return creqit.utils.formatters.format_value(*args, **kwargs)
...
def get_print(
	...
	from creqit.utils.pdf import get_pdf
	from creqit.website.serve import get_response_without_exception_handling
...
def attach_print(
	...
	from creqit.translate import print_language
	from creqit.utils import scrub_urls
	from creqit.utils.pdf import get_pdf
...
def publish_progress(*args, **kwargs):
	...
	"""
	import creqit.realtime

	return creqit.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return creqit.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: creqit.local.cache[namespace]
	:param key: creqit.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import creqit.utils.background_jobs

	return creqit.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return creqit.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(module=None, with_more_info=False, allow_site=True, filter=None, max_size=100_000, file_count=20):
	...
	from creqit.utils.logger import get_logger
...
def safe_eval(code, eval_globals=None, eval_locals=None):
	...
	from creqit.utils.safe_exec import safe_eval
...
def get_active_domains():
	from creqit.core.doctype.domain_settings.domain_settings import get_active_domains
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	Example
	>>> creqit.get_version("User", "foobar@gmail.com")
	>>>
	[
	        {
	...
	if meta.track_changes:
		...
		from creqit.utils import dictify, safe_json_loads, squashify
...
def parse_json(val):
	from creqit.utils import parse_json
...
def mock(type, size=1, locale="en"):
	...
	from creqit.utils import squashify
...
def validate_and_sanitize_search_inputs(fn):
	...
	def wrapper(*args, **kwargs):
		from creqit.desk.search import sanitize_searchfield
...
from creqit.utils.error import log_error

if _tune_gc:
	# generational GC gets triggered after certain allocs (g0) which is 700 by default.
	# This number is quite small for creqit where a single query can potentially create 700+
	# objects easily.
