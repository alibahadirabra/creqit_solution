...
class TestFullTextSearch(IntegrationTestCase):
	...
	def test_search_term(self):
		...
		self.assertEqual(res[0], "sw/creqit")
	...
	def test_update_index(self):
		...
		self.index.update_index({"name": "sw/creqit", "content": """Awesomecreqit"""})

		res = self.index.search("CommonSearchTerm")
		self.assertTrue("sw/creqit" not in res)

		res = self.index.search("Awesomecreqit")
		self.assertEqual(res[0], "sw/creqit")

		# Update new doc
		self.index.update_index({"name": "sw/creqitbooks", "content": """DesktopAccounting"""})

		res = self.index.search("DesktopAccounting")
		self.assertEqual(res[0], "sw/creqitbooks")
...
def get_index():
	return TestWrapper("test_creqit_index")
...
def get_documents():
	...
	docs.append(
		{
			"name": "sw/creqit",
			"content": """creqit is a free and open-source integrated Enterprise Resource Planning software developed by
			creqit Technologies Pvt. Ltd. and is built on MariaDB database system using a Python based server-side framework.
			creqit is a generic ERP software used by manufacturers, distributors and services companies. CommonSearchTerm""",
		}
	)
	...
	docs.append(
		{
			"name": "sw/creqit",
			"content": """creqit Framework is a full-stack web framework, that includes everything you need to build and
