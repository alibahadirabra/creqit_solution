...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "creqit", "dist", type)
	...
	assets_json = creqit.read_file("assets/assets.json")
	if assets_json:
		assets_json = creqit.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/creqit"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["creqit/" + name for name in missing_assets]
			bundle(build_mode, files=files_to_build)
	else:
		# no assets.json, run full build
		bundle(build_mode, apps="creqit")


def get_assets_link(creqit_head) -> str:
	import requests

	tag = getoutput(
		r"cd ../apps/creqit && git show-ref --tags -d | grep %s | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{}//'" % creqit_head
	)

	if tag:
		# if tag exists, download assets from github release
		url = f"https://github.com/creqit/creqit/releases/download/{tag}/assets.tar.gz"
	else:
		url = f"http://assets.frappeframework.com/{creqit_head}.tar.gz"

	if not requests.head(url):
		reference = f"Release {tag}" if tag else f"Commit {creqit_head}"
		raise AssetsDontExistError(f"Assets for {reference} don't exist")

	return url


def fetch_assets(url, creqit_head):
	click.secho("Retrieving assets...", fg="yellow")

	prefix = mkdtemp(prefix="creqit-assets-", suffix=creqit_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	click.echo(click.style("✔", fg="green") + f" Downloaded creqit assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./creqit-bench/sites", "")
...
def download_creqit_assets(verbose=True) -> bool:
	"""Download and set up creqit assets if they exist based on the current commit HEAD.
	Return True if correctly setup else return False.
	"""
	creqit_head = getoutput("cd ../apps/creqit && git rev-parse HEAD")

	if not creqit_head:
		return False

	try:
		url = get_assets_link(creqit_head)
		assets_archive = fetch_assets(url, creqit_head)
...
def symlink(target, link_name, overwrite=False):
	...
	while True:
		temp_link_name = f"tmp{creqit.generate_hash()}"
...
def setup():
	...
	for app in creqit.get_all_apps(True):
		try:
			pymodules.append(creqit.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(creqit.local.sites_path, "assets")
...
def bundle(
	...
	creqit_app_path = creqit.get_app_source_path("creqit")
	creqit.commands.popen(command, cwd=creqit_app_path, env=get_node_env(), raise_err=True)
...
def watch(apps=None):
	...
	live_reload = creqit.utils.cint(os.environ.get("LIVE_RELOAD", creqit.conf.live_reload))
	...
	check_node_executable()
	creqit_app_path = creqit.get_app_source_path("creqit")
	creqit.commands.popen(command, cwd=creqit_app_path, env=get_node_env())
...
def generate_assets_map():
	...
	for app_name in creqit.get_all_apps():
		app_doc_path = None

		pymodule = creqit.get_module(app_name)
...
def html_to_js_template(path, content):
	"""Return HTML template content as Javascript code, by adding it to `creqit.templates`."""
	return """creqit.templates["{key}"] = '{content}';\n""".format(
