...
def get_dynamic_link_map(for_delete=False):
	...
	"""
	if getattr(creqit.local, "dynamic_link_map", None) is None or creqit.flags.in_test:
		# Build from scratch
		dynamic_link_map = {}
		...
		creqit.local.dynamic_link_map = dynamic_link_map
	return creqit.local.dynamic_link_map
...
def get_dynamic_links():
	...
	for query in dynamic_link_queries:
		df += creqit.db.sql(query, as_dict=True)
	return df
