...
from creqit.model.base_document import (
...
from creqit.utils import cast, cint, cstr
...
def get_meta(doctype: str | Document, cached=True) -> "_Meta":
	...
	if cached and not isinstance(doctype, Document):
		if meta := creqit.cache.hget("doctype_meta", doctype_name):
			return meta

	meta = Meta(doctype)
	creqit.cache.hset("doctype_meta", meta.name, meta)
	return meta
...
def get_table_columns(doctype):
	return creqit.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = creqit.scrub(doctype)
	with open(creqit.get_app_path("creqit", "core", "doctype", fname, fname + ".json")) as f:
...
class Meta(Document):
	...
	standard_set_once_fields = (
		creqit._dict(fieldname="creation", fieldtype="Datetime"),
		creqit._dict(fieldname="owner", fieldtype="Data"),
	)
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(creqit._dict(fieldtype="Data", fieldname="name", label="Name"))

		return fields

	def get_valid_columns(self) -> list[str]:
		if not hasattr(self, "_valid_columns"):
			table_exists = creqit.db.table_exists(self.name)
	...
	def get_valid_fields(self) -> list[str]:
		if not hasattr(self, "_valid_fields"):
			if (creqit.flags.in_install or creqit.flags.in_migrate) and self.name in self.special_doctypes:
	...
	def add_custom_fields(self):
		if not creqit.db.table_exists("Custom Field"):
			return

		custom_fields = creqit.db.get_values(
			"Custom Field",
	...
	def apply_property_setters(self):
		...
		"""
		if not creqit.db.table_exists("Property Setter"):
			return

		property_setters = creqit.db.get_values(
			"Property Setter",
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(creqit._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permitted_fieldnames(
		...
		*,
		...
		if not permission_type:
			permission_type = "select" if creqit.only_has_select_perm(self.name, user=user) else "read"
		...
		if 0 not in permlevel_access and permission_type in ("read", "select"):
			if creqit.share.get_shared(self.name, user, rights=[permission_type], limit=1):
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = creqit.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other creqit applications via hooks.
		"""
		data = creqit._dict()
		if not self.custom:
			try:
				...
				if hasattr(module, "get_data"):
					data = creqit._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in creqit.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = creqit._dict(creqit.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = creqit.scrub(self.module)
		doctype = creqit.scrub(self.name)
		template_path = creqit.get_module_path(
...
def is_single(doctype):
	try:
		return creqit.db.get_value("DocType", doctype, "issingle")
	except IndexError:
		raise Exception("Cannot determine whether %s is single" % doctype)


def get_parent_dt(dt):
	if not creqit.is_table(dt):
		return ""

	return (
		creqit.db.get_value(
			"DocField",
			{"fieldtype": ("in", creqit.model.table_fields), "options": dt},
			"parent",
		)
		or ""
	)
...
def set_fieldname(field_id, fieldname):
	creqit.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(creqit.local, "field_currency", None):
		creqit.local.field_currency = creqit._dict()

	if not (
		creqit.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.get("parent")
			and creqit.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = creqit.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if creqit.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = creqit.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			creqit.local.field_currency.setdefault((doc.doctype, ref_docname), creqit._dict()).setdefault(
				df.fieldname, currency
			)

	return creqit.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
		doc.get("parent") and creqit.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)


def get_field_precision(df, doc=None, currency=None):
	"""get precision based on DocField options and fieldvalue in doc"""
	from creqit.locale import get_number_format

	if df.precision:
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(creqit.db.get_default("currency_precision"))
		if not precision:
			number_format = get_number_format()
			precision = number_format.precision
	else:
		precision = cint(creqit.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return creqit._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return creqit._dict(fieldname=fieldname, fieldtype="Int")

		elif fieldname in ("owner", "modified_by"):
			return creqit._dict(fieldname=fieldname, fieldtype="Link", options="User")

		return creqit._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in creqit.get_all("DocType", filters=filters, pluck="name"):
		try:
...
def trim_table(doctype, dry_run=True):
	creqit.cache.hdel("table_columns", f"tab{doctype}")
	ignore_fields = default_fields + optional_fields + child_table_fields
	columns = creqit.db.get_table_columns(doctype)
	fields = creqit.get_meta(doctype, cached=False).get_fieldnames_with_value()
	...
	if columns_to_remove and not dry_run:
		...
		creqit.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")
