...
class VirtualDoctype(Protocol):
	...
	- DocType controller has to inherit from `creqit.model.document.Document` class
	...
	@staticmethod
	def get_list(**kwargs) -> list[creqit._dict]:
...
def validate_controller(doctype: str) -> None:
	try:
		...
		creqit.msgprint(_("Failed to import virtual doctype {}, is controller file present?").format(doctype))
		return
		...
		if not isinstance(method, staticmethod):
			creqit.msgprint(
				_("Virtual DocType {} requires a static method called {} found {}").format(
					creqit.bold(doctype), creqit.bold(m), creqit.bold(_as_str(method))
				),
