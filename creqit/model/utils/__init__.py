...
def set_default(doc, key):
	...
	if not doc.is_default:
		creqit.db.set(doc, "is_default", 1)
...
def set_field_property(filters, key, value):
	...
	docs = [
		creqit.get_doc("DocType", d.parent)
		for d in creqit.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(creqit.ValidationError):
	pass
	...
	for _ in range(5):
		if "{% include" in content:
			...
			for path in paths:
				...
				with open(creqit.get_app_path(app, app_path), encoding="utf-8") as f:
					include = f.read()
...
def is_virtual_doctype(doctype: str):
	if creqit.flags.in_install or creqit.flags.in_migrate:
		if creqit.db.has_column("DocType", "is_virtual"):
			return creqit.db.get_value("DocType", doctype, "is_virtual")
	else:
		return getattr(creqit.get_meta(doctype), "is_virtual", False)
...
def is_single_doctype(doctype: str) -> bool:
	...
	if creqit.flags.in_install or creqit.flags.in_migrate:
		...
		return getattr(creqit.get_meta(doctype), "issingle", False)
