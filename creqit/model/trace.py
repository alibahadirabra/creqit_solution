"""
Traced Fields for creqit

This module provides utilities for creating traced fields in creqit documents,
...
- Seamlessly integrate with creqit's document model

Example of standard usage:
		from creqit.model.trace import TracedDocument, traced_field
...
See creqit.tests.classes.context_managers for a context manager built into test classes.
"""
...
class TracedValue:
	"""
	A descriptor class for creating traced fields in creqit documents.

	This class allows for monitoring and validating changes to specific fields
	in a creqit document. It can enforce forbidden values and apply custom
	validation logic.
	...
	def __set__(self, obj, value):
		...
		if value in self.forbidden_values:
			if creqit.flags.in_test:
				creqit.throw(f"{self.field_name} cannot be set to {value}", AssertionError)
			else:
				creqit.throw(f"{self.field_name} cannot be set to {value}")
		...
		if self.custom_validation:
			try:
				...
				if creqit.flags.in_test:
					creqit.throw(str(e), AssertionError)
				else:
					creqit.throw(str(e))
...
class TracedDocument(Document):
	"""
	A base class for creqit documents with traced fields.

	This class extends creqit's Document class to provide support for
	traced fields created with TracedValue.

	Attributes:
	        Inherits all attributes from creqit.model.document.Document
	"""
...
def traced_field(*args, **kwargs):
	...
	This function simplifies the creation of traced fields in creqit documents.
...
from creqit.deprecation_dumpster import model_trace_traced_field_context as traced_field_context
