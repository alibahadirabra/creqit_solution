...
import creqit
from creqit import _, is_whitelisted, msgprint
from creqit.core.doctype.file.utils import relink_mismatched_files
from creqit.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from creqit.desk.form.document_follow import follow_document
from creqit.integrations.doctype.webhook import run_webhooks
from creqit.model import optional_fields, table_fields
from creqit.model.base_document import BaseDocument, get_controller
from creqit.model.docstatus import DocStatus
from creqit.model.naming import set_new_name, validate_name
from creqit.model.utils import is_virtual_doctype, simple_singledispatch
from creqit.model.workflow import set_workflow_state_on_action, validate_workflow
from creqit.types import DF
from creqit.utils import compare, cstr, date_diff, file_lock, flt, now
from creqit.utils.data import get_absolute_url, get_datetime, get_timedelta, getdate
from creqit.utils.global_search import update_global_search
...
def get_doc(*args, **kwargs) -> "Document":
	"""Return a `creqit.model.Document` object.
...
def read_only_document(context=None):
	...
	def read_only_method(func):
		...
		def wrapper(self, *args, **kwargs):
			...
			if context:
				error_msg += f" ({context})"
			raise creqit.DatabaseModificationError(error_msg)

		return wrapper

	# Use a thread-local variable to track nested invocations
	if not hasattr(creqit.local, "read_only_depth"):
		creqit.local.read_only_depth = 0

	try:
		# Increment the depth counter
		creqit.local.read_only_depth += 1

		# Only apply read-only methods if this is the outermost invocation
		if creqit.local.read_only_depth == 1:
...
class Document(BaseDocument):
	...
	def load_from_db(self) -> "Self":
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = creqit.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = creqit.new_doc(self.doctype, as_dict=True)
			...
			d = creqit.db.get_value(
				doctype=self.doctype, filters=self.name, fieldname="*", **get_value_kwargs
			)

			if not d:
				creqit.throw(
					_("{0} {1} not found").format(_(self.doctype), self.name), creqit.DoesNotExistError
				)
		...
		for df in self._get_table_fields():
			...
			# During creqit is installed, the property "is_virtual" is not available in tabDocType, so
			# we need to filter those cases for the access to creqit.db.get_value() as it would crash otherwise.
	...
	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `creqit.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self.raise_no_permission_to(permtype)

	def has_permission(self, permtype="read", *, debug=False, user=None) -> bool:
		"""
		Call `creqit.permissions.has_permission` if `ignore_permissions` flag isn't truthy
		...
		return creqit.permissions.has_permission(self.doctype, permtype, self, debug=debug, user=user)

	def raise_no_permission_to(self, perm_type):
		"""Raise `creqit.PermissionError`."""
		creqit.flags.error_message = (
			_("Insufficient Permission for {0}").format(_(self.doctype)) + f" ({creqit.bold(_(perm_type))})"
		)
		raise creqit.PermissionError
	...
	def insert(
		...
		if not (creqit.flags.in_migrate or creqit.local.flags.in_install or creqit.flags.in_setup_wizard):
			if creqit.get_cached_value("User", creqit.session.user, "follow_created_documents"):
				follow_document(self.doctype, self.name, creqit.session.user)
		return self

	def check_if_locked(self):
		if self.creation and self.is_locked:
			raise creqit.DocumentLockedError
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Self":
		...
		self.flags.ignore_version = creqit.flags.in_test if ignore_version is None else ignore_version
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `creqit.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		creqit.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in creqit.db.value_cache:
			del creqit.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = creqit.session.user
		...
		if self.is_new() and not (
			creqit.flags.in_install or creqit.flags.in_patch or creqit.flags.in_migrate
		):
		...
		creqit.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					creqit.bold(_(self.doctype)),
					_("Row"),
					self.idx,
					_("Value cannot be negative for"),
					creqit.bold(_(df.label, context=df.parent)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), creqit.bold(_(df.label, context=df.parent))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				creqit.throw(msg, creqit.NonNegativeError, title=_("Negative Value"))
	...
	def validate_workflow(self):
		...
		if creqit.flags.in_install == "creqit":
			return
				...
				if fail:
					creqit.throw(
						_("Value cannot be changed for {0}").format(
							creqit.bold(self.meta.get_label(field.fieldname))
						),
						exc=creqit.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += creqit.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in creqit.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or creqit.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = creqit.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = creqit.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		if self.is_new():
			new_doc = creqit.new_doc(self.doctype, as_dict=True)
			self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = creqit.new_doc(df.options, parent_doc=self, parentfield=df.fieldname, as_dict=True)
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			creqit.msgprint(
				...
				raise_exception=creqit.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.draft():
			if self.docstatus.is_draft():
				...
				raise creqit.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise creqit.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise creqit.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if creqit.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise creqit.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			creqit.throw(_("Could not find {0}").format(msg), creqit.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			creqit.throw(_("Cannot link cancelled document: {0}").format(msg), creqit.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			(creqit.flags.in_import and creqit.flags.mute_emails)
			or creqit.flags.in_patch
		...
		from creqit.email.doctype.notification.notification import evaluate_alert
		...
		if self.flags.notifications is None:
			...
			self.flags.notifications = creqit.cache.hget("notifications", self.doctype, _get_notifications)
	...
	def _rename(self, name: str, merge: bool = False, force: bool = False, validate_rename: bool = True):
		"""Rename the document. Triggers creqit.rename_doc, then reloads."""
	...
	def discard(self):
		...
		if not self.docstatus == DocStatus.draft():
			raise creqit.ValidationError(_("Only draft documents can be discarded"), self.docstatus)
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = creqit.get_doc(self.doctype, self.name, for_update=True)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in creqit.flags.currently_saving:
			creqit.flags.currently_saving.remove((self.doctype, self.name))

	def clear_cache(self):
		creqit.clear_document_cache(self.doctype, self.name)
	...
	def reset_seen(self):
		...
		if (
			...
			creqit.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([creqit.session.user]), update_modified=False
			)
	...
	def notify_update(self):
		...
		if not self.meta.get("read_only") and not self.meta.get("issingle") and not self.meta.get("istable"):
			data = {"doctype": self.doctype, "name": self.name, "user": creqit.session.user}
			creqit.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run creqit.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in creqit.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", creqit.session.user)
	...
	def db_get(self, fieldname):
		...
		return creqit.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from creqit.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def hook(f):
		...
		def compose(fn, *hooks):
			def runner(self, method, *args, **kwargs):
				...
				for f in hooks:
					try:
						creqit.db._disable_transaction_control += 1
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			creqit.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or creqit.EmptyTableError
			)
		...
		return creqit.get_doc(
			{
				...
				"comment_email": comment_email or creqit.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not creqit.flags.read_only and not self.meta.issingle:
			_seen = self.get("_seen") or []
			_seen = creqit.parse_json(_seen)
			...
			if user not in _seen:
				...
				creqit.local.flags.commit = True
	...
	def add_viewed(self, user=None, force=False, unique_views=False):
		...
		if unique_views and creqit.db.exists(
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		if existing_key:
			doc = creqit.get_doc("Document Share Key", existing_key)
		else:
			doc = creqit.new_doc("Document Share Key")
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", creqit._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", creqit._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		return enqueue(
			"creqit.model.document.execute_action",
	...
	def lock(self, timeout=None):
		...
		creqit.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in creqit.local.locked_documents:
			creqit.local.locked_documents.remove(self)
		...
		if date_diff(to_date, from_date) < 0:
			creqit.throw(
				_("{0} must be after {1}").format(
					creqit.bold(_(self.meta.get_label(to_date_field))),
					creqit.bold(_(self.meta.get_label(from_date_field))),
				),
				creqit.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = creqit.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from creqit.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from creqit.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = creqit.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		if creqit.message_log:
			msg = creqit.message_log[-1].get("message")
		else:
			msg = "<pre><code>" + creqit.get_traceback() + "</pre></code>"
...
def bulk_insert(
	...
	for child_table in doctype_meta.get_table_fields():
		valid_column_map[child_table.options] = creqit.get_meta(child_table.options).get_valid_columns()
...
def _document_values_generator(
	...
	for doc in documents:
		...
		doc.owner = doc.modified_by = creqit.session.user
...
def unlock_document(doctype: str, name: str):
	creqit.get_doc(doctype, name).unlock()
	creqit.msgprint(creqit._("Document Unlocked"), alert=True)
