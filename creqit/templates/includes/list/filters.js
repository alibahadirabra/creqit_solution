function setup_list_filters() {
	...
	creqit.call('creqit.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = creqit.utils.get_query_params()[df.fieldname];
			const f = creqit.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(creqit.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = creqit.utils.make_query_string(query_params);
...
creqit.ready(setup_list_filters);
