...
from creqit.database.database import is_query_type
...
class RecorderConfig:
	...
	def __post_init__(self):
		if not (self.record_jobs or self.record_requests):
			creqit.throw("You must record one of jobs or requests")

	def store(self):
		creqit.cache.set_value(RECORDER_CONFIG_FLAG, self, expires_in_sec=RECORDER_AUTO_DISABLE)

	@classmethod
	def retrieve(cls):
		return creqit.cache.get_value(RECORDER_CONFIG_FLAG) or cls()

	@staticmethod
	def delete():
		creqit.cache.delete_value(RECORDER_CONFIG_FLAG)


def record_sql(*args, **kwargs):
	start_time = time.monotonic()
	result = creqit.db._sql(*args, **kwargs)
	end_time = time.monotonic()

	query = getattr(creqit.db, "last_query", None)
	if not query or isinstance(result, str):
		# run=0, doesn't actually run the query so last_query won't be present
		return result

	stack = []
	if creqit.local._recorder.config.capture_stack:
	...
	creqit.local._recorder.register(data)
	return result


def get_current_stack_frames():
	from creqit.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX
...
def post_process():
	...
	"""
	creqit.db.rollback()
	creqit.db.begin(read_only=True)  # Explicitly start read only transaction

	config = RecorderConfig.retrieve()
	result = list(creqit.cache.hgetall(RECORDER_REQUEST_HASH).values())
	...
	for request in result:
		for call in request["calls"]:
			...
			if config.explain and is_query_type(formatted_query, ("select", "update", "delete")):
				...
				try:
					call["explain_result"] = creqit.db.sql(f"EXPLAIN {formatted_query}", as_dict=True)
				except Exception:
					pass
		mark_duplicates(request)
		creqit.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
...
def record(force=False):
	if creqit.cache.get_value(RECORDER_INTERCEPT_FLAG) or force:
		creqit.local._recorder = Recorder(force=force)


def dump():
	if hasattr(creqit.local, "_recorder"):
		creqit.local._recorder.dump()
...
class Recorder:
	def __init__(self, force=False):
		...
		if (
			...
			and creqit.request
			and self.config.request_filter in creqit.request.path
		):
			self.path = creqit.request.path
			self.cmd = creqit.local.form_dict.cmd or ""
			self.method = creqit.request.method
			self.headers = dict(creqit.local.request.headers)
			self.form_dict = creqit.local.form_dict
			self.event_type = "HTTP Request"
		elif self.config.record_jobs and creqit.job and self.config.jobs_filter in creqit.job.method:
			self.event_type = "Background Job"
			self.path = creqit.job.method
			self.cmd = None
			self.method = None
			self.headers = None
			self.form_dict = None
		...
		self.uuid = creqit.generate_hash(length=10)
	...
	def dump(self):
		...
		creqit.cache.hset(RECORDER_REQUEST_SPARSE_HASH, self.uuid, request_data)

		request_data["calls"] = self.calls
		request_data["headers"] = self.headers
		request_data["form_dict"] = self.form_dict
		request_data["profile"] = profiler_output
		creqit.cache.hset(RECORDER_REQUEST_HASH, self.uuid, request_data)
	...
	@staticmethod
	def _patch_sql():
		creqit.db._sql = creqit.db.sql
		creqit.db.sql = record_sql

	@staticmethod
	def _unpatch_sql():
		creqit.db.sql = creqit.db._sql
...
def do_not_record(function):
	...
	def wrapper(*args, **kwargs):
		if hasattr(creqit.local, "_recorder"):
			creqit.local._recorder.cleanup()
			del creqit.local._recorder
...
def administrator_only(function):
	...
	def wrapper(*args, **kwargs):
		if creqit.session.user != "Administrator":
			creqit.throw(_("Only Administrator is allowed to use Recorder"))
		return function(*args, **kwargs)
...
def status(*args, **kwargs):
	return bool(creqit.cache.get_value(RECORDER_INTERCEPT_FLAG))
...
def start(
	...
	**kwargs,
	...
	creqit.cache.set_value(RECORDER_INTERCEPT_FLAG, 1, expires_in_sec=RECORDER_AUTO_DISABLE)
...
def stop(*args, **kwargs):
	creqit.cache.delete_value(RECORDER_INTERCEPT_FLAG)
	creqit.enqueue(post_process, now=creqit.flags.in_test)
...
def get(uuid=None, *args, **kwargs):
	if uuid:
		result = creqit.cache.hget(RECORDER_REQUEST_HASH, uuid)
	else:
		result = list(creqit.cache.hgetall(RECORDER_REQUEST_SPARSE_HASH).values())
	return result
...
def export_data(*args, **kwargs):
	return list(creqit.cache.hgetall(RECORDER_REQUEST_HASH).values())
...
def delete(*args, **kwargs):
	creqit.cache.delete_value(RECORDER_REQUEST_SPARSE_HASH)
	creqit.cache.delete_value(RECORDER_REQUEST_HASH)
...
def record_queries(func: Callable):
	...
	def wrapped(*args, **kwargs):
		record(force=True)
		creqit.local._recorder.path = f"Function call: {func.__module__}.{func.__qualname__}"
...
def import_data(file: str) -> None:
	file_doc = creqit.get_doc("File", {"file_url": file})
	file_content = json.loads(file_doc.get_content())
	for request in file_content:
		creqit.cache.hset(RECORDER_REQUEST_SPARSE_HASH, request["uuid"], request)
		creqit.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
