...
from creqit.deprecation_dumpster import gzip_compress, gzip_decompress, make_esc
...
def get_fullname(user=None):
	...
	if not hasattr(creqit.local, "fullnames"):
		creqit.local.fullnames = {}

	if not creqit.local.fullnames.get(user):
		p = creqit.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			creqit.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			creqit.local.fullnames[user] = user

	return creqit.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return creqit.db.get_value("User", user, "email")
...
def validate_phone_number_with_country_code(phone_number: str, fieldname: str) -> None:
	...
	try:
		...
		if not valid_number:
			creqit.throw(
				error_message.format(creqit.bold(phone_number), creqit.bold(fieldname)),
				title=error_title,
				exc=creqit.InvalidPhoneNumberError,
			)
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		creqit.throw(
			creqit._("{0} is not a valid Phone Number").format(phone_number), creqit.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	* anything else is considered invalid
	...
	Note: "Name" here is name of a person, not the primary key in creqit doctypes.
	"""
	if not name:
		return False
	...
	if not match and throw:
		creqit.throw(creqit._("{0} is not a valid Name").format(name), creqit.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = creqit.utils.escape_html(e)
				creqit.throw(
					creqit._("{0} is not a valid Email Address").format(invalid_email),
...
def validate_url(
	...
	if not is_valid and throw:
		creqit.throw(creqit._("'{0}' is not a valid URL").format(creqit.bold(txt)))
...
def has_gravatar(email: str) -> str:
	...
	if creqit.flags.in_import or creqit.flags.in_install or creqit.flags.in_test:
...
def get_gravatar_url(email: str, default: Literal["mm", "404"] = "mm") -> str:
	...
	"""
	hexdigest = hashlib.md5(creqit.as_unicode(email).encode("utf-8"), usedforsecurity=False).hexdigest()
	...
	return Format(
		custom_var_printers=[
			...
			*[(variable_name, lambda *a, **kw: placeholder) for variable_name in blocklist],
			...
			(["_secret", creqit._dict, lambda *a, **kw: False], dict_printer),
		],
	)


def log(event, details):
	creqit.logger(event).info(details)
...
def get_defaults(key=None):
	...
	"""
	return creqit.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return creqit.db.set_default(key, val)
...
def execute_in_shell(cmd, verbose=False, low_priority=False, check_exit_code=False):
	...
	if failed:
		raise creqit.CommandFailedError(
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = creqit.local.site_path
	return os.path.join(base, *path)


def get_site_base_path():
	return creqit.local.site_path
...
def get_bench_path():
	return os.environ.get("creqit_BENCH_ROOT") or os.path.realpath(
		os.path.join(os.path.dirname(creqit.__file__), "..", "..", "..")
	)


def get_bench_id():
	return creqit.get_conf().get("bench_id", get_bench_path().strip("/").replace("/", "-"))


def get_site_id(site=None):
	return f"{site or creqit.local.site}@{get_bench_id()}"
...
def get_test_client(use_cookies=True) -> Client:
	"""Return an test instance of the creqit WSGI."""
	from creqit.app import application

	return Client(application, use_cookies=use_cookies)


def get_hook_method(hook_name, fallback=None):
	method = creqit.get_hooks().get(hook_name)
	if method:
		method = creqit.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in creqit.get_hooks(hook):
		out = out or creqit.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l, absolute=False):
	...
	if not getattr(creqit.local, "request", None) or is_cli():  # pragma: no cover
		lt = len(txt)
		try:
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in INCLUDE_DIRECTIVE_PATTERN.findall(html_format):
			for app_name in creqit.get_installed_apps():
				include_path = creqit.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(creqit.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else creqit.utils.md_to_html(text)
...
def get_site_info():
	from creqit.email.queue import get_emails_sent_this_month
	from creqit.utils.user import get_system_managers

	# only get system users
	users = creqit.get_all(
		"User",
		filters={"user_type": "System User", "name": ("not in", creqit.STANDARD_USERS)},
	...
	system_settings = creqit.db.get_singles_dict("System Settings")
	space_usage = creqit._dict((creqit.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": creqit.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in creqit.get_hooks("get_site_info"):
		site_info.update(creqit.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return json.loads(creqit.as_json(site_info))
...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                bench --site creqit.local execute creqit.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
		db_count[doctype] = creqit.db.count(doctype)

	return json.loads(creqit.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Pass a doctype or a series of doctypes to get the count of docs in them
	Parameters:
	        fn: creqit function to be called
	...
	Example:
	        via terminal:
	                bench --site creqit.local execute creqit.utils.call --args '''["creqit.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
	return json.loads(creqit.as_json(creqit.call(fn, *args, **kwargs)))
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, int | float):
			filters = creqit.as_unicode(filters)
...
def set_request(**kwargs):
	...
	creqit.local.request = Request(builder.get_environ())
...
def get_html_for_route(route):
	...
	return creqit.safe_decode(response.get_data())
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(creqit.local.sites_path, "assets/assets.json")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return creqit.utils.random_string(8)


def get_assets_json():
	def _get_assets():
		# get merged assets.json and assets-rtl.json
		assets = creqit.parse_json(creqit.read_file("assets/assets.json"))

		if assets_rtl := creqit.read_file("assets/assets-rtl.json"):
			assets.update(creqit.parse_json(assets_rtl))

		return assets

	if not hasattr(creqit.local, "assets_json"):
		if not creqit.conf.developer_mode:
			creqit.local.assets_json = creqit.cache.get_value(
				"assets_json",
				_get_assets,
				shared=True,
			)
			...
			creqit.local.assets_json = _get_assets()

	return creqit.local.assets_json
...
def add_user_info(user: str | list[str] | set[str], user_info: dict[str, _UserInfo]) -> None:
	...
	for info in missing_info:
		user_info.setdefault(info.name, creqit._dict()).update(
...
class CallbackManager:
	...
	Example usage: creqit.db.after_commit
	"""
