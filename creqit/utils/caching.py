...
def request_cache(func: Callable) -> Callable:
	...
	creqit.local.request_cache. The cache only persists for the current request
	...
	def wrapper(*args, **kwargs):
		if not getattr(creqit.local, "initialised", None):
			return func(*args, **kwargs)
		if not hasattr(creqit.local, "request_cache"):
			creqit.local.request_cache = defaultdict(dict)
		...
		try:
			return creqit.local.request_cache[func][args_key]
		except KeyError:
			return_val = func(*args, **kwargs)
			creqit.local.request_cache[func][args_key] = return_val
			return return_val
...
def site_cache(ttl: int | None = None, maxsize: int | None = None) -> Callable:
	...
	creqit.utils.caching._SITE_CACHE. The cache persists on the parent process.
	...
	workers, use redis (creqit.cache API) instead.
	...
	def time_cache_wrapper(func: Callable | None = None) -> Callable:
		...
		def site_cache_wrapper(*args, **kwargs):
			if getattr(creqit.local, "initialised", None):
				...
				if hasattr(func, "maxsize") and len(_SITE_CACHE[func_key][creqit.local.site]) >= func.maxsize:
					_SITE_CACHE[func_key][creqit.local.site].pop(
						next(iter(_SITE_CACHE[func_key][creqit.local.site])), None
					)

				if func_call_key not in _SITE_CACHE[func_key][creqit.local.site]:
					_SITE_CACHE[func_key][creqit.local.site][func_call_key] = func(*args, **kwargs)

				return _SITE_CACHE[func_key][creqit.local.site][func_call_key]
...
def redis_cache(ttl: int | None = 3600, user: str | bool | None = None, shared: bool = False) -> Callable:
	...
	def wrapper(func: Callable | None = None) -> Callable:
		...
		def clear_cache():
			creqit.cache.delete_keys(func_key)
		...
		def redis_cache_wrapper(*args, **kwargs):
			...
			if creqit.cache.exists(func_call_key, user=user, shared=shared):
				return creqit.cache.get_value(func_call_key, user=user, shared=shared)
			val = func(*args, **kwargs)
			ttl = getattr(func, "ttl", 3600)
			creqit.cache.set_value(func_call_key, val, expires_in_sec=ttl, user=user, shared=shared)
			return val
