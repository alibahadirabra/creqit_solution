...
from creqit.database.database import Database, EmptyQueryValues


class creqitIntegration(Integration):
	identifier = "creqit"
...
def set_scope(scope):
	if job := rq.get_current_job():
		...
		context = creqit._dict({"scheduled": False, "wait": 0})
		...
		if creqit.form_dict.cmd:
			path = f"/api/method/{creqit.form_dict.cmd}"
		else:
			path = creqit.request.path
	...
	scope.set_user({"id": creqit.local.site})
	user = getattr(creqit.session, "user", "Unidentified")
	scope.set_tag("creqit_user", user)
	# Extract `X-creqit-Request-ID` to store as a separate field if its present
	if trace_id := creqit.monitor.get_trace_id():
		scope.set_tag("creqit_trace_id", trace_id)
	...
	try:
		...
		with hub.configure_scope() as scope:
			if (
				...
				set_scope(scope)
			if creqit.request:
				evt_processor = _make_wsgi_event_processor(creqit.request.environ, False)
				scope.add_event_processor(evt_processor)
				if creqit.request.is_json:
					scope.set_context("JSON Body", creqit.request.json)
				elif creqit.request.form:
					scope.set_context("Form Data", creqit.request.form)

		if client := hub.client:
			exc_info = sys.exc_info()
			if any(exc_info):
				# Don't report errors which we can't "fix" in code
				if isinstance(exc_info[1], creqit.ValidationError | creqit.PermissionError):
					return
		...
		creqit.logger().error("Failed to capture exception", exc_info=True)
