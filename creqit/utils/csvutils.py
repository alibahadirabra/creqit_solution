...
from creqit.core.doctype.file.file import FILE_ENCODING_OPTIONS
from creqit.utils import cint, comma_or, cstr, flt


def read_csv_content_from_attached_file(doc):
	fileid = creqit.get_all(
		"File",
		fields=["name"],
	...
	try:
		_file = creqit.get_doc("File", fileid)
...
def read_csv_content(fcontent):
	...
	content = [creqit.safe_decode(line) for line in fcontent.splitlines(True)]
	...
	try:
		...
		dialect = sniffer.sniff(sample="\n".join(sample_content), delimiters=creqit.flags.delimiter_options)
	except csv.Error:
	...
	try:
		...
		creqit.msgprint(_("Not a valid Comma Separated Value (CSV File)"))
		raise
...
def send_csv_to_client(args):
	...
	creqit.response["result"] = cstr(to_csv(args.data))
	creqit.response["doctype"] = args.filename
...
def build_csv_response(data, filename):
	creqit.response["result"] = cstr(to_csv(data))
	creqit.response["doctype"] = filename
	creqit.response["type"] = "csv"
...
def check_record(d):
	...
	from creqit.utils.dateutils import parse_date
	...
	for key in d:
		...
		if docfield:
			if docfield.reqd and (val == "" or val is None):
				creqit.msgprint(_("{0} is required").format(docfield.label), raise_exception=1)
...
def import_doc(d, doctype, overwrite, row_idx, submit=False, ignore_links=False):
	...
	if d.get("name") and creqit.db.exists(doctype, d["name"]):
		if overwrite:
			doc = creqit.get_doc(doctype, d["name"])
