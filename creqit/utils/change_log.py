...
import creqit
from creqit import _, safe_decode
...
def get_change_log(user=None):
	...
	last_known_versions = creqit._dict(
		json.loads(creqit.db.get_value("User", user, "last_known_versions") or "{}")
	)
	...
	if "creqit" in current_versions:
		set_in_change_log("creqit", current_versions["creqit"], change_log)

	return change_log


def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(creqit.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = creqit.read_file(file_path)
...
def get_versions():
	...
	Example:
	        ...
	        {
	                "creqit": {
	                        "title": "creqit Framework",
	                        "version": "5.0.0"
	                }
	        }"""
	versions = {}
	for app in creqit.get_installed_apps(_ensure_on_bench=True):
		app_hooks = creqit.get_hooks(app_name=app)
		versions[app] = {
		...
		try:
			versions[app]["version"] = creqit.get_attr(app + ".__version__")
...
def check_for_update():
	if creqit.get_system_settings("disable_system_update_notification"):
		return

	updates = creqit._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def has_app_update_notifications() -> bool:
	return bool(creqit.cache.sismember("changelog-update-user-set", creqit.session.user))
		...
		for vuln in advisory["vulnerabilities"]:
			with suppress(Exception):
				...
				if (
					...
					# XXX: this is not 100% correct, but works for creqit
...
def add_message_to_redis(update_json):
	...
	creqit.cache.set_value("changelog-update-info", json.dumps(update_json))
	user_list = [x.name for x in creqit.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in creqit.get_roles(user)]
	creqit.cache.sadd("changelog-update-user-set", *system_managers)
...
def show_update_popup():
	...
	update_info = creqit.cache.get_value("changelog-update-info")
	if not update_info:
		return
	...
	update_message = ""
	if creqit.cache.sismember("changelog-update-user-set", user):
		for update_type in updates:
			release_links = ""
			for app in updates[update_type]:
				app = creqit._dict(app)
				security_msg = ""
				if app.security_issues:
					...
					security_msg = security_msg.format(creqit.bold(app.security_issues))
	...
	if on_creqitcloud():
		primary_action = {
			"label": _("Update from creqit Cloud"),
			"client_action": "window.open",
			"args": f"https://creqitcloud.com/dashboard/sites/{creqit.local.site}",
		}
	...
	if update_message:
		...
		creqit.cache.srem("changelog-update-user-set", user)


def get_pyproject(app: str) -> dict | None:
	from tomli import load

	pyproject_path = creqit.get_app_path(app, "..", "pyproject.toml")
