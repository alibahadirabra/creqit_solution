...
def creqitClickWrapper(cls, handler):
	class Cls(cls):
...
def main():
	...
	commands.update({"get-creqit-commands": get_creqit_commands, "get-creqit-help": get_creqit_help})
	creqitClickWrapper(click.Group, handle_exception)(commands=commands)(prog_name="bench")


def get_app_groups() -> dict[str, click.Group]:
	"""Get all app groups, put them in main group "creqit" since bench is
	...
	return dict(
		creqit=click.group(
			name="creqit", commands=commands, cls=creqitClickWrapper(click.Group, handle_exception)
		)(app_group)
	)
...
def get_app_group(app: str) -> click.Group:
	if app_commands := get_app_commands(app):
		return click.group(
			name=app, commands=app_commands, cls=creqitClickWrapper(click.Group, handle_exception)
		)(app_group)
...
def app_group(ctx, site=False, force=False, verbose=False, profile=False):
	...
	if ctx.info_name == "creqit":
		ctx.info_name = ""


def get_sites(site_arg: str) -> list[str]:
	if site_arg == "all":
		return creqit.utils.get_sites()
	elif site_arg:
		return [site_arg]
	elif os.environ.get("creqit_SITE"):
		return [os.environ.get("creqit_SITE")]
	elif default_site := creqit.get_conf().default_site:
		return [default_site]
	# This is not supported, just added here for warning.
	elif (site := creqit.read_file("currentsite.txt")) and site.strip():
		click.secho(
			dedent(
				f"""
...
@click.command("get-creqit-commands")
def get_creqit_commands():
	commands = list(get_app_commands("creqit"))
...
@click.command("get-creqit-help")
def get_creqit_help():
	print(click.Context(get_app_groups()["creqit"]).get_help())


def get_apps():
	return creqit.get_all_apps(with_internal_apps=False, sites_path=".")


if __name__ == "__main__":
	if not creqit._dev_server:
