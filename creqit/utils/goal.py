...
from creqit.query_builder.functions import DateFormat, Function
from creqit.query_builder.utils import DocType
from creqit.utils.data import add_to_date, cstr, flt, now_datetime
...
def get_monthly_results(
	...
	date_format = "%m-%Y" if creqit.db.db_type != "postgres" else "MM-YYYY"
...
def get_monthly_goal_graph_data(
	...
	if history and "{" in cstr(history):
		with suppress(ValueError):
			month_to_value_dict = creqit.parse_json(history)

	if month_to_value_dict is None:  # nosemgrep
		doc_filter = {}
		with suppress(ValueError):
			doc_filter = creqit.parse_json(filters or "{}")
