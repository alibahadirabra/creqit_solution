...
class RedisWrapper(redis.Redis):
	...
	def __call__(self):
		"""WARNING: Added for backward compatibility to support creqit.cache().method(...)"""
		return self
	...
	def make_key(self, key, user=None, shared=False):
		...
		return f"{creqit.conf.db_name}|{key}".encode()
	...
	def get_value(self, key, generator=None, user=None, expires=False, shared=False):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in creqit.local
		"""
		original_key = key
		key = self.make_key(key, user, shared)

		if key in creqit.local.cache:
			val = creqit.local.cache[key]
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(creqit.local.cache) if regex.match(cstr(k))]
	...
	def delete_value(self, keys, user=None, make_keys=True, shared=False):
		...
		for key in keys:
			creqit.local.cache.pop(key, None)
	...
	def hset(
		...
		**kwargs,
		...
		# set in local
		creqit.local.cache.setdefault(_name, {})[key] = value
	...
	def hget(self, name, key, generator=None, shared=False):
		_name = self.make_key(name, shared=shared)
		if _name not in creqit.local.cache:
			creqit.local.cache[_name] = {}

		if not key:
			return None

		if key in creqit.local.cache[_name]:
			return creqit.local.cache[_name][key]
		...
		if value is not None:
			...
			creqit.local.cache[_name][key] = value
		elif generator:
			value = generator()
	...
	def hdel(
		...
		:param shared: shared creqit key or not
		:param pipeline: A redis.client.Pipeline object, if this transaction is to be run in a pipeline
		"""
		_name = self.make_key(name, shared=shared)

		name_in_local_cache = _name in creqit.local.cache

		if not isinstance(keys, list | tuple):
			if name_in_local_cache and keys in creqit.local.cache[_name]:
				del creqit.local.cache[_name][keys]
			if pipeline:
		...
		for key in keys:
			if name_in_local_cache:
				if key in creqit.local.cache[_name]:
					del creqit.local.cache[_name][key]
...
def setup_cache():
	if creqit.conf.redis_cache_sentinel_enabled:
		sentinels = [tuple(node.split(":")) for node in creqit.conf.get("redis_cache_sentinels", [])]
		sentinel = get_sentinel_connection(
			sentinels=sentinels,
			sentinel_username=creqit.conf.get("redis_cache_sentinel_username"),
			sentinel_password=creqit.conf.get("redis_cache_sentinel_password"),
			master_username=creqit.conf.get("redis_cache_master_username"),
			master_password=creqit.conf.get("redis_cache_master_password"),
		)
		return sentinel.master_for(
			creqit.conf.get("redis_cache_master_service"),
			redis_class=RedisWrapper,
		)

	return RedisWrapper.from_url(creqit.conf.get("redis_cache"))
