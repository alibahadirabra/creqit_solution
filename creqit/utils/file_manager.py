...
import creqit
from creqit import _, conf
from creqit.query_builder.utils import DocType
from creqit.utils import call_hook_method, cint, cstr, encode, get_files_path, get_hook_method
...
def get_file_url(file_data_name):
	data = creqit.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = creqit.form_dict.doctype
	dn = creqit.form_dict.docname
	file_url = creqit.form_dict.file_url
	filename = creqit.form_dict.filename
	creqit.form_dict.is_private = cint(creqit.form_dict.is_private)

	if not filename and not file_url:
		creqit.msgprint(_("Please select a file or url"), raise_exception=True)
	...
	if dt and dn:
		...
		comment = creqit.get_doc(dt, dn).add_comment(
			"Attachment",
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	# 	creqit.msgprint("URL must start with 'http://' or 'https://'")
	# 	return None, None

	file_url = unquote(file_url)
	file_size = creqit.form_dict.file_size
	...
	try:
		...
		return creqit.get_doc("File", f.duplicate_entry)
	return f


def get_uploaded_content():
	# should not be unicode when reading a file, hence using creqit.form
	if "filedata" in creqit.form_dict:
		if "," in creqit.form_dict.filedata:
			creqit.form_dict.filedata = creqit.form_dict.filedata.rsplit(",", 1)[1]
		creqit.uploaded_content = safe_b64decode(creqit.form_dict.filedata)
		creqit.uploaded_filename = creqit.form_dict.filename
		return creqit.uploaded_filename, creqit.uploaded_content
	else:
		creqit.msgprint(_("No file attached"))
		return None, None
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in creqit.get_all(
		"File", {"content_hash": content_hash, "is_private": is_private}, pluck="name"
	):
		b = creqit.get_doc("File", name)
		return {k: b.get(k) for k in creqit.get_hooks()["write_file_keys"]}
	return False
...
def write_file(content, fname, is_private=0):
	...
	creqit.create_folder(file_path)
	# write the file
	if isinstance(content, str):
...
def remove_all(dt, dn, from_delete=False, delete_permanently=False):
	...
	try:
		for fid in creqit.get_all("File", {"attached_to_doctype": dt, "attached_to_name": dn}, pluck="name"):
			if from_delete:
				# If deleting a doc, directly delete files
				creqit.delete_doc("File", fid, ignore_permissions=True, delete_permanently=delete_permanently)
			else:
...
def remove_file(
	...
	if not (attached_to_doctype and attached_to_name):
		attached = creqit.db.get_value("File", fid, ["attached_to_doctype", "attached_to_name", "file_name"])
		if attached:
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = creqit.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = creqit.flags.in_web_form or doc.has_permission("write")
		if not file_name:
			file_name = creqit.db.get_value("File", fid, "file_name")
...
def delete_file(path):
	...
	if path:
		...
		if parts[0] == "files":
			path = creqit.utils.get_site_path("public", "files", parts[-1])

		else:
			path = creqit.utils.get_site_path("private", "files", parts[-1])
...
def get_file_name(fname, optional_suffix):
	...
	n_records = creqit.get_all("File", {"file_name": fname}, pluck="name")
