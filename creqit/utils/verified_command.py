...
def get_secret():
	from creqit.utils.password import get_encryption_key

	return creqit.local.conf.get("secret") or get_encryption_key()


def verify_request():
	"""Verify if the incoming signed request if it is correct."""
	query_string = creqit.safe_decode(
		creqit.local.flags.signed_query_string or getattr(creqit.request, "query_string", None)
	)
	...
	if signature_string in query_string:
		...
		valid_method = creqit.request.method == "GET"
		valid_request_data = not (creqit.request.form or creqit.request.data)
...
def get_url(cmd, params, nonce=None, secret=None):
	...
	return creqit.utils.get_url("".join(["api/method/", cmd, "?", urlencode(params)]))


def get_signature(params, nonce, secret=None):
	params = "".join(creqit.utils.cstr(p) for p in params.values())
	if not secret:
		secret = creqit.local.conf.get("secret") or "secret"
