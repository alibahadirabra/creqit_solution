...
from creqit.utils import CallbackManager, cint, get_bench_id
from creqit.utils.commands import log
from creqit.utils.redis_queue import RedisQueue
...
def get_queues_timeout() -> dict[str, int]:
	...
	"""
	common_site_config = creqit.get_conf()
...
def enqueue(
	...
	**kwargs,
	...
	duplicate calls
	:param now: if now=True, the method is executed via creqit.call()
	...
	if deduplicate:
		if not job_id:
			creqit.throw(_("`job_id` paramater is required for deduplication."))
		job = get_job(job_id)
		if job and job.get_status() in (JobStatus.QUEUED, JobStatus.STARTED):
			creqit.logger().debug(f"Not queueing job {job.id} because it is in queue already")
			return
		elif job:
	...
	if not is_async and not creqit.flags.in_test:
		from creqit.deprecation_dumpster import deprecation_warning
	...
	call_directly = now or (not is_async and not creqit.flags.in_test)
	if call_directly:
		return creqit.call(method, **kwargs)

	try:
		q = get_queue(queue, is_async=is_async)
	except ConnectionError:
		if creqit.local.flags.in_migrate:
	...
	def enqueue_call():
		return q.enqueue_call(
			...
			kwargs=queue_args,
			at_front=at_front,
			failure_ttl=creqit.conf.get("rq_job_failure_ttl") or RQ_JOB_FAILURE_TTL,
			result_ttl=creqit.conf.get("rq_results_ttl") or RQ_RESULTS_TTL,
			job_id=job_id,
		)

	if enqueue_after_commit:
		creqit.db.after_commit.add(enqueue_call)
		return
...
def enqueue_doc(doctype, name=None, method=None, queue="default", timeout=300, now=False, **kwargs):
	...
	return enqueue(
		"creqit.utils.background_jobs.run_doc_method",
		doctype=doctype,
		name=name,
		doc_method=method,
		queue=queue,
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(creqit.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	for before_job_task in creqit.get_hooks("before_job"):
		creqit.call(before_job_task, method=method_name, kwargs=kwargs, transaction_type="job")

	try:
		retval = method(**kwargs)

	except (creqit.db.InternalError, creqit.RetryBackgroundJobError) as e:
		creqit.db.rollback()

		if retry < 5 and (
			isinstance(e, creqit.RetryBackgroundJobError)
			or (creqit.db.is_deadlocked(e) or creqit.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			...
			creqit.log_error(title=method_name)
			raise

	except Exception as e:
		creqit.db.rollback()
		creqit.log_error(title=method_name)
		creqit.monitor.add_data_to_monitor(exception=e.__class__.__name__)
		creqit.db.commit()
		...
		for after_job_task in creqit.get_hooks("after_job"):
			creqit.call(after_job_task, method=method_name, kwargs=kwargs, result=retval)
		creqit.local.job.after_job.run()
...
class creqitWorker(Worker):
	def work(self, *args, **kwargs):
		self.start_creqit_scheduler()
	...
	def start_creqit_scheduler(self):
		from creqit.utils.scheduler import start_scheduler
...
def start_worker_pool(
	...
	import creqit.database.query  # sqlparse and indirect imports
	import creqit.query_builder  # pypika
	import creqit.utils  # common utils
	import creqit.utils.safe_exec
	import creqit.utils.scheduler
	...
	pool = WorkerPool(
		...
		worker_class=creqitWorker,  # Auto starts scheduler with workerpool
	)
...
def validate_queue(queue: str, default_queue_list: list | None = None) -> None:
	...
	if queue not in default_queue_list:
		creqit.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn(username=None, password=None):
	if not hasattr(creqit.local, "conf"):
		raise Exception("You need to call creqit.init")

	elif not creqit.local.conf.redis_queue:
		raise Exception("redis_queue missing in common_site_config.json")

	global _redis_queue_conn

	cred = creqit._dict()
	if creqit.conf.get("use_rq_auth"):
		if username:
			cred["username"] = username
			cred["password"] = password
		else:
			cred["username"] = creqit.get_site_config().rq_username or get_bench_id()
			cred["password"] = creqit.get_site_config().rq_password
	...
	try:
		...
		log(
			f"Please make sure that Redis Queue runs @ {creqit.get_conf().redis_queue}. Redis reported error: {e!s}",
			colour="red",
		)
		raise
...
def enqueue_test_job():
	enqueue("creqit.utils.background_jobs.test_job", s=100)
...
def create_job_id(job_id: str | None = None) -> str:
	...
	if not job_id:
		job_id = str(uuid4())
	return f"{creqit.local.site}::{job_id}"
...
def truncate_failed_registry(job, connection, type, value, traceback):
	...
	from creqit.utils import create_batch

	conf = creqit.conf if creqit.conf else creqit.get_conf(site=job.kwargs.get("site"))
...
def flush_telemetry():
	...
	to push events."""
	ph = getattr(creqit.local, "posthog", None)
	with suppress(Exception):
		ph and ph.flush()


def _start_sentry():
	sentry_dsn = os.getenv("creqit_SENTRY_DSN")
	if not sentry_dsn:
		return
