...
def make_boilerplate(dest, app_name, no_git=False):
	...
	app_name = creqit.scrub(app_name)
...
def _get_user_inputs(app_name):
	...
	new_app_config = {
		...
		"branch_name": {"prompt": "Branch Name", "default": get_app_branch("creqit")},
	}
...
def copy_from_creqit(rel_path: str, new_app_path: str):
	"""Copy files from creqit app to new app."""
	src = Path(creqit.get_app_path("creqit", "..")) / rel_path
...
def _create_app_boilerplate(dest, hooks, no_git=False):
	creqit.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, creqit.scrub(hooks.app_title)),
		with_init=True,
	)
	creqit.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True)
	creqit.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	creqit.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "pages"), with_init=True
	)
	creqit.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "includes"))
	creqit.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	creqit.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "css"))
	creqit.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "js"))
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "hooks.py"), "w") as f:
		f.write(creqit.as_unicode(hooks_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "patches.txt"), "w") as f:
		f.write(creqit.as_unicode(patches_template.format(**hooks)))

	app_directory = os.path.join(dest, hooks.app_name)

	copy_from_creqit(".editorconfig", app_directory)
	copy_from_creqit(".eslintrc", app_directory)
	...
	with open(os.path.join(dest, hooks.app_name, "README.md"), "w") as f:
		f.write(creqit.as_unicode(readme_template.format(**hooks)))

	if not no_git:
		with open(os.path.join(dest, hooks.app_name, ".gitignore"), "w") as f:
			f.write(creqit.as_unicode(gitignore_template.format(app_name=hooks.app_name)))
...
class PatchCreator:
	def __init__(self):
		self.all_apps = creqit.get_all_apps(sites_path=".", with_internal_apps=False)
	...
	def _ask_app_name(self):
		...
		self.app_dir = pathlib.Path(creqit.get_app_path(self.app))
	...
	def _ask_patch_meta_info(self):
		...
		default_filename = creqit.scrub(self.docstring) + ".py"
...
dynamic = ["version"]
dependencies = [
    # "creqit~=15.0.0" # Installed and managed by bench.
]
...
typing-modules = ["creqit.types.DF"]
...
# Desk Notifications
# ------------------
# See creqit.core.notifications.get_notification_config
...
# 	"creqit.desk.doctype.event.event.get_events": "{app_name}.event.get_events"
# }}
#
# each overriding function accepts a `data` argument;
# generated from the base implementation of the doctype dashboard,
# along with any modifications made in other creqit apps
    ...
    steps:
      ...
      - name: Setup
        run: |
          ...
          bench init --skip-redis-config-generation --skip-assets --python "$(which python)" ~/creqit-bench
          mariadb --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL character_set_server = 'utf8mb4'"
          mariadb --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL collation_server = 'utf8mb4_unicode_ci'"

      - name: Install
        working-directory: /home/runner/creqit-bench
        run: |
...
# Read docs to understand patches: https://creqitframework.com/docs/v14/user/en/database-migrations
  ...
  linter:
    ...
    steps:
      ...
      - name: Download Semgrep rules
        run: git clone --depth 1 https://github.com/creqit/semgrep-rules.git creqit-semgrep-rules

      - name: Run Semgrep rules
        run: |
          pip install semgrep
          semgrep ci --config ./creqit-semgrep-rules/rules --config r/python.lang.correctness
...
You can install this app using the [bench](https://github.com/creqit/bench) CLI:
...
- Linters: Runs [creqit Semgrep Rules](https://github.com/creqit/semgrep-rules) and [pip-audit](https://pypi.org/project/pip-audit/) on every pull request.
