...
from creqit.www.printview import validate_print_permission
...
def download_multi_pdf_async(
	...
	creqit.local.response["http_status_code"] = http.HTTPStatus.CREATED
...
def _download_multi_pdf(
	...
	if not isinstance(doctype, dict):
		...
		for idx, ss in enumerate(result):
			try:
				...
				if task_id:
					creqit.publish_realtime(task_id=task_id, message={"message": "Failed"})
		...
		if task_id is None:
			creqit.local.response.filename = "{doctype}.pdf".format(
		...
		for doctype_name in doctype:
			...
			for doc_name in doctype[doctype_name]:
				try:
					...
					if task_id:
						creqit.publish_realtime(task_id=task_id, message="Failed")
					creqit.log_error(
		...
		if task_id is None:
			creqit.local.response.filename = f"{name}.pdf"
	...
	with BytesIO() as merged_pdf:
		...
		if task_id:
			...
			_file.save()
			creqit.publish_realtime(f"task_complete:{task_id}", message={"file_url": _file.unique_url})
		else:
			creqit.local.response.filecontent = merged_pdf.getvalue()
			creqit.local.response.type = "pdf"


from creqit.deprecation_dumpster import read_multi_pdf
...
def download_pdf(doctype, name, format=None, doc=None, no_letterhead=0, language=None, letterhead=None):
	doc = doc or creqit.get_doc(doctype, name)
	validate_print_permission(doc)

	with print_language(language):
		pdf_file = creqit.get_print(
			doctype, name, format, doc=doc, as_pdf=True, letterhead=letterhead, no_letterhead=no_letterhead
		)

	creqit.local.response.filename = "{name}.pdf".format(name=name.replace(" ", "-").replace("/", "-"))
	creqit.local.response.filecontent = pdf_file
	creqit.local.response.type = "pdf"
...
def report_to_pdf(html, orientation="Landscape"):
	...
	creqit.local.response.filename = "report.pdf"
	creqit.local.response.filecontent = get_pdf(html, {"orientation": orientation})
	creqit.local.response.type = "pdf"
...
def print_by_server(
	...
	print_settings = creqit.get_doc("Network Printer Settings", printer_setting)
	try:
		import cups
	except ImportError:
		creqit.throw(_("You need to install pycups to use this feature!"))
	...
	try:
		...
		output = PdfWriter()
		output = creqit.get_print(
			doctype, name, print_format, doc=doc, no_letterhead=no_letterhead, as_pdf=True, output=output
		)
		if not file_path:
			file_path = os.path.join("/", "tmp", f"creqit-pdf-{creqit.generate_hash()}.pdf")
		...
		if (
			...
			creqit.throw(_("PDF generation failed"))
	except cups.IPPError:
		creqit.throw(_("Printing failed"))
