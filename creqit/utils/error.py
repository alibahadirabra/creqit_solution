...
EXCLUDE_EXCEPTIONS = (
	...
	creqit.CSRFTokenError,  # CSRF covers OAuth too
	creqit.SecurityException,
	creqit.InReadOnlyMode,
)
...
def log_error(title=None, message=None, reference_doctype=None, reference_name=None, *, defer_insert=False):
	...
	from creqit.utils.sentry import capture_exception
	...
	traceback = creqit.as_unicode(traceback or creqit.get_traceback(with_context=True))
	...
	if creqit.flags.read_only or defer_insert:
...
def log_error_snapshot(exception: Exception):
	...
	logger = creqit.logger(with_more_info=True)
	...
	def decorator_raise_error_on_no_output(func):
		...
		def wrapper_raise_error_on_no_output(*args, **kwargs):
			...
			if (not response) and raise_error:
				creqit.throw(error_message, error_type or Exception)
			return response
...
def guess_exception_source(exception: str) -> str | None:
	...
	- For creqit.throws the exception source is possibly present after skipping creqit.throw frames
	...
	with suppress(Exception):
		...
		if (probably_source := apps.most_common(1)) and probably_source[0][0] != "creqit":
