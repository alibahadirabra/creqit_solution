# Copyright (c) 2024, creqit Technologies Pvt. Ltd. and contributors
...
class IssueSummary:
	...
	def get_issues(self):
		...
		self.entries = creqit.db.get_all(
			"Issue",
			fields=[
	...
	def get_summary_data(self):
		self.issue_summary_data = creqit._dict()
			...
			if self.filters.based_on == "Assigned To":
				if d._assign:
					for entry in json.loads(d._assign):
						self.issue_summary_data.setdefault(entry, creqit._dict()).setdefault(status, 0.0)
						self.issue_summary_data.setdefault(entry, creqit._dict()).setdefault(
				...
				self.issue_summary_data.setdefault(value, creqit._dict()).setdefault(status, 0.0)
				self.issue_summary_data.setdefault(value, creqit._dict()).setdefault(agreement_status, 0.0)
				self.issue_summary_data.setdefault(value, creqit._dict()).setdefault("total_issues", 0.0)
		...
		if issues:
			if self.filters.based_on == "Assigned To":
				...
				for entry in data:
					...
					for metric in metrics_list:
						self.issue_summary_data.setdefault(value, creqit._dict()).setdefault(metric, 0.0)
