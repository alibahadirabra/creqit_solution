...
import creqit
import creqit.desk.form.assign_to
from creqit.automation.doctype.assignment_rule.test_assignment_rule import (
	TEST_DOCTYPE,
	_make_test_record,
	create_test_doctype,
)
from creqit.desk.form.load import get_assignments
from creqit.desk.listview import get_group_by_count
...
class TestAssign(IntegrationTestCase):
	...
	def test_assign(self):
		...
		if not creqit.db.exists("User", "test@example.com"):
			creqit.get_doc({"doctype": "User", "email": "test@example.com", "first_name": "Test"}).insert()
	...
	def _test_basic_assign_on_document(self, doc):
		...
		creqit.desk.form.assign_to.remove(doc.doctype, doc.name, "test@example.com")

		# assignment is cleared
		assignments = creqit.desk.form.assign_to.get(dict(doctype=doc.doctype, name=doc.name))
	...
	def test_assignment_count(self):
		...
		if not creqit.db.exists("User", "test_assign1@example.com"):
			creqit.get_doc(
				{
					"doctype": "User",
		...
		if not creqit.db.exists("User", "test_assign2@example.com"):
			creqit.get_doc(
				{
					"doctype": "User",
	...
	def test_assignment_removal(self):
		...
		# remove assignment
		creqit.db.set_value("ToDo", new_todo[0].name, "allocated_to", "")

		self.assertFalse(get_assignments("ToDo", todo.name))


def assign(doc, user):
	return creqit.desk.form.assign_to.add(
		{
			"assign_to": [user],
			"doctype": doc.doctype,
			"name": doc.name,
