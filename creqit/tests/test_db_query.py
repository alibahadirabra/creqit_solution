...
from creqit.database.utils import DefaultOrderBy
from creqit.desk.reportview import get_filters_cond
from creqit.handler import execute_cmd
from creqit.model.db_query import DatabaseQuery, get_between_date_filter
from creqit.permissions import add_user_permission, clear_user_permissions_for_doctype
from creqit.query_builder import Column
...
class TestDBQuery(IntegrationTestCase):
	...
	def test_child_table_field_syntax(self):
		note = creqit.get_doc(
			doctype="Note",
			title=f"Test {creqit.utils.random_string(8)}",
			content="test",
	...
	def test_child_table_join(self):
		creqit.delete_doc_if_exists("DocType", "Parent DocType 1")
		creqit.delete_doc_if_exists("DocType", "Parent DocType 2")
		creqit.delete_doc_if_exists("DocType", "Child DocType")
		# child table
		creqit.get_doc(
			{
				"doctype": "DocType",
		...
		# clear records
		creqit.db.delete("Parent DocType 1")
		creqit.db.delete("Parent DocType 2")
		creqit.db.delete("Child DocType")
		...
		# test query
		results1 = creqit.get_all("Parent DocType 1", fields=["name", "child.title as child_title"])
		results2 = creqit.get_all("Parent DocType 2", fields=["name", "child.title as child_title"])
	...
	def test_link_field_syntax(self):
		todo = creqit.get_doc(doctype="ToDo", description="Test ToDo", allocated_to="Administrator").insert()
		result = creqit.get_all(
			"ToDo",
	...
	def test_build_match_conditions(self):
		...
		test2user = creqit.get_doc("User", "test2@example.com")
	...
	def test_string_as_field(self):
		self.assertEqual(
			creqit.get_all("DocType", as_list=True), creqit.get_all("DocType", fields="name", as_list=True)
		)

	def test_none_filter(self):
		query = creqit.qb.get_query("DocType", fields="name", filters={"restrict_to_domain": None})
	...
	def test_between_filters_date_bounds(self):
		date_df = creqit._dict(fieldtype="Date")
		datetime_df = creqit._dict(fieldtype="Datetime")
		today = creqit.utils.nowdate()
	...
	def test_ignore_permissions_for_get_filters_cond(self):
		creqit.set_user("test2@example.com")
		self.assertRaises(creqit.PermissionError, get_filters_cond, "DocType", dict(istable=1), [])
	...
	def test_query_fields_sanitizer(self):
		...
		with self.assertRaises(creqit.DataError):
			DatabaseQuery("DocType").execute(
				fields=[
					"name",
					"issingle",
	...
	def test_nested_permission(self):
		...
		from creqit.core.page.permission_manager.permission_manager import update
	...
	def test_of_not_of_descendant_ancestors(self):
		...
		data = creqit.get_all("Nested DocType", {"name": ("descendants of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)

		data = creqit.get_all("Nested DocType", {"name": ("descendants of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = creqit.get_all("Nested DocType", {"name": ("ancestors of", "Level 2 A")})
		...
		data = creqit.get_all("Nested DocType", {"name": ("ancestors of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = creqit.get_all("Nested DocType", {"name": ("not descendants of", "Level 2 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		...
		data = creqit.get_all("Nested DocType", {"name": ("not descendants of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		...
		data = creqit.get_all("Nested DocType", {"name": ("not ancestors of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = creqit.get_all("Nested DocType", {"name": ("not ancestors of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = creqit.get_all("Nested DocType", {"name": ("ancestors of", "Root")})
		self.assertTrue(len(data) == 0)
		self.assertTrue(
			len(creqit.get_all("Nested DocType", {"name": ("not ancestors of", "Root")}))
			== len(creqit.get_all("Nested DocType"))
		)
	...
	def test_is_set_is_not_set(self):
		...
		creqit.db.set_value("DocType", "Property Setter", "autoname", None, update_modified=False)
	...
	def test_virtual_field_get_list(self):
		try:
			creqit.get_list("Prepared Report", ["*"])
			creqit.get_list("Scheduled Job Type", ["*"])
		except Exception:
	...
	def test_prepare_select_args(self):
		# creqit.get_all inserts modified field into order_by clause
	...
	def test_column_comparison(self):
		...
		users_unedited = creqit.get_all(
			"User",
			filters={"creation": Column("modified")},
			fields=["name", "creation", "modified"],
			limit=1,
		)
		users_edited = creqit.get_all(
			"User",
			filters={"creation": ("!=", Column("modified"))},
			fields=["name", "creation", "modified"],
			limit=1,
	...
	def test_cast_name(self):
		...
		creqit.delete_doc_if_exists("DocType", "autoinc_dt_test")
	...
	def test_fieldname_starting_with_int(self):
		...
		creqit.delete_doc_if_exists("DocType", "dt_with_int_named_fieldname")
		creqit.delete_doc_if_exists("DocType", "table_dt")
		...
		dt_data = creqit.get_doc({"doctype": "dt_with_int_named_fieldname", "1field": "10"}).insert(
			ignore_permissions=True
		)
	...
	def test_permission_query_condition(self):
		from creqit.desk.doctype.dashboard_settings.dashboard_settings import (
		...
		dashboard_settings = creqit.db.sql(
			f"""
				SELECT name
	...
	def test_virtual_doctype(self):
		...
		with patch(
			"creqit.controllers",
			new={creqit.local.site: {"Virtual DocType": VirtualDocType}},
		):
			creqit.get_all("Virtual DocType", filters={"name": "test"}, fields=["name"], limit=1)

	def test_coalesce_with_in_ops(self):
		self.assertNotIn("ifnull", creqit.get_all("User", {"first_name": ("in", ["a", "b"])}, run=0))
		self.assertIn("ifnull", creqit.get_all("User", {"first_name": ("in", ["a", None])}, run=0))
		self.assertIn("ifnull", creqit.get_all("User", {"first_name": ("in", ["a", ""])}, run=0))
		self.assertIn("ifnull", creqit.get_all("User", {"first_name": ("in", [])}, run=0))
		self.assertIn("ifnull", creqit.get_all("User", {"first_name": ("not in", ["a"])}, run=0))
		self.assertIn("ifnull", creqit.get_all("User", {"first_name": ("not in", [])}, run=0))
		self.assertIn("ifnull", creqit.get_all("User", {"first_name": ("not in", [""])}, run=0))

		# primary key is never nullable
		self.assertNotIn("ifnull", creqit.get_all("User", {"name": ("in", ["a", None])}, run=0))
		self.assertNotIn("ifnull", creqit.get_all("User", {"name": ("in", ["a", ""])}, run=0))
		self.assertNotIn("ifnull", creqit.get_all("User", {"name": ("in", (""))}, run=0))
		self.assertNotIn("ifnull", creqit.get_all("User", {"name": ("in", ())}, run=0))

	def test_coalesce_with_datetime_ops(self):
		self.assertNotIn("ifnull", creqit.get_all("User", {"last_active": (">", "2022-01-01")}, run=0))
		self.assertNotIn("ifnull", creqit.get_all("User", {"creation": ("<", "2022-01-01")}, run=0))
		self.assertNotIn(
			"ifnull",
			creqit.get_all("User", {"last_active": ("between", ("2022-01-01", "2023-01-01"))}, run=0),
		)
		self.assertIn("ifnull", creqit.get_all("User", {"last_active": ("<", "2022-01-01")}, run=0))

	def test_ambiguous_linked_tables(self):
		from creqit.desk.reportview import get

		if not creqit.db.exists("DocType", "Related Todos"):
			creqit.get_doc(
				{
					"doctype": "DocType",
					"custom": 1,
			...
			creqit.db.delete("Related Todos")

		todo_one = creqit.get_doc(
			{
				"doctype": "ToDo",
		...
		creqit.form_dict.doctype = "Related Todos"
		creqit.form_dict.fields = [
	...
	def test_select_star_expansion(self):
		count = creqit.get_list("Language", ["SUM(1)", "COUNT(*)"], as_list=1, order_by=None)[0]
		self.assertEqual(count[0], creqit.db.count("Language"))
		self.assertEqual(count[1], creqit.db.count("Language"))
...
class TestReportView(IntegrationTestCase):
	...
	def test_get_count(self):
		...
		count = execute_cmd("creqit.desk.reportview.get_count")
		creqit.local.form_dict = creqit._dict(
			{
				"doctype": "DocType",
		...
		dict_filter_response = execute_cmd("creqit.desk.reportview.get_count")
		...
		child_filter_response = execute_cmd("creqit.desk.reportview.get_count")
		current_value = creqit.db.sql(
	...
	def test_reportview_get(self):
		...
		response = execute_cmd("creqit.desk.reportview.get")
	...
	def test_reportview_get_permlevel_system_users(self):
		with setup_patched_blog_post(), setup_test_user(set_user=True):
			...
			self.assertEqual(creqit.get_list("Blog Post", "published"), [])
	...
	def test_db_filter_not_set(self):
		...
		"""
		creqit.get_doc({"doctype": "ToDo", "description": "filter test"}).insert()
		creqit.get_doc({"doctype": "ToDo", "description": "filter test", "reference_name": ""}).insert()
		...
		self.assertEqual(
			len(creqit.get_all("ToDo", filters={"reference_name": ["is", "not set"]})),
			creqit.db.count("ToDo", {"reference_name": ["is", "not set"]}),
		)


def add_child_table_to_blog_post():
	child_table = creqit.get_doc(
		{
			"doctype": "DocType",
			"istable": 1,
			"custom": 1,
...
def create_nested_doctype():
	if creqit.db.exists("DocType", "Nested DocType"):
		return
...
def create_nested_doctype_records():
	...
	for r in records:
		d = creqit.new_doc("Nested DocType")
		d.update(r)
