...
if TYPE_CHECKING:
	from creqit.model import Document
...
def freeze_time(time_to_freeze: Any, is_utc: bool = False, *args: Any, **kwargs: Any) -> None:
	...
	from creqit.utils.data import get_datetime, get_system_timezone
...
def patch_hooks(overridden_hooks: dict) -> None:
	...
	with patch.object(creqit, "get_hooks", patched_hooks):
		yield
...
def change_settings(doctype, settings_dict=None, /, commit=False, **settings) -> None:
	...
	settings = creqit.get_doc(doctype)
	...
	creqit.db.value_cache[settings] = {}
	if commit:
		creqit.db.commit()
	yield
	settings = creqit.get_doc(doctype)
...
def switch_site(site: str) -> None:
	...
	old_site = creqit.local.site
	creqit.init(site, force=True)
	creqit.connect()
	yield
	creqit.init(old_site, force=True)
	creqit.connect()
...
def enable_safe_exec() -> None:
	...
	from creqit.utils.safe_exec import SAFE_EXEC_CONFIG_KEY

	conf = os.path.join(creqit.local.sites_path, "common_site_config.json")
...
def trace_fields(
	...
	**field_configs: dict[str, dict[str, list | Callable]],
	...
	Yields:
	    Document class
	"""
	from creqit.model.trace import traced_field
...
# NOTE: declare those who should also be made available directly creqit.tests.* namespace
