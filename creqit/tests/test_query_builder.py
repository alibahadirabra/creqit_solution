...
import creqit
from creqit.query_builder import Case
from creqit.query_builder.builder import Function
from creqit.query_builder.custom import ConstantColumn
from creqit.query_builder.functions import (
	Cast_,
	Coalesce,
	CombineDatetime,
	Date,
...
def run_only_if(dbtype: db_type_is) -> Callable:
	return unittest.skipIf(db_type_is(creqit.conf.db_type) != dbtype, f"Only runs for {dbtype.value}")
...
class TestCustomFunctionsMariaDB(IntegrationTestCase):
	...
	def test_constant_column(self):
		query = creqit.qb.from_("DocType").select("name", ConstantColumn("John").as_("User"))
		self.assertEqual(query.get_sql(), "SELECT `name`,'John' `User` FROM `tabDocType`")

	def test_timestamp(self):
		note = creqit.qb.DocType("Note")
		self.assertEqual(
	...
	def test_time(self):
		...
		select_query = creqit.qb.from_(note).select(CombineDatetime(note.posting_date, note.posting_time))
	...
	def test_cast(self):
		...
		self.assertEqual(
			creqit.qb.from_("red").from_(note).select("other", Cast_(note.name, "varchar")).get_sql(),
	...
	def test_round(self):
		...
		query = creqit.qb.from_(note).select(Round(note.price))
		self.assertEqual("select round(`price`,0) from `tabnote`", str(query).lower())

		query = creqit.qb.from_(note).select(Round(note.price, 3))
		self.assertEqual("select round(`price`,3) from `tabnote`", str(query).lower())

	def test_truncate(self):
		note = creqit.qb.DocType("Note")
		query = creqit.qb.from_(note).select(Truncate(note.price, 3))
...
class TestBuilderBase:
	def test_adding_tabs(self):
		self.assertEqual("tabNotes", creqit.qb.DocType("Notes").get_sql())
		self.assertEqual("__Auth", creqit.qb.DocType("__Auth").get_sql())
		self.assertEqual("Notes", creqit.qb.Table("Notes").get_sql())

	def test_run_patcher(self):
		query = creqit.qb.from_("ToDo").select("*").limit(1)
	...
	def test_agg_funcs(self):
		creqit.db.truncate("Communication")
		sample_data = {
		...
		creqit.get_doc(sample_data).insert()
		sample_data["rating"] = 3
		creqit.get_doc(sample_data).insert()
		sample_data["rating"] = 4
		creqit.get_doc(sample_data).insert()
		self.assertEqual(creqit.qb.max("Communication", "rating"), 4)
		self.assertEqual(creqit.qb.min("Communication", "rating"), 1)
		self.assertAlmostEqual(creqit.qb.avg("Communication", "rating"), 2.666, places=2)
		self.assertEqual(creqit.qb.sum("Communication", "rating"), 8.0)
		creqit.db.rollback()
...
class TestParameterization(IntegrationTestCase):
	def test_where_conditions(self):
		...
		query = creqit.qb.from_(DocType).select(DocType.name).where(DocType.owner == "Administrator' --")
	...
	def test_set_conditions(self):
		DocType = creqit.qb.DocType("DocType")
		query = creqit.qb.update(DocType).set(DocType.value, "some_value")
	...
	def test_where_conditions_functions(self):
		DocType = creqit.qb.DocType("DocType")
		query = (
			creqit.qb.from_(DocType).select(DocType.name).where(Coalesce(DocType.search_fields == "subject"))
		)
	...
	def test_case(self):
		DocType = creqit.qb.DocType("DocType")
		query = creqit.qb.from_(DocType).select(
			Case()
	...
	def test_case_in_update(self):
		DocType = creqit.qb.DocType("DocType")
		query = creqit.qb.update(DocType).set(
			"parent",
			Case()
			.when(DocType.search_fields == "value", "other_value")
			.when(Coalesce(DocType.search_fields == "subject_in_function"), "true_value")
...
class TestBuilderMaria(IntegrationTestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual("SELECT * FROM `tabNotes`", creqit.qb.from_("Notes").select("*").get_sql())
		self.assertEqual("SELECT * FROM `__Auth`", creqit.qb.from_("__Auth").select("*").get_sql())

	def test_get_qb_type(self):
		from creqit.query_builder import get_query_builder

		qb = get_query_builder(creqit.db.db_type)
...
class TestBuilderPostgres(IntegrationTestCase, TestBuilderBase):
	...
	def test_replace_tables(self):
		info_schema = creqit.qb.Schema("information_schema")
		self.assertEqual(
			'SELECT * FROM "pg_stat_all_tables"',
			creqit.qb.from_(info_schema.tables).select("*").get_sql(),
		)

	def test_replace_fields_post(self):
		self.assertEqual("relname", creqit.qb.Field("table_name").get_sql())
...
class TestMisc(IntegrationTestCase):
	...
	def test_union(self):
		...
		role = creqit.qb.DocType("Role")
		users = creqit.qb.from_(user).select(user.name)
		roles = creqit.qb.from_(role).select(role.name)
