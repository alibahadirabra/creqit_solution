...
import creqit
from creqit.desk.listview import get_group_by_count, get_list_settings, set_list_settings
...
class TestListView(IntegrationTestCase):
	def setUp(self):
		if creqit.db.exists("List View Settings", "DocType"):
			creqit.delete_doc("List View Settings", "DocType")
	...
	def test_get_list_settings_with_default_settings(self):
		creqit.get_doc({"doctype": "List View Settings", "name": "DocType"}).insert()
	...
	def test_get_list_settings_with_non_default_settings(self):
		creqit.get_doc({"doctype": "List View Settings", "name": "DocType", "disable_count": 1}).insert()
		settings = get_list_settings("DocType")
		self.assertIsNotNone(settings)
	...
	def test_set_list_settings_without_settings(self):
		...
		settings = creqit.get_doc("List View Settings", "DocType")
	...
	def test_list_view_child_table_filter_with_created_by_filter(self):
		if creqit.db.exists("Note", "Test created by filter with child table filter"):
			creqit.delete_doc("Note", "Test created by filter with child table filter")
	...
	def test_list_view_comment_count(self):
		...
		creqit.form_dict.limit = "1"
		creqit.form_dict.fields = [
			"`tabDocType`.`name`",
		]

		for with_comment_count in (1, True, "1"):
			creqit.form_dict.with_comment_count = with_comment_count
