...
import creqit
from creqit.installer import parse_app_name
from creqit.model.document import Document
from creqit.tests import IntegrationTestCase, MockedRequestTestCase
...
from creqit.utils.change_log import (
	get_source_url,
	parse_github_url,
)
from creqit.utils.data import (
	add_to_date,
	add_years,
	cast,
	cint,
...
from creqit.utils.dateutils import get_dates_from_timegrain
from creqit.utils.diff import _get_value_from_version, get_version_diff, version_query
from creqit.utils.identicon import Identicon
from creqit.utils.image import optimize_image, strip_exif_data
from creqit.utils.make_random import can_make, get_random, how_many
from creqit.utils.response import json_handler
from creqit.utils.synchronization import LockTimeoutError, filelock
	...
	def test_scrub_urls(self):
		html = """
			...
			<div class="more-info">
				...
				<img src="/assets/creqit/test.jpg">
			</div>
			<div style="background-image: url('/assets/creqit/bg.jpg')">
		...
		self.assertTrue(f'<img src="{url}/assets/creqit/test.jpg">' in html)
		self.assertTrue(f"style=\"background-image: url('{url}/assets/creqit/bg.jpg') !important\"" in html)
...
class TestHTMLUtils(IntegrationTestCase):
	def test_clean_email_html(self):
		from creqit.utils.html_utils import clean_email_html
	...
	def test_sanitize_html(self):
		from creqit.utils.html_utils import sanitize_html
...
class TestValidationUtils(IntegrationTestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://creqit.io", throw=True))
		...
		self.assertRaises(creqit.ValidationError, validate_url, "creqit", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://creqit.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(validate_url("bolo://creqit.io", valid_schemes=("http", "https", "ftp", "ftps")))
		self.assertRaises(
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@creqit.com"))
		self.assertTrue(validate_email_address("someone@creqit.com, anyone@creqit.io"))
		self.assertTrue(validate_email_address("test%201@creqit.com"))

		# Invalid address
		self.assertFalse(validate_email_address("someone"))
		self.assertFalse(validate_email_address("someone@----.com"))
		self.assertFalse(validate_email_address("test 1@creqit.com"))
		self.assertFalse(validate_email_address("test@example.com test2@example.com,undisclosed-recipient"))

		# Invalid with throw
		self.assertRaises(
			creqit.InvalidEmailAddressError,
			validate_email_address,
			"someone.com",
			throw=True,
		)

		self.assertEqual(validate_email_address("Some%20One@creqit.com"), "Some%20One@creqit.com")
		self.assertEqual(
			validate_email_address("erp+Job%20Applicant=JA00004@creqit.com"),
			"erp+Job%20Applicant=JA00004@creqit.com",
		)
	...
	def test_valid_phone(self):
		...
		self.assertRaises(
			creqit.InvalidPhoneNumberError,
			validate_phone_number_with_country_code,
			"+420 1234567890",
			"field",
		)
	...
	def test_validate_name(self):
		...
		for name in invalid_names:
			self.assertRaises(creqit.InvalidNameError, validate_name, name, True)


class TestImage(IntegrationTestCase):
	def test_strip_exif_data(self):
		original_image = Image.open(creqit.get_app_path("creqit", "tests", "data", "exif_sample_image.jpg"))
		original_image_content = open(
			creqit.get_app_path("creqit", "tests", "data", "exif_sample_image.jpg"),
			mode="rb",
		).read()
	...
	def test_optimize_image(self):
		image_file_path = creqit.get_app_path("creqit", "tests", "data", "sample_image_for_optimization.jpg")
...
class TestPythonExpressions(IntegrationTestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(creqit.ValidationError, validate_python_code, expr)
...
class TestDiffUtils(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.doc = creqit.get_doc(doctype="Client Script", dt="Client Script", name="test_client_script")
		cls.doc.insert()
		cls.doc.script = "2;"
...
class TestDateUtils(IntegrationTestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(creqit.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				creqit.utils.get_first_day_of_week("2020-12-25"),
				creqit.utils.getdate("2020-12-21"),
			)
			self.assertEqual(
				creqit.utils.get_first_day_of_week("2020-12-20"),
				creqit.utils.getdate("2020-12-14"),
			)

		# Sunday as start of the week
		self.assertEqual(
			creqit.utils.get_first_day_of_week("2020-12-25"),
			creqit.utils.getdate("2020-12-20"),
		)
		self.assertEqual(
			creqit.utils.get_first_day_of_week("2020-12-21"),
			creqit.utils.getdate("2020-12-20"),
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			creqit.utils.get_last_day_of_week("2020-12-24"),
			creqit.utils.getdate("2020-12-26"),
		)
		self.assertEqual(
			creqit.utils.get_last_day_of_week("2020-12-28"),
			creqit.utils.getdate("2021-01-02"),
		)

	def test_is_last_day_of_the_month(self):
		self.assertEqual(creqit.utils.is_last_day_of_the_month("2020-12-24"), False)
		self.assertEqual(creqit.utils.is_last_day_of_the_month("2020-12-31"), True)
...
class TestResponse(IntegrationTestCase):
	def test_json_handler(self):
		...
		GOOD_OBJECT = {
			...
			"doc": [
				creqit.get_doc("System Settings"),
			],
			"iter": [
				{1, 2, 3},
				(1, 2, 3),
...
class TestXlsxUtils(IntegrationTestCase):
	def test_unescape(self):
		from creqit.utils.xlsxutils import handle_html
...
class TestLinkTitle(IntegrationTestCase):
	def test_link_title_doctypes_in_boot_info(self):
		...
		from creqit.boot import get_link_title_doctypes
	...
	def test_link_titles_on_getdoc(self):
		...
		link_titles = creqit.local.response["_link_titles"]
...
class TestAppParser(MockedRequestTestCase):
	def test_app_name_parser(self):
		self.responses.add(
			"HEAD",
			"https://api.github.com/repos/creqit/healthcare",
			status=200,
			json={},
		)
		bench_path = get_bench_path()
		creqit_app = os.path.join(bench_path, "apps", "creqit")
		self.assertEqual("creqit", parse_app_name(creqit_app))
		self.assertEqual("healthcare", parse_app_name("healthcare"))
		self.assertEqual("healthcare", parse_app_name("https://github.com/creqit/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("git@github.com:creqit/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("creqit/healthcare@develop"))
...
class TestIntrospectionMagic(IntegrationTestCase):
	...
	def test_get_newargs(self):
		...
		self.assertEqual(creqit.get_newargs(f, safe_kwargs), safe_kwargs)

		unsafe_args = dict(safe_kwargs)
		unsafe_args.update({"ignore_permissions": True, "flags": {"ignore_mandatory": True}})
		self.assertEqual(creqit.get_newargs(f, unsafe_args), safe_kwargs)
	...
	def test_strip_off_kwargs_when_not_supported(self):
		...
		self.assertEqual(creqit.get_newargs(f, args), {"b": 1})

		# No args
		self.assertEqual(creqit.get_newargs(lambda: None, args), {})
...
class TestLazyLoader(IntegrationTestCase):
	def test_lazy_import_module(self):
		from creqit.utils.lazy_loader import lazy_import

		with Capturing() as output:
			ls = lazy_import("creqit.tests.data.load_sleep")
		self.assertEqual(output, [])

		with Capturing() as output:
			ls.time
		self.assertEqual(["Module `creqit.tests.data.load_sleep` loaded"], output)


class TestIdenticon(IntegrationTestCase):
	def test_get_gravatar(self):
		# developers@creqit.io has a gravatar linked so str URL will be returned
		creqit.flags.in_test = False
		gravatar_url = get_gravatar("developers@creqit.io")
		creqit.flags.in_test = True
		...
		gravatar_url = get_gravatar(f"developers{random_string(6)}@creqit.io")
		self.assertIsInstance(gravatar_url, str)
...
class TestMiscUtils(IntegrationTestCase):
	...
	def test_get_all_sites(self):
		self.assertIn(creqit.local.site, get_sites())

	def test_get_site_info(self):
		info = get_site_info()

		installed_apps = [app["app_name"] for app in info["installed_apps"]]
		self.assertIn("creqit", installed_apps)
...
class TestTypingValidations(IntegrationTestCase):
	...
	def test_validate_whitelisted_api(self):
		...
		for fn, args, kwargs in test_cases:
			with self.assertRaisesRegex(creqit.creqitTypeError, self.ERR_REGEX):
				fn(*args, **kwargs)

	def test_validate_whitelisted_doc_method(self):
		report = creqit.get_last_doc("Report")
...
class TestTBSanitization(IntegrationTestCase):
	def test_traceback_sanitzation(self):
		try:
			...
			args = creqit._dict({"password": "42", "pwd": "42", "safe": "safe_value"})  # noqa: F841
			raise Exception
		except Exception:
...
class TestRounding(IntegrationTestCase):
	...
	def test_default_rounding(self):
		self.assertEqual(creqit.get_system_settings("rounding_method"), "Banker's Rounding")
...
class TestArgumentTypingValidations(IntegrationTestCase):
	def test_validate_argument_types(self):
		...
		from creqit.core.doctype.doctype.doctype import DocType
		from creqit.utils.typing_validations import (
			creqitTypeError,
		...
		self.assertEqual(test_simple_types(1, 2, "1"), (1, 2.0, True))
		with self.assertRaises(creqitTypeError):
		...
		doctype = creqit.get_last_doc("DocType")
...
class TestChangeLog(IntegrationTestCase):
	def test_get_remote_url(self):
		self.assertIsInstance(get_source_url("creqit"), str)

	def test_parse_github_url(self):
		# using creqit as repo in order to be different from the owner
		owner, repo = parse_github_url("https://github.com/creqit/creqit.git")
		self.assertEqual(owner, "creqit")
		self.assertEqual(repo, "creqit")

		owner, repo = parse_github_url("https://github.com/creqit/creqit")
		self.assertEqual(owner, "creqit")
		self.assertEqual(repo, "creqit")

		owner, repo = parse_github_url("git@github.com:creqit/creqit.git")
		self.assertEqual(owner, "creqit")
		self.assertEqual(repo, "creqit")
...
class TestURLTrackers(IntegrationTestCase):
	def test_add_trackers_to_url(self):
		...
		with patch("creqit.db.get_value") as mock_get_value:
	...
	def test_map_trackers(self):
		...
		expected = {
			"utm_source": creqit.get_doc("UTM Source", "test_source"),
			"utm_medium": creqit.get_doc("UTM Medium", "test_medium"),
			"utm_campaign": creqit.get_doc("UTM Campaign", "test_campaign"),
			"utm_content": "test_content",
		}
