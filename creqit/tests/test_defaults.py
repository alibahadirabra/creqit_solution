...
class TestDefaults(IntegrationTestCase):
	...
	def test_user_permission_on_defaults(self):
		...
		old_user = creqit.session.user
		user = "test@example.com"
		creqit.set_user(user)

		perm_doc = creqit.get_doc(
			doctype="User Permission", user=creqit.session.user, allow="Language", for_value="en-GB"
		...
		creqit.delete_doc("User Permission", perm_doc.name)
		creqit.set_user(old_user)
	...
	def test_user_permission_defaults(self):
		...
		perm_doc = creqit.get_doc(
			doctype="User Permission", user=creqit.session.user, allow="Country", for_value="India"
		).insert(ignore_permissions=True)

		creqit.db.set_value("User Permission", perm_doc.name, "is_default", 1)
		set_global_default("Country", "United States")
		self.assertEqual(get_user_default("Country"), "India")

		creqit.db.set_value("User Permission", perm_doc.name, "is_default", 0)
		clear_user_default("Country")
		self.assertEqual(get_user_default("Country"), None)

		perm_doc = creqit.get_doc(
			doctype="User Permission", user=creqit.session.user, allow="Country", for_value="United States"
		).insert(ignore_permissions=True)
