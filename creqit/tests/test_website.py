...
from creqit.website.serve import get_response, get_response_content
from creqit.website.utils import build_response, clear_website_cache, get_home_page
...
class TestWebsite(IntegrationTestCase):
	...
	def test_home_page(self):
		...
		role = creqit.get_doc(doctype="Role", role_name="home-page-test", desk_access=0).insert(
		...
		creqit.db.set_value("Role", "home-page-test", "home_page", "home-page-test")
		creqit.set_user("test-user-for-home-page@example.com")
		self.assertEqual(get_home_page(), "home-page-test")

		creqit.set_user("Administrator")
		creqit.db.set_value("Role", "home-page-test", "home_page", "")

		# home page via portal settings
		creqit.db.set_single_value("Portal Settings", "default_portal_home", "test-portal-home")

		creqit.set_user("test-user-for-home-page@example.com")
		creqit.cache.hdel("home_page", creqit.session.user)
		self.assertEqual(get_home_page(), "test-portal-home")

		creqit.db.set_single_value("Portal Settings", "default_portal_home", "")
		clear_website_cache()

		# home page via website settings
		creqit.db.set_single_value("Website Settings", "home_page", "contact")
		self.assertEqual(get_home_page(), "contact")

		creqit.db.set_single_value("Website Settings", "home_page", None)
		clear_website_cache()
		...
		with patch.object(
			...
			"get_hooks",
			patched_get_hooks(
				"get_website_user_home_page", ["creqit.www._test._test_home_page.get_website_user_home_page"]
			),
		):
			self.assertEqual(get_home_page(), "_test/_test_folder")

		clear_website_cache()
		with patch.object(creqit, "get_hooks", patched_get_hooks("website_user_home_page", ["login"])):
			self.assertEqual(get_home_page(), "login")

		clear_website_cache()
		with patch.object(creqit, "get_hooks", patched_get_hooks("home_page", ["about"])):
			self.assertEqual(get_home_page(), "about")

		clear_website_cache()
		with patch.object(
			creqit, "get_hooks", patched_get_hooks("role_home_page", {"home-page-test": ["home-page-test"]})
		):
			self.assertEqual(get_home_page(), "home-page-test")
	...
	def test_page_load(self):
		...
		html = creqit.safe_decode(response.get_data())
	...
	def test_redirect(self):
		...
		creqit.hooks.website_redirects = [
		...
		website_settings = creqit.get_doc("Website Settings")
		...
		delattr(creqit.hooks, "website_redirects")
		creqit.cache.delete_key("app_hooks")
	...
	def test_custom_page_renderer(self):
		...
		def patched_get_hooks(*args, **kwargs):
			...
			if args and args[0] == "page_renderer":
				return_value = ["creqit.tests.test_website.CustomPageRenderer"]
			return return_value

		with patch.object(creqit, "get_hooks", patched_get_hooks):
	...
	def test_printview_page(self):
		creqit.db.value_cache[("DocType", "Language", "name")] = (("Language",),)
		creqit.set_user("Administrator")
	...
	def test_json_sidebar_data(self):
		creqit.flags.look_for_sidebar = False
		content = get_response_content("/_test/_test_folder/_test_page")
		self.assertNotIn("Test Sidebar", content)
		clear_website_cache()
		creqit.flags.look_for_sidebar = True
		content = get_response_content("/_test/_test_folder/_test_page")
	...
	def test_caching(self):
		# to enable caching
		creqit.flags.force_website_cache = True
	...
	def test_safe_render(self):
		...
		self.assertNotIn("creqit.exceptions.ValidationError: Illegal template", content)

	def test_never_render(self):
		from pathlib import Path
		from random import choices

		WWW = Path(creqit.get_app_path("creqit")) / "www"
	...
	def test_resolve_class(self):
		from creqit.utils.jinja_globals import resolve_class

		context = creqit._dict(primary=True)
		...
		self.assertEqual(
			creqit.render_template(content, context), '<a class="btn btn-default btn-primary">Test</a>'
		)
	...
	def test_app_include(self):
		...
		with patch.object(creqit, "get_hooks", patched_get_hooks):
			creqit.set_user("Administrator")
			creqit.hooks.app_include_js.append("test_app_include.js")
			creqit.hooks.app_include_css.append("test_app_include.css")
			creqit.conf.update({"app_include_js": ["test_app_include_via_site_config.js"]})
			creqit.conf.update({"app_include_css": ["test_app_include_via_site_config.css"]})
