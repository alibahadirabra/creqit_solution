...
from creqit.desk.form import linked_with
...
class TestLinkedWith(IntegrationTestCase):
	...
	def test_get_doctype_references_by_dlink_field(self):
		...
		parent_record = creqit.get_doc({"doctype": "Parent DocType"}).insert()
	...
	def test_check_delete_integrity(self):
		...
		doc = creqit.get_doc({"doctype": "Parent DocType"}).insert()
		doc.submit()
		doc.cancel()

		amendment = creqit.copy_doc(doc)
		amendment.amended_from = doc.name
		amendment.docstatus = 0
		amendment.insert()
		amendment.submit()

		self.assertRaises(creqit.LinkExistsError, doc.delete)
	...
	def test_reserved_keywords(self):
		...
		linked_doc = creqit.new_doc(dt_name).insert().submit()

		second_doc = (
			creqit.new_doc(dt_name, **{"from": linked_doc.doctype, "order": linked_doc.name})
			.insert()
			.submit()
		)

		with savepoint(creqit.LinkExistsError):
