...
import creqit
from creqit.app import make_form_dict
from creqit.core.doctype.doctype.test_doctype import new_doctype
from creqit.desk.doctype.note.note import Note
from creqit.model.naming import make_autoname, parse_naming_series, revert_series_if_last
from creqit.tests import IntegrationTestCase
from creqit.utils import cint, now_datetime, set_request
...
class TestDocument(IntegrationTestCase):
	...
	def test_load_single(self):
		d = creqit.get_doc("Website Settings", "Website Settings")
	...
	def test_insert(self):
		...
		self.assertEqual(creqit.db.get_value("Event", d.name, "subject"), "test-doc-test-event 1")
	...
	def test_website_route_default(self):
		...
		doc = creqit.get_doc({"doctype": parent, "child_table": [{"some_fieldname": "xasd"}]}).insert()
	...
	def test_insert_with_child(self):
		...
		d.insert()
		self.assertTrue(d.name.startswith("EV"))
		self.assertEqual(creqit.db.get_value("Event", d.name, "subject"), "test-doc-test-event 2")
	...
	def test_update(self):
		...
		self.assertEqual(creqit.db.get_value(d.doctype, d.name, "subject"), "subject changed")
	...
	def test_discard_transitions(self):
		...
		d.submit()
		self.assertRaises(creqit.ValidationError, d.discard)
		d.reload()
	...
	def test_save_on_discard_throws(self):
		from creqit.desk.doctype.event.event import Event
		...
		self.assertRaises(creqit.ValidationError, d3.discard)
	...
	def test_mandatory(self):
		...
		creqit.delete_doc_if_exists("User", "test_mandatory@example.com", 1)
		...
		self.assertRaises(creqit.MandatoryError, d.insert)

		d.set("first_name", "Test Mandatory")
		d.insert()
		self.assertEqual(creqit.db.get_value("User", d.name), d.name)

	def test_text_editor_field(self):
		try:
			creqit.get_doc(doctype="Activity Log", subject="test", message='<img src="test.png" />').insert()
		except creqit.MandatoryError:
			self.fail("Text Editor false positive mandatory error")

	def test_conflict_validation(self):
		d1 = self.test_insert()
		d2 = creqit.get_doc(d1.doctype, d1.name)
		d1.save()
		self.assertRaises(creqit.TimestampMismatchError, d2.save)

	def test_conflict_validation_single(self):
		d1 = creqit.get_doc("Website Settings", "Website Settings")
		d1.home_page = "test-web-page-1"

		d2 = creqit.get_doc("Website Settings", "Website Settings")
	...
	def test_permission(self):
		...
		self.assertRaises(creqit.PermissionError, self.test_insert)
		creqit.set_user("Administrator")

	def test_permission_single(self):
		creqit.set_user("Guest")
		d = creqit.get_doc("Website Settings", "Website Settings")
		self.assertRaises(creqit.PermissionError, d.save)
		creqit.set_user("Administrator")

	def test_link_validation(self):
		creqit.delete_doc_if_exists("User", "test_link_validation@example.com", 1)
		...
		self.assertRaises(creqit.LinkValidationError, d.insert)
	...
	def test_validate(self):
		...
		self.assertRaises(creqit.ValidationError, d.validate)
		self.assertRaises(creqit.ValidationError, d.run_method, "validate")
		self.assertRaises(creqit.ValidationError, d.save)
	...
	def test_new_doc_with_fields(self):
		user = creqit.new_doc("User", first_name="wizard")
		self.assertEqual(user.first_name, "wizard")

	def test_update_after_submit(self):
		d = self.test_insert()
		d.starts_on = "2014-09-09"
		self.assertRaises(creqit.UpdateAfterSubmitError, d.validate_update_after_submit)
	...
	def test_varchar_length(self):
		...
		self.assertRaises(creqit.CharacterLengthExceededError, d.save)
	...
	def test_naming_series(self):
		...
		for series in data:
			...
			old_current = creqit.db.get_value("Series", prefix, "current", order_by="name")

			revert_series_if_last(series, name)
			new_current = cint(creqit.db.get_value("Series", prefix, "current", order_by="name"))

			self.assertEqual(cint(old_current) - 1, new_current)

	def test_non_negative_check(self):
		creqit.delete_doc_if_exists("Currency", "creqit Coin", 1)

		d = creqit.get_doc(
			{"doctype": "Currency", "currency_name": "creqit Coin", "smallest_currency_fraction_value": -1}
		)

		self.assertRaises(creqit.NonNegativeError, d.insert)

		d.set("smallest_currency_fraction_value", 1)
		d.insert()
		self.assertEqual(creqit.db.get_value("Currency", d.name), d.name)
	...
	def test_get_formatted(self):
		...
		creqit.delete_doc_if_exists("Currency", "INR", 1)
		...
		d = creqit.get_doc({"doctype": "Test Formatted", "currency": 100000})
	...
	def test_virtual_fields(self):
		...
		creqit.db.delete("Custom Field", {"dt": "Note", "fieldname": "age"})
		...
		def patch_note(class_=None):
			return patch("creqit.controllers", new={creqit.local.site: {"Note": class_ or CustomTestNote}})

		@contextmanager
		def customize_note(with_options=False):
			options = (
				"creqit.utils.now_datetime() - creqit.utils.get_datetime(doc.creation)"
				if with_options
				else ""
			)
			custom_field = creqit.get_doc(
				{
	...
	def test_error_on_saving_new_doc_with_name(self):
		...
		doc = creqit.get_doc(
			{
				"doctype": "ToDo",
				"description": "this should raise creqit.DoesNotExistError",
				"name": "lets-trick-doc-save",
			}
		)

		self.assertRaises(creqit.DoesNotExistError, doc.save)
	...
	def test_validate_from_to_dates(self):
		...
		self.assertRaises(
			creqit.exceptions.InvalidDates, doc.validate_from_to_dates, "start_date", "end_date"
		)

	def test_db_set_singles(self):
		c = creqit.get_doc("Contact Us Settings")
		key, val = "email_id", "admin1@example.com"
		c.db_set(key, val)
		changed_val = creqit.db.get_single_value(c.doctype, key)
...
class TestDocumentWebView(IntegrationTestCase):
	def get(self, path, user="Guest"):
		...
		make_form_dict(creqit.local.request)
	...
	def test_web_view_link_authentication(self):
		...
		# expire the key
		document_key_doc = creqit.get_doc("Document Share Key", {"key": document_key})
	...
	def test_base_class_set_correctly_on_has_web_view_change(self):
		...
		from creqit.modules.utils import get_doc_path, scrub

		creqit.flags.allow_doctype_export = True

		creqit.delete_doc_if_exists("DocType", "Test WebViewDocType", force=1)
	...
	def test_bulk_inserts(self):
		from creqit.model.document import bulk_insert

		doctype = "Role Profile"
		child_field = "roles"
		child_doctype = creqit.get_meta(doctype).get_field(child_field).options
		...
		def doc_generator():
			for _ in range(21):
				...
				doc.role_profile = creqit.generate_hash()
		...
		all_docs = set(creqit.get_all(doctype, pluck="name"))
		all_child_docs = set(
			creqit.get_all(
