...
from creqit.utils import get_bench_path, set_request
...
first_lang, second_lang, third_lang, fourth_lang, fifth_lang = choices(
	...
	creqit.get_all("Language", pluck="name", filters=[["name", "not like", "en%"], ["enabled", "=", 1]]),
	k=5,
)
...
class TestTranslate(IntegrationTestCase):
	...
	def tearDown(self):
		creqit.form_dict.pop("_lang", None)
		if self._testMethodName in self.guest_sessions_required:
	...
	def test_clear_cache(self):
		...
		self.assertIsNotNone(creqit.cache.hget(USER_TRANSLATION_KEY, creqit.local.lang))
		self.assertIsNotNone(creqit.cache.hget(MERGED_TRANSLATION_KEY, creqit.local.lang))

		clear_cache()

		self.assertIsNone(creqit.cache.hget(USER_TRANSLATION_KEY, creqit.local.lang))
		self.assertIsNone(creqit.cache.hget(MERGED_TRANSLATION_KEY, creqit.local.lang))

	def test_extract_message_from_file(self):
		data = creqit.translate.get_messages_from_file(translation_string_file)
		bench_path = get_bench_path()
		file_path = creqit.get_app_path("creqit", "tests", "translation_test_file.txt")
	...
	def test_request_language_resolution_with_form_dict(self):
		"""Test for creqit.translate.get_language

		Case 1: creqit.form_dict._lang is set
		"""

		creqit.form_dict._lang = first_lang

		with patch.object(creqit.translate, "get_preferred_language_cookie", return_value=second_lang):
	...
	def test_request_language_resolution_with_cookie(self):
		...
		Case 2: creqit.form_dict._lang is not set, but preferred_language cookie is
		"""

		with patch.object(creqit.translate, "get_preferred_language_cookie", return_value="fr"):
	...
	def test_guest_request_language_resolution_with_cookie(self):
		...
		Case 3: creqit.form_dict._lang is not set, but preferred_language cookie is [Guest User]
		"""
	...
	def test_guest_request_language_resolution_with_request_header(self):
		...
		Case 4: creqit.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is [Guest User]
		"""
	...
	def test_request_language_resolution_with_request_header(self):
		...
		Case 5: creqit.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is
		"""
	...
	def test_load_all_translate_files(self):
		...
		verify_translation_files("creqit")
	...
	def test_python_extractor(self):
		code = textwrap.dedent(
			...
			creqit._("attr")
			_("name")
			creqit._("attr with", context="attr context")
...
def verify_translation_files(app):
	...
	translations_dir = Path(creqit.get_app_path(app)) / "translations"
