...
import creqit
from creqit.auth import LoginAttemptTracker
from creqit.creqitclient import AuthError, creqitClient
from creqit.sessions import Session, get_expired_sessions, get_expiry_in_seconds
from creqit.tests import IntegrationTestCase
from creqit.tests.test_api import creqitAPITestCase
from creqit.utils import get_datetime, get_site_url, now
from creqit.utils.data import add_to_date
from creqit.www.login import _generate_temporary_login_link
...
class TestAuth(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.HOST_NAME = creqit.get_site_config().host_name or get_site_url(creqit.local.site)
	...
	def tearDownClass(cls):
		creqit.delete_doc("User", cls.test_user_email, force=True)
		creqit.local.request_ip = None
		creqit.form_dict.email = None
		creqit.local.response["http_status_code"] = None

	def set_system_settings(self, k, v):
		creqit.db.set_single_value("System Settings", k, v)
		creqit.clear_cache()
		creqit.db.commit()
	...
	def test_allow_login_using_mobile(self):
		...
		creqitClient(self.HOST_NAME, self.test_user_mobile, self.test_user_password)
		creqitClient(self.HOST_NAME, self.test_user_email, self.test_user_password)

		# login by username should fail
		with self.assertRaises(AuthError):
			creqitClient(self.HOST_NAME, self.test_user_name, self.test_user_password)
	...
	def test_deny_multiple_login(self):
		...
		first_login = creqitClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		first_login.get_list("ToDo")

		second_login = creqitClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		second_login.get_list("ToDo")
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")

		third_login = creqitClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")
		with self.assertRaises(Exception):
			second_login.get_list("ToDo")
	...
	def test_disable_user_pass_login(self):
		creqitClient(self.HOST_NAME, self.test_user_email, self.test_user_password).get_list("ToDo")
	...
	def test_correct_cookie_expiry_set(self):
		client = creqitClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
...
class TestSessionExpirty(creqitAPITestCase):
	def test_session_expires(self):
		sid = self.sid  # triggers login for test case login
		s: Session = creqit.local.session_obj
