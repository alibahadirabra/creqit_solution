...
import creqit
from creqit.auth import HTTPRequest, get_login_attempt_tracker, validate_ip_address
...
from creqit.utils import cint, set_request


class TestTwoFactor(IntegrationTestCase):
	def setUp(self):
		self.http_requests = create_http_request()
		self.login_manager = creqit.local.login_manager
	...
	def tearDown(self):
		creqit.local.response["verification"] = None
		creqit.local.response["tmp_id"] = None
		disable_2fa()
		creqit.clear_cache(user=self.user)
	...
	def test_authenticate_for_2factor(self):
		"""Verification obj and tmp_id should be set in creqit.local."""
		authenticate_for_2factor(self.user)
		verification_obj = creqit.local.response["verification"]
		tmp_id = creqit.local.response["tmp_id"]
		self.assertTrue(verification_obj)
		self.assertTrue(tmp_id)
		for k in ["_usr", "_pwd", "_otp_secret"]:
			self.assertTrue(creqit.cache.get(f"{tmp_id}{k}"), f"{k} not available")
	...
	def test_two_factor_is_enabled(self):
		...
		# Scenario 3
		enable_2fa()
		user = creqit.get_doc("User", self.user)
		user.restrict_ip = creqit.local.request_ip
		user.save()
		self.assertTrue(should_run_2fa(self.user))
	...
	def test_confirm_otp_token(self):
		...
		creqit.flags.otp_expiry = 2
		authenticate_for_2factor(self.user)
		tmp_id = creqit.local.response["tmp_id"]
		otp = "wrongotp"
		with self.assertRaises(creqit.AuthenticationError):
		...
		creqit.flags.otp_expiry = None
		if creqit.flags.tests_verbose:
	...
	def test_render_string_template(self):
		...
		args = {"issuer_name": "creqit Technologies"}
		_str = "Verification Code from {{issuer_name}}"
		_str = creqit.render_template(_str, args)
		self.assertEqual(_str, "Verification Code from creqit Technologies")
	...
	def test_otp_attempt_tracker(self):
		...
		with self.assertRaises(creqit.SecurityException):
			confirm_otp_token(self.login_manager, otp=otp, tmp_id=tmp_id)
...
def create_http_request():
	...
	enable_2fa()
	creqit.form_dict["usr"] = "test@example.com"
	creqit.form_dict["pwd"] = "Eastern_43A1W"
	creqit.local.form_dict["cmd"] = "login"
	return HTTPRequest()


def enable_2fa(bypass_two_factor_auth=0, bypass_restrict_ip_check=0):
	"""Enable Two factor in system settings."""
	system_settings = creqit.get_doc("System Settings")
...
def toggle_2fa_all_role(state=None):
	...
	all_role = creqit.get_doc("Role", "All")
