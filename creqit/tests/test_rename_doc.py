...
from creqit.model.rename_doc import bulk_rename, update_document_title
from creqit.modules.utils import get_doc_path
...
def patch_db(endpoints: list[str] | None = None):
	...
	for point in endpoints:
		x = patch(f"creqit.db.{point}", new=lambda: True)
		patched_endpoints.append(x)

	savepoint = "SAVEPOINT_for_test_bulk_rename"
	creqit.db.savepoint(save_point=savepoint)
	try:
...
class TestRenameDoc(IntegrationTestCase):
	...
	def setUpClass(self):
		...
		super().setUpClass()
		self._original_developer_flag = creqit.conf.developer_mode
		creqit.conf.developer_mode = 1
	...
	def tearDownClass(self):
		...
		creqit.delete_doc_if_exists("DocType", "Renamed Doc")
		creqit.db.sql_ddl("drop table if exists `tabRenamed Doc`")

		# delete the documents created
		for docname in self.available_documents:
			creqit.delete_doc(self.test_doctype, docname)

		for dt in self.doctype.values():
			if creqit.db.exists("DocType", dt):
				creqit.delete_doc("DocType", dt)
				creqit.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{dt}`")

		# reset original value of developer_mode conf
		creqit.conf.developer_mode = self._original_developer_flag

	def setUp(self):
		creqit.flags.link_fields = {}
		if self._testMethodName == "test_doc_rename_method":
			self.property_setter = creqit.get_doc(
				{
	...
	def test_rename_doc(self):
		"""Rename an existing document via creqit.rename_doc"""
		old_name = choice(self.available_documents)
		new_name = old_name + ".new"
		self.assertEqual(new_name, creqit.rename_doc(self.test_doctype, old_name, new_name, force=True))
		self.available_documents.remove(old_name)
		self.available_documents.append(new_name)

	def test_merging_docs(self):
		"""Merge two documents via creqit.rename_doc"""
		first_todo, second_todo = sample(self.available_documents, 2)

		second_todo_doc = creqit.get_doc(self.test_doctype, second_todo)
		second_todo_doc.priority = "High"
		second_todo_doc.save()

		merged_todo = creqit.rename_doc(self.test_doctype, first_todo, second_todo, merge=True, force=True)
		merged_todo_doc = creqit.get_doc(self.test_doctype, merged_todo)
		self.available_documents.remove(first_todo)

		with self.assertRaises(DoesNotExistError):
			creqit.get_doc(self.test_doctype, first_todo)
	...
	def test_rename_controllers(self):
		...
		self.assertNotEqual(get_controller(self.doctype.old), creqit.model.document.Document)

		old_doctype_path = get_doc_path("Custom", "DocType", self.doctype.old)

		# rename doc via wrapper API accessible via /desk
		creqit.rename_doc("DocType", self.doctype.old, self.doctype.new)

		# check if database and controllers are updated
		self.assertTrue(creqit.db.exists("DocType", self.doctype.new))
		self.assertFalse(creqit.db.exists("DocType", self.doctype.old))
		self.assertFalse(os.path.exists(old_doctype_path))

	def test_rename_doctype(self):
		"""Rename DocType via creqit.rename_doc"""
		from creqit.core.doctype.doctype.test_doctype import new_doctype

		if not creqit.db.exists("DocType", "Rename This"):
			new_doctype(
				"Rename This",
				fields=[
					{
		...
		to_rename_record = creqit.get_doc(
			{"doctype": "Rename This", "linked_to_doctype": "Rename This"}
		).insert()

		# Rename doctype
		self.assertEqual(
			"Renamed Doc", creqit.rename_doc("DocType", "Rename This", "Renamed Doc", force=True)
		)

		# Test if Doctype value has changed in Link field
		linked_to_doctype = creqit.db.get_value("Renamed Doc", to_rename_record.name, "linked_to_doctype")
		...
		new_name = "ToDo"
		self.assertEqual(new_name, creqit.rename_doc("Renamed Doc", old_name, new_name, force=True))

	def test_update_document_title_api(self):
		test_doctype = "Module Def"
		test_doc = creqit.get_doc(
			{
				"doctype": test_doctype,
				"module_name": f"Test-test_update_document_title_api-{creqit.generate_hash()}",
				"custom": True,
			}
		)
		...
		doc_before = creqit.get_doc(test_doctype, dn)
		return_value = update_document_title(doctype=dt, docname=dn, new_name=new_name)
		doc_after = creqit.get_doc(test_doctype, return_value)
	...
	def test_bulk_rename(self):
		...
		with patch_db(["commit", "rollback"]), patch("creqit.enqueue") as enqueue:
	...
	def test_doc_rename_method(self):
		...
		new_name = f"{name}-{creqit.generate_hash(length=4)}"
		doc = creqit.get_doc(self.test_doctype, name)
		doc.rename(new_name, merge=creqit.db.exists(self.test_doctype, new_name))
	...
	def test_parenttype(self):
		...
		parent_a_instance = creqit.get_doc(
			doctype=parent_a.name, test_table=[{"some_fieldname": "x"}], name="XYZ"
		).insert()

		parent_b_instance = creqit.get_doc(
