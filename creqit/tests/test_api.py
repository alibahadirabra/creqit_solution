...
from creqit.utils import cint, get_test_client, get_url

try:
	_site = creqit.local.site
except Exception:
	_site = None
...
class ThreadWithReturnValue(Thread):
	...
	def run(self):
		if self._target is not None:
			with patch("creqit.app.get_site_name", return_value=self.site):
				header_patch = patch("creqit.get_request_header", new=patch_request_header)
...
class creqitAPITestCase(IntegrationTestCase):
...
class TestResourceAPI(creqitAPITestCase):
	DOCTYPE = "ToDo"
	...
	def setUpClass(cls):
		super().setUpClass()
		for _ in range(20):
			doc = creqit.get_doc({"doctype": "ToDo", "description": creqit.mock("paragraph")}).insert()
	...
	def tearDownClass(cls):
		for name in cls.GENERATED_DOCUMENTS:
			creqit.delete_doc_if_exists(cls.DOCTYPE, name)
		creqit.db.commit()
	...
	def test_get_list_dict(self):
		...
		json = creqit._dict(response.json)
		self.assertEqual(response.status_code, 200)
	...
	def test_create_document(self):
		data = {"description": creqit.mock("paragraph"), "sid": self.sid}
	...
	def test_update_document(self):
		generated_desc = creqit.mock("paragraph")
	...
	def test_run_doc_method(self):
		...
		if response.status_code == 403:
			...
			self.assertEqual(
				response.json.get("exception"), "creqit.exceptions.PermissionError: Not permitted"
			)
...
class TestMethodAPI(creqitAPITestCase):
	def test_ping(self):
	...
	def test_get_user_info(self):
		# test 3: test for /api/method/creqit.realtime.get_user_info
		response = self.get(self.method("creqit.realtime.get_user_info"))
		self.assertEqual(response.status_code, 200)
		self.assertIsInstance(response.json, dict)
	...
	def test_auth_cycle(self):
		...
		response = self.get(self.method("creqit.auth.get_logged_user"))
	...
	def test_logs(self):
		method = "creqit.tests.test_api.test"

		def get_message(resp, msg_type):
			return creqit.parse_json(creqit.parse_json(creqit.parse_json(resp.json)[msg_type])[0])
	...
	def test_array_response(self):
		method = "creqit.tests.test_api.test_array"
...
class TestReadOnlyMode(creqitAPITestCase):
	...
	def test_blocked_writes(self):
		with suppress_stdout():
			response = self.post(
				self.resource("ToDo"), {"description": creqit.mock("paragraph"), "sid": self.sid}
			)
		self.assertEqual(response.status_code, 503)
		self.assertEqual(response.json["exc_type"], "InReadOnlyMode")


class TestWSGIApp(creqitAPITestCase):
	def test_request_hooks(self):
		self.addCleanup(lambda: _test_REQ_HOOK.clear())

		with self.patch_hooks(
			{
				"before_request": ["creqit.tests.test_api.before_request"],
				"after_request": ["creqit.tests.test_api.after_request"],
			}
		):
...
class TestResponse(creqitAPITestCase):
	def test_generate_pdf(self):
		response = self.get(
			"/api/method/creqit.utils.print_format.download_pdf",
	...
	def test_binary_and_csv_response(self):
		def download_template(file_type):
			...
			return self.post(
				"/api/method/creqit.core.doctype.data_import.data_import.download_template",
				{
					"sid": self.sid,
					"doctype": "User",
		...
		from creqit.desk.utils import provide_binary_file
	...
	def test_download_private_file_with_unique_url(self):
		test_content = creqit.generate_hash()
		file = creqit.get_doc(
			{
				"doctype": "File",
...
def generate_admin_keys():
	from creqit.core.doctype.user.user import generate_keys
...
def test(*, fail=False, handled=True, message="Failed"):
	if fail:
		if handled:
			creqit.throw(message)
		else:
			1 / 0
	else:
		creqit.msgprint(message)
