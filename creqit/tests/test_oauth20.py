...
import creqit
from creqit.integrations.oauth2 import encode_params
from creqit.tests import IntegrationTestCase
from creqit.tests.test_api import get_test_client, make_request, suppress_stdout
from creqit.tests.utils import make_test_records

if TYPE_CHECKING:
	from creqit.integrations.doctype.social_login_key.social_login_key import SocialLoginKey


class creqitRequestTestCase(IntegrationTestCase):
	@property
	def sid(self) -> str:
		if not getattr(self, "_sid", None):
			...
			creqit.local.login_manager.login_as("test@example.com")
			self._sid = creqit.session.sid
...
class TestOAuth20(creqitRequestTestCase):
	...
	def setUpClass(cls):
		...
		# Set creqit server URL reqired for id_token generation
		creqit_login_key: "SocialLoginKey" = creqit.new_doc("Social Login Key")
		creqit_login_key.get_social_login_provider("creqit", initialize=True)
		creqit_login_key.base_url = creqit.utils.get_url()
		creqit_login_key.enable_social_login = 0
		creqit_login_key.insert(ignore_if_duplicate=True)
		creqit.db.commit()

	def setUp(self):
		self.TEST_CLIENT = get_test_client()
		self.oauth_client = creqit.new_doc("OAuth Client")
	...
	def test_login_using_authorization_code(self):
		...
		resp = self.get(
			"/api/method/creqit.integrations.oauth2.authorize",
			{
		...
		token_response = self.post(
			"/api/method/creqit.integrations.oauth2.get_token",
	...
	def test_revoke_token(self):
		client = creqit.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = self.post(
			"/api/method/creqit.integrations.oauth2.revoke_token",
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = creqit.get_doc("OAuth Client", self.client_id)
		...
		try:
			session.get(
				get_full_url("/api/method/creqit.integrations.oauth2.authorize"),
...
def check_valid_openid_response(access_token=None, client: "creqitRequestTestCase" = None):
	"""Return True for valid response."""
	# Use token in header
	headers = {}
	URL = "/api/method/creqit.integrations.oauth2.openid_profile"
...
def get_full_url(endpoint):
	...
	return urljoin(creqit.utils.get_url(), endpoint)
