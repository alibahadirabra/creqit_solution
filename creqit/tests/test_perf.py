...
class TestPerformance(IntegrationTestCase):
	def reset_request_specific_caches(self):
		...
		creqit.destroy()  # releases everything on creqit.local
		creqit.init(self.TEST_SITE)
		creqit.connect()
		creqit.clear_cache()

	def setUp(self) -> None:
		self.HOST = creqit.utils.get_site_url(creqit.local.site)

		self.reset_request_specific_caches()

	def test_meta_caching(self):
		creqit.clear_cache()
		creqit.get_meta("User")
	...
	def test_permitted_fieldnames(self):
		...
		doc = creqit.new_doc("Prepared Report")
		# load permitted fieldnames once
		doc.permitted_fieldnames

		with patch("creqit.model.base_document.get_permitted_fields") as mocked:
			doc.as_dict()
			# get_permitted_fields should not be called again
			mocked.assert_not_called()

	def test_set_value_query_count(self):
		creqit.db.set_value("User", "Administrator", "interest", "Nothing")

		with self.assertQueryCount(1):
			creqit.db.set_value("User", "Administrator", "interest", "Nothing")

		with self.assertQueryCount(1):
			creqit.db.set_value("User", {"user_type": "System User"}, "interest", "Nothing")
	...
	def test_get_value_limits(self):
		...
		# Warm up code
		creqit.db.get_values("User", filters=filters[0], limit=1)
		for filter in filters:
			with self.assertRowsRead(1):
				self.assertEqual(1, len(creqit.db.get_values("User", filters=filter, limit=1)))
			with self.assertRowsRead(2):
				self.assertEqual(2, len(creqit.db.get_values("User", filters=filter, limit=2)))

			self.assertEqual(
				len(creqit.db.get_values("User", filters=filter)), creqit.db.count("User", filter)
			)

			with self.assertRowsRead(1):
				creqit.db.get_value("User", filters=filter)

			with self.assertRowsRead(1):
				creqit.db.exists("User", filter)
	...
	def test_req_per_seconds_basic(self):
		...
		req_count = 1000
		client = creqitClient(self.HOST, "Administrator", self.ADMIN_PASSWORD)
	...
	def test_consistent_build_version(self):
		from creqit.utils import get_build_version
...
class TestOverheadCalls(creqitAPITestCase):
	...
	def test_get_doc_overheads(self):
		sid = self.sid
		tables = len(creqit.get_meta("User").get_table_fields())
