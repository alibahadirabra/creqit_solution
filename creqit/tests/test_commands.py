...
import creqit
import creqit.commands.scheduler
import creqit.commands.site
import creqit.commands.utils
import creqit.recorder
from creqit.installer import add_to_installed_apps, remove_app
...
from creqit.utils import add_to_date, get_bench_path, get_bench_relative_path, now
from creqit.utils.backups import BackupGenerator, fetch_latest_backups
from creqit.utils.jinja_globals import bundled_asset
from creqit.utils.scheduler import enable_scheduler, is_scheduler_inactive

_result: Result | None = None
TEST_SITE = "commands-site-O4PN2QK.test"  # added random string tag to avoid collisions
CLI_CONTEXT = creqit._dict(sites=[TEST_SITE])
...
def missing_in_backup(doctypes: list, file: os.PathLike) -> list:
	...
	"""
	predicate = 'COPY public."tab{}"' if creqit.conf.db_type == "postgres" else "CREATE TABLE `tab{}`"
...
@contextmanager
def maintain_locals():
	pre_site = creqit.local.site
	pre_flags = creqit.local.flags.copy()
	pre_db = creqit.local.db

	try:
		yield
	finally:
		post_site = getattr(creqit.local, "site", None)
		if not post_site or post_site != pre_site:
			creqit.init(pre_site)
			creqit.local.db = pre_db
			creqit.local.flags.update(pre_flags)
...
def cli(cmd: Command, args: list | None = None):
	with maintain_locals():
		...
		patch_ctx = patch("creqit.commands.pass_context", pass_test_context)
...
class BaseTestCommands(IntegrationTestCase):
	...
	def setup_test_site(cls):
		cmd_config = {
			"test_site": TEST_SITE,
			"admin_password": creqit.conf.admin_password,
			"db_type": creqit.conf.db_type,
			"db_root_username": creqit.conf.root_login,
			"db_root_password": creqit.conf.root_password,
		}
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("bench --site {site} execute creqit.db.get_database_size")
		self.assertEqual(self.returncode, 0)
		self.assertIsInstance(float(self.stdout), float)

		# test 2: execute a command accessing a normal attribute
		self.execute("bench --site {site} execute creqit.local.site")
		self.assertEqual(self.returncode, 0)
		self.assertIsNotNone(self.stderr)

		# test 3: execute a command expecting an errored output as lacol won't exist
		self.execute("bench --site {site} execute creqit.lacol.site")
		self.assertEqual(self.returncode, 1)
		self.assertIsNotNone(self.stderr)

		# test 4: execute a command with kwargs
		self.execute(
			"bench --site {site} execute creqit.bold --kwargs '{put_here}'",
			{"put_here": '{"text": "DocType"}'},  # avoid escaping errors
		)
		self.assertEqual(self.returncode, 0)
		self.assertEqual(self.stdout, creqit.bold(text="DocType"))
	...
	def test_restore(self):
		...
		global_config = {
			"admin_password": creqit.conf.admin_password,
			"root_login": creqit.conf.root_login,
			"root_password": creqit.conf.root_password,
			"db_type": creqit.conf.db_type,
		}
		...
		with self.switch_site(TEST_SITE):
			public_file = creqit.new_doc(
				"File", file_name=f"test_{creqit.generate_hash()}", content=creqit.generate_hash()
			).insert()
			private_file = creqit.new_doc(
				"File", file_name=f"test_{creqit.generate_hash()}", content=creqit.generate_hash()
			).insert()
		...
		self.execute(
			"bench --site {test_site} execute creqit.utils.backups.fetch_latest_backups",
			site_data,
		)
		...
		with self.switch_site(TEST_SITE):
			...
			creqit.db.sql_ddl("DROP TABLE IF EXISTS `tabToDo`")
		...
		with self.switch_site(TEST_SITE):
			self.assertTrue(creqit.db.table_exists("ToDo", cached=False))
		...
		self.execute(
			"bench --site {test_site} execute" " creqit.utils.backups.fetch_latest_backups --kwargs {kw}",
			site_data,
		)
	...
	def test_partial_restore(self):
			...
			creqit.get_doc(
				{
					...
					"description": creqit.mock("paragraph"),
				}
			).insert()
		creqit.db.commit()
		todo_count = creqit.db.count("ToDo")
		...
		self.assertEqual(self.returncode, 0)
		self.assertEqual(creqit.db.count("ToDo"), todo_count)
	...
	def test_recorder(self):
		...
		creqit.local.cache = {}
		self.assertEqual(creqit.recorder.status(), True)

		self.execute("bench --site {site} stop-recording")
		creqit.local.cache = {}
		self.assertEqual(creqit.recorder.status(), False)
	...
	def test_list_apps(self):
		...
		doctype = creqit.get_single("Installed Applications").installed_applications
		if doctype:
	...
	def test_creqit_site_env(self):
		os.putenv("creqit_SITE", creqit.local.site)
		self.execute("bench execute creqit.ping")
		self.assertEqual(self.returncode, 0)
	...
	def test_set_password(self):
		from creqit.utils.password import check_password
		...
		original_password = creqit.conf.admin_password or "admin"
	...
	@skipIf(
		not (creqit.conf.root_password and creqit.conf.admin_password and creqit.conf.db_type == "mariadb"),
	...
	def test_bench_drop_site_should_archive_site(self):
		...
		self.execute(
			...
			f"--admin-password {creqit.conf.admin_password} "
			f"--db-root-username {creqit.conf.root_login} "
			f"--db-root-password {creqit.conf.root_password} "
			f"--db-type {creqit.conf.db_type} "
		)
		self.assertEqual(self.returncode, 0)
	...
	def test_force_install_app(self):
		...
		# set admin password in site_config as when creqit force installs, we don't have the conf
		self.execute(f"bench --site {TEST_SITE} set-config admin_password {creqit.conf.admin_password}")

		# try installing the creqit_docs app again on test site
		self.execute(f"bench --site {TEST_SITE} install-app {app_name}")
		self.assertIn(f"{app_name} already installed", self.stdout)
		self.assertEqual(self.returncode, 0)

		# force install creqit_docs app on the test site
	...
	def test_different_db_username(self):
		...
		kwargs = {
			...
			"db_root_username": creqit.conf.root_login,
			"db_root_password": creqit.conf.root_password or "",
		}
		self.execute(
	...
	def test_existing_db_username(self):
		...
		if creqit.conf.db_type == "mariadb":
			from creqit.database.mariadb.setup_db import get_root_connection

			root_conn = get_root_connection()
			root_conn.sql(f"CREATE USER '{user}'@'localhost'")
		else:
			from creqit.database.postgres.setup_db import get_root_connection
...
class TestBackups(BaseTestCommands):
	...
	site_backup_path = creqit.utils.get_site_path("private", "backups")
	...
	@skipIf(
		not (creqit.conf.db_type == "mariadb"),
		"Only for MariaDB",
	)
	...
	def test_backup_fails_with_exit_code(self):
		...
		odb = BackupGenerator(
			creqit.conf.db_name,
			creqit.conf.db_name,
			creqit.conf.db_password + "INCORRECT PASSWORD",
	...
	def test_clear_log_table(self):
		d = creqit.get_doc(doctype="Error Log", title="Something").insert()
		d.db_set("creation", "2010-01-01", update_modified=False)
		creqit.db.commit()

		tables_before = creqit.db.get_tables(cached=False)

		self.execute("bench --site {site} clear-log-table --days=30 --doctype='Error Log'")
		self.assertEqual(self.returncode, 0)
		creqit.db.commit()

		self.assertFalse(creqit.db.exists("Error Log", d.name))
		tables_after = creqit.db.get_tables(cached=False)
	...
	def test_backup_only_specific_doctypes(self):
		"""Take a backup with (include) backup options set in the site config `creqit.conf.backup.includes`"""
		self.execute(
	...
	def test_backup_excluding_specific_doctypes(self):
		"""Take a backup with (exclude) backup options set (`creqit.conf.backup.excludes`, `--exclude`)"""
		# test 1: take a backup with creqit.conf.backup.excludes
		self.execute(
	...
	def test_selective_backup_priority_resolution(self):
		"""Take a backup with conflicting backup options set (`creqit.conf.excludes`, `--include`)"""
		self.execute(
	...
	def test_dont_backup_conf(self):
		"""Take a backup ignoring creqit.conf.backup settings (with --ignore-backup-conf option)"""
...
class TestRemoveApp(IntegrationTestCase):
	def test_delete_modules(self):
		from creqit.installer import (
			_delete_doctypes,
			_delete_modules,
			_get_module_linked_doctype_field_map,
		)

		test_module = creqit.new_doc("Module Def")

		test_module.update({"module_name": "RemoveThis", "app_name": "creqit"})
		test_module.save()

		module_def_linked_doctype = creqit.get_doc(
			{
				"doctype": "DocType",
		...
		self.assertFalse(creqit.db.exists("Module Def", test_module.module_name))
		self.assertFalse(creqit.db.exists("DocType", module_def_linked_doctype.name))

	def test_dry_run(self):
		"""Check if dry run in not destructive."""

		# nothing to assert, if this fails rest of the test suite will crumble.
		remove_app("creqit", dry_run=True, yes=True, no_backup=True)


class TestSiteMigration(BaseTestCommands):
	def test_migrate_cli(self):
		with cli(creqit.commands.site.migrate) as result:
...
class TestAddNewUser(BaseTestCommands):
	def test_create_user(self):
		...
		self.assertEqual(self.returncode, 0)
		user = creqit.get_doc("User", "test@gmail.com")
...
class TestBenchBuild(BaseTestCommands):
	def test_build_assets_size_check(self):
		with cli(creqit.commands.utils.build, "--force --production --app creqit") as result:
			self.assertEqual(result.exit_code, 0)
			self.assertEqual(result.exception, None)
		...
		for chunk in default_bundle:
			abs_path = Path.cwd() / creqit.local.sites_path / bundled_asset(chunk)[1:]
...
class TestDBUtils(BaseTestCommands):
	def test_db_add_index(self):
		...
		creqit.db.rollback()
		index_name = creqit.db.get_index_name((field,))
		self.assertTrue(creqit.db.has_index("tabUser", index_name))
		meta = creqit.get_meta("User", cached=False)
...
class TestSchedulerUtils(BaseTestCommands):
	...
	def test_ready_for_migrate(self):
		with cli(creqit.commands.scheduler.ready_for_migration) as result:
			self.assertEqual(result.exit_code, 0)


class TestCommandUtils(IntegrationTestCase):
	def test_bench_helper(self):
		from creqit.utils.bench_helper import get_app_groups

		app_groups = get_app_groups()
		self.assertIn("creqit", app_groups)
		self.assertIsInstance(app_groups["creqit"], click.Group)
...
class TestSchedulerCLI(BaseTestCommands):
	...
	def test_scheduler_status(self):
		...
		parsed_output = creqit.parse_json(self.stdout)
		self.assertEqual(self.returncode, 0)
