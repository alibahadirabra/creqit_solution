...
from creqit.utils import get_bench_id
from creqit.utils.background_jobs import get_redis_conn
...
class TestRedisAuth(IntegrationTestCase):
	...
	@patch.dict(creqit.conf, {"bench_id": "test_bench", "use_rq_auth": False})
	...
	def test_rq_namespace(self):
		...
		# Current bench ID
		bench_id = creqit.conf.get("bench_id")
		conn = get_redis_conn()
		...
		conn.acl_deluser(username)
		creqit.conf.update({"bench_id": tmp_bench_id})
		_ = RedisQueue(conn).add_user(username, password)
		...
		creqit.conf.update({"bench_id": bench_id})
		conn.acl_deluser(username)
