...
class DBTable:
	def __init__(self, doctype, meta=None):
		...
		self.meta = meta or creqit.get_meta(doctype, False)
	...
	def sync(self):
		...
		if self.is_new():
			...
			creqit.cache.hdel("table_columns", self.table_name)
			self.alter()
	...
	def get_column_definitions(self):
		column_list = [*creqit.db.DEFAULT_COLUMNS]
		ret = []
	...
	def get_index_definitions(self):
		return [
			...
			if (
				col.set_index
				and not col.unique
				and col.fieldtype in creqit.db.type_map
				and creqit.db.type_map.get(col.fieldtype)[0] not in ("text", "longtext")
			)
		]
	...
	def get_columns_from_docfields(self):
		...
		if not self.meta.get("istable"):
			for fieldname in creqit.db.OPTIONAL_COLUMNS:
	...
	def validate(self):
		...
		columns = [
			creqit._dict({"fieldname": f, "fieldtype": "Data"}) for f in creqit.db.STANDARD_VARCHAR_COLUMNS
		]
		if self.meta.get("istable"):
			columns += [
				creqit._dict({"fieldname": f, "fieldtype": "Data"}) for f in creqit.db.CHILD_TABLE_COLUMNS
			]
		...
		for col in columns:
			if len(col.fieldname) >= 64:
				creqit.throw(
					_("Fieldname is limited to 64 characters ({0})").format(creqit.bold(col.fieldname))
				)

			if "varchar" in creqit.db.type_map.get(col.fieldtype, ()):
				# validate length range
				new_length = cint(col.length) or cint(creqit.db.VARCHAR_LEN)
				if not (1 <= new_length <= 1000):
					creqit.throw(_("Length of {0} should be between 1 and 1000").format(col.fieldname))
				...
				if cint(current_length) != cint(new_length):
					try:
						...
						max_length = creqit.db.sql(
							f"""SELECT MAX(CHAR_LENGTH(`{col.fieldname}`)) FROM `tab{self.doctype}`"""
						)

					except creqit.db.InternalError as e:
						if creqit.db.is_missing_column(e):
					...
					if max_length and max_length[0][0] and max_length[0][0] > new_length:
						...
						creqit.msgprint(info_message)

	def is_new(self):
		return self.table_name not in creqit.db.get_tables()

	def setup_table_columns(self):
		# TODO: figure out a way to get key data
		for c in creqit.db.get_table_columns_description(self.table_name):
...
class DbColumn:
	...
	def get_definition(self, for_modification=False):
		...
		if self.fieldtype in ("Check", "Int"):
			...
			self.default
			and (self.default not in creqit.db.DEFAULT_SHORTCUTS)
			and not cstr(self.default).startswith(":")
		):
			default = creqit.db.escape(self.default)

		if self.not_nullable and null:
			if default is None:
				default = get_not_null_defaults(self.fieldtype)
				if isinstance(default, str):
					default = creqit.db.escape(default)
			null = False
	...
	if special_characters := SPECIAL_CHAR_PATTERN.findall(n):
		...
		creqit.throw(
			_("Fieldname {0} cannot have special characters like {1}").format(
				creqit.bold(cstr(n)), special_characters
			),
			creqit.db.InvalidColumnName,
		)
	return n


def validate_column_length(fieldname):
	if len(fieldname) > creqit.db.MAX_COLUMN_LENGTH:
		creqit.throw(_("Fieldname is limited to 64 characters ({0})").format(fieldname))


def get_definition(fieldtype, precision=None, length=None, *, options=None):
	d = creqit.db.type_map.get(fieldtype)
	...
	if (
		...
		and creqit.db.exists("DocType", options)
		and creqit.get_meta(options).autoname == "UUID"
	):
		d = ("uuid", None)
	...
	if fieldtype == "Int" and length and length > 11:
		...
		d = creqit.db.type_map.get("Long Int")
	...
	if size:
		...
		if length:
			if coltype == "varchar":
				...
				# NOTE: this will only be applicable for mariadb as creqit implements int
