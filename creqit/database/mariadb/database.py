...
from creqit.database.mariadb.schema import MariaDBTable
from creqit.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if creqit.conf.local_infile:
			conn_settings["local_infile"] = creqit.conf.local_infile

		if creqit.conf.db_ssl_ca and creqit.conf.db_ssl_cert and creqit.conf.db_ssl_key:
			conn_settings["ssl"] = {
				"ca": creqit.conf.db_ssl_ca,
				"cert": creqit.conf.db_ssl_cert,
				"key": creqit.conf.db_ssl_key,
			}
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		# Also, given we're promoting use of the PyPika builder via creqit.qb, the use
		# of this method should be limited.

		# pymysql expects unicode argument to escape_string with Python 3
		s = creqit.as_unicode(escape_string(creqit.as_unicode(s)), "utf-8").replace("`", "\\`")
	...
	def get_column_index(self, table_name: str, fieldname: str, unique: bool = False) -> creqit._dict | None:
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = creqit.cache.get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			creqit.cache.set_value("db_tables", tables)
