...
from creqit.database.schema import DBTable, get_definition
...
class PostgresTable(DBTable):
	def create(self):
		varchar_len = creqit.db.VARCHAR_LEN
		...
		if not self.meta.issingle and self.meta.autoname == "autoincrement":
			creqit.db.create_sequence(self.doctype, check_not_exists=True)
	...
	def create_indexes(self):
		...
		if create_index_query:
			# nosemgrep
			creqit.db.sql(create_index_query)
	...
	def alter(self):
		...
		for col in self.set_default:
			...
			if col.fieldtype in ("Check", "Int"):
				...
				col_default = f"{creqit.db.escape(col.default)}"
		...
		try:
			if query:
				...
				# nosemgrep
				creqit.db.sql(final_alter_query)
			if change_nullability:
				# nosemgrep
				creqit.db.sql(f"ALTER TABLE `{self.table_name}` {','.join(change_nullability)}")
			if create_contraint_query:
				# nosemgrep
				creqit.db.sql(create_contraint_query)
			if drop_contraint_query:
				# nosemgrep
				creqit.db.sql(drop_contraint_query)
		except Exception as e:
			# sanitize
			if creqit.db.is_duplicate_fieldname(e):
				creqit.throw(str(e))
			elif creqit.db.is_duplicate_entry(e):
	...
	def alter_primary_key(self) -> str | None:
		...
		if autoname == "UUID" and creqit.db.get_column_type(self.doctype, "name") != "uuid":
			if not creqit.db.get_value(self.doctype, {}, order_by=None):
		...
		if autoname != "UUID" and creqit.db.get_column_type(self.doctype, "name") == "uuid":
			return f"alter column `name` TYPE varchar({creqit.db.VARCHAR_LEN})"
