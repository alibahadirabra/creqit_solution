...
from creqit.database.postgres.schema import PostgresTable
from creqit.database.utils import EmptyQueryValues, LazyDecode
from creqit.utils import cstr, get_table_name
...
class PostgresExceptionUtil:
	...
	def is_statement_timeout(e):
		return PostgresDatabase.is_timedout(e) or isinstance(e, creqit.QueryTimeoutError)
...
class PostgresDatabase(PostgresExceptionUtil, Database):
	...
	@property
	def db_schema(self):
		return creqit.conf.get("db_schema", "public").replace("'", "").replace('"', "")
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		if (columns := creqit.cache.hget("table_columns", table)) is not None:
			return columns
	...
	def describe(self, doctype: str) -> list | tuple:
		...
		return self.sql(
			f"SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_NAME = '{table_name}' and table_schema='{creqit.conf.get('db_schema', 'public')}'"
		)
