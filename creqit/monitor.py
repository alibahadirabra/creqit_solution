...
def start(transaction_type="request", method=None, kwargs=None):
	if creqit.conf.monitor:
		creqit.local.monitor = Monitor(transaction_type, method, kwargs)


def stop(response=None):
	if hasattr(creqit.local, "monitor"):
		creqit.local.monitor.dump(response)
...
def add_data_to_monitor(**kwargs) -> None:
	...
	if hasattr(creqit.local, "monitor"):
		creqit.local.monitor.add_custom_data(**kwargs)


def get_trace_id() -> str | None:
	"""Get unique ID for current transaction."""
	if monitor := getattr(creqit.local, "monitor", None):
		return monitor.data.uuid


def log_file():
	return os.path.join(creqit.utils.get_bench_path(), "logs", "monitor.json.log")
...
class Monitor:
	...
	def collect_request_meta(self):
		self.data.request = creqit._dict(
			{
				"ip": creqit.local.request_ip,
				"method": creqit.request.method,
				"path": creqit.request.path,
			}
		)

		if request_id := creqit.request.headers.get("X-creqit-Request-Id"):
			self.data.uuid = request_id

	def collect_job_meta(self, method, kwargs):
		self.data.job = creqit._dict({"method": method, "scheduled": False, "wait": 0})
		...
		try:
			...
			if self.data.transaction_type == "request":
				...
				if hasattr(creqit.local, "rate_limiter"):
					limiter = creqit.local.rate_limiter
	...
	def store(self):
		...
		length = creqit.cache.rpush(MONITOR_REDIS_KEY, serialized)
		if cint(length) > MONITOR_MAX_ENTRIES:
			creqit.cache.ltrim(MONITOR_REDIS_KEY, 1, -1)
...
def flush():
	try:
		...
		if logs:
			logs = list(map(creqit.safe_decode, logs))
			with open(log_file(), "a", os.O_NONBLOCK) as f:
				f.write("\n".join(logs))
				f.write("\n")
			# Remove fetched entries from cache
			creqit.cache.ltrim(MONITOR_REDIS_KEY, len(logs) - 1, -1)
	except Exception:
		traceback.print_exc()
