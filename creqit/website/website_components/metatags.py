...
class MetaTags:
	def __init__(self, path, context):
		...
		self.tags = creqit._dict(self.context.get("metatags") or {})
	...
	def init_metatags_from_context(self):
		...
		if self.tags.get("image"):
			self.tags["image"] = creqit.utils.get_url(self.tags["image"])

		self.tags["language"] = creqit.local.lang or "en"
	...
	def set_metatags_from_website_route_meta(self):
		...
		if route == "":
			# homepage
			route = creqit.db.get_single_value("Website Settings", "home_page")

		route_exists = (
			route and not route.endswith((".js", ".css")) and creqit.db.exists("Website Route Meta", route)
		)

		if route_exists:
			website_route_meta = creqit.get_doc("Website Route Meta", route)
