...
class TestWebPage(IntegrationTestCase):
	def setUp(self):
		creqit.db.delete("Web Page")
		for t in self.globalTestRecords["Web Page"]:
			creqit.get_doc(t).insert()
	...
	def test_dynamic_route(self):
		web_page = creqit.get_doc(
			...
			content_type="HTML",
			dynamic_template=1,
			main_section_html="<div>{{ creqit.form_dict.doctype }}</div>",
		).insert()
		try:
			from creqit.utils import get_html_for_route
