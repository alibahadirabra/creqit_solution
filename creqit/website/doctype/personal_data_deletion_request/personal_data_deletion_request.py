...
from creqit.desk.doctype.notification_settings.notification_settings import is_email_notifications_enabled
from creqit.model.document import Document
from creqit.utils import get_datetime, get_fullname, time_diff_in_hours
...
class PersonalDataDeletionRequest(Document):
	...
	if TYPE_CHECKING:
		...
		from creqit.website.doctype.personal_data_deletion_step.personal_data_deletion_step import (
	...
	def __init__(self, *args, **kwargs):
		...
		self.user_data_fields = creqit.get_hooks("user_data_fields")
	...
	def autoname(self):
		from creqit.model.naming import set_name_from_naming_options
		...
		domain = creqit.local.site.replace("_", "-")
		...
		creqit.utils.validate_email_address(self.email, throw=True)

	def after_insert(self):
		self.send_verification_mail()

	def generate_url_for_confirmation(self):
		params = {"email": self.email, "name": self.name, "host_name": creqit.local.site}
		api = creqit.utils.get_url(
			"/api/method/creqit.website.doctype.personal_data_deletion_request"
	...
	def disable_user(self):
		user = creqit.get_doc("User", self.email)
		user.enabled = False
		user.save()
	...
	def send_verification_mail(self):
		...
		creqit.sendmail(
			...
			args={
				"email": self.email,
				"name": self.name,
				"host_name": creqit.utils.get_url(),
				"link": url,
			},
	...
	def notify_system_managers(self):
		...
		creqit.sendmail(
			...
			args={"user": self.email, "url": creqit.utils.get_url(self.get_url())},
	...
	def validate_data_anonymization(self):
		...
		if self.status != "Pending Approval":
			creqit.throw(_("This request has not yet been approved by the user."))
	...
	def generate_anonymization_dict(self, ref):
		...
		meta = creqit.get_meta(ref["doctype"])
	...
	def redact_doc(self, doc, ref):
		...
		if ref.get("rename") and doc["name"] != self.anon:
			creqit.rename_doc(ref["doctype"], doc["name"], self.anon, force=True, show_alert=False)
	...
	def _anonymize_data(self, email=None, anon=None, set_data=True, commit=False):
		...
		creqit.rename_doc("User", email, anon, force=True, show_alert=False)
	...
	def __redact_partial_match_data(self, doctype):
		...
		for df in creqit.get_meta(doctype["doctype"]).fields:
...
def process_data_deletion_request():
	auto_account_deletion = creqit.db.get_single_value("Website Settings", "auto_account_deletion")
	if auto_account_deletion < 1:
		return

	requests = creqit.get_all(
		"Personal Data Deletion Request", filters={"status": "Pending Approval"}, pluck="name"
	)

	for request in requests:
		doc = creqit.get_doc("Personal Data Deletion Request", request)
...
def confirm_deletion(email, name, host_name):
	...
	doc = creqit.get_doc("Personal Data Deletion Request", name)
	host_name = creqit.utils.get_url()
