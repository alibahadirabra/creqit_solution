...
from creqit.core.doctype.file.utils import remove_file_by_url
from creqit.desk.form.meta import get_code_files_via_hooks
from creqit.modules.utils import export_module_json, get_doc_module
from creqit.rate_limiter import rate_limit
from creqit.utils import dict_with_keys, strip_html
from creqit.utils.caching import redis_cache
from creqit.website.utils import get_boot_data, get_comment_list, get_sidebar_items
from creqit.website.website_generator import WebsiteGenerator
...
class WebForm(WebsiteGenerator):
	...
	if TYPE_CHECKING:
		...
		from creqit.website.doctype.web_form_field.web_form_field import WebFormField
		from creqit.website.doctype.web_form_list_column.web_form_list_column import WebFormListColumn
	...
	website = creqit._dict(no_cache=1)

	def validate(self):
		super().validate()

		if not self.module:
			self.module = creqit.db.get_value("DocType", self.doc_type, "module")

		in_user_env = not (
			creqit.flags.in_install
			or creqit.flags.in_patch
			or creqit.flags.in_test
			or creqit.flags.in_fixtures
		)
		if in_user_env and self.is_standard and not creqit.conf.developer_mode:
			...
			if self.has_value_changed("published"):
				...
				creqit.throw(_("You need to be in developer mode to edit a Standard Web Form"))

		if not creqit.flags.in_import:
	...
	def validate_fields(self):
		...
		meta = creqit.get_meta(self.doc_type)
		missing = [
			df.fieldname
		...
		if missing:
			creqit.throw(_("Following fields are missing:") + "<br>" + "<br>".join(missing))
...
"""
	...
	def get_context(self, context):
		...
		if creqit.form_dict.name:
			...
			if not creqit.db.exists(self.doc_type, creqit.form_dict.name):
				raise creqit.PageDoesNotExistError()

			if not self.has_web_form_permission(self.doc_type, creqit.form_dict.name):
				creqit.throw(
					_("You don't have the permissions to access this document"), creqit.PermissionError
				)

		if creqit.local.path == self.route:
			path = f"/{self.route}/list" if self.show_list else f"/{self.route}/new"
			creqit.redirect(path)

		if creqit.form_dict.is_list and not self.show_list:
			creqit.redirect(f"/{self.route}/new")

		if creqit.form_dict.is_edit and not self.allow_edit:
			context.in_view_mode = True
			creqit.redirect(f"/{self.route}/{creqit.form_dict.name}")

		if creqit.form_dict.is_edit:
			context.in_edit_mode = True

		if creqit.form_dict.is_read:
			context.in_view_mode = True

		if (
			not creqit.form_dict.is_edit
			and not creqit.form_dict.is_read
			and self.allow_edit
			and creqit.form_dict.name
		):
			context.in_edit_mode = True
			creqit.redirect(f"/{creqit.local.path}/edit")

		if (
			creqit.session.user != "Guest"
			and self.login_required
			and not self.allow_multiple
			and not creqit.form_dict.name
			and not creqit.form_dict.is_list
		):
			condition_json = json.loads(self.condition_json) if self.condition_json else []
			condition_json.append(["owner", "=", creqit.session.user])
			names = creqit.get_all(self.doc_type, filters=condition_json, pluck="name")
			if names:
				context.in_view_mode = True
				creqit.redirect(f"/{self.route}/{names[0]}")

		# Show new form when
		# - User is Guest
		# - Login not required
		route_to_new = creqit.session.user == "Guest" or not self.login_required
		if not creqit.form_dict.is_new and route_to_new:
			creqit.redirect(f"/{self.route}/new")

		self.reset_field_parent()

		# add keys from form_dict to context
		context.update(dict_with_keys(creqit.form_dict, ["is_list", "is_new", "is_edit", "is_read"]))
		...
		context.boot["link_title_doctypes"] = creqit.boot.get_link_title_doctypes()
	...
	def load_translations(self, context):
		...
		context.translated_messages = creqit.as_json({message: _(message) for message in messages if message})
	...
	def load_form_data(self, context):
		...
		if self.breadcrumbs:
			context.parents = creqit.safe_eval(self.breadcrumbs, {"_": _})

		if self.show_list and creqit.form_dict.is_new:
			context.title = _("New {0}").format(context.title)

		context.has_header = (creqit.form_dict.name or creqit.form_dict.is_new) and (
			creqit.session.user != "Guest" or not self.login_required
		)

		if context.success_message:
			context.success_message = creqit.db.escape(context.success_message.replace("\n", "<br>")).strip(
				"'"
			)
		...
		# load reference doc
		if creqit.form_dict.name:
			context.doc_name = creqit.form_dict.name
			context.reference_doc = creqit.get_doc(self.doc_type, context.doc_name)
			...
			if self.show_attachments:
				context.attachments = creqit.get_all(
					"File",
					filters={
	...
	def add_custom_context_and_script(self, context):
		...
		if self.is_standard:
			...
			if os.path.exists(js_path):
				script = creqit.render_template(open(js_path).read(), context)

				for path in get_code_files_via_hooks("webform_include_js", context.doc_type):
					custom_js = creqit.render_template(open(path).read(), context)
	...
	def has_web_form_permission(self, doctype, name, ptype="read"):
		...
		if self.apply_document_permissions:
			return creqit.get_doc(doctype, name).has_permission(permtype=ptype)

		# owner matches
		elif creqit.db.get_value(doctype, name, "owner") == creqit.session.user:
			return True

		elif creqit.has_website_permission(name, ptype=ptype, doctype=doctype):
			return True
...
def accept(web_form, data):
	"""Save the web form"""
	data = creqit._dict(json.loads(data))

	files = []
	files_to_delete = []

	web_form = creqit.get_doc("Web Form", web_form)
	doctype = web_form.doc_type
	user = creqit.session.user

	if web_form.anonymous and creqit.session.user != "Guest":
		creqit.session.user = "Guest"

	if data.name and not web_form.allow_edit:
		creqit.throw(_("You are not allowed to update this Web Form Document"))

	creqit.flags.in_web_form = True
	meta = creqit.get_meta(doctype)

	if data.name:
		# update
		doc = creqit.get_doc(doctype, data.name)
	else:
		# insert
	...
	if doc.name:
		...
		# insert
		if web_form.login_required and creqit.session.user == "Guest":
			creqit.throw(_("You must login to submit this form"))
	...
	if web_form.anonymous and creqit.session.user == "Guest" and user:
		creqit.session.user = user

	creqit.flags.web_form_doc = doc
	return doc


@creqit.whitelist()
def delete(web_form_name, docname):
	web_form = creqit.get_doc("Web Form", web_form_name)

	owner = creqit.db.get_value(web_form.doc_type, docname, "owner")
	if creqit.session.user == owner and web_form.allow_delete:
		creqit.delete_doc(web_form.doc_type, docname, ignore_permissions=True)
	else:
		raise creqit.PermissionError("Not Allowed")
...
def get_form_data(doctype, docname=None, web_form_name=None):
	...
	if creqit.session.user != "Guest" and not docname and not web_form.allow_multiple:
		docname = creqit.db.get_value(doctype, {"owner": creqit.session.user}, "name")
...
def get_link_options(web_form_name, doctype, allow_read_on_all_link_options=False):
	web_form: WebForm = creqit.get_doc("Web Form", web_form_name)

	if web_form.login_required and creqit.session.user == "Guest":
		creqit.throw(_("You must be logged in to use this form."), creqit.PermissionError)

	if not web_form.published or not any(f for f in web_form.web_form_fields if f.options == doctype):
		creqit.throw(
			_("You don't have permission to access the {0} DocType.").format(doctype), creqit.PermissionError
		)

	link_options, filters = [], {}
	if web_form.login_required and not allow_read_on_all_link_options:
		filters = {"owner": creqit.session.user}
	...
	link_options = creqit.get_all(doctype, filters, fields)
...
def get_published_web_forms() -> dict[str, str]:
	return creqit.get_all("Web Form", ["name", "route", "modified"], {"published": 1})
