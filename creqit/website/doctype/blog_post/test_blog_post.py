...
from creqit.utils import random_string, set_request
from creqit.website.doctype.blog_post.blog_post import get_blog_list
...
class TestBlogPost(IntegrationTestCase):
	...
	def test_generator_not_found(self):
		pages = creqit.get_all("Blog Post", fields=["name", "route"], filters={"published": 0}, limit=1)

		route = f"test-route-{creqit.generate_hash(length=5)}"

		creqit.db.set_value("Blog Post", pages[0].name, "route", route)
	...
	def test_category_link(self):
		...
		blog_page_html = creqit.safe_decode(blog_page_response.get_data())
		...
		cached_value = creqit.db.value_cache.get(("DocType", "Blog Post", "name"))
		creqit.db.value_cache[("DocType", "Blog Post", "name")] = (("Blog Post",),)
		...
		category_page_html = creqit.safe_decode(category_page_response.get_data())
		# Category page should contain the blog post title
		self.assertIn(blog.title, category_page_html)

		# Cleanup
		creqit.db.value_cache[("DocType", "Blog Post", "name")] = cached_value
		creqit.delete_doc("Blog Post", blog.name)
		creqit.delete_doc("Blog Category", blog.blog_category)
	...
	def test_blog_pagination(self):
		...
		filters = creqit._dict({"blog_category": scrub(category_title)})
		...
		for blog in blogs:
			creqit.delete_doc(blog.doctype, blog.name)
		creqit.delete_doc("Blog Category", blogs[0].blog_category)

	def test_caching(self):
		# to enable caching
		creqit.flags.force_website_cache = True
		print(creqit.session.user)
	...
	def test_like_dislike(self):
		...
		creqit.db.delete("Comment", {"comment_type": "Like", "reference_doctype": "Blog Post"})

		from creqit.templates.includes.likes.likes import like
...
def make_test_blog(category_title="Test Blog Category"):
	...
	if not creqit.db.exists("Blog Category", category_name):
		creqit.get_doc(doctype="Blog Category", title=category_title).insert()
	if not creqit.db.exists("Blogger", "test-blogger"):
		creqit.get_doc(doctype="Blogger", short_name="test-blogger", full_name="Test Blogger").insert()
