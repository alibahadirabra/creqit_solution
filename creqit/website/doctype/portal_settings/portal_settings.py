...
class PortalSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from creqit.website.doctype.portal_menu_item.portal_menu_item import PortalMenuItem
	...
	def sync_menu(self):
		...
		dirty = False
		for item in creqit.get_hooks("standard_portal_menu_items"):
			if item.get("role") and not creqit.db.exists("Role", item.get("role")):
				creqit.get_doc({"doctype": "Role", "role_name": item.get("role"), "desk_access": 0}).insert()
	...
	def remove_deleted_doctype_items(self):
		existing_doctypes = set(creqit.get_list("DocType", pluck="name"))
	...
	def validate(self):
		if creqit.request and self.default_portal_home:
