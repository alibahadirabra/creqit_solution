...
def delete_page_cache(path):
	...
	if path:
		creqit.cache.hdel_names(groups, path)
		creqit.cache.delete_value("full_index")
	else:
		groups.append("full_index")
		creqit.cache.delete_value(groups)
...
def can_cache(no_cache=False):
	if creqit.flags.force_website_cache:
		return True
	if creqit.conf.disable_website_cache or creqit.conf.developer_mode:
		return False
	if getattr(creqit.local, "no_cache", False):
		return False
	return not no_cache


def get_comment_list(doctype, name):
	comments = creqit.get_all(
		"Comment",
		...
		or_filters=[["owner", "=", creqit.session.user], ["published", "=", 1]],
	)
	...
	communications = creqit.get_all(
		...
		or_filters=[
			["recipients", "like", f"%{creqit.session.user}%"],
			["cc", "like", f"%{creqit.session.user}%"],
			["bcc", "like", f"%{creqit.session.user}%"],
		],
	)
...
def get_home_page():
	if creqit.local.flags.home_page and not creqit.flags.in_test:
		return creqit.local.flags.home_page
	...
	def _get_home_page():
		...
		if creqit.session.user != "Guest":
			# by role
			for role in creqit.get_roles():
				home_page = creqit.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = creqit.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = creqit.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if creqit.session.user == "Guest" else "me"
	...
	return creqit.cache.hget("home_page", creqit.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = creqit.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = creqit.get_attr(home_page_method[-1])(creqit.session.user)
	elif creqit.get_hooks("website_user_home_page"):
		home_page = creqit.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = creqit.get_hooks("role_home_page")
		if role_home_page:
			for role in creqit.get_roles():
	...
	if not home_page:
		home_page = creqit.get_hooks("home_page")
		if home_page:
...
def get_boot_data():
	...
	return {
		...
		"sysdefaults": {
			"float_precision": cint(creqit.get_system_settings("float_precision")) or 3,
		...
		"time_zone": {
			...
			"user": creqit.db.get_value("User", creqit.session.user, "time_zone") or get_system_timezone(),
		},
		"assets_json": get_assets_json(),
		"sitename": creqit.local.site,
	}


def is_signup_disabled():
	return creqit.get_website_settings("disable_signup")
...
def get_toc(route, url_prefix=None, app=None):
	...
	return creqit.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from creqit.website.router import get_pages

	if not creqit.local.flags.children_map:
		...
		children_map = creqit.cache.get_value("website_full_index", _build)

		creqit.local.flags.children_map = children_map

	return creqit.local.flags.children_map
...
def clear_cache(path=None):
	...
	if path:
		creqit.cache.hdel("website_redirects", path)
		delete_page_cache(path)
	else:
		clear_sitemap()
		creqit.clear_cache("Guest")
		keys += [
			"portal_menu_items",
			"home_page",
			"website_route_rules",
	...
	creqit.cache.delete_value(keys)

	for method in creqit.get_hooks("website_clear_cache"):
		creqit.get_attr(method)(path)
...
def get_sidebar_items(parent_sidebar, basepath=None):
	...
	hooks = creqit.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else creqit.flags.look_for_sidebar
...
def get_portal_sidebar_items():
	sidebar_items = creqit.cache.hget("portal_menu_items", creqit.session.user)
	if sidebar_items is None:
		sidebar_items = []
		roles = creqit.get_roles()
		portal_settings = creqit.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = creqit.get_hooks("portal_menu_items")
		if items_via_hooks:
			for i in items_via_hooks:
				i["enabled"] = 1
			add_items(sidebar_items, items_via_hooks)

		creqit.cache.hset("portal_menu_items", creqit.session.user, sidebar_items)
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path) as sidebarfile:
		try:
			...
			creqit.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		if can_cache():
			html = None
			page_cache = creqit.cache.hget("website_page", args[0].path)
			if page_cache and creqit.local.lang in page_cache:
				html = page_cache[creqit.local.lang]
			if html:
				creqit.local.response.from_cache = True
				creqit.local.response.can_cache = True
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = creqit.cache.hget("website_page", args[0].path) or {}
			page_cache[creqit.local.lang] = html
			creqit.cache.hset("website_page", args[0].path, page_cache)
			creqit.local.response.can_cache = True
...
def build_response(path, data, http_status_code, headers: dict | None = None):
	...
	response.headers["X-From-Cache"] = creqit.local.response.from_cache or False
...
def add_preload_for_bundled_assets(response):
	links = [f"<{css}>; rel=preload; as=style" for css in creqit.local.preload_assets["style"]]
	links.extend(f"<{js}>; rel=preload; as=script" for js in creqit.local.preload_assets["script"])

	version = get_build_version()
	links.extend(
		f"<{svg}?v={version}>; rel=preload; as=fetch; crossorigin"
		for svg in creqit.local.preload_assets["icons"]
	)
