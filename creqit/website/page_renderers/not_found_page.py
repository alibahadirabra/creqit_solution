...
from creqit.website.utils import can_cache
...
class NotFoundPage(TemplatePage):
	def __init__(self, path, http_status_code=None):
		...
		self.request_url = creqit.local.request.url if hasattr(creqit.local, "request") else ""
		path = "404"
	...
	def render(self):
		if self.can_cache_404():
			creqit.cache.hset("website_404", self.request_url, True)
		return super().render()
