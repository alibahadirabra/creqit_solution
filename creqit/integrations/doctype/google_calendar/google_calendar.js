...
creqit.ui.form.on("Google Calendar", {
	refresh: function (frm) {
		...
		creqit.realtime.on("import_google_calendar", (data) => {
			if (data.progress) {
		...
		if (frm.doc.refresh_token) {
			frm.add_custom_button(__("Sync Calendar"), function () {
				...
				creqit
					.call({
						method: "creqit.integrations.doctype.google_calendar.google_calendar.sync",
						args: {
	...
	authorize_google_calendar_access: function (frm) {
		...
		creqit.call({
			method: "creqit.integrations.doctype.google_calendar.google_calendar.authorize_access",
			args: {
				g_calendar: frm.doc.name,
