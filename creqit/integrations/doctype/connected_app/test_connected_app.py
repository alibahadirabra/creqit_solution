...
import creqit
from creqit.integrations.doctype.social_login_key.test_social_login_key import (
...
def get_connected_app():
	...
	connected_app = creqit.new_doc(doctype)
	connected_app.provider_name = "creqit"
...
def get_oauth_client():
	oauth_client = creqit.new_doc("OAuth Client")
...
class TestConnectedApp(IntegrationTestCase):
	def setUp(self):
		...
		creqit comes with it's own oAuth2 provider that we can test against. The
		...
		creqit's "Authorization URL" and "Access Token URL" (actually they're
	...
	def test_web_application_flow(self):
		...
		resp = oauth2_session.get(urljoin(self.base_url, "/api/method/creqit.auth.get_logged_user"))
	...
	def tearDown(self):
		...
		if getattr(self, "oauth_client", None):
			tokens = creqit.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.name})
			for token in tokens:
				doc = creqit.get_doc("OAuth Bearer Token", token.name)
				doc.delete()

			codes = creqit.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.name})
			for code in codes:
				doc = creqit.get_doc("OAuth Authorization Code", code.name)
				doc.delete()
