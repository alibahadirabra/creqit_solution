...
if any((os.getenv("CI"), creqit.conf.developer_mode, creqit.conf.allow_tests)):
...
class ConnectedApp(Document):
	...
	if TYPE_CHECKING:
		from creqit.integrations.doctype.oauth_scope.oauth_scope import OAuthScope
		from creqit.integrations.doctype.query_parameters.query_parameters import QueryParameters
	...
	def validate(self):
		base_url = creqit.utils.get_url()
		callback_path = (
			"/api/method/creqit.integrations.doctype.connected_app.connected_app.callback/" + self.name
		)
	...
	def initiate_web_application_flow(self, user=None, success_uri=None):
		...
		if not token_cache:
			token_cache = creqit.new_doc("Token Cache")
	...
	def get_user_token(self, user=None, success_uri=None):
		...
		creqit.local.response["location"] = redirect
		return redirect
	...
	def get_token_cache(self, user):
		...
		if creqit.db.exists("Token Cache", token_cache_name):
			token_cache = creqit.get_doc("Token Cache", token_cache_name)
...
@creqit.whitelist(methods=["GET"], allow_guest=True)
def callback(code=None, state=None):
	...
	if creqit.session.user == "Guest":
		creqit.local.response["type"] = "redirect"
		creqit.local.response["location"] = "/login?" + urlencode({"redirect-to": creqit.request.url})
		return

	path = creqit.request.path[1:].split("/")
	if len(path) != 4 or not path[3]:
		creqit.throw(_("Invalid Parameters."))

	connected_app = creqit.get_doc("Connected App", path[3])
	token_cache = creqit.get_doc("Token Cache", connected_app.name + "-" + creqit.session.user)

	if state != token_cache.state:
		creqit.throw(_("Invalid token state! Check if the token has been created by the OAuth user."))
	...
	creqit.local.response["type"] = "redirect"
	creqit.local.response["location"] = token_cache.get("success_uri") or connected_app.get_url()


@creqit.whitelist()
def has_token(connected_app, connected_user=None):
	app = creqit.get_doc("Connected App", connected_app)
	token_cache = app.get_token_cache(connected_user or creqit.session.user)
