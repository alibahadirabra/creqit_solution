...
class GoogleContacts(Document):
	...
	def validate(self):
		if not creqit.db.get_single_value("Google Settings", "enable"):
			creqit.throw(_("Enable Google API in Google Settings."))

	def get_access_token(self):
		if not self.refresh_token:
			button_label = creqit.bold(_("Allow Google Contacts Access"))
...
def authorize_access(g_contact, reauthorize=False, code=None):
	...
	"""
	contact = creqit.get_doc("Google Contacts", g_contact)
...
def get_google_contacts_object(g_contact):
	...
	account = creqit.get_doc("Google Contacts", g_contact)
	oauth_obj = GoogleOAuth("contacts")
...
def sync(g_contact=None):
	...
	google_contacts = creqit.get_list("Google Contacts", filters=filters)
...
def sync_contacts_from_google_contacts(g_contact):
	...
	creqit.db.set_value("Google Contacts", account.name, "last_sync_on", creqit.utils.now_datetime())

	for idx, connection in enumerate(results):
		creqit.publish_realtime(
			"import_google_contacts", dict(progress=idx + 1, total=len(results)), user=creqit.session.user
		)
		# Work-around to fix
		# https://github.com/creqit/creqit/issues/22648
...
def insert_contacts_to_google_contacts(doc, method=None):
	...
	"""
	if (
		not creqit.db.exists("Google Contacts", {"name": doc.google_contacts})
	...
	try:
		...
		creqit.db.set_value("Contact", doc.name, "google_contacts_id", contact.get("resourceName"))
	except HttpError as err:
		creqit.msgprint(
...
def update_contacts_to_google_contacts(doc, method=None):
	...
	try:
		...
		creqit.msgprint(_("Contact Synced with Google Contacts."))
	except HttpError as err:
		creqit.msgprint(
