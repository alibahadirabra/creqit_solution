...
import creqit
from creqit.integrations.doctype.webhook import flush_webhook_execution_queue
from creqit.integrations.doctype.webhook.webhook import (
	enqueue_webhook,
	get_webhook_data,
	get_webhook_headers,
)
...
def get_test_webhook(config):
	wh = creqit.get_doc(config)
	if not wh.name:
		wh.name = creqit.generate_hash()
	wh.insert()
	wh.reload()
	try:
		yield wh
...
class TestWebhook(IntegrationTestCase):
	...
	def setUpClass(cls):
		...
		creqit.db.delete("Webhook")
		# Delete existing logs if any
		creqit.db.delete("Webhook Request Log")
		super().setUpClass()
	...
	def setUp(self):
		...
		if creqit.db.exists("Webhook", webhook_fields):
			self.webhook = creqit.get_doc("Webhook", webhook_fields)
		else:
			self.webhook = creqit.new_doc("Webhook")
			self.webhook.update(webhook_fields)

		# create a User document
		self.user = creqit.new_doc("User")
		self.user.first_name = creqit.mock("name")
		self.user.email = creqit.mock("email")
		self.user.save()

		# Create another test user specific to this test
		self.test_user = creqit.new_doc("User")
	...
	def test_webhook_trigger_with_enabled_webhooks(self):
		...
		webhooks = creqit.cache.get_value("webhooks")
		self.assertTrue("User" in webhooks)
		self.assertEqual(len(webhooks.get("User")), 1)

		# only 1 hook (enabled) must be queued
		self.assertEqual(len(creqit.local._webhook_queue), 1)
		execution = creqit.local._webhook_queue[0]
	...
	def test_validate_doc_events(self):
		...
		self.assertRaises(creqit.ValidationError, self.webhook.save)
	...
	def test_webhook_req_log_creation(self):
		...
		if not creqit.db.get_value("User", "user2@integration.webhooks.test.com"):
			user = creqit.get_doc(
				{"doctype": "User", "email": "user2@integration.webhooks.test.com", "first_name": "user2"}
			).insert()
		else:
			user = creqit.get_doc("User", "user2@integration.webhooks.test.com")

		webhook = creqit.get_doc("Webhook", {"webhook_doctype": "User"})
		enqueue_webhook(user, webhook)

		self.assertTrue(creqit.get_all("Webhook Request Log", pluck="name"))
	...
	def test_webhook_with_array_body(self):
		...
		doc = creqit.new_doc("Note")
		doc.title = "Test Webhook Note"
		final_title = creqit.generate_hash()
		...
		with get_test_webhook(wh_config):
			...
			log = creqit.get_last_doc("Webhook Request Log")
