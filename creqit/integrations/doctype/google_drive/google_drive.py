...
from creqit.utils import get_backups_path, get_bench_path
...
class GoogleDrive(Document):
	...
	def get_access_token(self):
		if not self.refresh_token:
			button_label = creqit.bold(_("Allow Google Drive Access"))
...
def authorize_access(reauthorize=False, code=None):
	...
	oauth_code = creqit.db.get_single_value("Google Drive", "authorization_code") if not code else code
	oauth_obj = GoogleOAuth("drive")

	if not oauth_code or reauthorize:
		if reauthorize:
			creqit.db.set_single_value("Google Drive", "backup_folder_id", "")
...
def get_google_drive_object():
	...
	account = creqit.get_doc("Google Drive")
	oauth_obj = GoogleOAuth("drive")
...
def check_for_folder_in_google_drive():
	...
	def _create_folder_in_google_drive(google_drive, account):
		...
		try:
			...
			creqit.db.set_single_value("Google Drive", "backup_folder_id", folder.get("id"))
			creqit.db.commit()
	...
	try:
		...
		creqit.throw(_("Google Drive - Could not find folder in Google Drive - Error Code {0}").format(e))

	for f in google_drive_folders.get("files"):
		if f.get("name") == account.backup_folder_name:
			creqit.db.set_single_value("Google Drive", "backup_folder_id", f.get("id"))
			creqit.db.commit()
...
def take_backup():
	...
	enqueue(
		"creqit.integrations.doctype.google_drive.google_drive.upload_system_backup_to_google_drive",
		queue="long",
		timeout=1500,
	)
...
def upload_system_backup_to_google_drive():
	...
	for fileurl in file_urls:
		...
		try:
			...
			creqit.throw(_("Google Drive - Could not locate - {0}").format(e))
	...
	creqit.db.set_single_value("Google Drive", "last_backup_on", creqit.utils.now_datetime())
	send_email(True, "Google Drive", "Google Drive", "email")
	return _("Google Drive Backup Successful.")


def daily_backup():
	drive_settings = creqit.db.get_singles_dict("Google Drive", cast=True)
