...
import creqit
from creqit.utils import get_request_session
...
def make_request(method: str, url: str, auth=None, headers=None, data=None, json=None, params=None):
	...
	try:
		...
		response = creqit.flags.integration_request = s.request(
		...
		if creqit.flags.integration_request_doc:
			creqit.flags.integration_request_doc.log_error()
		else:
			creqit.log_error()
		raise exc
...
def create_request_log(
	...
	**kwargs,
	...
	integration_request = creqit.get_doc(
		{
...
def get_json(obj):
	return obj if isinstance(obj, str) else creqit.as_json(obj, indent=1)
