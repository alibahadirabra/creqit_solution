...
import creqit
from creqit.utils import get_request_site_address

CALLBACK_METHOD = "/api/method/creqit.integrations.google_oauth.callback"
_SCOPES = {
...
_DOMAIN_CALLBACK_METHODS = {
	"mail": "creqit.email.oauth.authorize_google_access",
	"contacts": "creqit.integrations.doctype.google_contacts.google_contacts.authorize_access",
	"drive": "creqit.integrations.doctype.google_drive.google_drive.authorize_access",
	"indexing": "creqit.website.doctype.website_settings.google_indexing.authorize_access",
}
...
class GoogleOAuth:
	...
	def __init__(self, domain: str, validate: bool = True):
		self.google_settings = creqit.get_single("Google Settings")
	...
	def validate_google_settings(self):
		...
		if not self.google_settings.enable:
			creqit.throw(creqit._("Please enable {} before continuing.").format(google_settings))

		if not (self.google_settings.client_id and self.google_settings.client_secret):
			creqit.throw(creqit._("Please update {} before continuing.").format(google_settings))
...
def handle_response(
	...
	if "error" in response:
		creqit.log_error(creqit._(error_title), creqit._(response.get("error_description", error_message)))

		if raise_err:
			creqit.throw(creqit._(error_title), GoogleAuthenticationError, creqit._(error_message))
...
@creqit.whitelist(methods=["GET"])
def callback(state: str, code: str | None = None, error: str | None = None) -> None:
	"""Common callback for google integrations.
	Invokes functions using `creqit.get_attr` and also adds required (keyworded) arguments
	along with committing and redirecting us back to creqit site."""
	...
	if not error:
		if (domain := state.pop("domain")) in _DOMAIN_CALLBACK_METHODS:
			...
			creqit.get_attr(_DOMAIN_CALLBACK_METHODS[domain])(**state)
	...
	creqit.local.response["location"] = f"{redirect}?{failure_query_param if error else success_query_param}"
