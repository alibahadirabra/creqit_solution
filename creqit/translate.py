...
import creqit
from creqit.query_builder import DocType, Field
from creqit.utils import cstr, get_bench_path, is_html, strip, strip_html_tags, unique
...
def get_language(lang_list: list | None = None) -> str:
	"""Set `creqit.local.lang` from HTTP headers at beginning of request
	...
	"""
	is_logged_in = creqit.session.user != "Guest"

	# fetch language from form_dict
	if creqit.form_dict._lang:
		language = get_lang_code(creqit.form_dict._lang or get_parent_language(creqit.form_dict._lang))
		if language:
			return language
	...
	accept_language = list(creqit.request.accept_languages.values())
	...
	return creqit.db.get_default("lang") or "en"
...
def get_user_lang(user: str | None = None) -> str:
	"""Set creqit.local.lang from user preferences on session beginning or resumption"""
	user = user or creqit.session.user

	# User.language => Session Defaults => creqit.local.lang => 'en'
	return (
		creqit.get_cached_value("User", user, "language")
		or creqit.db.get_default("lang")
...
def get_lang_code(lang: str) -> str | None:
	return creqit.db.get_value("Language", {"name": lang}) or creqit.db.get_value(
...
def set_default_language(lang):
	...
	if creqit.db.get_default("lang") != lang:
		creqit.db.set_default("lang", lang)
	creqit.local.lang = lang


def get_lang_dict():
	"""Return all languages in dict format, full name is the key e.g. `{"english":"en"}`."""
	return dict(
		creqit.get_all("Language", fields=["language_name", "name"], order_by="creation", as_list=True)
	)
...
def get_messages_for_boot():
	...
	return get_all_translations(creqit.local.lang)
...
def get_all_translations(lang: str) -> dict[str, str]:
	...
	def _merge_translations():
		from creqit.geo.country_info import get_translated_countries
	...
	try:
		return creqit.cache.hget(MERGED_TRANSLATION_KEY, lang, generator=_merge_translations)
	except Exception:
		...
		creqit.logger().error("Unable to load translations", exc_info=True)
		return {}
...
def get_translations_from_apps(lang, apps=None):
	...
	translations = {}
	from creqit.gettext.translate import get_translations_from_mo

	for app in apps or creqit.get_installed_apps(_ensure_on_bench=True):
...
def get_translations_from_csv(lang, app):
	return get_translation_dict_from_file(
		os.path.join(creqit.get_app_path(app, "translations"), lang + ".csv"), lang, app
	)
	...
	if os.path.exists(path):
		...
		for item in csv_content:
			if len(item) == 3 and item[2]:
				...
				creqit.log_error(message=msg, title="Error in translation file")
				if throw:
					creqit.throw(msg, title="Error in translation file")
...
def get_user_translations(lang):
	...
	return creqit.cache.hget(USER_TRANSLATION_KEY, lang, generator=_read_from_db)


def clear_cache():
	"""Clear all translation assets from :meth:`creqit.cache`"""
	creqit.cache.delete_value(
...
def get_messages_for_app(app, deduplicate=True):
	...
	messages = []
	modules = [creqit.unscrub(m) for m in creqit.local.app_modules[app]]
	...
	if modules:
		if isinstance(modules, str):
			modules = [modules]
		filtered_doctypes = (
			creqit.qb.from_("DocType").where(Field("module").isin(modules)).select("name").run(pluck=True)
		)
		...
		# pages
		filtered_pages = (
			creqit.qb.from_("Page").where(Field("module").isin(modules)).select("name", "title").run()
		)
...
def get_messages_from_navbar():
	...
	labels = creqit.get_all("Navbar Item", filters={"item_label": ("is", "set")}, pluck="item_label")
...
def get_messages_from_doctype(name):
	...
	from creqit.gettext.extractors.utils import is_translatable

	messages = []
	meta = creqit.get_meta(name)
	...
	if not meta.custom:
		doctype_file_path = creqit.get_module_path(meta.module, "doctype", meta.name, meta.name)
...
def get_messages_from_workflow(doctype=None, app_name=None):
	...
	workflows = []
	if doctype:
		workflows = creqit.get_all("Workflow", filters={"document_type": doctype})
	else:
		fixtures = creqit.get_hooks("fixtures", app_name=app_name) or []
		for fixture in fixtures:
			if isinstance(fixture, str) and fixture == "Workflow":
				workflows = creqit.get_all("Workflow")
				break
			elif isinstance(fixture, dict) and fixture.get("dt", fixture.get("doctype")) == "Workflow":
				workflows.extend(creqit.get_all("Workflow", filters=fixture.get("filters")))

	messages = []
	document_state = DocType("Workflow Document State")
	for w in workflows:
		states = creqit.db.get_values(
			document_state,
...
def get_messages_from_page(name):
	"""Return all translatable strings from a :class:`creqit.core.doctype.Page`."""
	return _get_messages_from_page_or_report("Page", name)


def get_messages_from_report(name):
	"""Return all translatable strings from a :class:`creqit.core.doctype.Report`."""
	from creqit.gettext.extractors.utils import is_translatable

	report = creqit.get_doc("Report", name)
	messages = _get_messages_from_page_or_report(
		"Report", name, creqit.db.get_value("DocType", report.ref_doctype, "module")
	)
...
def _get_messages_from_page_or_report(doctype, name, module=None):
	if not module:
		module = creqit.db.get_value(doctype, name, "module")

	doc_path = creqit.get_module_path(module, doctype, name)

	messages = get_messages_from_file(os.path.join(doc_path, creqit.scrub(name) + ".py"))
...
def get_server_messages(app):
	"""Extracts all translatable strings (tagged with :func:`creqit._`) from Python modules
	inside an app"""
	messages = []
	file_extensions = (".py", ".html", ".js", ".vue")
	app_walk = os.walk(creqit.get_app_path(app))
...
def get_messages_from_include_files(app_name=None):
	...
	from creqit.utils.jinja_globals import bundled_asset

	messages = []
	app_include_js = creqit.get_hooks("app_include_js", app_name=app_name) or []
	web_include_js = creqit.get_hooks("web_include_js", app_name=app_name) or []
	include_js = app_include_js + web_include_js

	for js_path in include_js:
		file_path = bundled_asset(js_path)
		relative_path = os.path.join(creqit.local.sites_path, file_path.lstrip("/"))
...
def get_all_messages_from_js_files(app_name=None):
	...
	messages = []
	for app in [app_name] if app_name else creqit.get_installed_apps(_ensure_on_bench=True):
		if os.path.exists(creqit.get_app_path(app, "public")):
			for basepath, folders, files in os.walk(creqit.get_app_path(app, "public")):  # noqa: B007
				if "creqit/public/js/lib" in basepath:
					continue
...
def get_messages_from_file(path: str) -> list[tuple[str, str, str | None, int]]:
	...
	from creqit.gettext.extractors.utils import extract_messages_from_code

	creqit.flags.setdefault("scanned_files", set())
	# TODO: Find better alternative
	# To avoid duplicate scan
	if path in creqit.flags.scanned_files:
		return []

	creqit.flags.scanned_files.add(path)
...
def extract_messages_from_javascript_code(code: str) -> list[tuple[int, str, str | None]]:
	...
	messages = []
	from creqit.gettext.extractors.javascript import extract_javascript
...
def update_translations(lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	for key, value in zip(
		creqit.get_file_items(untranslated_file, ignore_empty_lines=False),
		creqit.get_file_items(translated_file, ignore_empty_lines=False),
		strict=False,
	):
...
def import_translations(lang, path):
	...
	for app in creqit.get_all_apps(True):
...
def migrate_translations(source_app, target_app):
	...
	strings_in_source_app = [m[1] for m in creqit.translate.get_messages_for_app(source_app)]
	strings_in_target_app = [m[1] for m in creqit.translate.get_messages_for_app(target_app)]

	strings_in_target_app_but_not_in_source_app = list(
		set(strings_in_target_app) - set(strings_in_source_app)
	)

	languages = creqit.translate.get_all_languages()

	source_app_translations_dir = creqit.get_app_path(source_app, "translations")
	target_app_translations_dir = creqit.get_app_path(target_app, "translations")
...
def rebuild_all_translation_files():
	...
	for lang in get_all_languages():
		for app in creqit.get_all_apps():
...
def write_translations_file(app, lang, full_dict=None, app_messages=None):
	...
	tpath = creqit.get_app_path(app, "translations")
	creqit.create_folder(tpath)
	write_csv_file(os.path.join(tpath, lang + ".csv"), app_messages, full_dict or get_all_translations(lang))


def send_translations(translation_dict):
	"""Append translated dict in `creqit.local.response`"""
	if "__messages" not in creqit.local.response:
		creqit.local.response["__messages"] = {}

	creqit.local.response["__messages"].update(translation_dict)
...
def update_translations_for_source(source=None, translation_dict=None):
	...
	translation_records = creqit.db.get_values(
...
def get_all_languages(with_language_name: bool = False) -> list:
	...
	def get_language_codes():
		return creqit.get_all("Language", filters={"enabled": 1}, pluck="name")

	def get_all_language_with_name():
		return creqit.get_all("Language", ["language_code", "language_name"], {"enabled": 1})

	if with_language_name:
		return creqit.cache.get_value("languages_with_name", get_all_language_with_name)
	else:
		return creqit.cache.get_value("languages", get_language_codes)


def get_preferred_language_cookie():
	return creqit.request.cookies.get("preferred_language")


def get_translated_doctypes():
	dts = creqit.get_all("DocType", {"translated_doctype": 1}, pluck="name")
...
def print_language(language: str):
	...
	with print_language("de"):
	    html = creqit.get_print(...)
	```
	"""
	if not language or language == creqit.local.lang:
		# do nothing
		yield
		return

	# remember original values
	_lang = creqit.local.lang
	_jenv = creqit.local.jenv

	# set language, empty any existing lang_full_dict and jenv
	creqit.local.lang = language
	creqit.local.jenv = None

	yield

	# restore original values
	creqit.local.lang = _lang
	creqit.local.jenv = _jenv
