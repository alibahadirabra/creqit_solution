...
def execute():
	...
	for name, content in creqit.get_all(
		...
		creqit.db.set_value("Comment", name, "content", content, update_modified=False)
	...
	for name, content in creqit.get_all(
		...
		if content and content.startswith("Removed "):
			creqit.db.set_value("Comment", name, "content", content[8:], update_modified=False)
