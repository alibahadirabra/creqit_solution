import creqit
from creqit.model.naming import is_autoincremented
...
def execute():
	...
	for doctype in possible_log_types:
		if (
			creqit.db.exists("DocType", doctype)
			and _is_implicit_int_pk(doctype)
			and not is_autoincremented(doctype)
		):
			creqit.db.change_column_type(
				doctype,
				"name",
				type=f"varchar({creqit.db.VARCHAR_LEN})",
				nullable=True,
			)
...
def _is_implicit_int_pk(doctype: str) -> bool:
	...
	if creqit.db.db_type == "mariadb":
		...
		values = (creqit.db.cur_db_name,)

	col_type = creqit.db.sql(query, values)
