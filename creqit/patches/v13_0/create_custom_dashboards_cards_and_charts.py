...
from creqit.utils.dashboard import get_dashboards_with_link


def execute():
	if (
		not creqit.db.table_exists("Dashboard Chart")
		or not creqit.db.table_exists("Number Card")
		or not creqit.db.table_exists("Dashboard")
	):
		return

	creqit.reload_doc("desk", "doctype", "dashboard_chart")
	creqit.reload_doc("desk", "doctype", "number_card")
	creqit.reload_doc("desk", "doctype", "dashboard")
...
def get_modified_docs(doctype):
	return creqit.get_all(doctype, filters={"owner": "Administrator", "modified_by": ["!=", "Administrator"]})


def rename_modified_doc(docname, doctype):
	new_name = docname + " Custom"
	try:
		creqit.rename_doc(doctype, docname, new_name)
