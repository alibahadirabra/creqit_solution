...
def execute():
	...
	creqit.reload_doc("automation", "doctype", "Auto Repeat")
	creqit.db.sql("update `tabDocPerm` set submit=0, cancel=0, amend=0 where parent='Auto Repeat'")
	creqit.db.sql("update `tabAuto Repeat` set docstatus=0 where docstatus=1 or docstatus=2")

	for entry in creqit.get_all("Auto Repeat"):
		doc = creqit.get_doc("Auto Repeat", entry.name)

		# create custom field for allow auto repeat
		fields = creqit.get_meta(doc.reference_doctype).fields
