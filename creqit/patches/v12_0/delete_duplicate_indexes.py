...
def execute():
	...
	all_tables = creqit.db.get_tables()
	final_deletion_map = creqit._dict()

	for table in all_tables:
		indexes_to_keep_map = creqit._dict()
		indexes_to_delete = []
		index_info = creqit.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				creqit.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")
			except Exception as e:
				creqit.log_error("Failed to drop index")
