...
from creqit.creqitclient import creqitClient, creqitOAuth2Client
from creqit.utils import convert_utc_to_system_timezone, get_datetime, get_datetime_str, get_system_timezone


class creqitMail:
	"""Class to interact with the creqit Mail API."""
	...
	def get_client(
		...
		api_secret: str | None = None,
		access_token: str | None = None,
	) -> creqitClient | creqitOAuth2Client:
		"""Returns a creqitClient or creqitOAuth2Client instance."""

		if hasattr(creqit.local, "creqit_mail_clients"):
			if client := creqit.local.creqit_mail_clients.get(mailbox):
				return client
		else:
			creqit.local.creqit_mail_clients = {}

		client = (
			creqitOAuth2Client(url=site, access_token=access_token)
			if access_token
			else creqitClient(url=site, api_key=api_key, api_secret=api_secret)
		)
		creqit.local.creqit_mail_clients[mailbox] = client
	...
	def request(
		...
		"""Makes a request to the creqit Mail API."""
	...
	def send_raw(self, sender: str, recipients: str | list, message: str) -> None:
		"""Sends an email using the creqit Mail API."""
	...
	def send_newsletter(self, sender: str, recipients: str | list, message: str) -> None:
		"""Sends an newsletter using the creqit Mail API."""
	...
	def pull_raw(self, limit: int = 50, last_synced_at: str | None = None) -> dict[str, list[str] | str]:
		"""Pulls emails from the mailbox using the creqit Mail API."""
		...
		headers = {"X-Site": creqit.utils.get_url()}
