...
import creqit
from creqit import _, safe_encode, task
from creqit.core.utils import html2text
from creqit.database.database import savepoint
from creqit.email.doctype.email_account.email_account import EmailAccount
from creqit.email.email_body import add_attachment, get_email, get_formatted_html
from creqit.email.creqitmail import creqitMail
from creqit.email.queue import get_unsubcribed_url, get_unsubscribe_message
from creqit.email.smtp import SMTPServer
from creqit.model.document import Document
from creqit.query_builder import DocType, Interval
...
from creqit.utils.verified_command import get_signed_params
...
class EmailQueue(Document):
	...
	if TYPE_CHECKING:
		from creqit.email.doctype.email_queue_recipient.email_queue_recipient import EmailQueueRecipient
	...
	def prevent_email_queue_delete(self):
		if creqit.session.user != "Administrator":
			creqit.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = creqit.get_doc(values)
	...
	def new(cls, doc_data, ignore_permissions=False) -> "EmailQueue":
		...
		doc = creqit.new_doc(cls.DOCTYPE)
		doc.update(data)
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = creqit.get_doc("Communication", self.communication)
	...
	def get_email_account(self, raise_error=False):
		if self.email_account:
			return creqit.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			creqit.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(creqit.db.get_default("suspend_email_queue")) == 1
		):
			return False

		return True

	def send(self, smtp_server_instance: SMTPServer = None, creqit_mail_client: creqitMail = None):
		"""Send emails to recipients."""
		if not self.can_send_now():
			return

		with SendMailContext(self, smtp_server_instance, creqit_mail_client) as ctx:
			...
			for recipient in self.recipients:
				...
				elif not creqit.flags.in_test or creqit.flags.testing_email:
					if ctx.email_account_doc.service == "creqit Mail":
						if self.reference_doctype == "Newsletter":
							ctx.creqit_mail_client.send_newsletter(
								sender=self.sender,
								recipients=recipient.recipient,
								message=message.decode("utf-8"),
							)
						else:
							ctx.creqit_mail_client.send_raw(
								sender=self.sender,
								recipients=recipient.recipient,
								message=message.decode("utf-8"),
							)
			...
			if creqit.flags.in_test and not creqit.flags.testing_email:
				creqit.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = creqit.qb.DocType("Email Queue")
		email_recipient = creqit.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			creqit.qb.from_(email_queue).delete().where(email_queue.creation < (Now() - Interval(days=days)))
		).run()
		...
		(
			creqit.qb.from_(email_recipient)
			.delete()
			.where(email_recipient.creation < (Now() - Interval(days=days)))
		).run()


from creqit.deprecation_dumpster import send_mail as _send_mail
...
class SendMailContext:
	def __init__(
		...
		creqit_mail_client: creqitMail = None,
	):
		self.queue_doc: EmailQueue = queue_doc
		self.smtp_server: SMTPServer = smtp_server_instance
		self.creqit_mail_client: creqitMail = creqit_mail_client
	...
	def fetch_outgoing_server(self):
		...
		if self.email_account_doc.service == "creqit Mail":
			if not self.creqit_mail_client:
				self.creqit_mail_client = self.email_account_doc.get_creqit_mail_client()
	...
	def __exit__(self, exc_type, exc_val, exc_tb):
		if exc_type:
			update_fields = {"error": creqit.get_traceback()}
	...
	def get_tracker_str(self, recipient_email) -> str:
		...
		if self.queue_doc.get("email_read_tracker_url"):
			...
			tracker_url = f"{get_url()}/api/method/creqit.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = creqit.get_doc("File", file_filters)
				...
				print_format_file = creqit.attach_print(**attachment)
	...
	def _store_file(self, file_name, content):
		if not creqit.get_system_settings("store_attached_pdf_document"):
			return

		file_data = creqit._dict(file_name=file_name, is_private=1)
		...
		if creqit.db.exists("File", file_data):
			return

		file = creqit.new_doc("File", **file_data)
...
def retry_sending(queues: str | list[str]):
	if not creqit.has_permission("Email Queue", throw=True):
		return
	...
	email_queue = creqit.qb.DocType("Email Queue")
	creqit.qb.update(email_queue).set(email_queue.status, "Not Sent").set(email_queue.modified, now()).set(
		email_queue.modified_by, creqit.session.user
...
def toggle_sending(enable):
	...
	creqit.db.set_default("suspend_email_queue", 0 if sbool(enable) else 1)


def on_doctype_update():
	"""Add index in `tabCommunication` for `(reference_doctype, reference_name)`"""
	creqit.db.add_index("Email Queue", ("status", "send_after", "priority", "creation"), "index_bulk_flush")

	creqit.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(creqit.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def __init__(
		...
		:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/creqit.email.queue.unsubscribe`.
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/creqit.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				creqit.qb.from_(EmailUnsubscribe)
	...
	def process(self, send_now=False) -> EmailQueue | None:
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in creqit.utils.create_batch(final_recipients, 1000):
				creqit.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=creqit.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=creqit.flags.in_test or send_now,
					queue="long",
				)
	...
	def send_emails(self, queue_data, final_recipients):
		...
		creqit_mail_client = None
		smtp_server_instance = None
		for r in final_recipients:
			recipients = list(set([r, *self.final_cc(), *self.bcc]))
			q = EmailQueue.new({**queue_data, **{"recipients": recipients}}, ignore_permissions=True)
			if not creqit_mail_client and not smtp_server_instance:
				email_account = q.get_email_account(raise_error=True)

				if email_account.service == "creqit Mail":
					creqit_mail_client = email_account.get_creqit_mail_client()
				else:
					smtp_server_instance = email_account.get_smtp_server()

			with suppress(Exception):
				q.send(smtp_server_instance=smtp_server_instance, creqit_mail_client=creqit_mail_client)
	...
	def as_dict(self, include_recipients=True):
		...
		except creqit.InvalidEmailAddressError:
