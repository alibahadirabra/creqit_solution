...
class EmailDomain(Document):
	...
	def validate(self):
		...
		if creqit.local.flags.in_patch or creqit.local.flags.in_test or creqit.local.flags.in_install:
			return
	...
	def on_update(self):
		...
		for email_account in creqit.get_all("Email Account", filters={"domain": self.name}):
			try:
