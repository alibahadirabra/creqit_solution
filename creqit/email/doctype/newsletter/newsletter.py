...
from creqit.email.doctype.email_group.email_group import add_subscribers
from creqit.rate_limiter import rate_limit
from creqit.utils.safe_exec import is_job_queued
from creqit.utils.verified_command import get_signed_params, verify_request
...
class Newsletter(WebsiteGenerator):
	...
	if TYPE_CHECKING:
		from creqit.email.doctype.newsletter_attachment.newsletter_attachment import NewsletterAttachment
		from creqit.email.doctype.newsletter_email_group.newsletter_email_group import NewsletterEmailGroup
	...
	def get_sending_status(self):
		count_by_status = creqit.get_all(
			"Email Queue",
		...
		emails_queued = is_job_queued(
			job_name=creqit.utils.get_job_name("send_bulk_emails_for", self.doctype, self.name),
			queue="long",
		)
	...
	def send_test_email(self, email):
		test_emails = creqit.utils.validate_email_address(email, throw=True)
		self.send_newsletter(emails=test_emails, test_email=True)
		creqit.msgprint(_("Test email sent to {0}").format(email), alert=True)
	...
	def validate_newsletter_status(self):
		if self.email_sent:
			creqit.throw(_("Newsletter has already been sent"), exc=NewsletterAlreadySentError)

		if self.get("__islocal"):
			creqit.throw(_("Please save the Newsletter before sending"), exc=NewsletterNotSavedError)

	def validate_newsletter_recipients(self):
		if not self.newsletter_recipients:
			creqit.throw(_("Newsletter should have atleast one recipient"), exc=NoRecipientFoundError)

	def validate_sender_address(self):
		"""Validate self.send_from is a valid email address or not."""
		if self.sender_email:
			creqit.utils.validate_email_address(self.sender_email, throw=True)
			self.send_from = (
	...
	def validate_publishing(self):
		if self.send_webview_link and not self.published:
			creqit.throw(_("Newsletter must be published to send webview link in email"))

	def validate_scheduling_date(self):
		if (
			self.schedule_sending
			and creqit.utils.get_datetime(self.schedule_send) < creqit.utils.now_datetime()
		):
			creqit.throw(_("Past dates are not allowed for Scheduling."))
	...
	def queue_all(self):
		...
		self.email_sent_at = creqit.utils.now()
	...
	def send_newsletter(self, emails: list[str], test_email: bool = False):
		...
		sender = self.send_from or creqit.utils.get_formatted_email(self.owner)
		args = self.as_dict()
		args["message"] = self.get_message(medium="email")

		is_auto_commit_set = bool(creqit.db.auto_commit_on_many_writes)
		creqit.db.auto_commit_on_many_writes = not creqit.flags.in_test
		...
		creqit.sendmail(
			...
			if test_email
			else "/api/method/creqit.email.doctype.newsletter.newsletter.newsletter_email_read",
		)

		creqit.db.auto_commit_on_many_writes = is_auto_commit_set

	def get_message(self, medium=None) -> str:
		message = self.message
		if self.content_type == "Markdown":
			message = creqit.utils.md_to_html(self.message_md)
		if self.content_type == "HTML":
			message = self.message_html

		html = creqit.render_template(message, {"doc": self.as_dict()})
	...
	def add_source(self, html: str, medium="None") -> str:
		...
		for link in links:
			...
			if href and not href.startswith("#"):
				if not creqit.utils.is_site_link(href):
					continue
				new_href = creqit.utils.add_trackers_to_url(
	...
	def get_email_groups(self) -> list[str]:
		...
		return [x.email_group for x in self.email_group] or creqit.get_all(
...
def confirmed_unsubscribe(email, group):
	...
	creqit.flags.ignore_permissions = True
	doc = creqit.get_doc("Email Group Member", {"email": email, "email_group": group})
...
def subscribe(email, email_group=None):
	...
	api_endpoint = creqit.utils.get_url(
		"/api/method/creqit.email.doctype.newsletter.newsletter.confirm_subscription"
	)
	...
	email_confirmation_template = creqit.db.get_value(
	...
	if email_confirmation_template:
		...
		email_template = creqit.get_doc("Email Template", email_confirmation_template)
		email_subject = email_template.subject
		content = creqit.render_template(email_template.response, args)
	else:
...
def confirm_subscription(email, email_group=None):
	...
	try:
		group = creqit.get_doc("Email Group", email_group)
	except creqit.DoesNotExistError:
		group = creqit.get_doc({"doctype": "Email Group", "title": email_group}).insert(
	...
	if welcome_url:
		...
		creqit.local.response["location"] = welcome_url
	else:
...
def send_scheduled_email():
	...
	scheduled_newsletter = creqit.get_all(
		"Newsletter",
		filters={
			"schedule_send": ("<=", creqit.utils.now_datetime()),
			"email_sent": False,
	...
	for newsletter_name in scheduled_newsletter:
		try:
			newsletter = creqit.get_doc("Newsletter", newsletter_name)
			...
			creqit.db.set_value("Newsletter", newsletter_name, "email_sent", 0)
...
def newsletter_email_read(recipient_email=None, reference_doctype=None, reference_name=None):
	...
	verify_request()
	try:
		doc = creqit.get_cached_doc("Newsletter", reference_name)
		if doc.add_viewed(recipient_email, force=True, unique_views=True):
			newsletter = creqit.qb.DocType("Newsletter")
			(
				creqit.qb.update(newsletter)
		...
		creqit.response.update(creqit.utils.get_imaginary_pixel_response())


def get_default_email_group():
	return _("Website", lang=creqit.db.get_default("language"))
