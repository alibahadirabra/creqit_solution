...
def get_test_notification(config):
	try:
		notification = creqit.get_doc(doctype="Notification", **config).insert()
		yield notification
	finally:
		notification.delete()
		creqit.db.commit()
...
class TestNotification(IntegrationTestCase):
	def setUp(self):
		...
		if not creqit.db.exists("Notification", {"name": "ToDo Status Update"}, "name"):
			notification = creqit.new_doc("Notification")
		...
		if not creqit.db.exists("Notification", {"name": "Contact Status Update"}, "name"):
			notification = creqit.new_doc("Notification")
	...
	def test_new_and_save(self):
		...
		self.assertEqual(creqit.db.get_value("Communication", communication.name, "subject"), "__testing__")

	def test_condition(self):
		"""Check notification is triggered based on a condition."""
		event = creqit.new_doc("Event")
		event.subject = "test"
	...
	def test_invalid_condition(self):
		...
		recipent = creqit.new_doc("Notification Recipient")
		recipent.receiver_by_document_field = "owner"

		notification.recipents = recipent
		notification.condition = "test"

		self.assertRaises(creqit.ValidationError, notification.save)
		notification.delete()
	...
	def test_minutes_positive_offset(self):
		...
		with get_test_notification(notification) as n:
			creqit.db.delete("Notification Log", {"subject": n.subject})
			# Run the offset alerts
			trigger_notifications(None, "offset")

			# Check if the notification was triggered
			self.assertEqual(1, creqit.db.count("Notification Log", {"subject": n.subject}))
	...
	def test_minutes_offset_validation(self):
		...
		self.assertRaises(creqit.ValidationError, notification.insert)
	...
	def test_date_changed(self):
		...
		creqit.get_doc(
			"Scheduled Job Type",
			dict(method="creqit.email.doctype.notification.notification.trigger_daily_alerts"),
		).execute()
		...
		event.starts_on = creqit.utils.add_days(creqit.utils.nowdate(), 2) + " 12:00:00"
		event.save()
	...
	def test_cc_jinja(self):
		creqit.db.delete("User", {"email": "test_jinja@example.com"})
		creqit.db.delete("Email Queue")
		creqit.db.delete("Email Queue Recipient")

		test_user = creqit.new_doc("User")
		...
		self.assertTrue(creqit.db.get_value("Email Queue Recipient", {"recipient": "test_jinja@example.com"}))
	...
	def test_notification_value_change_casted_types(self):
		...
		with get_test_notification(notification) as n:
			...
			user.birth_date = creqit.utils.add_days(user.birth_date, 1)
			user.save()

			user.reload()
			user.birth_date = creqit.utils.getdate(user.birth_date)
			user.save()
			self.assertEqual(1, creqit.db.count("Notification Log", {"subject": n.subject}))

	@classmethod
	def tearDownClass(cls):
		creqit.delete_doc_if_exists("Notification", "ToDo Status Update")
		creqit.delete_doc_if_exists("Notification", "Contact Status Update")
...
❯ bench run-tests --module creqit.email.doctype.notification.test_notification --case TestNotificationOffsetRange
...
# 		self.event = creqit.new_doc("Event")
...
# 		creqit.db.delete("Event", {"name": self.event.name})
# 		creqit.db.delete("Notification", {"name": self.notification["name"]})
# 		creqit.db.delete("Notification Log", {"subject": self.notification["subject"]})
...
# 						self.assertEqual(
# 							1,
# 							creqit.db.count("Notification Log", {"subject": n.subject}),
