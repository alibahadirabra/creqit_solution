...
function oauth_access(frm) {
	creqit.model.with_doc("Connected App", frm.doc.connected_app, () => {
		const connected_app = creqit.get_doc("Connected App", frm.doc.connected_app);
		return creqit.call({
			doc: connected_app,
...
function set_default_max_attachment_size(frm) {
	if (frm.doc.__islocal && !frm.doc["attachment_limit"]) {
		creqit.call({
			method: "creqit.core.api.file.get_max_file_size",
			callback: function (r) {
				if (!r.exc) {
...
creqit.ui.form.on("Email Account", {
	service: function (frm) {
		$.each(creqit.email_defaults[frm.doc.service], function (key, value) {
			frm.set_value(key, value);
		});
		if (!frm.doc.use_imap) {
			$.each(creqit.email_defaults_pop[frm.doc.service], function (key, value) {
				frm.set_value(key, value);
			});
		}
	},
	...
	onload: function (frm) {
		...
		frm.set_query(
			"append_to",
			"creqit.email.doctype.email_account.email_account.get_append_to"
		);
		frm.set_query("append_to", "imap_folder", function () {
			return {
				query: "creqit.email.doctype.email_account.email_account.get_append_to",
			};
		});
	...
	refresh: function (frm) {
		...
		if (creqit.route_flags.delete_user_from_locals && creqit.route_flags.linked_user) {
			delete creqit.route_flags.delete_user_from_locals;
			delete locals["User"][creqit.route_flags.linked_user];
		}

		if (!frm.is_dirty() && frm.doc.enable_incoming) {
			frm.add_custom_button(__("Pull Emails"), () => {
				creqit.dom.freeze(__("Pulling emails..."));
				frm.call({
					method: "pull_emails",
					...
					if (!(r._server_messages && r._server_messages.length)) {
						creqit.show_alert({ message: __("Emails Pulled"), indicator: "green" });
					}
				});
			});
		}
	...
	validate_creqit_mail_settings: function (frm) {
		if (frm.doc.service == "creqit Mail") {
			creqit.call({
				doc: frm.doc,
				method: "validate_creqit_mail_settings",
			});
		}
	},
	...
	show_oauth_authorization_message(frm) {
		if (
			...
			creqit.call({
				method: "creqit.integrations.doctype.connected_app.connected_app.has_token",
				args: {
	...
	domain: creqit.utils.debounce((frm) => {
		if (frm.doc.domain) {
			creqit.call({
	...
	email_sync_option: function (frm) {
		...
		if (frm.doc.email_sync_option == "ALL") {
			...
			creqit.confirm(msg, null, function () {
	...
	warn_autoreply_on_incoming: function (frm) {
		if (frm.doc.enable_incoming && frm.doc.enable_auto_reply && frm.doc.__islocal) {
			...
			creqit.confirm(msg, null, function () {
				...
				creqit.show_alert({ message: __("Disabled Auto Reply"), indicator: "blue" });
			});
		}
	},
});
