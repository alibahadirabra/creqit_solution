...
import creqit
from creqit import _, are_emails_muted, safe_encode
from creqit.desk.form import assign_to
from creqit.email.doctype.email_domain.email_domain import EMAIL_DOMAIN_FIELDS
from creqit.email.creqitmail import creqitMail
from creqit.email.receive import EmailServer, InboundMail, SentEmailInInboxError
from creqit.email.smtp import SMTPServer
from creqit.email.utils import get_port
from creqit.model.document import Document
from creqit.utils import cint, comma_or, cstr, parse_addr, validate_email_address
from creqit.utils.background_jobs import enqueue, get_jobs
from creqit.utils.jinja import render_template
...
def cache_email_account(cache_name):
	def decorator_cache_email_account(func):
		...
		def wrapper_cache_email_account(*args, **kwargs):
			if not hasattr(creqit.local, cache_name):
				setattr(creqit.local, cache_name, {})

			cached_accounts = getattr(creqit.local, cache_name)
...
class EmailAccount(Document):
	...
	if TYPE_CHECKING:
		from creqit.email.doctype.imap_folder.imap_folder import IMAPFolder
		...
		service: DF.Literal[
			"", "creqit Mail", "GMail", "Sendgrid", "SparkPost", "Yahoo Mail", "Outlook.com", "Yandex.Mail"
		]
	...
	def validate(self):
		...
		if self.login_id_is_different:
			if not self.login_id:
				creqit.throw(_("Login Id is required"))
		else:
			self.login_id = None

		if self.service == "creqit Mail":
			self.use_imap = 0
			self.always_use_account_email_id_as_sender = 1

			if self.auth_method == "Basic" or self.get_oauth_token():
				self.validate_creqit_mail_settings()

		# validate the imap settings
		if self.enable_incoming and self.use_imap and len(self.imap_folder) <= 0:
			creqit.throw(_("You need to set one IMAP folder for {0}").format(creqit.bold(self.email_id)))

		if creqit.local.flags.in_patch or creqit.local.flags.in_test:
			return
		...
		if (
			not creqit.local.flags.in_install
			and not self.awaiting_password
			and not self.service == "creqit Mail"
		):
			if validate_oauth or self.password or self.smtp_server in ("127.0.0.1", "localhost"):
				...
				if self.enable_incoming or (self.enable_outgoing and not self.no_smtp_authentication):
					if not use_oauth:
						creqit.throw(_("Password is required or select Awaiting Password"))

		if self.notify_if_unreplied:
			if not self.send_notification_to:
				creqit.throw(_("{0} is mandatory").format(self.meta.get_label("send_notification_to")))
			...
			for folder in self.imap_folder:
				if folder.append_to:
					...
					if folder.append_to not in valid_doctypes:
						creqit.throw(_("Append To can be one of {0}").format(comma_or(valid_doctypes)))

	@creqit.whitelist()
	def validate_creqit_mail_settings(self):
		if self.service == "creqit Mail":
			creqit_mail_client = self.get_creqit_mail_client()
			creqit_mail_client.validate(for_inbound=self.enable_incoming, for_outbound=self.enable_outgoing)

	def validate_smtp_conn(self):
		if not self.smtp_server:
			creqit.throw(_("SMTP Server is required"))
	...
	def before_save(self):
		...
		if not self.enable_incoming and self.default_incoming:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					creqit.bold(_("Default Incoming")),
					creqit.bold(_("Enable Incoming")),
				)
			)
		if not self.enable_outgoing and self.default_outgoing:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					creqit.bold(_("Default Outgoing")),
					creqit.bold(_("Enable Outgoing")),
				)
			)
		if messages:
			if len(messages) == 1:
	...
	def there_must_be_only_one_default(self):
		...
		for field in ("default_incoming", "default_outgoing"):
			...
			for email_account in creqit.get_all("Email Account", filters={field: 1}):
				if email_account.name == self.name:
					continue

				email_account = creqit.get_doc("Email Account", email_account.name)
				email_account.set(field, 0)
				email_account.save()

	@creqit.whitelist()
	def get_domain_values(self, domain: str):
		return creqit.db.get_value("Email Domain", domain, EMAIL_DOMAIN_FIELDS, as_dict=True)
	...
	def get_incoming_server(self, in_receive=False, email_sync_rule="UNSEEN"):
		...
		if not args.get("host"):
			creqit.throw(_("{0} is required").format("Email Server"))

		email_server = EmailServer(creqit._dict(args))
	...
	def check_email_server_connection(self, email_server, in_receive):
		...
		try:
			...
			if in_receive:
				...
				description = creqit.message_log.pop() if creqit.message_log else "Socket Error"
	...
	def _password(self):
		raise_exception = not (
			self.auth_method == "OAuth" or self.no_smtp_authentication or creqit.flags.in_test
		)
	...
	def from_record(cls, record):
		email_account = creqit.new_doc(cls.DOCTYPE)
	...
	def find_one_by_filters(cls, **kwargs) -> "EmailAccount":
		name = creqit.db.get_value(cls.DOCTYPE, kwargs)
	...
	def get_account_details_from_site_config(cls):
		if not creqit.conf.get("mail_server"):
			return {}
		...
		field_to_conf_name_map = {
			...
			"name": {"conf_names": ("email_sender_name",), "default": "creqit"},
		...
		for doc_field_name, d in field_to_conf_name_map.items():
			...
			value = [creqit.conf.get(k) for k in conf_names if creqit.conf.get(k)]
	...
	def get_creqit_mail_client(self):
		return self._creqit_mail_client

	@functools.cached_property
	def _creqit_mail_client(self):
		if self.auth_method == "OAuth":
			if access_token := self.get_access_token():
				return creqitMail(self.creqit_mail_site, self.email_id, access_token=access_token)

			creqit.throw(
				_("Please Authorize OAuth for Email Account {0}").format(
					creqit.bold(self.email_account_name)
				),
				title=_("creqit Mail OAuth Error"),
			)
		else:
			return creqitMail(
				self.creqit_mail_site, self.email_id, self.api_key, self.get_password("api_secret")
			)
	...
	def handle_incoming_connect_error(self, description):
		if self.get_failed_attempts_count() > 5:
			...
			creqit.enqueue(self._disable_broken_incoming_account, description=description)
		else:
			self.set_failed_attempts_count(self.get_failed_attempts_count() + 1)

	def _disable_broken_incoming_account(self, description):
		if creqit.flags.in_test:
			return
	...
	def set_failed_attempts_count(self, value):
		creqit.cache.set_value(f"{self.name}:email-account-failed-attempts", value)

	def get_failed_attempts_count(self):
		return cint(creqit.cache.get_value(f"{self.name}:email-account-failed-attempts"))
	...
	def receive(self):
		...
		for mail in inbound_mails:
			try:
				...
				try:
					...
					if self.use_imap:
						self.handle_bad_emails(mail.uid, mail.raw_message, creqit.get_traceback())
					exceptions.append(creqit.get_traceback())
				except Exception:
					creqit.db.rollback()
				else:
					creqit.db.commit()
		...
		if exceptions:
			raise Exception(creqit.as_json(exceptions))
	...
	def get_inbound_mails(self) -> list[InboundMail]:
		...
		try:
			if self.service == "creqit Mail":
				creqit_mail_client = self.get_creqit_mail_client()
				messages = creqit_mail_client.pull_raw(last_synced_at=self.last_synced_at)
	...
	def handle_bad_emails(self, uid, raw, reason):
		...
		if cint(self.use_imap):
			...
			unhandled_email = creqit.get_doc(
				{
					"raw": raw_str,
					"uid": uid,
					"reason": reason,
	...
	def send_auto_reply(self, communication, email):
		...
		from creqit.core.doctype.communication.email import (
		...
		if self.enable_auto_reply:
			...
			creqit.sendmail(
				...
				or creqit.get_template("templates/emails/auto_reply.html").render(communication.as_dict()),
	...
	def on_trash(self):
		...
		Communication = creqit.qb.DocType("Communication")
		creqit.qb.update(Communication).set(Communication.email_account, "").where(
	...
	def after_rename(self, old, new, merge=False):
		creqit.db.set_value("Email Account", new, "email_account_name", new)
	...
	def check_automatic_linking_email_account(self):
		if self.enable_automatic_linking:
			if not self.enable_incoming:
				creqit.throw(_("Automatic Linking can be activated only if Incoming is enabled."))

			if creqit.db.exists(
				"Email Account",
				{"enable_automatic_linking": 1, "name": ("!=", self.name)},
			):
				creqit.throw(_("Automatic Linking can be activated only for one Email Account."))
	...
	def get_oauth_token(self):
		if self.auth_method == "OAuth":
			connected_app = creqit.get_doc("Connected App", self.connected_app)
...
def get_append_to(doctype=None, txt=None, searchfield=None, start=None, page_len=None, filters=None):
	...
	email_append_to_list = [
		dt.name for dt in creqit.get_all("DocType", filters=filters, fields=["name", "email_append_to"])
	]
	# Set Email Append To DocTypes set via Customize Form
	email_append_to_list.extend(
		dt.doc_type
		for dt in creqit.get_list(
			"Property Setter",
...
def notify_unreplied():
	...
	for email_account in creqit.get_all(
		"Email Account",
		"name",
		...
		if append_to:
			...
			for comm in creqit.get_all(
				...
				comm = creqit.get_doc("Communication", comm.name)

				if creqit.db.get_value(comm.reference_doctype, comm.reference_name, "status") == "Open":
...
def pull(now=False):
	...
	from creqit.integrations.doctype.connected_app.connected_app import has_token

	doctype = creqit.qb.DocType("Email Account")
	email_accounts = (
	...
	for email_account in email_accounts:
		...
		if now:
			...
			queued_jobs = get_jobs(site=creqit.local.site, key="job_name")[creqit.local.site]
...
def pull_emails(email_account: str) -> None:
	...
	creqit.has_permission("Email Account", "read", throw=True)
	...
	if job_name not in queued_jobs:
		...
		creqit.msgprint(_("Emails are already being pulled from this account."))


def pull_from_email_account(email_account):
	"""Runs within a worker process"""
	email_account = creqit.get_doc("Email Account", email_account)
...
def setup_user_email_inbox(email_account, awaiting_password, email_id, enable_outgoing, used_oauth):
	...
	from creqit.core.doctype.user.user import ask_pass_update
	...
	user_names = creqit.db.get_values("User", {"email": email_id}, as_dict=True)
	if not user_names:
		return
	...
	if update_user_email_settings:
		UserEmail = creqit.qb.DocType("User Email")
		creqit.qb.update(UserEmail).set(UserEmail.awaiting_password, (awaiting_password or 0)).set(
		...
		users = " and ".join([creqit.bold(user.get("name")) for user in user_names])
		creqit.msgprint(_("Enabled email inbox for user {0}").format(users))
	ask_pass_update()
...
def remove_user_email_inbox(email_account):
	...
	for user in users:
		doc = creqit.get_doc("User", user.get("name"))
...
def set_email_password(email_account, password):
	account = creqit.get_doc("Email Account", email_account)
