...
from creqit.utils import cint, cstr, get_traceback


class InvalidEmailCredentials(creqit.ValidationError):
	pass
...
class SMTPServer:
	def __init__(
		...
		if not self.server:
			creqit.msgprint(
				...
				raise_exception=creqit.OutgoingEmailError,
			)
	...
	def session(self):
		...
		try:
			...
			if not _session:
				creqit.msgprint(
					_("Could not connect to outgoing email server"), raise_exception=creqit.OutgoingEmailError
				)
			...
			if self.use_oauth:
				...
				if res[0] != 235:
					creqit.msgprint(res[1], raise_exception=creqit.OutgoingEmailError)
	...
	def _enqueue_connection_closure(self):
		if creqit.request and hasattr(creqit.request, "after_response"):
			creqit.request.after_response.add(self.quit)
		elif creqit.job:
			creqit.job.after_job.add(self.quit)
		elif not creqit.flags.in_test:
			# Console?
			import atexit
