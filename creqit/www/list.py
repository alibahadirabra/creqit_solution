...
from creqit.model.document import Document, get_controller
from creqit.utils import cint, quoted
from creqit.website.path_resolver import resolve_path
...
def get_context(context, **dict_params):
	...
	creqit.local.form_dict.update(dict_params)
	doctype = creqit.local.form_dict.doctype
	context.parents = [{"route": "me", "title": _("My Account")}]
	context.meta = creqit.get_meta(doctype)
	context.update(get_list_context(context, doctype) or {})
	context.doctype = doctype
	context.txt = creqit.local.form_dict.txt
	context.update(get(**creqit.local.form_dict))
...
def get(doctype, txt=None, limit_start=0, limit=20, pathname=None, **kwargs):
	...
	list_context = creqit.flags.list_context
	...
	for doc in raw_result:
		doc.doctype = doctype
		new_context = creqit._dict(doc=doc, meta=meta, list_view_fields=list_view_fields)

		if not list_context.get_list and not isinstance(new_context.doc, Document):
			new_context.doc = creqit.get_doc(doc.doctype, doc.name)
			new_context.update(new_context.doc.as_dict())

		if not creqit.flags.in_test:
			pathname = pathname or creqit.local.request.path
			new_context["pathname"] = pathname.strip("/ ")
		new_context.update(list_context)
		set_route(new_context)
		rendered_row = creqit.render_template(row_template, new_context, is_path=True)
...
def get_list_data(
	...
	if creqit.is_table(doctype):
		creqit.throw(_("Child DocTypes are not allowed"), title=_("Invalid DocType"))

	if not txt and creqit.form_dict.search:
		txt = creqit.form_dict.search
		del creqit.form_dict["search"]

	controller = get_controller(doctype)
	meta = creqit.get_meta(doctype)

	filters = prepare_filters(doctype, controller, kwargs)
	list_context = get_list_context(creqit._dict(), doctype, web_form_name)
	...
	creqit.flags.list_context = list_context
...
def prepare_filters(doctype, controller, kwargs):
	...
	if filters.pathname:
		...
		for key, val in creqit.local.form_dict.items():
...
def get_list_context(context, doctype, web_form_name=None):
	...
	from creqit.website.doctype.web_form.web_form import get_web_form_module

	list_context = context or creqit._dict()
	meta = creqit.get_meta(doctype)
	...
	def update_context_from_module(module, list_context):
		...
		if hasattr(module, "get_list_context"):
			out = creqit._dict(module.get_list_context(list_context) or {})
			if out:
				list_context = out
		return list_context
	...
	if meta.custom and web_form_name:
		webform_list_contexts = creqit.get_hooks("webform_list_context")
		if webform_list_contexts and not creqit.get_doc("Module Def", meta.module).custom:
			out = creqit._dict(creqit.get_attr(webform_list_contexts[0])(meta.module) or {})
			if out:
				list_context = out
