import creqit
from creqit.email.doctype.newsletter.newsletter import confirmed_unsubscribe
...
def get_context(context):
	...
	if "email" in creqit.form_dict and creqit.request.method == "GET":
		if verify_request():
			user_email = creqit.form_dict["email"]
			context.email = user_email
			title = creqit.form_dict.get("name")
	...
	elif "user_email" in creqit.form_dict and creqit.request.method == "POST":
		context.status = "unsubscribed"
		email = creqit.form_dict["user_email"]
		email_group = get_email_groups(email)
		for group in email_group:
			if group.email_group in creqit.form_dict:
