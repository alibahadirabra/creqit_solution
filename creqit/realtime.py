...
def publish_progress(percent, title=None, doctype=None, docname=None, description=None, task_id=None):
	publish_realtime(
		...
		user=None if doctype and docname else creqit.session.user,
		doctype=doctype,
		docname=docname,
		task_id=task_id,
	)
...
def publish_realtime(
	...
	if not task_id and hasattr(creqit.local, "task_id"):
		task_id = creqit.local.task_id
	...
	if after_commit:
		if not hasattr(creqit.local, "_realtime_log"):
			creqit.local._realtime_log = []
			creqit.db.after_commit.add(flush_realtime_log)
			creqit.db.after_rollback.add(clear_realtime_log)

		params = [event, message, room]
		if params not in creqit.local._realtime_log:
			creqit.local._realtime_log.append(params)
	else:
		emit_via_redis(event, message, room)


def flush_realtime_log():
	for args in creqit.local._realtime_log:
		creqit.realtime.emit_via_redis(*args)

	clear_realtime_log()


def clear_realtime_log():
	if hasattr(creqit.local, "_realtime_log"):
		del creqit.local._realtime_log
...
def emit_via_redis(event, message, room):
	...
	from creqit.utils.background_jobs import get_redis_connection_without_auth
	...
	with suppress(redis.exceptions.ConnectionError):
		...
		r.publish(
			"events",
			creqit.as_json(
				{"event": event, "message": message, "room": room, "namespace": creqit.local.site}
			),
		)
...
def has_permission(doctype: str, name: str) -> bool:
	if not creqit.has_permission(doctype=doctype, doc=name, ptype="read"):
...
def get_user_info():
	return {
		"user": creqit.session.user,
		"user_type": creqit.session.data.user_type,
