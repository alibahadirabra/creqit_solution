"""
creqitClient is a library that helps you connect with other creqit systems
"""
import base64
import json
...
class creqitException(Exception):
	pass
...
class creqitClient:
	def __init__(
		...
		api_key=None,
		api_secret=None,
		creqit_authorization_source=None,
	):
		import requests
		...
		self.creqit_authorization_source = creqit_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.creqit_authorization_source:
				auth_source = {"creqit-Authorization-Source": self.creqit_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": creqit.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return creqit._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "creqit.client.insert_many", "docs": creqit.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": creqit.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return creqit._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "creqit.client.bulk_update", "docs": creqit.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "creqit.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "creqit.client.submit", "doc": creqit.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "creqit.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": creqit.as_json(filters),
			}
		)
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "creqit.client.cancel", "doctype": doctype, "name": name})
	...
	def migrate_doctype(self, doctype, filters=None, update=None, verbose=1, exclude=None, preprocess=None):
		...
		for doc in docs:
			...
			if doctype != "User" and not creqit.db.exists("User", doc.get("owner")):
				creqit.get_doc(
					{
						"doctype": "User",
						"email": doc.get("owner"),
	...
	def migrate_single(self, doctype):
		...
		doc.modified = creqit.db.get_single_value(doctype, "modified")
	...
	def post_process(self, response):
		...
		if rjson and ("exc" in rjson) and rjson["exc"]:
			try:
				...
				exc = "creqitClient Request Failed\n\n" + exc
			except Exception:
				exc = rjson["exc"]

			raise creqitException(exc)
		if "message" in rjson:
...
class creqitOAuth2Client(creqitClient):
