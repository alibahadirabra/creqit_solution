...
def trigger_scheduler_event(context: CliCtxObj, event):
	...
	for site in context.sites:
		try:
			...
			creqit.connect()
			try:
				creqit.get_doc("Scheduled Job Type", {"method": event}).execute()
...
def enable_scheduler(context: CliCtxObj):
	...
	for site in context.sites:
		try:
			creqit.init(site)
			creqit.connect()
			creqit.utils.scheduler.enable_scheduler()
			creqit.db.commit()
...
def disable_scheduler(context: CliCtxObj):
	...
	for site in context.sites:
		try:
			creqit.init(site)
			creqit.connect()
			creqit.utils.scheduler.disable_scheduler()
			creqit.db.commit()
...
def scheduler(context: CliCtxObj, state: str, format: str, verbose: bool = False, site: str | None = None):
	...
	import creqit
	from creqit.utils.scheduler import is_scheduler_inactive, toggle_scheduler
	...
	with creqit.init_site(site=site):
		match state:
			case "status":
				creqit.connect()
...
def doctor(context: CliCtxObj, site=None):
	...
	from creqit.utils.doctor import doctor as _doctor
...
def show_pending_jobs(context: CliCtxObj, site=None):
	...
	from creqit.utils.doctor import pending_jobs as _pending_jobs
...
def start_worker(queue, quiet=False, rq_username=None, rq_password=None, burst=False, strategy=None):
	...
	from creqit.utils.background_jobs import start_worker
...
def start_worker_pool(queue, quiet=False, num_workers=2, burst=False):
	...
	from creqit.utils.background_jobs import start_worker_pool
