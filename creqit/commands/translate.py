...
def build_message_files(context: CliCtxObj):
	...
	for site in context.sites:
		try:
			creqit.init(site)
			creqit.connect()
			creqit.translate.rebuild_all_translation_files()
		finally:
			creqit.destroy()
...
@click.argument("app")  # , help="App name eg. creqit")
@pass_context
def new_language(context: CliCtxObj, lang_code, app):
	...
	# init site
	creqit.init(site=context.sites[0])
	creqit.connect()
	creqit.translate.write_translations_file(app, lang_code)

	print(f"File created at ./apps/{app}/{app}/translations/{lang_code}.csv")
	print("You will need to add the language in creqit/geo/languages.csv, if you haven't done it already.")
...
def get_untranslated(context: CliCtxObj, lang, untranslated_file, app="_ALL_APPS", all=None):
	...
	try:
		creqit.init(site)
		creqit.connect()
		creqit.translate.get_untranslated(lang, untranslated_file, get_all=all, app=app)
	finally:
		creqit.destroy()
...
def update_translations(context: CliCtxObj, lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	try:
		creqit.init(site)
		creqit.connect()
		creqit.translate.update_translations(lang, untranslated_file, translated_file, app=app)
	finally:
		creqit.destroy()
...
def import_translations(context: CliCtxObj, lang, path):
	...
	try:
		creqit.init(site)
		creqit.connect()
		creqit.translate.import_translations(lang, path)
	finally:
		creqit.destroy()
...
def migrate_translations(context: CliCtxObj, source_app, target_app):
	...
	try:
		creqit.init(site)
		creqit.connect()
		creqit.translate.migrate_translations(source_app, target_app)
	finally:
		creqit.destroy()
