...
if TYPE_CHECKING:
	...
	from creqit.testing import TestRunner
...
def main(
	...
	from creqit.testing.environment import _cleanup_after_tests, _initialize_test_environment

	testing_module_logger = logging.getLogger("creqit.testing")
	...
	try:
		...
		for app, category, result in results:
			_print_result(creqit.unscrub(app or "Unspecified App"), creqit.unscrub(category), result)
...
def _load_doctype_list(doctype_list_path):
	...
	with open(creqit.get_app_path(app, path)) as f:
...
def _run_module_def_tests(app, module_def, runner: "TestRunner", force) -> "TestRunner":
	...
	from creqit.testing import discover_doctype_tests
...
def _get_doctypes_for_module_def(app, module_def):
	...
	doctypes = []
	doctypes_ = creqit.get_list(
		"DocType",
		filters={"module": module_def, "istable": 0},
		fields=["name", "module"],
		as_list=True,
	)
	from creqit.modules import get_module_name
...
@click.option(
	"--doctype-list-path",
	help="Path to .txt file for list of doctypes. Example creqit/tests/server/agriculture.txt",
)
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		...
		creqit.init(site)
		allow_tests = creqit.get_conf().allow_tests
...
@click.option("--app", help="For App", default="creqit")
...
def run_parallel_tests(
	...
	with CodeCoverage(with_coverage, app):
		site = get_site(context)
		if use_orchestrator:
			from creqit.parallel_test_runner import ParallelTestWithOrchestrator

			ParallelTestWithOrchestrator(app, site=site)
		else:
			from creqit.parallel_test_runner import ParallelTestRunner
...
def run_ui_tests(
	...
	site = get_site(context)
	creqit.init(site)
	app_base_path = creqit.get_app_source_path(app)
	site_url = creqit.utils.get_site_url(site)
	admin_password = creqit.get_conf().admin_password
	...
	node_bin = subprocess.getoutput("(cd ../creqit && yarn bin)")
	...
	if not (
		...
		creqit.commands.popen(f"(cd ../creqit && yarn add {packages} --no-lockfile)")
	...
	creqit.commands.popen(formatted_command, cwd=app_base_path, raise_err=True)
