...
def pass_context(f):
	...
	def _func(ctx, *args, **kwargs):
		...
		try:
			...
			creqit.exceptions.SiteNotSpecifiedError,
			creqit.exceptions.IncorrectSitePath,
			creqit.exceptions.CommandFailedError,
		) as e:
...
def get_site(context, raise_err=True):
	try:
		...
		if raise_err:
			raise creqit.SiteNotSpecifiedError
		return None
...
def get_commands():
	...
	clickable_link = "https://creqitframework.com/docs"
	all_commands = (
		scheduler_commands
		+ site_commands
		+ testing_commands
