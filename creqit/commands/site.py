...
import creqit
from creqit.commands import get_site, pass_context
from creqit.exceptions import SiteNotSpecifiedError
from creqit.utils import CallbackManager
from creqit.utils.bench_helper import CliCtxObj
...
def new_site(
	...
	"Create a new site"
	from creqit.installer import _new_site

	creqit.init(site, new_site=True)
...
def _restore(
	*,
	...
	from creqit.installer import extract_files
	from creqit.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key

	err, out = creqit.utils.execute_in_shell(f"file {sql_file_path}", check_exit_code=True)
	if err:
...
def restore_backup(
	...
	from creqit.installer import _new_site, is_downgrade, is_partial, validate_database_sql
	...
	if is_partial(sql_file_path):
		click.secho(
			"Partial Backup file detected. You cannot use a partial file to restore a creqit site.",
			fg="red",
		)
		click.secho(
	...
	# Check if the backup is of an older version of creqit and the user hasn't specified force
...
def partial_restore(context: CliCtxObj, sql_file_path, verbose, encryption_key=None):
	from creqit.installer import is_partial, partial_restore
	from creqit.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key
	...
	if "cipher" in out.decode().split(":")[-1].strip():
		...
		with decrypt_backup(sql_file_path, key):
			if not is_partial(sql_file_path):
				click.secho(
					"Full backup file detected. Use `bench restore` to restore a creqit Site.",
					fg="red",
				)
				sys.exit(1)
...
def _reinstall(
	...
	try:
		...
		creqit.connect()
		creqit.clear_cache()
		installed = creqit.get_installed_apps()
		creqit.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context: CliCtxObj, apps, force=False):
	...
	from creqit.installer import install_app as _install_app
	from creqit.utils.synchronization import filelock
	...
	for site in context.sites:
		...
		with filelock("install_app", timeout=1):
			for app in apps:
				...
				except creqit.IncompatibleApp as err:
					err_msg = f":\n{err}" if str(err) else ""
					print(f"App {app} is Incompatible with Site {site}{err_msg}")
					exit_code = 1
				except Exception as err:
					err_msg = f": {err!s}\n{creqit.get_traceback(with_context=True)}"
...
def list_apps(context: CliCtxObj, format):
	...
	for site in context.sites:
		...
		apps = creqit.get_single("Installed Applications").installed_applications
		if apps:
			installed_apps_info.extend(format_app(app) for app in apps)
		else:
			installed_apps_info.extend(creqit.get_installed_apps())
	...
	if format == "json":
		click.echo(creqit.as_json(summary_dict))
...
def add_db_index(context: CliCtxObj, doctype, column):
	...
	for site in context.sites:
		...
		creqit.connect()
		try:
			creqit.db.add_index(doctype, columns)
			if len(columns) == 1:
...
def describe_database_table(context, doctype, column):
	...
	from creqit.core.doctype.recorder.recorder import _fetch_table_stats

	for site in context.sites:
		creqit.init(site=site)
		creqit.connect()
		try:
...
def add_system_manager(context: CliCtxObj, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		...
		creqit.connect()
		try:
			creqit.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			creqit.db.commit()
		finally:
			creqit.destroy()
	if not context.sites:
...
def disable_user(context: CliCtxObj, email):
	...
	site = get_site(context)
	with creqit.init_site(site):
		creqit.connect()
		user = creqit.get_doc("User", email)
		user.enabled = 0
...
def migrate(context: CliCtxObj, skip_failing=False, skip_search_index=False):
	...
	from creqit.migrate import SiteMigration
...
def migrate_to():
	...
	from creqit.integrations.creqit_providers import migrate_to
...
def run_patch(context: CliCtxObj, module, force):
	...
	for site in context.sites:
		...
		try:
			creqit.connect()
			creqit.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			creqit.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context: CliCtxObj, module, doctype, docname):
	...
	for site in context.sites:
		try:
			creqit.init(site)
			creqit.connect()
			creqit.reload_doc(module, doctype, docname, force=context.force)
			creqit.db.commit()
		finally:
			creqit.destroy()
	if not context.sites:
...
def reload_doctype(context: CliCtxObj, doctype):
	...
	for site in context.sites:
		try:
			creqit.init(site)
			creqit.connect()
			creqit.reload_doctype(doctype, force=context.force)
			creqit.db.commit()
		finally:
			creqit.destroy()
	if not context.sites:
...
def add_to_hosts(context: CliCtxObj):
	"Add site to hosts"
	for site in context.sites:
		creqit.commands.popen(f"echo 127.0.0.1\t{site} | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
	...
	for site in context.sites:
		try:
			...
			if rollback_callback:
				...
				rollback_callback = None
			if verbose:
				print(creqit.get_traceback(with_context=True))
			exit_code = 1
			continue
		if creqit.get_system_settings("encrypt_backup") and creqit.get_site_config().encryption_key:
			click.secho(
...
def remove_from_installed_apps(context: CliCtxObj, app):
	...
	ensure_app_not_creqit(app)
	from creqit.installer import remove_from_installed_apps
...
def uninstall(context: CliCtxObj, app, dry_run, yes, no_backup, force):
	...
	ensure_app_not_creqit(app)
	from creqit.installer import remove_app
	from creqit.utils.synchronization import filelock
...
def _drop_site(
	...
	from creqit.database import drop_user_and_database
	from creqit.utils.backups import scheduled_backup
	...
	creqit.flags.root_login = db_root_username
	creqit.flags.root_password = db_root_password

	drop_user_and_database(creqit.conf.db_name, creqit.conf.db_user)

	archived_sites_path = archived_sites_path or os.path.join(
		creqit.utils.get_bench_path(), "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	creqit.init(site)
	old_path = creqit.utils.get_site_path()
...
def set_last_active_for_user(context: CliCtxObj, user=None):
	...
	from creqit.core.doctype.user.user import get_system_users
	...
	with creqit.init_site(site):
		...
		creqit.db.set_value("User", user, "last_active", now_datetime())
		creqit.db.commit()
...
def publish_realtime(context: CliCtxObj, event, message, room, user, doctype, docname, after_commit):
	...
	from creqit import publish_realtime
...
def browse(context: CliCtxObj, site, user=None):
	...
	if site not in creqit.utils.get_sites():
	...
	if user:
		...
		if creqit.conf.developer_mode or user == "Administrator":
			...
			sid = f"/app?sid={creqit.session.sid}"
		else:
			click.echo("Please enable developer mode to login as a user")

	url = f"{creqit.utils.get_site_url(site)}{sid}"
...
def start_ngrok(context: CliCtxObj, bind_tls, use_default_authtoken):
	...
	ngrok_authtoken = creqit.conf.ngrok_authtoken
...
def clear_log_table(context: CliCtxObj, doctype, days, no_backup):
	...
	"""
	from creqit.core.doctype.log_settings.log_settings import LOG_DOCTYPES
	from creqit.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from creqit.utils.backups import scheduled_backup
	...
	if doctype not in LOG_DOCTYPES:
		raise creqit.ValidationError(f"Unsupported logging DocType: {doctype}")
...
def trim_database(context: CliCtxObj, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		database_tables: list[str] = (
			creqit.qb.from_(information_schema.tables)
			.select(table_name)
			.where(information_schema.tables.table_schema == creqit.conf.db_name)
			.where(information_schema.tables.table_type == "BASE TABLE")
			.run(pluck=True)
		)
		doctype_tables = creqit.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			...
			for table in TABLES_TO_DROP:
				...
				creqit.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[creqit.local.site] = TABLES_TO_DROP
		creqit.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"database",
		creqit.conf.db_type,
		f"framework_{creqit.conf.db_type}.sql",
	)
...
def trim_tables(context: CliCtxObj, dry_run, format, no_backup):
	...
	from creqit.model.meta import trim_tables
	from creqit.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {creqit.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {creqit.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({creqit.local.site: data}, indent=1, sort_keys=True))
	else:
		from creqit.utils.commands import render_table
...
def ensure_app_not_creqit(app: str) -> None:
	"""
	Ensure that the app name passed is not 'creqit'

	:param app: Name of the app
	:return: Nothing
	"""
	if app == "creqit":
		click.secho("You cannot remove or uninstall the app `creqit`", fg="red")
		sys.exit(1)
...
def bypass_patch(context: CliCtxObj, patch_name: str, yes: bool):
	...
	from creqit.modules.patch_handler import update_patch_log
