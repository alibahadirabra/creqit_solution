...
from creqit.coverage import CodeCoverage
from creqit.exceptions import SiteNotSpecifiedError
from creqit.utils import cint, update_progress_bar
...
@click.option(
	...
	envvar="creqit_HARD_LINK_ASSETS",
)
...
def build(
	...
	from creqit.build import bundle, download_creqit_assets
	from creqit.gettext.translate import compile_translations
	...
	with filelock("bench_build", is_global=True, timeout=10):
		...
		if not (force or apps or os.environ.get("CI")):
			# skip building creqit if assets exist remotely
			skip_creqit = download_creqit_assets(verbose=verbose)
		else:
			skip_creqit = False

		# don't minify in developer_mode for faster builds
		development = creqit.local.conf.developer_mode or creqit.local.dev_server
...
def watch(apps=None):
	...
	from creqit.build import watch
...
def destroy_all_sessions(context: CliCtxObj, reason=None):
	...
	for site in context.sites:
		try:
			creqit.init(site)
			creqit.connect()
			creqit.sessions.clear_all_sessions(reason)
			creqit.db.commit()
		finally:
			creqit.destroy()
	if not context.sites:
...
def show_config(context: CliCtxObj, format):
	...
	for site in context.sites:
		...
		configuration = creqit.get_site_config(sites_path=sites_path, site_path=site)
	...
	if format == "json":
		click.echo(creqit.as_json(sites_config))
...
def reset_perms(context: CliCtxObj):
	...
	from creqit.permissions import reset_perms

	for site in context.sites:
		try:
			creqit.init(site)
			creqit.connect()
			for d in creqit.db.sql_list(
				"""select name from `tabDocType`
				where istable=0 and custom=0"""
			):
				creqit.clear_cache(doctype=d)
				reset_perms(d)
		finally:
			creqit.destroy()
	if not context.sites:
...
def execute(context: CliCtxObj, method, args=None, kwargs=None, profile=False):
	...
	for site in context.sites:
		...
		try:
			...
			try:
				ret = creqit.get_attr(method)(*fn_args, **fn_kwargs)
			except Exception:
...
def export_doc(context: CliCtxObj, doctype, docname):
	...
	for site in context.sites:
		try:
			creqit.init(site)
			creqit.connect()
			creqit.modules.export_doc(doctype, docname)
		finally:
			creqit.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def export_json(context: CliCtxObj, doctype, path, name=None):
	...
	from creqit.core.doctype.data_import.data_import import export_json
...
def export_csv(context: CliCtxObj, doctype, path):
	...
	from creqit.core.doctype.data_import.data_import import export_csv
...
def data_import(
	...
	from creqit.core.doctype.data_import.data_import import import_file
...
def bulk_rename(context: CliCtxObj, doctype, path):
	...
	from creqit.model.rename_doc import bulk_rename
	from creqit.utils.csvutils import read_csv_content
...
def postgres(context: CliCtxObj, extra_args):
	...
	site = get_site(context)
	creqit.init(site)
	creqit.conf.db_type = "postgres"
	_enter_console(extra_args=extra_args)


def _enter_console(extra_args=None):
	from creqit.database import get_command
	from creqit.utils import get_site_path
	...
	bin, args, bin_name = get_command(
		...
		user=creqit.conf.db_user,
		password=creqit.conf.db_password,
...
def jupyter(context: CliCtxObj):
	...
	jupyter_notebooks_path = os.path.abspath(creqit.get_site_path("jupyter_notebooks"))
	sites_path = os.path.abspath(creqit.get_site_path(".."))
...
Run the following in your first cell to connect notebook to creqit
```
import creqit
creqit.init('{site}', sites_path='{sites_path}')
creqit.connect()
creqit.local.lang = creqit.db.get_default('lang')
creqit.db.connect()
```
	"""
	)
...
def store_logs(terminal: "InteractiveShellEmbed") -> None:
	...
	creqit.log_level = 20  # info
	with suppress(Exception):
		logger = creqit.logger("ipython")
...
def console(context: CliCtxObj, autoreload=False):
	...
	all_apps = creqit.get_installed_apps()
	failed_to_import = []
...
def transform_database(context: CliCtxObj, table, engine, row_format, failfast):
	...
	if table == "all":
		...
		queried_tables = (
			...
			.where(
				...
				& (information_schema.tables.table_schema == creqit.conf.db_name)
			)
			.run()
		)
	...
	for current, table in enumerate(tables):
		...
		try:
			creqit.db.sql(f"ALTER TABLE `{table}`{values_to_set}")
...
def serve(
	...
	with CodeCoverage(with_coverage, "creqit"):
		if with_coverage:
		...
		try:
			...
			if args:
				...
				if args.startswith("/api/method"):
					creqit.local.form_dict.cmd = args.split("?", 1)[0].split("/")[-1]
			elif path:
			...
			creqit.handler.execute_cmd(creqit.form_dict.cmd)

			print(creqit.response)
		finally:
			creqit.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def make_app(destination, app_name, no_git=False):
	...
	from creqit.utils.boilerplate import make_boilerplate
...
def create_patch():
	...
	from creqit.utils.boilerplate import PatchCreator
...
def get_version(output):
	...
	from creqit.utils.change_log import get_app_branch
	from creqit.utils.commands import render_table

	creqit.init("")
	data = []

	for app in sorted(creqit.get_all_apps()):
		module = creqit.get_module(app)
		app_hooks = creqit.get_module(app + ".hooks")

		app_info = creqit._dict()

		try:
			app_info.commit = Repo(creqit.get_app_source_path(app)).head.object.hexsha[:7]
...
def rebuild_global_search(context: CliCtxObj, static_pages=False):
	...
	from creqit.utils.global_search import (
