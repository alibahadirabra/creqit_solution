creqit.pages["print-format-builder"].on_page_load = function (wrapper) {
	creqit.print_format_builder = new creqit.PrintFormatBuilder(wrapper);
	creqit.breadcrumbs.add("Setup", "Print Format");
};

creqit.pages["print-format-builder"].on_page_show = function (wrapper) {
	var route = creqit.get_route();
	if (route.length > 1) {
		creqit.model.with_doc("Print Format", route[1], function () {
			creqit.print_format_builder.print_format = creqit.get_doc("Print Format", route[1]);
			creqit.print_format_builder.refresh();
		});
	} else if (creqit.route_options) {
		if (creqit.route_options.make_new) {
			let { doctype, name, based_on, beta } = creqit.route_options;
			creqit.route_options = null;
			creqit.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			creqit.print_format_builder.print_format = creqit.route_options.doc;
			creqit.route_options = null;
			creqit.print_format_builder.refresh();
		}
	}
...
creqit.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = creqit.ui.make_app_page({
			parent: this.parent,
	...
	show_start() {
		this.page.main.html(creqit.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = creqit.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function () {
			...
			if (!name) return;
			creqit.model.with_doc("Print Format", name, function (doc) {
				creqit.set_route("print-format-builder", name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = creqit.ui.form.make_control({
		...
		this.name_input = creqit.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function () {
			...
			if (!(doctype && name)) {
				creqit.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		creqit.call({
			method: "creqit.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
	...
	setup_print_format() {
		var me = this;
		creqit.model.with_doctype(this.print_format.doc_type, function (doctype) {
			me.meta = creqit.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function () {
				creqit.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(creqit.render_template("print_format_builder_sidebar", { fields: fields }))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(
			creqit.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function (i, f) {
			...
			if (!f.name && f.fieldname) {
				...
				if (f.fieldname === "_custom_html") {
					...
					f = $.extend(
						creqit.meta.get_docfield(me.print_format.doc_type, f.fieldname) || {},
						f
					);
				}
			}
			...
			if (f.fieldtype === "Section Break") {
				...
				set_column();
			} else if (!creqit.model.layout_fields.includes(f.fieldtype)) {
	...
	init_visible_columns(f) {
		f.visible_columns = [];
		$.each(creqit.get_meta(f.options).fields, function (i, _f) {
			if (
			...
			onAdd: function (evt) {
				...
				if (!$item.hasClass("print-format-builder-field")) {
					...
					if (fieldname === "_custom_html") {
						...
						field = creqit.meta.get_docfield(me.print_format.doc_type, fieldname);
					}

					var html = creqit.render_template("print_format_builder_field", {
						field: field,
						me: me,
					});
	...
	setup_add_section() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function () {
			...
			var $section = $(
				creqit.render_template("print_format_builder_section", {
					section: section,
					me: me,
				})
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			var doc_fields = creqit.get_meta(doctype).fields;
			...
			// render checkboxes
			$(
				creqit.render_template("print_format_builder_column_selector", {
					fields: fields,
	...
	save_print_format() {
		...
		creqit.call({
			...
			callback: function (r) {
				...
				creqit.show_alert({ message: __("Saved"), indicator: "green" });
			},
		});
	}
};
