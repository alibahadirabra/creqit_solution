creqit.pages["print-format-builder-beta"].on_page_load = function (wrapper) {
	...
	if (creqit.boot.developer_mode) {
		...
		creqit.hot_update.push(() => load_print_format_builder_beta(wrapper));
	}
};

creqit.pages["print-format-builder-beta"].on_page_show = function (wrapper) {
...
function load_print_format_builder_beta(wrapper) {
	...
	if (route.length > 1) {
		creqit.require("print_format_builder.bundle.js").then(() => {
			creqit.print_format_builder = new creqit.ui.PrintFormatBuilder({
				wrapper: $parent,
				page: wrapper.page,
		...
		let d = new creqit.ui.Dialog({
			...
			primary_action({ action, doctype, print_format, print_format_name }) {
				if (action === "Edit") {
					creqit.set_route("print-format-builder-beta", print_format);
					...
					creqit.db
						...
						.then((doc) => {
							creqit.set_route("print-format-builder-beta", doc.name);
						})
						.finally(() => {
