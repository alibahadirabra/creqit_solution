...
from creqit.utils.weasyprint import download_pdf, get_html
...
class PrintFormat(Document):
	...
	def validate(self):
		if (
			...
			and not creqit.flags.in_install
			and not creqit.flags.in_test
		):
			creqit.throw(creqit._("Standard Print Format cannot be updated"))

		# old_doc_type is required for clearing item cache
		self.old_doc_type = creqit.db.get_value("Print Format", self.name, "doc_type")
		...
		if self.custom_format and self.raw_printing and not self.raw_commands:
			creqit.throw(_("{0} are required").format(creqit.bold(_("Raw Commands"))), creqit.MandatoryError)

		if self.custom_format and not self.html and not self.raw_printing:
			creqit.throw(_("{0} is required").format(creqit.bold(_("HTML"))), creqit.MandatoryError)
	...
	def on_update(self):
		if hasattr(self, "old_doc_type") and self.old_doc_type:
			creqit.clear_cache(doctype=self.old_doc_type)
		if self.doc_type:
...
def make_default(name):
	...
	creqit.has_permission("Print Format", "write")

	print_format = creqit.get_doc("Print Format", name)

	doctype = creqit.get_doc("DocType", print_format.doc_type)
	if doctype.custom:
	...
	creqit.msgprint(
		creqit._("{0} is now default print format for {1} doctype").format(
			creqit.bold(name), creqit.bold(print_format.doc_type)
		)
	)
