...
class TestPrintFormat(IntegrationTestCase):
	def test_print_user(self, style=None):
		print_html = creqit.get_print("User", "Administrator", style=style)
	...
	def test_export_doc(self):
		doc: "PrintFormat" = creqit.get_doc("Print Format", self.globalTestRecords["Print Format"][0]["name"])

		# this is only to make export_doc happy
		doc.standard = "Yes"
		_before = creqit.conf.developer_mode
		creqit.conf.developer_mode = True
		export_path = doc.export_doc()
		creqit.conf.developer_mode = _before
		...
		with open(exported_doc_path) as f:
			exported_doc = creqit.parse_json(f.read())
