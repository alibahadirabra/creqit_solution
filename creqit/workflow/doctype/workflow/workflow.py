...
class Workflow(Document):
	...
	if TYPE_CHECKING:
		...
		from creqit.workflow.doctype.workflow_document_state.workflow_document_state import (
			WorkflowDocumentState,
		)
		from creqit.workflow.doctype.workflow_transition.workflow_transition import WorkflowTransition
	...
	def on_update(self):
		creqit.clear_cache(doctype=self.document_type)

	def create_custom_field_for_workflow_state(self):
		creqit.clear_cache(doctype=self.document_type)
		meta = creqit.get_meta(self.document_type)
	...
	def validate_docstatus(self):
		def get_state(state):
			...
			creqit.throw(creqit._("{0} not a valid State").format(state))
		...
		for t in self.transitions:
			...
			if state.doc_status == "2":
				creqit.throw(
					creqit._("Cannot change state of Cancelled Document. Transition row {0}").format(t.idx)
				)
			...
			if state.doc_status == "0" and next_state.doc_status == "2":
				creqit.throw(creqit._("Cannot cancel before submitting. See Transition {0}").format(t.idx))
...
def get_workflow_state_count(doctype, workflow_state_field, states):
	creqit.has_permission(doctype=doctype, ptype="read", throw=True)
	states = creqit.parse_json(states)

	if workflow_state_field in creqit.get_meta(doctype).get_valid_columns():
