...
from creqit.desk.form.utils import get_pdf_link
from creqit.desk.notifications import clear_doctype_notifications
...
from creqit.utils import get_datetime, get_url
from creqit.utils.background_jobs import enqueue
from creqit.utils.data import get_link_to_form
from creqit.utils.user import get_users_with_role
...
class WorkflowAction(Document):
	...
	if TYPE_CHECKING:
		...
		from creqit.workflow.doctype.workflow_action_permitted_role.workflow_action_permitted_role import (
...
def on_doctype_update():
	...
	creqit.db.add_index("Workflow Action", ["reference_name", "reference_doctype", "status"])
...
def get_permission_query_conditions(user):
	...
	permitted_workflow_actions = (
		creqit.qb.from_(WorkflowAction)
...
def has_permission(doc, user):
	...
	return not permitted_roles.isdisjoint(creqit.get_roles(user))
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = creqit.session.user
	...
	if logged_in_user == "Guest":
		creqit.set_user(logged_in_user)
...
def return_success_page(doc):
	creqit.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), creqit.bold(doc.get("name")), creqit.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	creqit.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			creqit.bold(doc.get("name")),
			creqit.bold(doc_workflow_state),
			creqit.bold(creqit.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def get_allowed_roles(user, workflow, workflow_state):
	user = user if user else creqit.session.user

	allowed_roles = creqit.get_all(
		"Workflow Transition",
		fields="allowed",
	...
	user_roles = set(creqit.get_roles(user))
...
def update_completed_workflow_actions_using_role(user=None, workflow_action=None):
	...
	(
		creqit.qb.update(WorkflowAction)
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = creqit._dict(
					{
						"possible_actions": [],
						"email": creqit.db.get_value("User", user, "email"),
					}
				)
...
def create_workflow_actions_for_roles(roles, doc):
	if not roles:
		return
	workflow_action = creqit.get_doc(
		{
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = "/api/method/creqit.workflow.doctype.workflow_action.workflow_action.apply_action"
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/creqit.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def get_common_email_args(doc):
	...
	if email_template:
		subject = creqit.render_template(email_template.subject, vars(doc))
		response = creqit.render_template(email_template.response, vars(doc))
	else:
	...
	lang = doc.get("language") or (
		creqit.get_cached_value("Print Format", print_format, "default_print_language")
		if print_format
		else None
	)
...
def get_email_template(doc):
	...
	template_name = creqit.db.get_value(
	...
	if not template_name:
		return
	return creqit.get_doc("Email Template", template_name)


def get_state_optional_field_value(workflow_name, state):
	return creqit.get_cached_value(
