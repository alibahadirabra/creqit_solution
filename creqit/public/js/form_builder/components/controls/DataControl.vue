...
if (props.df.fieldtype === "Datetime") {
	let time_zone_text = creqit.boot.time_zone
		? creqit.boot.time_zone.user
		: creqit.sys_defaults.time_zone;
...
<template>
	<div class="control creqit-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div
			...
			v-html="creqit.utils.icon('folder-normal', 'md')"
		/>
		<!-- phone selector icon -->
		<div
			v-if="df.fieldtype == 'Phone'"
			class="selected-phone no-value"
			v-html="creqit.utils.icon('down', 'sm')"
		/>
	</div>
</template>
