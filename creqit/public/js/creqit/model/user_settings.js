creqit.provide("creqit.model.user_settings");

$.extend(creqit.model.user_settings, {
	get: function (doctype) {
		return creqit
			.call("creqit.model.utils.user_settings.get", { doctype })
			.then((r) => JSON.parse(r.message || "{}"));
	},
	save: function (doctype, key, value) {
		if (creqit.session.user === "Guest") return Promise.resolve();

		const old_user_settings = creqit.model.user_settings[doctype] || {};
	...
	remove: function (doctype, key) {
		var user_settings = creqit.model.user_settings[doctype] || {};
	...
	update: function (doctype, user_settings) {
		if (creqit.session.user === "Guest") return Promise.resolve();
		return creqit.call({
			method: "creqit.model.utils.user_settings.save",
			args: {
				doctype: doctype,
				user_settings: user_settings,
			},
			callback: function (r) {
				creqit.model.user_settings[doctype] = r.message;
			},
		});
	},
});

creqit.get_user_settings = function (doctype, key) {
	var settings = creqit.model.user_settings[doctype] || {};
	if (key) {
