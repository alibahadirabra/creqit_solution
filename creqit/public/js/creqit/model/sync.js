...
Object.assign(creqit.model, {
	docinfo: {},
	sync: function (r) {
		/* docs:
			extract docs, docinfo (attachments, comments, assignments)
			from incoming request and set in `locals` and `creqit.model.docinfo`
		*/
		var isPlain;
		...
		if (r.docs) {
			for (var i = 0, l = r.docs.length; i < l; i++) {
				...
				if (locals[d.doctype] && locals[d.doctype][d.name]) {
					// update values
					creqit.model.update_in_locals(d);
				} else {
					creqit.model.add_to_locals(d);
				}
				...
				if (d.localname) {
					creqit.model.rename_after_save(d, i);
				}
			}
		}

		creqit.model.sync_docinfo(r);
		return r.docs;
	},

	rename_after_save: (d, i) => {
		creqit.model.new_names[d.localname] = d.name;
		$(document).trigger("rename", [d.doctype, d.localname, d.name]);
		delete locals[d.doctype][d.localname];

		// update docinfo to new dict keys
		if (i === 0) {
			creqit.model.docinfo[d.doctype][d.name] = creqit.model.docinfo[d.doctype][d.localname];
			creqit.model.docinfo[d.doctype][d.localname] = undefined;
		}
	},
	...
	sync_docinfo: (r) => {
		...
		if (r.docinfo) {
			...
			if (!creqit.model.docinfo[doctype]) {
				creqit.model.docinfo[doctype] = {};
			}
			creqit.model.docinfo[doctype][name] = r.docinfo;

			// copy values to creqit.boot.user_info
			Object.assign(creqit.boot.user_info, r.docinfo.user_info);
		}
	...
	add_to_locals: function (doc) {
		...
		if (!doc.name && doc.__islocal) {
			...
			if (!doc.parentfield) creqit.model.clear_doc(doc);

			doc.name = creqit.model.get_new_name(doc.doctype);

			if (!doc.parentfield)
				creqit.provide("creqit.model.docinfo." + doc.doctype + "." + doc.name);
		}
	...
	update_in_locals: function (doc) {
		...
		for (let fieldname in doc) {
			let df = creqit.meta.get_field(doc.doctype, fieldname);
			if (df && creqit.model.table_fields.includes(df.fieldtype)) {
				// table
				...
				for (let i = 0; i < doc[fieldname].length; i++) {
					...
					if (local_d) {
						...
						if (!d.name) {
							...
							d.name = creqit.model.get_new_name(doc.doctype);
						}
