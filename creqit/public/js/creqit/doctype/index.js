...
*/
creqit.model.DocTypeController = class DocTypeController extends creqit.ui.form.Controller {
	setup() {
		// setup formatters for fieldtype
		creqit.meta.docfield_map[
	...
	show_db_utilization() {
		...
		creqit
			.xcall("creqit.core.doctype.doctype.doctype.get_row_size_utilization", {
				doctype,
			})
			.then((r) => {
				if (r < 50.0) return;
	...
	setup_fetch_from_fields(doc, doctype, docname) {
		...
		let row = creqit.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			creqit.model.with_doctype(link_doctype, () => {
				let fields = creqit.meta
