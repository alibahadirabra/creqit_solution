...
creqit.views.ListView = class ListView extends creqit.views.BaseList {
	static load_last_view() {
		...
		creqit.after_ajax(() => {
			...
			if (!creqit.user.has_role("Administrator")) {
				const interval = setInterval(() => {
					...
					const menuItems = menuBtnGroup?.querySelector("ul.dropdown-menu");
		...
		if (route.length === 2) {
			...
			creqit.set_route(
				"list",
				creqit.router.doctype_layout || doctype,
				creqit.views.is_valid(last_view) ? last_view.toLowerCase() : "list"
			);
			return true;
		}
		return false;
	...
	constructor(opts) {
		super(opts);
		this.show();
		this.debounced_refresh = creqit.utils.debounce(
	...
	has_permissions() {
		return creqit.perm.has_perm(this.doctype, 0, "read");
	}
	...
	check_permissions() {
		if (!this.has_permissions()) {
			creqit.set_route("");
			creqit.throw(__("Not permitted to view {0}", [this.doctype]));
		}
	}
	...
	validate_filters(filters) {
		...
		valid_fields = valid_fields.concat(creqit.model.std_fields_list);
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = creqit.perm.get_match_rules(this.doctype);
		if (match_rules_list.length) {
			this.restricted_list = $(
				`<button class="btn btn-xs restricted-button flex align-center">
					${creqit.utils.icon("restriction", "xs")}
				</button>`
			)
	...
	show_restrictions(match_rules_list = []) {
		creqit.msgprint(
			creqit.render_template("list_view_permission_restrictions", {
	...
	async set_fields() {
		...
		await Promise.all(
			fields.map((f) => {
				return new Promise((resolve) => {
					const df =
						typeof f === "string" ? creqit.meta.get_docfield(this.doctype, f) : f;
					if (
						df &&
						df.fieldtype == "Link" &&
						creqit.boot.link_title_doctypes.includes(df.options)
					) {
						creqit.model.with_doctype(df.options, () => {
							const meta = creqit.get_meta(df.options);
		...
		this.fields.forEach((f) => {
			const df = creqit.meta.get_docfield(f[1], f[0]);
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = creqit.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise((resolve) => {
			if (this.required_libs) {
				creqit.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
	...
	set_primary_action() {
		if (this.can_create && !creqit.boot.read_only) {
			const doctype_name = __(creqit.router.doctype_layout) || __(this.doctype);
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (f[2] === "=" && creqit.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
		creqit.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = creqit.meta.get_docfield.bind(null, this.doctype);
		...
		if (creqit.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: "Status",
			});
		...
		this.columns = this.columns.concat(
			fields_in_list_view
				.filter((df) => {
					if (creqit.has_indicator(this.doctype) && df.fieldname === "status") {
						return false;
					}
	...
	before_refresh() {
		if (creqit.route_options && this.filter_area) {
	...
	before_render() {
		...
		creqit.model.user_settings.save(this.doctype, "last_view", this.view_name);
		...
		const $columns = this.columns
			.map((col) => {
				let classes = [
					...
					creqit.model.is_numeric_field(col.df) ? "text-right" : "",
				].join(" ");
	...
	get_column_html(col, doc) {
		...
		let translated_doctypes = creqit.boot?.translated_doctypes || [];
		...
		const format = () => {
			if (df.fieldtype === "Percent") {
				...
				return creqit.format(value, df, null, doc);
			}
		};
		...
		const field_html = () => {
			...
			if (strip_html_required) {
				...
				_value =
					typeof value_display === "string"
						? creqit.utils.escape_html(value_display)
						: value_display;
			}
			...
			if (df.fieldtype === "Image") {
				html = df.options
					...
					: `<div class="missing-image small">
						${creqit.utils.icon("restriction")}
					</div>`;
			} else if (df.fieldtype === "Select") {
				html = `<span class="filterable indicator-pill ${creqit.utils.guess_colour(
					_value
				)} ellipsis"
			...
			} else if (creqit.model.html_fieldtypes.includes(df.fieldtype)) {
				html = `<span class="ellipsis">
					${_value}
				</span>`;
			} else {
				html = `<a class="filterable ellipsis"
					data-filter="${fieldname},=,${creqit.utils.escape_html(value)}">
					${format()}
				</a>`;
			}

			return `<span class="ellipsis"
				title="${__(label)}: ${creqit.utils.escape_html(_value)}">
				${html}
			</span>`;
		};
		...
		const css_class = [
			"list-row-col ellipsis",
			class_map[col.type],
			creqit.model.is_numeric_field(df) ? "text-right" : "",
		].join(" ");
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments d-flex align-items-center">
					${creqit.avatar_group(assigned_users, 3, { filterable: true })[0].outerHTML}
				</div>`;
		}
		...
		// 		${creqit.utils.icon("es-line-chat-alt")}
	...
	get_form_link(doc) {
		...
		return `/app/${encodeURIComponent(
			creqit.router.slug(creqit.router.doctype_layout || this.doctype)
		)}/${encodeURIComponent(cstr(doc.name))}`;
	}

	get_seen_class(doc) {
		const seen_by = doc._seen ? JSON.parse(doc._seen) : [];
		return seen_by.includes(creqit.session.user) ? "" : "regular";//bold yerine regular verildi <<creqit.v1.sevval
	}
	//tablo >> alanı gizlendiği için fonksiyonu kaldırıldı <<sevval
	// get_like_html(doc) {
	// 	const liked_by = doc._liked_by ? JSON.parse(doc._liked_by) : [];
	// 	const is_liked = liked_by.includes(creqit.session.user);
	// 	const title = liked_by.map((u) => creqit.user_info(u).fullname).join(", ");
	...
	get_subject_text(doc, title) {
		...
		if (creqit.model.html_fieldtypes.includes(subject_field.fieldtype)) {
			// NOTE: this is very slow, so only do it for HTML fields
			return creqit.utils.html2text(value);
		} else {
			return value;
		}
	}
	...
	get_indicator_html(doc, show_workflow_state) {
		const indicator = creqit.get_indicator(doc, this.doctype, show_workflow_state);
		...
		if (indicator) {
			const text = indicator[0] || "";//creqit.v1.sevval//
			const iconType = indicator[3] || "";//creqit.v1.sevval//
			const iconPosition = indicator[4] === 'left' ? 'indicator-icon-left' : 'indicator-icon-right';//creqit.v1.sevval//
			return `<span class="indicator-pill ${
				...
				${
					iconType ? `<div class="indicator-icon" style="fill:${indicator[1]};">${creqit.utils.icon(iconType, "lg")}</div>` : ""
				}
			</span>`;//creqit.v1.sevval//
		}
		return "";
	}

	get_indicator_dot(doc) {
		const indicator = creqit.get_indicator(doc, this.doctype);
		if (!indicator) return "";
	...
	get_image_url(doc) {
		...
		if (window.cordova && !creqit.utils.is_url(url)) {
			url = creqit.base_url + url;
		}
		return url || null;
	}
	...
	setup_filterable() {
		...
		this.$result.on("click", ".filterable", (e) => {
			...
			const filters_to_apply = filters.map((f) => {
				f = f.split(",");
				if (f[2] === "Today") {
					f[2] = creqit.datetime.get_today();
				} else if (f[2] == "User") {
					f[2] = creqit.session.user;
				}
	...
	setup_list_click() {
		this.$result.on("click", ".list-row, .image-view-header, .file-header", (e) => {
			...
			if (link) {
				creqit.set_route(link.pathname);
				return false;
			}
		});
	}
	...
	setup_like() {
		this.$result.on("click", ".like-action", (e) => {
			...
			creqit.ui.toggle_like($this, doctype, name);
	...
	setup_realtime_updates() {
		...
		creqit.realtime.doctype_subscribe(this.doctype);
		creqit.realtime.off("list_update");
		creqit.realtime.on("list_update", (data) => {
	...
	disable_realtime_updates() {
		creqit.realtime.doctype_unsubscribe(this.doctype);
	...
	process_document_refreshes() {
		...
		creqit.call(call_args).then(({ message }) => {
			if (!message) return;
			const data = creqit.utils.dict(message.keys, message.values);
	...
	save_view_user_settings(obj) {
		return creqit.model.user_settings.save(this.doctype, this.view_name, obj);
	}

	on_update() {}

	update_url_with_filters() {
		if (creqit.get_route_str() == this.page_name && !this.report_name) {
	...
	get_menu_items() {
		...
		if (creqit.model.can_import(doctype, null, this.meta)) {
			items.push({
				label: __("Import", null, "Button in list view menu"),
				action: () =>
					creqit.set_route("list", "data-import", {
		...
		if (creqit.user_roles.includes("System Manager")) {
			items.push({
				...
				action: () =>
					creqit.set_route("permission-manager", {
						doctype,
					}),
				standard: true,
			});
		...
		if (
			creqit.model.can_create("Custom Field") &&
			creqit.model.can_create("Property Setter")
		) {
			items.push({
				...
				action: () => {
					if (!this.meta) return;
					if (this.meta.custom) {
						creqit.set_route("form", "doctype", doctype);
					} else if (!this.meta.custom) {
						creqit.set_route("form", "customize-form", {
							doc_type: doctype,
						});
					}
				},
		...
		if (creqit.user.has_role("System Manager") && creqit.boot.developer_mode) {
			// edit doctype
			items.push({
				label: __("Edit DocType", null, "Button in list view menu"),
				action: () => creqit.set_route("form", "doctype", doctype),
				standard: true,
			});
		}
	...
	show_list_settings() {
		creqit.model.with_doctype(this.doctype, () => {
			new ListSettings({
				listview: this,
				doctype: this.doctype,
				settings: this.list_view_settings,
				meta: creqit.get_meta(this.doctype),
			});
		});
	}
	...
	get_workflow_action_menu_items() {
		...
		if (creqit.model.has_workflow(this.doctype)) {
			const actions = creqit.workflow.get_all_transition_actions(this.doctype);
			actions.forEach((action) => {
				workflow_actions.push({
					...
					action: () => {
						...
						creqit
							.xcall("creqit.model.workflow.bulk_workflow_approval", {
	...
	toggle_workflow_actions() {
		if (!creqit.model.has_workflow(this.doctype)) return;
		...
		creqit
			.xcall("creqit.model.workflow.get_common_transition_actions", {
				docs: checked_items,
				doctype: this.doctype,
			})
			.then((actions) => {
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return (
				field_doc.fieldname &&
				creqit.model.is_value_type(field_doc) &&
		...
		const has_submit_permission = (doctype) => {
			return creqit.perm.has_perm(doctype, 0, "submit");
		};
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					creqit.meta.get_docfields(doctype).forEach((field_doc) => {
		...
		// bulk edit
		if (has_editable_fields(doctype) && !creqit.model.has_workflow(doctype)) {
		...
		// bulk printing
		if (creqit.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// bulk submit
		if (
			creqit.model.is_submittable(doctype) &&
			has_submit_permission(doctype) &&
			!creqit.model.has_workflow(doctype)
		) {
			actions_menu_items.push(bulk_submit());
		}

		// bulk cancel
		if (creqit.model.can_cancel(doctype) && !creqit.model.has_workflow(doctype)) {
			actions_menu_items.push(bulk_cancel());
		}

		// bulk delete
		if (creqit.model.can_delete(doctype) && !creqit.model.has_workflow(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		if (!params.toString() && creqit.route_options) {
			params = new Map(Object.entries(creqit.route_options));
		}
		...
		params.forEach((value, field) => {
			...
			if (!doctype) {
				doctype = creqit.meta.get_doctype_for_field(this.doctype, field);
			}
...
creqit.get_list_view = (doctype) => {
	let route = `List/${doctype}/List`;
	return creqit.views.list_view[route];
};
