...
creqit.provide("creqit.views.list_view");

window.cur_list = null;
creqit.views.ListFactory = class ListFactory extends creqit.views.Factory {
	make(route) {
		const me = this;
		const doctype = route[1];

		// List / Gantt / Kanban / etc
		let view_name = creqit.utils.to_title_case(route[2] || "List");
		...
		let view_class = creqit.views[view_name + "View"];
		if (!view_class) view_class = creqit.views.ListView;
		...
		creqit.provide("creqit.views.list_view." + doctype);

		creqit.views.list_view[me.page_name] = new view_class({
			doctype: doctype,
	...
	re_route_to_view() {
		...
		const last_route = creqit.route_history.slice(-2)[0];
		if (
			this.route[0] === "List" &&
			this.route.length === 2 &&
			creqit.views.list_view[doctype] &&
			last_route &&
			last_route[0] === "List" &&
	...
	set_module_breadcrumb() {
		if (creqit.route_history.length > 1) {
			const prev_route = creqit.route_history[creqit.route_history.length - 2];
			if (prev_route[0] === "modules") {
				const doctype = this.route[1],
					module = prev_route[1];
				if (creqit.module_links[module] && creqit.module_links[module].includes(doctype)) {
					// save the last page from the breadcrumb was accessed
					creqit.breadcrumbs.set_doctype_module(doctype, module);
				}
			}
		}
	}
	...
	set_cur_list() {
		cur_list = creqit.views.list_view[this.page_name];
