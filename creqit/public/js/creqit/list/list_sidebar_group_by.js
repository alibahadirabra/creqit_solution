...
creqit.views.ListGroupBy = class ListGroupBy {
	constructor(opts) {
		$.extend(this, opts);
		this.make_wrapper();
	...
	render_group_by_items() {
		let get_item_html = (fieldname) => {
			...
			if (fieldname === "assigned_to") {
				...
				label = creqit.meta.get_label(this.doctype, fieldname);
				let docfield = creqit.meta.get_docfield(this.doctype, fieldname);
				if (!docfield) {
					return;
				}
			...
			return `<div class="group-by-field list-link">
						...
						href="#" onclick="return false;">
							...
							<span>${creqit.utils.icon("select", "xs")}</span>
						</a>
	...
	setup_search($dropdown) {
		creqit.utils.setup_search($dropdown, ".group-by-item", ".group-by-value", "data-name");
	}
	...
	get_group_by_count(field) {
		...
		return creqit.call("creqit.desk.listview.get_group_by_count", args).then((r) => {
			let field_counts = r.message || [];
			field_counts = field_counts.filter((f) => f.count !== 0);
			let current_user = field_counts.find((f) => f.name === creqit.session.user);
			field_counts = field_counts.filter(
				(f) => !["Guest", "Administrator", creqit.session.user].includes(f.name)
			);
			// Set creqit.session.user on top of the list
	...
	get_dropdown_html(field, fieldtype, applied = false) {
		...
		if (field.name == null) {
			label = __("Not Set");
		} else if (field.name === creqit.session.user) {
			label = __("Me");
		...
		let applied_html = applied
			? `<span class="applied"> ${creqit.utils.icon("tick", "xs")} </span>`
			: "";
