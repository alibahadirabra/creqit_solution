...
creqit.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = creqit.utils.to_title_case(route[2] || "");
		if (route.length > 2 && creqit.views.view_modes.includes(view_name)) {
	...
	set_route(view, calendar_name) {
		...
		if (search_params) {
			creqit.route_options = Object.fromEntries(search_params);
		}
	...
	setup_views() {
		const views = {
			...
			Calendar: {
				condition: creqit.views.calendar[this.doctype],
			...
			Inbox: {
				condition: this.doctype === "Communication" && creqit.boot.email_accounts.length,
				action: () => this.set_route("inbox"),
				current_view_handler: () => {
					const accounts = this.get_email_accounts();
					let default_action;
					if (has_common(creqit.user_roles, ["System Manager", "Administrator"])) {
						default_action = {
							label: __("New Email Account"),
							action: () => creqit.new_doc("Email Account"),
						};
					}
			...
			Tree: {
				condition:
					creqit.treeview_settings[this.doctype] ||
					creqit.get_meta(this.doctype).is_tree,
		...
		creqit.views.view_modes.forEach((view) => {
	...
	setup_kanban_switcher(kanbans) {
		...
		if (can_create) {
			this.page.add_custom_menu_item(
				...
				() => creqit.views.KanbanView.show_kanban_dialog(this.doctype),
				true
			);
		}
	}
	...
	get_page_name() {
		return creqit.utils.to_title_case(creqit.get_route().slice(-1)[0] || "");
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(creqit.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
			...
			creqit.db.get_value(
				...
				(board) => {
					if (!$.isEmptyObject(board)) {
						creqit.set_route("list", doctype, "kanban", board.name);
					} else {
						creqit.views.KanbanView.show_kanban_dialog(doctype);
					}
				}
			);
		}
		...
		const last_opened_kanban =
			creqit.model.user_settings[this.doctype]["Kanban"]?.last_kanban_board;
		if (!last_opened_kanban) {
			fetch_kanban_board(this.doctype);
		} else {
			creqit.db.exists("Kanban Board", last_opened_kanban).then((exists) => {
				if (exists) {
					creqit.set_route("list", this.doctype, "kanban", last_opened_kanban);
				} else {
					fetch_kanban_board(this.doctype);
				}
			});
	...
	get_calendars() {
		...
		return creqit.db
			...
			.then((result) => {
				...
				if (creqit.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = creqit.boot.email_accounts;
	...
	slug() {
		return creqit.router.slug(creqit.router.doctype_layout || this.doctype);
	}
};
