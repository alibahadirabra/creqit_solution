...
creqit.provide("creqit.ui.form.handlers");
...
creqit.ui.form.get_event_handler_list = function (doctype, fieldname) {
	if (!creqit.ui.form.handlers[doctype]) {
		creqit.ui.form.handlers[doctype] = {};
	}
	if (!creqit.ui.form.handlers[doctype][fieldname]) {
		creqit.ui.form.handlers[doctype][fieldname] = [];
	}
	return creqit.ui.form.handlers[doctype][fieldname];
};

creqit.ui.form.on = creqit.ui.form.on_change = function (doctype, fieldname, handler) {
	var add_handler = function (fieldname, handler) {
		var handler_list = creqit.ui.form.get_event_handler_list(doctype, fieldname);
...
creqit.ui.form.off = function (doctype, fieldname, handler) {
	var handler_list = creqit.ui.form.get_event_handler_list(doctype, fieldname);
...
creqit.ui.form.trigger = function (doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
};

creqit.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = creqit.get_doc(doctype, name);
	...
	has_handler(event_name) {
		...
		return (
			creqit.ui.form.handlers[this.frm.doctype] &&
			creqit.ui.form.handlers[this.frm.doctype][event_name]
		);
	}
	...
	get_handlers(event_name, doctype) {
		...
		if (creqit.ui.form.handlers[doctype] && creqit.ui.form.handlers[doctype][event_name]) {
			$.each(creqit.ui.form.handlers[doctype][event_name], function (i, fn) {
	...
	setup() {
		...
		$.each(this.frm.fields, function (i, field) {
			...
			if (creqit.model.table_fields.includes(field.df.fieldtype)) {
				$.each(
					creqit.meta.get_docfields(field.df.options, me.frm.docname),
					function (i, df) {
						setup_add_fetch(df);
					}
				);
		...
		// css
		doctype.__css && creqit.dom.set_style(doctype.__css);

		this.trigger("setup");
	}

	log_error(caller, e) {
		creqit.show_alert({ message: __("Error in Client Script."), indicator: "error" });
