...
creqit.ui.form.Sidebar = class {
	constructor(opts) {
		$.extend(this, opts);
	}

	make() {
		var sidebar_content = creqit.render_template("form_sidebar", {
			doctype: this.frm.doctype,
			frm: this.frm,
			can_write: creqit.model.can_write(this.frm.doctype, this.frm.docname),
		});
		...
		creqit.ui.form.setup_user_image_event(this.frm);
	...
	setup_keyboard_shortcuts() {
		...
		creqit.ui.keys.get_shortcut_group(this.page).add(assignment_link);
	}
	...
	refresh() {
		if (this.frm.doc.__islocal) {
			...
			creqit.ui.form.set_user_image(this.frm);
		}
	}
	...
	refresh_web_view_count() {
		if (this.frm.doc.route && cint(creqit.boot.website_tracking_enabled)) {
			...
			creqit.utils.get_page_view_count(route).then((res) => {
				this.sidebar
					.find(".pageview-count")
					.removeClass("hidden")
	...
	refresh_creation_modified() {
		...
		let avatar_group = creqit.avatar_group(user_list, 5, {
			align: "left",
			overlap: true,
		});
			...
			creqit.call({
				...
				callback: function (res) {
					...
					el.on("click", function () {
						creqit.set_route("Form", "Auto Repeat", me.frm.doc.auto_repeat);
					});
				},
			});
		}
	...
	show_webhook_request_log_status() {
		...
		if (docinfo.webhook_request_log_exists) {
			...
			el.show();
			el.on("click", () => {
				creqit.set_route("List", "Webhook Request Log", {
					reference_doctype: this.frm.doc.doctype,
	...
	make_tags() {
		...
		this.frm.tags = new creqit.ui.TagEditor({
			parent: tags_parent,
	...
	make_attachments() {
		var me = this;
		this.frm.attachments = new creqit.ui.form.Attachments({
	...
	make_assignments() {
		this.frm.assign_to = new creqit.ui.form.AssignTo({
	...
	make_shared() {
		this.frm.shared = new creqit.ui.form.Share({
			frm: this.frm,
	...
	make_review() {
		...
		if (creqit.boot.energy_points_enabled && !this.frm.is_new()) {
			this.frm.reviews = new creqit.ui.form.Review({
	...
	reload_docinfo(callback) {
		creqit.call({
			method: "creqit.desk.form.load.get_docinfo",
			args: {
				doctype: this.frm.doctype,
