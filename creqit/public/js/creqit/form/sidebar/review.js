...
creqit.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = creqit.boot.points;
	...
	update_points() {
		return creqit
			.xcall("creqit.social.doctype.energy_point_log.energy_point_log.get_energy_points", {
				user: creqit.session.user,
			})
			.then((data) => {
				creqit.boot.points = data;
				this.points = data;
			});
	}
	...
	show_review_dialog() {
		...
		const review_dialog = new creqit.ui.Dialog({
			title: __("Add Review"),
			fields: [
				{
					fieldname: "to_user",
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return creqit.msgprint(__("You do not have enough points"));
				}
				creqit
					.xcall("creqit.social.doctype.energy_point_log.energy_point_log.review", {
						doc: {
	...
	setup_detail_popover(el, data) {
		let subject = "";
		let fullname = creqit.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${creqit.datetime.comment_when(
			data.creation
		)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${creqit.utils.icon("review")}
							${subject}

							<p class="mt-1">
								<!-- ${creqit.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${creqit.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;
