...
creqit.ui.form.get_open_grid_form = function () {
	return $(".grid-row-open").data("grid_row");
};

creqit.ui.form.close_grid_form = function () {
	var open_form = creqit.ui.form.get_open_grid_form();
	open_form && open_form.hide_form();

	// hide editable row too
	if (creqit.ui.form.editable_row) {
...
export default class Grid {
	constructor(opts) {
		...
		this.debounced_refresh = creqit.utils.debounce(this.debounced_refresh, 100);
	}
	...
	make() {
		...
		creqit.utils.bind_actions_with_object(this.wrapper, this);
	...
	set_doc_url() {
		let unsupported_fieldtypes = creqit.model.no_value_type.filter(
			(x) => creqit.model.table_fields.indexOf(x) === -1
		);
	...
	delete_all_rows() {
		creqit.confirm(__("Are you sure you want to delete all rows?"), () => {
	...
	scroll_to_top() {
		creqit.utils.scroll_to(this.wrapper);
	}
	...
	debounced_refresh_remove_rows_button = creqit.utils.debounce(
		this.refresh_remove_rows_button,
		100
	);
	...
	refresh() {
		...
		if (this.frm) {
			this.display_status = creqit.perm.get_field_display_status(
				this.df,
				this.frm.doc,
				this.perm
			);
	...
	setup_fields() {
		...
		if (this.frm && this.frm.docname) {
			...
			this.df = creqit.meta.get_docfield(
				this.frm.doctype,
				this.df.fieldname,
				this.frm.docname
			);
			...
			if (this.df.options) {
				this.df =
					creqit.meta.get_docfield(this.df.options, this.df.fieldname) ||
					this.df ||
					null;
			}
		}
		...
		if (this.doctype && this.frm) {
			this.docfields = creqit.meta.get_docfields(this.doctype, this.frm.docname);
		} else {
	...
	get_data_based_on_fieldtype(df, data, value) {
		...
		if (fieldtype === "Check") {
			value = creqit.utils.string_to_boolean(value);
			...
			let formatted_duration = creqit.utils.get_formatted_duration(fieldvalue);
			...
			let user_formatted_date = creqit.datetime.str_to_user(fieldvalue);
				...
				row.columns_list.forEach((column) => {
					...
					if (column.df.fieldname == fieldname) {
						if (show) {
							...
							if (row != creqit.ui.form.editable_row) {
	...
	get_docfield(fieldname) {
		return creqit.meta.get_docfield(
			this.doctype,
			fieldname,
	...
	setup_visible_columns() {
		...
		for (var ci in fields) {
			...
			if (
				...
				if (
					...
					const docfield = creqit.meta.docfield_map[df.parent][df.fieldname];
		...
		while (total_colsize < 11 && passes < 12) {
			for (var i in this.visible_columns) {
				...
				if (colsize > 1 && colsize < 11 && creqit.model.is_non_std_field(df.fieldname)) {
					if (
						passes < 3 &&
	...
	setup_user_defined_columns() {
		...
		let user_settings = creqit.get_user_settings(this.frm.doctype, "GridView");
		if (user_settings && user_settings[this.doctype] && user_settings[this.doctype].length) {
			this.user_defined_columns = user_settings[this.doctype]
				.map((row) => {
					let column = creqit.meta.get_docfield(this.doctype, row.fieldname);
	...
	set_multiple_add(link, qty) {
		...
		var link_field = creqit.meta.get_docfield(this.df.options, link);
	...
	setup_allow_bulk_edit() {
		...
		if (this.frm && this.frm.get_docfield(this.df.fieldname)?.allow_bulk_edit) {
			...
			const value_formatter_map = {
				Date: (val) => (val ? creqit.datetime.user_to_str(val) : val),
				Int: (val) => cint(val),
				Check: (val) => cint(val),
				Float: (val) => flt(val),
			...
			$(this.wrapper)
				...
				.on("click", () => {
					new creqit.ui.FileUploader({
						...
						on_success(file) {
							var data = creqit.utils.csv_to_array(
								creqit.utils.get_decoded_string(file.dataurl)
							);
								...
								if (i > 6) {
									...
									if (!blank_row) {
										...
										$.each(row, (ci, value) => {
											...
											var df = creqit.meta.get_docfield(
												me.df.options,
												fieldname
											);
											if (df) {
	...
	setup_download() {
		let title = this.df.label || creqit.model.unscrub(this.df.fieldname);
		$(this.wrapper)
			.find(".grid-download")
			.removeClass("hidden")
			.on("click", () => {
				...
				data.push(["------"]);
				$.each(creqit.get_meta(this.df.options).fields, (i, df) => {
					// don't include the read-only field in the template
					if (creqit.model.is_value_type(df.fieldtype)) {
						data[1].push(df.label);
						data[2].push(df.fieldname);
						let description = (df.description || "") + " ";
						if (df.fieldtype === "Date") {
							description += creqit.boot.sysdefaults.date_format;
						}
				...
				$.each(this.frm.doc[this.df.fieldname] || [], (i, d) => {
					...
					$.each(data[2], (i, fieldname) => {
						...
						if (docfields[i].fieldtype === "Date" && value) {
							value = creqit.datetime.str_to_user(value);
						}
				...
				creqit.tools.downloadify(data, null, title);
				return false;
			});
	}
