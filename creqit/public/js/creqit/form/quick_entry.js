...
creqit.quick_edit = function (doctype, name) {
	if (!name) name = doctype; // single
	creqit.db.get_doc(doctype, name).then((doc) => {
		creqit.ui.form.make_quick_entry(doctype, null, null, doc);
	});
};

creqit.ui.form.make_quick_entry = (doctype, after_insert, init_callback, doc, force) => {
	var trimmed_doctype = doctype.replace(/ /g, "");
	var controller_name = "QuickEntryForm";

	if (creqit.ui.form[trimmed_doctype + "QuickEntryForm"]) {
		controller_name = trimmed_doctype + "QuickEntryForm";
	}

	creqit.quick_entry = new creqit.ui.form[controller_name](
		doctype,
		after_insert,
		init_callback,
		doc,
	...
	return creqit.quick_entry.setup();
};

creqit.ui.form.QuickEntryForm = class QuickEntryForm extends creqit.ui.Dialog {
	...
	check_quick_entry_doc() {
		if (!this.doc) {
			this.doc = creqit.model.get_new_doc(this.doctype, null, null, true);
		}
	}
	...
	render_dialog() {
		...
		this.onhide = () => (creqit.quick_entry = null);
		this.show();
			...
			if (data) {
				...
				me.script_manager.trigger("validate").then(() => {
					me.insert().then(() => {
						...
						me.dialog.hide();
						setTimeout(function () {
							creqit.show_alert({ message: messagetxt, indicator: "green" }, 3);
						}, 500);
					});
				});
			}
	...
	insert() {
		...
		return new Promise((resolve) => {
			me.update_doc();
			creqit.call({
				method: "creqit.client.save",
				args: {
					doc: me.dialog.doc,
				},
				callback: function (r) {
					if (
						creqit.model.is_submittable(me.doctype) &&
						!creqit.model.has_workflow(me.doctype)
					) {
						creqit.run_serially([
							() => (me.dialog.working = true),
							() => {
	...
	process_after_insert(r) {
		...
		} else if (creqit._from_link) {
			creqit.ui.form.update_calling_link(this.doc);
	...
	open_form_if_not_list() {
		...
		if (route && !(route[0] === "List" && route[1] === doc.doctype)) {
			creqit.run_serially([() => creqit.set_route("Form", doc.doctype, doc.name)]);
		}
	}
	...
	open_doc(set_hooks) {
		...
		if (set_hooks && this.after_insert) {
			creqit.route_options = creqit.route_options || {};
			creqit.route_options.after_save = (frm) => {
				this.after_insert(frm);
			};
		}
		creqit.set_route("Form", this.doctype, this.doc.name);
	}
