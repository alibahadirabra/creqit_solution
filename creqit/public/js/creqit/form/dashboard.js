...
creqit.ui.form.Dashboard = class FormDashboard {
	...
	setup_dashboard_sections() {
		...
		this.heatmap_area = this.make_section({
			...
			is_dashboard_section: 1,
			body_html: `
				<div id="heatmap-${creqit.model.scrub(this.frm.doctype)}" class="heatmap"></div>
	...
	refresh() {
		this.reset();
		if (this.frm.doc.__islocal || !creqit.boot.desk_settings.dashboard) {
			return;
		}
	...
	render_links() {
		...
		$(creqit.render_template("form_links", this.data)).appendTo(transactions_area_body);
	...
	render_report_links() {
		...
		if (this.data.reports && this.data.reports.length) {
			$(creqit.render_template("report_links", this.data)).appendTo(parent);
			// bind reports
	...
	open_report($link) {
		...
		creqit.provide("creqit.route_options");
		creqit.route_options[fieldname] = this.frm.doc.name;
		creqit.set_route("query-report", report);
	}
	...
	open_document_list($link, show_open) {
		...
		if (
			...
			if (names.length) {
				creqit.route_options = { name: ["in", names] };
			} else {
				return false;
			}
		} else if (this.data.fieldname) {
			creqit.route_options = this.get_document_filter(doctype);
			if (show_open && creqit.ui.notifications) {
				creqit.ui.notifications.show_open_count_list(doctype);
			}
		}

		creqit.set_route("List", doctype, "List");
	}
	...
	set_open_count() {
		...
		let method = this.data.method || "creqit.desk.notifications.get_open_count";
		creqit.call({
			type: "GET",
			method: method,
			args: {
	...
	// heatmap
	render_heatmap() {
		this.heatmap = new creqit.Chart("#heatmap-" + creqit.model.scrub(this.frm.doctype), {
			type: "heatmap",
	...
	render_graph(args) {
		...
		this.chart = new creqit.Chart(".form-graph", args);
		if (!this.chart) {
			this.hide();
		}
	}
