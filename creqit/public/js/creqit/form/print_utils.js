creqit.ui.get_print_settings = function (pdf, callback, letter_head, pick_columns) {
	var print_settings = locals[":Print Settings"]["Print Settings"];

	var company = creqit.defaults.get_default("company");
	...
	return creqit.prompt(
		...
		function (data) {
			...
			if (data.letter_head) {
				data.letter_head = creqit.boot.letter_heads[print_settings.letter_head];
			}
			callback(data);
		},
		__("Print Settings")
...
creqit.ui.form.qz_connect = function () {
	return new Promise(function (resolve, reject) {
		creqit.ui.form.qz_init().then(() => {
			if (qz.websocket.isActive()) {
				// if already active, resolve immediately
				// creqit.show_alert({message: __('QZ Tray Connection Active!'), indicator: 'green'});
				resolve();
			} else {
				// try to connect once before firing the mimetype launcher
				creqit.show_alert({
							...
							qz.websocket
								...
								.then(
									...
									() => {
										creqit.throw(
											__(
												'Error connecting to QZ Tray Application...<br><br> You need to have QZ Tray application installed and running, to use the Raw Print feature.<br><br><a target="_blank" href="https://qz.io/download/">Click here to Download and install QZ Tray</a>.<br> <a target="_blank" href="https://creqit.com/docs/user/manual/en/setting-up/print/raw-printing">Click here to learn more about Raw Printing</a>.'
											)
										);
										reject();
									}
...
creqit.ui.form.qz_init = function () {
	...
	return new Promise((resolve) => {
		if (typeof qz === "object" && typeof qz.version === "string") {
			...
			let qz_required_assets = [
				"/assets/creqit/node_modules/js-sha256/build/sha256.min.js",
				"/assets/creqit/node_modules/qz-tray/qz-tray.js",
			];
			creqit.require(qz_required_assets, () => {
			...
			// note 'creqit.require' does not have callback on fail. Hence, any failure cannot be communicated to the user.
		}
	});
};

creqit.ui.form.qz_get_printer_list = function () {
	// returns the list of printers that are available to the QZ Tray
	return creqit.ui.form
		.qz_connect()
		.then(function () {
...
creqit.ui.form.qz_success = function () {
...
creqit.ui.form.qz_fail = function (e) {
	// notify qz errors
	creqit.show_alert(
		{
