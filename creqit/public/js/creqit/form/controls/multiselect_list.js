creqit.ui.form.ControlMultiSelectList = class ControlMultiSelectList extends (
	creqit.ui.form.ControlData
) {
	...
	set_selectable_items(options) {
		let html = options
			.map((option) => {
				...
				<div class="multiselect-check">${creqit.utils.icon("tick", "xs")}</div>
			</li>`;
			})
			.join("");
		if (!html) {
