...
creqit.ui.form.ControlTextEditor = class ControlTextEditor extends creqit.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on(
			"text-change",
			creqit.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on("keydown", (e) => {
			const key = creqit.ui.keys && creqit.ui.keys.get_key(e);
	...
	get_mention_options() {
		...
		return {
			...
			source: creqit.utils.debounce(async function (search_term, renderList) {
				let method =
					me.mention_search_method || "creqit.desk.search.get_names_for_mentions";
				let values = await creqit.xcall(method, {
					search_term,
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? creqit.utils.icon("users") : ""}`;
			},
		};
	}
	...
	parse(value) {
		...
		return creqit.dom.remove_script_and_style(value);
	}
