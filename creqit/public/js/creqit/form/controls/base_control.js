creqit.ui.form.Control = class BaseControl {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make_wrapper() {
		this.$wrapper = $("<div class='creqit-control'></div>").appendTo(this.parent);
	...
	get_status(explain) {
		...
		var status = creqit.perm.get_field_display_status(
			this.df,
			creqit.model.get_doc(this.doctype, this.docname),
		...
		let value = creqit.model.get_value(this.doctype, this.docname, this.df.fieldname);
	...
	show_translatable_button(value) {
		...
		if (
			...
			!creqit.model.can_write("Translation") ||
			!value
		)
			return;

		// Disable translation in website
		if (!creqit.views || !creqit.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on("click", () => {
				if (!this.doc.__islocal) {
					new creqit.views.TranslationManager({
						df: this.df,
