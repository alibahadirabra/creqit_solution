creqit.ui.form.ControlPassword = class ControlPassword extends creqit.ui.form.ControlData {
	...
	make_input() {
		...
		this.toggle_password.on("click", () => {
			if (this.$input.attr("type") === "password") {
				...
				this.toggle_password.html(creqit.utils.icon("hide", "sm"));
			} else {
				this.$input.attr("type", "password");
				this.toggle_password.html(creqit.utils.icon("unhide", "sm"));
			}
		});
	...
	get_password_strength(value) {
		...
		creqit.call({
			type: "POST",
			method: "creqit.core.doctype.user.user.test_password_strength",
			args: {
				new_password: value || "",
			},
			callback: function (r) {
