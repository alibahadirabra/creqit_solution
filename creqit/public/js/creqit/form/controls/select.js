creqit.ui.form.ControlSelect = class ControlSelect extends creqit.ui.form.ControlData {
	...
	set_icon(is_xs_input) {
		const select_icon_html = `<div class="select-icon ${is_xs_input ? "xs" : ""}">
				${creqit.utils.icon("select", is_xs_input ? "xs" : "sm")}
			</div>`;
		//creqit.v1.sevval//
		...
		if (this.only_input) {
			...
			this.$wrapper.append(search_icon_html);//creqit.v1.sevval//
		} else {
			this.$wrapper
				.find(".control-input")
				.addClass("flex align-center")
				.append(select_icon_html)
				.append(search_icon_html);//creqit.v1.sevval//
		}
	}
	...
	get_file_attachment_list() {
		if (!this.frm) return;
		var fl = creqit.model.docinfo[this.frm.doctype][this.frm.docname];
...
creqit.ui.form.add_options = function (input, options_list, sort, doctype) {
	let $select = $(input);
...
(function ($) {
	$.fn.add_options = function (options_list, sort) {
		return creqit.ui.form.add_options(this.get(0), options_list, sort);
	};
