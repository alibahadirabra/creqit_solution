creqit.ui.form.ControlInput = class ControlInput extends creqit.ui.form.Control {
	...
	make_wrapper() {
		if (this.only_input) {
			this.$wrapper = $('<div class="form-group creqit-control">').appendTo(this.parent);
		} else {
			this.$wrapper = $(
				`<div class="creqit-control">
	...
	refresh_input() {
		...
		if (me.disp_status != "None") {
			// refresh value
			if (me.frm) {
				me.value = creqit.model.get_value(me.doctype, me.docname, me.df.fieldname);
			} else if (me.doc) {
				...
				if (is_fetch_from_read_only) {
					$(me.disp_area).attr(
						...
						__(
							...
							me.df.fetch_from.split(".").map((value) => __(creqit.unscrub(value)))
						)
					);
				}
			}
	...
	set_disp_area(value) {
		...
		if (["Data", "Long Text", "Small Text", "Text", "Password"].includes(this.df.fieldtype)) {
			value = creqit.utils.escape_html(value);
		}
		let doc = this.doc || (this.frm && this.frm.doc);
		let display_value = creqit.format(value, this.df, { no_icon: true, inline: true }, doc);
