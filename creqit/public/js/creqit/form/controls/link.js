...
creqit.ui.form.recent_link_validations = {};

creqit.ui.form.ControlLink = class ControlLink extends creqit.ui.form.ControlData {
		...
		$(`<div class="link-field ui-front" style="position: relative;">
			...
			<span class="link-btn">
				<a class="btn-clear no-decoration">
					${creqit.utils.icon("close", "xs", "es-icon")}
				</a>
				<a class="btn-open no-decoration" title="${__("Open Link")}">
					${creqit.utils.icon("arrow-right", "xs")}
				</a>
			</span>
		</div>`).prependTo(this.input_area);
		this.set_icon();//creqit.v1.sevval//
		...
		this.$input.on("focus", function () {
			setTimeout(function () {
				if (me.$input.val() && me.get_options()) {
					...
					me.$link.toggle(true);
					me.$link_open.attr("href", creqit.utils.get_form_link(doctype, name));
	...
	set_icon() {
		...
		// 		${creqit.utils.icon("select")}
		// 	</div>`;

		const search_icon_html = `<div class="search-icon">
				${creqit.utils.icon("search")}
			</div>`;
	...
	get_reference_doctype() {
		...
		if (this.doctype) return this.doctype;
			...
			return creqit.get_route && creqit.get_route()[0] === "List"
				? creqit.get_route()[1]
				: null;
		}
	}
	...
	is_translatable() {
		return (creqit.boot?.translated_doctypes || []).includes(this.get_options());
	}
	is_title_link() {
		return (creqit.boot?.link_title_doctypes || []).includes(this.get_options());
	}
	async set_link_title(value) {
		...
		const link_title =
			creqit.utils.get_link_title(doctype, value) ||
			(await creqit.utils.fetch_link_title(doctype, value));
	...
	parse_validate_and_set_in_model(value, e, label) {
		...
		if (label) {
			...
			creqit.utils.add_link_title(this.df.options, value, label);
		}
	...
	new_doc() {
		...
		if (df && df.get_route_options_for_new_doc) {
			creqit.route_options = df.get_route_options_for_new_doc(this);
		} else {
			creqit.route_options = {};
		}

		// partially entered name field
		creqit.route_options.name_field = this.get_label_value();

		// reference to calling link
		creqit._from_link = creqit.utils.deep_clone(this);
		creqit._from_link_scrollY = $(document).scrollTop();

		creqit.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete() {
		...
		this.awesomplete = new Awesomplete(me.input, {
			...
			item: function (item) {
				...
				return $(`<div role="option">`)
					...
					.html(`<p title="${creqit.utils.escape_html(_label)}">${html}</p>`)
					.get(0);
			},
			sort: function () {
				return 0;
		...
		this.$input.on(
			...
			creqit.utils.debounce(function (e) {
				...
				var args = {
					...
					page_length: cint(creqit.boot.sysdefaults?.link_field_results_limit) || 10,
				};
					...
					callback: function (r) {
						...
						if (!me.df.only_select) {
							...
							let custom__link_options =
								creqit.ui.form.ControlLink.link_options &&
								creqit.ui.form.ControlLink.link_options(me);
						...
						r.message.forEach((item) => {
							creqit.utils.add_link_title(doctype, item.value, item.label);
						});
					},
				});
			}, 500)
		...
		this.$input.on("awesomplete-select", function (e) {
			...
			if (me.df.remember_last_selected_value) {
				creqit.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	toggle_href(doctype) {
		if (creqit.model.can_select(doctype) && !creqit.model.can_read(doctype)) {
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let docfield = creqit.meta.get_docfield(doctype, fieldname);
			let label = docfield ? docfield.label : creqit.model.unscrub(fieldname);
	...
	set_custom_query(args) {
		const is_valid_value = (value, key) => {
			...
			if (this.frm) {
				let field = creqit.meta.get_docfield(this.frm.doctype, key);
	...
	parse_filters(link_filters) {
		...
		link_filters.forEach((filter) => {
			...
			if (value?.startsWith?.("eval:")) {
				...
				value = creqit.utils.eval(value, context);
			}
	...
	validate_link_and_fetch(value) {
		...
		if (value) {
			return creqit
				.xcall("creqit.client.validate_link", {
					doctype: options,
					docname: value,
