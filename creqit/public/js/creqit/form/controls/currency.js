creqit.ui.form.ControlCurrency = class ControlCurrency extends creqit.ui.form.ControlFloat {
	...
	get_precision() {
		...
		if (typeof this.df.precision != "number" && !this.df.precision) {
			if (creqit.boot.sysdefaults.currency_precision) {
				this.df.precision = creqit.boot.sysdefaults.currency_precision;
			} else {
