	...
	<div class="tree-node" :class="{ opened: node.open }">
		...
		>
			...
			<a
				...
				v-html="creqit.utils.icon('external-link', 'sm')"
				@click.stop
			/>
		</span>
		<div v-if="node.file_url && creqit.utils.is_image_file(node.file_url)">
...
let icon = computed(() => {
	let icons = {
		open: creqit.utils.icon("folder-open", "md"),
		closed: creqit.utils.icon("folder-normal", "md"),
		leaf: creqit.utils.icon("primitive-dot", "xs"),
		search: creqit.utils.icon("search"),
	};
...
let open_file = (filename) => {
	return creqit.utils.get_form_link("File", filename);
};
