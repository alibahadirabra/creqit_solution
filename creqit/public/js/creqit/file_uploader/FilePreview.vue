<template>
	<div class="file-preview">
		<div class="file-icon">
			...
			<div class="fallback" v-else v-html="creqit.utils.icon('file', 'md')"></div>
		</div>
		<div>
			<div>
		...
		<div class="file-actions">
			<ProgressRing
				...
				:stroke="3"
			/>
			<div v-if="uploaded" v-html="creqit.utils.icon('solid-success', 'lg')"></div>
			<div v-if="file.failed" v-html="creqit.utils.icon('solid-error', 'lg')"></div>
			<div class="file-action-buttons">
				<button
					v-if="is_cropable"
					class="btn btn-crop muted"
					@click="emit('toggle_image_cropper')"
					v-html="creqit.utils.icon('crop', 'md')"
				></button>
				<button
					v-if="!uploaded && !file.uploading && !file.failed"
					class="btn muted"
					@click="emit('remove')"
					v-html="creqit.utils.icon('delete', 'md')"
				></button>
			</div>
		</div>
	</div>
