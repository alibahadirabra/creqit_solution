...
creqit.last_edited_communication = {};
const separator_element = "<div>---</div>";

creqit.views.CommunicationComposer = class {
	constructor(opts) {
		$.extend(this, opts);
		if (!this.doc) {
	...
	get_fields() {
		...
		const fields = [
			...
			{
				fieldtype: "Button",
				label: creqit.utils.icon("down", "xs"),
			...
			},
			{
				...
				fieldname: "content",
				onchange: creqit.utils.debounce(this.save_as_draft.bind(this), 300),
			},
			{
				fieldtype: "Button",
			...
			{ fieldtype: "Section Break" },
			{
				...
				default: creqit.boot.user.send_me_a_copy,
			},
			{
		...
		const email_accounts = creqit.boot.email_accounts.filter((account) => {
			return (
		...
		if (email_accounts.length) {
			...
			} else if (this.user_email_accounts.includes(creqit.session.user_email)) {
				this["sender"] = creqit.session.user_email;
			}
		}
	...
	guess_language() {
		...
		if (print_format != "Standard") {
			print_format_lang = creqit.get_doc(
				"Print Format",
				print_format
			)?.default_print_language;
		}

		let lang = document_lang || print_format_lang || creqit.boot.lang;
	...
	toggle_more_options(show_options) {
		...
		const label = creqit.utils.icon(show_options ? "up-line" : "down", "xs");
	...
	setup_multiselect_queries() {
		["recipients", "cc", "bcc"].forEach((field) => {
			this.dialog.fields_dict[field].get_data = () => {
				...
				creqit.call({
					method: "creqit.email.get_contact_list",
					args: args,
					callback: (r) => {
	...
	setup_email_template() {
		...
		function add_template() {
			...
			creqit.call({
				method: "creqit.email.doctype.email_template.email_template.get_email_template",
				args: {
		...
		creqit.utils.add_select_group_button(clear_and_add_template, email_template_actions);
	}
	...
	get_last_edited_communication(clear) {
		if (!creqit.last_edited_communication[this.doctype]) {
			creqit.last_edited_communication[this.doctype] = {};
		}

		if (clear || !creqit.last_edited_communication[this.doctype][this.key]) {
			creqit.last_edited_communication[this.doctype][this.key] = {};
		}

		return creqit.last_edited_communication[this.doctype][this.key];
	}
	...
	async set_values() {
		...
		const subject = this.subject ? creqit.utils.html2text(this.subject) : "";
	...
	setup_print() {
		...
		if (this.frm) {
			const print_formats = creqit.meta.get_print_formats(this.frm.meta.name);
	...
	setup_attach() {
		...
		$(`
			...
			<p class='add-more-attachments'>
				<button class='btn btn-xs btn-default'>
					${creqit.utils.icon("small-add", "xs")}&nbsp;
					${__("Add Attachment")}
				</button>
			</p>
		...
		attach
			...
			.on("click", () => new creqit.ui.FileUploader(args));
			...
			if (files.length) {
				$.each(files, (i, f) => {
					...
					if (!attachment_rows.find(`[data-file-name="${f.name}"]`).length) {
						f.file_url = creqit.urllib.get_full_url(f.file_url);
	...
	setup_email() {
		...
		$(fields.send_me_a_copy.input).on("click", () => {
			...
			creqit.db.set_value("User", creqit.session.user, "send_me_a_copy", val);
			creqit.boot.user.send_me_a_copy = val;
		});
	}
	...
	send_email(btn, form_values, selected_attachments, print_html, print_format) {
		...
		if (!form_values.recipients) {
			creqit.msgprint(__("Enter Email Recipient(s)"));
			return;
		}
		...
		if (this.frm && !creqit.model.can_email(this.doc.doctype, this.frm)) {
			creqit.msgprint(__("You are not allowed to send emails related to this document"));
			return;
		}

		return creqit.call({
			method: "creqit.core.doctype.communication.email.make",
			args: {
				...
				sender_full_name: form_values.sender ? creqit.user.full_name() : undefined,
			...
			callback(r) {
				if (!r.exc) {
					creqit.utils.play_sound("email");

					if (r.message["emails_not_sent_to"]) {
						creqit.msgprint(
							__("Email not sent to {0} (unsubscribed / disabled)", [
								creqit.utils.escape_html(r.message["emails_not_sent_to"]),
							])
						);
					}
	...
	is_print_letterhead_checked() {
		if (this.frm && $(this.frm.wrapper).find(".form-print-wrapper").is(":visible")) {
			...
			return (
				creqit.model.get_doc(":Print Settings", "Print Settings") || { with_letterhead: 1 }
			).with_letterhead
				? 1
				: 0;
		}
	...
	async get_signature(sender_email) {
		let signature = creqit.boot.user.email_signature;
		...
		if (!signature) {
			...
			const email_accounts = await creqit.db.get_list("Email Account", {
				filters: filters,
		...
		if (!creqit.utils.is_html(signature)) {
	...
	get_earlier_reply() {
		...
		const communication_date = creqit.datetime.global_date_format(
	...
	html2text(html) {
		...
		const text = creqit.utils.html2text(html);
		return text.replace(/\n{3,}/g, "\n\n");
	}
};
