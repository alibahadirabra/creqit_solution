...
creqit.provide("creqit.interaction_settings");

creqit.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		creqit.model.with_doctype("Event", () => {
			let categories = creqit.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new creqit.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = creqit.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = creqit.session.user;
		}
		return creqit.call({
	...
	add_attachments(doc, attachments) {
		creqit.call({
			method: "creqit.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,
