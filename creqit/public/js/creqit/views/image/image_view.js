/**
 * creqit.views.ImageView
 */
creqit.provide("creqit.views");

creqit.views.ImageView = class ImageView extends creqit.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = creqit.utils.escape_html(title);
	...
	get_attached_images() {
		return creqit
			.call({
				method: "creqit.core.api.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new creqit.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
	...
	get required_libs() {
		return [
			"assets/creqit/node_modules/photoswipe/src/photoswipe.css",
			"photoswipe.bundle.js",
		];
	}
};

creqit.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
		me.prepare();
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = creqit.render_template("photoswipe_dom");
	...
	_show(docname) {
		...
		// init
		this.pswp = new creqit.PhotoSwipe(options);
		this.pswp.init();
	}
};
