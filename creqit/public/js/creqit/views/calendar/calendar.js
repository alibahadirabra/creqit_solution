...
creqit.provide("creqit.views.calendar");
creqit.provide("creqit.views.calendars");

creqit.views.CalendarView = class CalendarView extends creqit.views.ListView {
	static load_last_view() {
		const route = creqit.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = creqit.get_user_settings(doctype)["Calendar"] || {};
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.calendar_settings = creqit.views.Calendar[this.doctype] || {};
			this.calendar_name = creqit.get_route()[3];
		});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then((options) => {
				this.calendar = new creqit.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise((resolve) => {
			if (calendar_name === "default") {
				Object.assign(options, creqit.views.calendar[this.doctype]);
				resolve(options);
			} else {
				creqit.model.with_doc("Calendar View", calendar_name, () => {
					const doc = creqit.get_doc("Calendar View", calendar_name);
					if (!doc) {
						creqit.show_alert(
						...
						creqit.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
...
creqit.views.Calendar = class Calendar {
	constructor(options) {
		$.extend(this, options);
	...
	make_page() {
		...
		$.each(creqit.boot.calendars, function (i, doctype) {
			if (creqit.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function () {
					creqit.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make() {
		...
		this.footnote_area = creqit.utils.set_footnote(
			this.footnote_area,
			this.$wrapper,
		...
		this.fullCalendar = new creqit.FullCalendar(this.$cal[0], this.cal_options);
	...
	set_css() {
		...
		this.$wrapper
			...
			.attr("class", "")
			.html(creqit.utils.icon("left"));
		this.$wrapper
			.find(`.fc-next-button span`)
			.attr("class", "")
			.html(creqit.utils.icon("right"));
		if (this.$wrapper.find(".fc-today-button svg").length == 0)
			this.$wrapper.find(".fc-today-button").prepend(creqit.utils.icon("today"));
	...
	get_system_datetime(date) {
		return creqit.datetime.convert_to_system_tz(date, true);
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			plugins: creqit.FullCalendar.Plugins,
			initialView: defaults.initialView || "dayGridMonth",
			locale: creqit.boot.lang,
			firstDay: 1,
			headerToolbar: {
			...
			events: function (info, successCallback, failureCallback) {
				return creqit.call({
					method: me.get_events_method || "creqit.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function (info) {
				...
				if (creqit.model.can_read(doctype)) {
					creqit.set_route("Form", doctype, info.event.id);
				}
			},
			...
			select: function (info) {
				...
				var event = creqit.model.get_new_doc(me.doctype);
				...
				creqit.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events(events) {
		...
		return (events || []).map((d) => {
			d.id = d.name;
			d.editable = creqit.model.can_write(d.doctype || me.doctype);
			...
			if (d.convertToUserTz) {
				d.start = creqit.datetime.convert_to_user_tz(d.start);
				d.end = creqit.datetime.convert_to_user_tz(d.end);
			}

			// show event on single day if start or end date is invalid
			if (!creqit.datetime.validate(d.start) && d.end) {
				d.start = creqit.datetime.add_days(d.end, -1);
			}

			if (d.start && !creqit.datetime.validate(d.end)) {
				d.end = creqit.datetime.add_days(d.start, 1);
			}

			me.prepare_colors(d);

			d.title = creqit.utils.html2text(d.title);
	...
	prepare_colors(d) {
		...
		if (this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = creqit.ui.color.validate_hex(color_name) ? color_name : "blue";
			}

			d.backgroundColor = creqit.ui.color.get(color_name, "extra-light");
			d.textColor = creqit.ui.color.get(color_name, "dark");
		} else {
			color = d.color;
			if (!creqit.ui.color.validate_hex(color) || !color) {
				color = creqit.ui.color.get("blue", "extra-light");
			}
			d.backgroundColor = color;
			d.textColor = creqit.ui.color.get_contrast_color(color);
		}
		return d;
	}
	update_event(event, revertFunc) {
		var me = this;
		creqit.model.remove_from_locals(me.doctype, event.id);
		return creqit.call({
			method: me.update_event_method || "creqit.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function (r) {
				if (r.exc) {
					creqit.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function () {
