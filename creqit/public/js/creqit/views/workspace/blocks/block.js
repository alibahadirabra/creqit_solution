...
export default class Block {
	...
	make(block, block_name, widget_type = block) {
		let block_data = this.config.page_data[block + "s"].items.find((obj) => {
			return (
				creqit.utils.unescape_html(obj.label) == creqit.utils.unescape_html(__(block_name))
			);
		});
		if (!block_data) return false;
		this.wrapper.innerHTML = "";
		block_data.in_customize_mode = !this.readOnly;
		this.block_widget = new creqit.widget.SingleWidgetGroup({
	...
	new(block, widget_type = block) {
		...
		this.dialog = new dialog_class({
			...
			primary_action: (widget) => {
				...
				this.block_widget = creqit.widget.make_widget({
					...widget,
	...
	add_new_block_button() {
		let $new_button = $(`
			<div class="new-block-button">${creqit.utils.icon("add-round", "lg")}</div>
		`);
		...
		this.dropdown_list = [
			{
				label: "Delete",
				title: "Delete Block",
				icon: creqit.utils.icon("delete-active", "sm"),
				action: () => this.api.blocks.delete(),
			},
			{
				label: "Expand",
				title: "Expand Block",
				icon: creqit.utils.icon("expand-alt", "sm"),
				action: () => this.increase_width(),
			},
			{
				label: "Shrink",
				title: "Shrink Block",
				icon: creqit.utils.icon("shrink", "sm"),
				action: () => this.decrease_width(),
			},
			{
				label: "Move Up",
				title: "Move Up",
				icon: creqit.utils.icon("up-arrow", "sm"),
				action: () => this.move_block("up"),
			},
			{
				label: "Move Down",
				title: "Move Down",
				icon: creqit.utils.icon("down-arrow", "sm"),
		...
		let $button = $(`
			<div class="dropdown-btn">
				<button class="btn btn-secondary btn-xs setting-btn" title="${__("Setting")}">
					${creqit.utils.icon("dot-horizontal", "xs")}
				</button>
