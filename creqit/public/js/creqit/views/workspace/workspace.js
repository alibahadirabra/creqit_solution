...
creqit.standard_pages["Workspaces"] = function () {
	var wrapper = creqit.container.add_page("Workspaces");
	...
	creqit.workspace = new creqit.views.Workspace(wrapper);
	$(wrapper).bind("show", function () {
		creqit.workspace.show();
	});
};

creqit.views.Workspace = class Workspace {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = wrapper.page;
		this.blocks = creqit.workspace_block.blocks;
		...
		this.sidebar = creqit.app.sidebar;
		this.sidebar.setup_pages();
		this.cached_pages = $.extend(true, {}, creqit.boot.sidebar_pages);
		this.has_access = creqit.boot.sidebar_pages.has_access;
		this.has_create_access = creqit.boot.sidebar_pages.has_create_access;
		this.show();
	...
	get_pages() {
		return creqit.xcall("creqit.desk.desktop.get_workspace_sidebar_items", null, "GET");
	}
	...
	show() {
		...
		if (!creqit.router.current_route[0]) {
			creqit.route_flags.replace_route = true;
			creqit.set_route(creqit.router.slug(page.public ? page.name : "private/" + page.name));
			return;
		}
	...
	get_data(page) {
		return creqit
			.call({
				method: "creqit.desk.desktop.get_desktop_page",
				args: {
			...
			.then((data) => {
				...
				return creqit.dashboard_utils.get_dashboard_settings().then((settings) => {
					if (settings) {
	...
	get_page_to_show() {
		...
		if (creqit.boot.user.default_workspace) {
			default_page = {
				name: creqit.boot.user.default_workspace.name,
				public: creqit.boot.user.default_workspace.public,
			};
		} else if (
		...
		if (this.sidebar.all_pages.length) {
			...
			if (!this._page.public) {
				...
				if (!app && this._page.module) {
					app = creqit.boot.module_app[creqit.router.slug(this._page.module)];
				}
				if (!app) app = "creqit";
			}
			...
			if (this.pages && this.pages[current_page.name]) {
				...
				await creqit.after_ajax(() => this.get_data(current_page));
			}
			...
			creqit.app.sidebar.set_current_app(app);
			this.wrapper.find(".workspace-title").html(__(this._page.title));
			this.wrapper
				.find(".workspace-icon")
				.html(creqit.utils.icon(this._page.icon || "folder-normal", "md"));
	...
	setup_actions(page) {
		...
		//creqit.v1.sevval alt kısım yorum satırına alındı new buton üstteki bloga dahil edildi
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			await this.editor.readOnly.toggle();
			this.is_read_only = true;
			creqit.boot.sidebar_pages = this.cached_pages;
			this.reload();
			creqit.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
		});

		if (page.name && this.has_access) {
			this.page.add_inner_button(__("Settings"), () => {
				creqit.set_route(`workspace/${page.name}`);
			});
		}
	}
	...
	initialize_new_page() {
		...
		const d = new creqit.ui.Dialog({
			...
			primary_action: (values) => {
				...
				let name = values.title + (values.is_public ? "" : "-" + creqit.session.user);
				let blocks = [
					{
						type: "header",
				...
				let new_page = {
					...
					for_user: values.is_public ? "" : creqit.session.user,
					icon: values.icon,
					indicator_color: values.indicator_color,
					parent_page: values.parent || "",
					is_editable: true,
					selected: true,
					app: creqit.current_app,
					type: values.type,
					...
					this.editor
						...
						.then(async () => {
							...
							this.create_page(new_page).then(() => {
								...
								let route = pre_url + creqit.router.slug(new_page.title);
	...
	create_page(new_page) {
		return new Promise((resolve) => {
			creqit.call({
				method: "creqit.desk.doctype.workspace.workspace.new_page",
				args: {
					new_page: new_page,
				},
				callback: (r) => {
					if (r.message) {
						...
						creqit.boot.sidebar_pages = r.message;
						this.sidebar.setup_pages();

						if (!creqit.boot.app_data_map["private"] && new_page.public === 0) {
	...
	initialize_editorjs(blocks) {
		this.tools = {
			...
			HeaderSize: creqit.workspace_block.tunes["header_size"],
		};
	...
	save_page(page) {
		...
		return this.editor
			...
			.then((outputData) => {
				...
				page.content = JSON.stringify(blocks);
				creqit.call({
					method: "creqit.desk.doctype.workspace.workspace.save_page",
					args: {
						name: page.name,
	...
	create_page_skeleton() {
		...
		this.body.prepend(creqit.render_template("workspace_loading_skeleton"));
	...
	register_awesomebar_shortcut() {
		"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter) => {
			...
			creqit.ui.keys.add_shortcut({ shortcut: letter, ...default_shortcut });
			creqit.ui.keys.add_shortcut({ shortcut: `shift+${letter}`, ...default_shortcut });
		});
	}
};
