/**
 * creqit.views.InboxView
 */

creqit.provide("creqit.views");

creqit.views.InboxView = class InboxView extends creqit.views.ListView {
	static load_last_view() {
		const route = creqit.get_route();
		if (!route[3] && creqit.boot.email_accounts.length) {
			let email_account;
			if (creqit.boot.email_accounts[0].email_id == "All Accounts") {
				email_account = "All Accounts";
			} else {
				email_account = creqit.boot.email_accounts[0].email_account;
			}
			creqit.set_route("List", "Communication", "Inbox", email_account);
			return true;
		...
		function is_valid(email_account) {
			return creqit.boot.email_accounts.find((d) => d.email_account === email_account);
		}
	}
	...
	setup_defaults() {
		...
		this.email_account = creqit.get_route()[3];
	...
	get_meta_html(email) {
		...
		if (email.reference_doctype && email.reference_doctype !== this.doctype) {
			link = `<a class="text-muted grey"
				href="${creqit.utils.get_form_link(email.reference_doctype, email.reference_name)}"
	...
	get_inbox_filters() {
		...
		if (email_account === "Sent") {
			...
			filters = default_filters.concat([
				...
				["Communication", "email_account", "in", creqit.boot.all_accounts, true],
			]);
		} else {
			var op = "=";
			if (email_account == "All Accounts") {
				op = "in";
				email_account = creqit.boot.all_accounts;
			}
	...
	get_no_result_message() {
		...
		} else if (!email_account && !creqit.boot.email_accounts.length) {
		...
		const html = creqit.model.can_create(args.doctype)
			? `<p>${args.msg}</p>
			<p>
	...
	make_new_doc() {
		if (!this.email_account && !creqit.boot.email_accounts.length) {
			creqit.route_options = {
				email_id: creqit.session.user_email,
			};
			creqit.new_doc("Email Account");
		} else {
