...
creqit.provide("creqit.treeview_settings");
creqit.provide("creqit.views.trees");
window.cur_tree = null;

creqit.views.TreeFactory = class TreeFactory extends creqit.views.Factory {
	make(route) {
		creqit.model.with_doctype(route[1], function () {
			var options = {
				doctype: route[1],
				meta: creqit.get_meta(route[1]),
			};

			if (
				!creqit.treeview_settings[route[1]] &&
				!creqit.meta.get_docfield(route[1], "is_group")
			) {
				creqit.msgprint(__("Tree view is not available for {0}", [route[1]]));
				return false;
			}
			$.extend(options, creqit.treeview_settings[route[1]] || {});
			creqit.views.trees[options.doctype] = new creqit.views.TreeView(options);
		});
	}
	...
	on_show() {
		...
		let treeview = creqit.views.trees[route[1]];
...
creqit.views.TreeView = class TreeView {
	constructor(opts) {
		var me = this;
		...
		this.get_tree_nodes = me.opts.get_tree_nodes || "creqit.desk.treeview.get_children";
	...
	get_permissions() {
		this.can_read = creqit.model.can_read(this.doctype);
		this.can_create =
			creqit.boot.user.can_create.indexOf(this.doctype) !== -1 ||
			creqit.boot.user.in_create.indexOf(this.doctype) !== -1;
		this.can_write = creqit.model.can_write(this.doctype);
		this.can_delete = creqit.model.can_delete(this.doctype);
	}
	make_page() {
		var me = this;
		if (!this.opts || !this.opts.do_not_make_page) {
			this.parent = creqit.container.add_page(this.page_name);
			$(this.parent).addClass("treeview");
			creqit.ui.make_app_page({ parent: this.parent, single_column: true });
			this.page = this.parent.page;
			creqit.container.change_to(this.page_name);
			creqit.breadcrumbs.add(
			...
			this.page.main.addClass("creqit-card");
		} else {
			this.page = this.opts.page;
			$(this.page[0]).addClass("creqit-card");
		}

		if (creqit.meta.has_field(me.doctype, "disabled")) {
			$(
	...
	make_filters() {
		var me = this;
		creqit.treeview_settings.filters = [];
		$.each(this.opts.filters || [], function (i, filter) {
			if (creqit.route_options && creqit.route_options[filter.fieldname]) {
				filter.default = creqit.route_options[filter.fieldname];
			}
	...
	rebuild_tree() {
		...
		creqit.call({
			method: "creqit.utils.nestedset.rebuild_tree",
			args: {
				doctype: me.doctype,
			},
			callback: function (r) {
	...
	select_node(node) {
		...
		if (this.opts.view_template) {
			...
			$(
				creqit.render_template(me.opts.view_template, {
					data: node.data,
					doctype: me.doctype,
				})
		...
		var toolbar = [
			{
				...
				click: function (node) {
					creqit.set_route("Form", me.doctype, node.label);
				},
			},
			{
				label: __("Add Child"),
			...
			},
			{
				...
				condition: function (node) {
					...
					if (me.doctype && creqit.get_meta(me.doctype)) {
						if (!creqit.get_meta(me.doctype).allow_rename) allow_rename = false;
					}
					return !node.is_root && me.can_write && allow_rename;
				},
				click: function (node) {
					creqit.model.rename_doc(me.doctype, node.label, function (new_name) {
			...
			},
			{
				...
				click: function (node) {
					creqit.model.delete_doc(me.doctype, node.label, function () {
						node.parent.remove();
					});
				},
				btnClass: "hidden-xs",
	...
	new_node() {
		...
		if (!(node && node.expandable)) {
			creqit.msgprint(__("Select a group node first."));
			return;
		}
		...
		d.set_primary_action(__("Create New"), function () {
			...
			d.hide();
			creqit.dom.freeze(__("Creating {0}", [me.doctype]));

			$.extend(args, v);
			return creqit.call({
				method: me.opts.add_tree_node || "creqit.desk.treeview.add_node",
				args: args,
				callback: function (r) {
					if (!r.exc) {
	...
	print_tree() {
		if (!creqit.model.can_print(this.doctype)) {
			creqit.msgprint(__("You are not allowed to print this report"));
			return false;
		}
		var tree = $(".tree:visible").html();
		var me = this;
		creqit.ui.get_print_settings(false, function (print_settings) {
			var title = __(me.docname || me.doctype);
			creqit.render_tree({ title: title, tree: tree, print_settings: print_settings });
			creqit.call({
				method: "creqit.core.doctype.access_log.access_log.make_access_log",
				args: {
					doctype: me.doctype,
	...
	set_menu_item() {
		...
		this.menu_items = [
			{
				label: __("View List"),
				action: function () {
					creqit.set_route("List", me.doctype);
				},
			},
			{
				label: __("Print"),
		...
		if (
			...
			creqit.meta.has_field(me.doctype, "lft") &&
			creqit.meta.has_field(me.doctype, "rgt")
		) {
