...
creqit.views.FileView = class FileView extends creqit.views.ListView {
	static load_last_view() {
		const route = creqit.get_route();
		if (route.length === 2) {
			const view_user_settings = creqit.get_user_settings("File", "File");
			creqit.set_route(
				"List",
				"File",
				view_user_settings.last_folder || creqit.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			creqit.views.FileView.grid_view = !creqit.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (creqit.views.FileView.grid_view) {
			Object.assign(args, {
		...
		return [
			{
				label: __("Home"),
				action: () => {
					creqit.set_route("List", "File", "Home");
				},
			},
			{
				label: __("New Folder"),
					...
					creqit.prompt(
						...
						(values) => {
							if (values.value.indexOf("/") > -1) {
								creqit.throw(__("Folder name should not include '/' (slash)"));
							}
							const data = {
							...
							creqit.call({
								method: "creqit.core.api.file.create_new_folder",
								args: data,
							});
						},
			...
			},
			{
				...
				action: () => {
					new creqit.ui.FileUploader({
						...
						on_success: (file) => {
							creqit.show_alert(__("Unzipping files..."));
							creqit
								.call("creqit.core.api.file.unzip_file", {
									name: file.name,
								})
								.then((r) => {
									if (r.message) {
										creqit.show_alert(__("Unzipped {0} files", [r.message]));
									}
								});
						},
					});
	...
	add_file_action_buttons() {
		this.$cut_button = this.page
			.add_button(__("Cut"), () => {
				creqit.file_manager.cut(this.get_checked_items(), this.current_folder);
				this.$checks.parents(".file-wrapper").addClass("cut");
			})
			.hide();

		this.$paste_btn = this.page
			.add_button(__("Paste"), () => creqit.file_manager.paste(this.current_folder))
			.hide();

		this.page.add_actions_menu_item(__("Export as zip"), () => {
			let docnames = this.get_checked_items(true);
			if (docnames.length) {
				open_url_post("/api/method/creqit.core.api.file.zip_files", {
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter((df) => creqit.model.is_value_type(df.fieldtype) && !df.hidden)
			.map((df) => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (creqit.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
	...
	before_render() {
		super.before_render();
		creqit.model.user_settings.save("File", "grid_view", creqit.views.FileView.grid_view);
	...
	get_header_html() {
		...
		let header_selector_html = !creqit.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
					"Select All"
			  )}">`
			: "";

		let header_columns_html = !creqit.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === creqit.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = creqit.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size ? creqit.form.formatters.FileSize(file.file_size) : "";
		...
		return `
			<div class="list-row-col ellipsis list-subject level">
				...
				<span class="level-item  ellipsis" title="${creqit.utils.escape_html(file.file_name)}">
					<a class="ellipsis" href="${route_url}" title="${creqit.utils.escape_html(file.file_name)}">
						${file.subject_html}
					</a>
				</span>
			</div>
		...
		this.$result.on("drop", (e) => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					creqit.file_manager.paste(folder_name);
					creqit.show_alert(`File ${file_name} moved to ${folder_name}`);
				}
			}
		});
	}
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!creqit.file_manager.can_paste ||
			creqit.file_manager.old_folder === this.current_folder;
...
creqit.views.FileView.grid_view = creqit.get_user_settings("File").grid_view || false;
