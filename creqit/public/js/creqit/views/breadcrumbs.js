...
creqit.breadcrumbs = {
	...
	add(module, doctype, type) {
		...
		this.all[creqit.breadcrumbs.current_page()] = obj;
		this.update();
	},

	current_page() {
		return creqit.get_route_str();
	},

	update() {
		var breadcrumbs = this.all[creqit.breadcrumbs.current_page()];
		...
		if (
			...
			creqit.workspace_map[breadcrumbs.workspace]?.app &&
			creqit.workspace_map[breadcrumbs.workspace]?.app != creqit.current_app
		) {
			creqit.app.sidebar.set_current_app(creqit.workspace_map[breadcrumbs.workspace].app);
		}
	...
	get last_route() {
		return creqit.route_history.slice(-2)[0];
	},
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (
			...
			(breadcrumbs.module_info.blocked ||
				!creqit.visible_modules.includes(breadcrumbs.module_info.module))
		) {
			return;
		}

		this.append_breadcrumb_element(
			`/app/${creqit.router.slug(breadcrumbs.workspace)}`,
	...
	set_workspace(breadcrumbs) {
		...
		if (this.last_route?.[0] == "Workspaces") {
			...
			if (
				breadcrumbs.module &&
				creqit.boot.module_wise_workspaces[breadcrumbs.module]?.includes(last_workspace)
			) {
				breadcrumbs.workspace = last_workspace;
			}
		} else {
			// choose from __workspaces
			const doctype_meta = creqit.get_meta(breadcrumbs.doctype);
			...
			if (breadcrumbs.module) {
				...
				breadcrumbs.module_info = creqit.get_module(breadcrumbs.module);

				// set workspace
				if (
					breadcrumbs.module_info &&
					creqit.boot.module_wise_workspaces[breadcrumbs.module]
				) {
					breadcrumbs.workspace =
						creqit.boot.module_wise_workspaces[breadcrumbs.module][0];
				}
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = creqit.get_meta(doctype);
		if (
			(doctype === "User" && !creqit.user.has_role("System Manager")) ||
			doctype_meta?.issingle
		) {
			// no user listview for non-system managers and single doctypes
		} else {
			let route;
			const doctype_route = creqit.router.slug(creqit.router.doctype_layout || doctype);
			if (doctype_meta?.is_tree) {
				let view = creqit.model.user_settings[doctype].last_view || "Tree";
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		let docname = creqit.get_route().slice(2).join("/");
		let docname_title;
		...
		let form_route = `/app/${creqit.router.slug(doctype)}/${encodeURIComponent(docname)}`;
		...
		if (view === "form") {
			...
			last_crumb.click((event) => {
				...
				creqit.utils.copy_to_clipboard(last_crumb.text());
			});
		}
	},
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		const doctype = breadcrumbs.doctype;
		const docname = creqit.get_route()[1];
		let dashboard_route = `/app/${creqit.router.slug(doctype)}/${docname}`;
		$(`<li><a href="${dashboard_route}">${__(docname)}</a></li>`).appendTo(this.$breadcrumbs);
	},

	setup_modules() {
		if (!creqit.visible_modules) {
			creqit.visible_modules = $.map(creqit.boot.allowed_workspaces, (m) => {
				return m.module;
			});
		}
	},
	...
	rename(doctype, old_name, new_name) {
		...
		delete creqit.breadcrumbs.all[old_route_str];
		this.update();
	},
