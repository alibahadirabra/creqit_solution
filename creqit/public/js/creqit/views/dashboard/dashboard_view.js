...
creqit.views.DashboardView = class DashboardView extends creqit.views.ListView {
	get view_name() {
		return "Dashboard";
	}
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.dashboard_settings =
				creqit.get_user_settings(this.doctype)["dashboard_settings"] || null;
		});
	}
	...
	save_dashboard_customization() {
		...
		creqit.model.user_settings.save(
			this.doctype,
			"dashboard_settings",
			this.dashboard_settings
		);
	...
	reset_dashboard_customization() {
		creqit.confirm(__("Are you sure you want to reset all customizations?"), () => {
			this.dashboard_settings = null;
			creqit.model.user_settings
		...
		const dialog = new creqit.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					get_query: () => {
						return {
							query: "creqit.desk.doctype.dashboard_chart.dashboard_chart.get_charts_for_user",
							filters: {
								document_type: this.doctype,
							},
						};
			...
			primary_action: (values) => {
				...
				if (chart.new_or_existing == "New Chart") {
					...
					creqit
						.xcall(
							"creqit.desk.doctype.dashboard_chart.dashboard_chart.create_dashboard_chart",
							{ args: chart }
						)
						.then((doc) => {
	...
	get_field_options() {
		...
		creqit.get_meta(this.doctype).fields.map((df) => {
