...
creqit.tags.utils = {
	get_tags: function (txt) {
		...
		if (!creqit.tags.tags) {
			creqit.tags.utils.fetch_tags();
			return [];
		}

		creqit.tags.tags.forEach((tag) => {
			const search_result = creqit.search.utils.fuzzy_search(txt, tag, true);
			if (search_result.score) {
				out.push({
					...
					onclick() {
						...
						creqit.searchdialog.search.init_search("#".concat(tag), "tags");
					},
				});
			}
		});
	...
	fetch_tags() {
		creqit.call({
			method: "creqit.desk.doctype.tag.tag.get_tags_list_for_awesomebar",
			callback: function (r) {
				if (r && r.message) {
					creqit.tags.tags = $.extend([], r.message);
				}
			},
		});
	},
	...
	get_tag_results: function (tag) {
		...
		return new Promise(function (resolve) {
			creqit.call({
				method: "creqit.desk.doctype.tag.tag.get_documents_for_tag",
				args: {
					tag: tag,
				},
				callback: function (r) {
