// creqit.ui.Capture
// Author - Achilles Rasquinha <achilles@creqit.io>
 ...
 *
 * @example
 * creqit._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
creqit._.get_data_uri = (element) => {
...
/**
 * @description creqit's Capture object.
 *
 * @example
 * const capture = creqit.ui.Capture()
 * capture.show()
 *
 ...
 */
creqit.ui.Capture = class {
	constructor(options = {}) {
		this.options = creqit.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...creqit.ui.Capture.OPTIONS, ...options };
	...
	show() {
		...
		if (cint(creqit.boot.sysdefaults.force_web_capture_mode_for_uploads)) {
			this.show_for_desktop();
		} else if (creqit.is_mobile()) {
	...
	build_dialog() {
		...
		me.$template = $(creqit.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch((err) => {
				if (me.options.error) {
					creqit.show_alert(creqit.ui.Capture.ERR_MESSAGE, 3);
				}
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__("Take Photo"), () => {
			const data_url = creqit._.get_data_uri(me.video);
...
creqit.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false,
};
creqit.ui.Capture.ERR_MESSAGE = __("Unable to load camera.");
creqit.ui.Capture.TEMPLATE = `
<div class="creqit-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${creqit.ui.Capture.ERR_MESSAGE}</video>
	</div>
