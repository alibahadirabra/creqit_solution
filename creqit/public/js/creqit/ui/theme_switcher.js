...
creqit.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
	...
	bind_events() {
		this.dialog.$wrapper.on("keydown", (e) => {
			...
			const key = creqit.ui.keys.get_key(e);
			let increment_by;
	...
	toggle_theme(theme) {
		...
		creqit.show_alert(__("Theme Changed"), 3);

		creqit.xcall("creqit.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme),
		});
	}
...
creqit.ui.add_system_theme_switch_listener = () => {
	creqit.ui.dark_theme_media_query.addEventListener("change", () => {
		creqit.ui.set_theme();
	});
};

creqit.ui.dark_theme_media_query = window.matchMedia("(prefers-color-scheme: dark)");

creqit.ui.set_theme = (theme) => {
	const root = document.documentElement;
	let theme_mode = root.getAttribute("data-theme-mode");
	if (!theme) {
		if (theme_mode === "automatic") {
			theme = creqit.ui.dark_theme_media_query.matches ? "dark" : "light";
		}
	}
