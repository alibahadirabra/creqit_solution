...
creqit.ui.AddressAutocompleteDialog = class AddressAutocompleteDialog {
	constructor(opts) {
	...
	_get_dialog() {
		...
		const dialog = new creqit.ui.Dialog({
			...
			fields: [
				{
					...
					reqd: 1,
					get_query:
						"creqit.integrations.doctype.geolocation_settings.geolocation_settings.autocomplete",
					onchange: () => {
						// Disable "Create Address" button if mandatory fields are missing
						creqit.model.with_doctype("Address", () => {
							const address = this.parse_selected_value();
							const mandatory_fields = creqit
								.get_meta("Address")
								.fields.filter(
									(field) =>
										field.reqd &&
			...
			primary_action: () => {
				...
				creqit.db.insert(address).then((doc) => {
			...
			secondary_action: () => {
				...
				const address = this.parse_selected_value();
				creqit.new_doc("Address", address);
			},
		});
