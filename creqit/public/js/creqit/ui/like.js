...
creqit.ui.is_liked = function (doc) {
	return creqit.ui.get_liked_by(doc).includes(creqit.session.user);
};

creqit.ui.get_liked_by = function (doc) {
	return doc._liked_by ? JSON.parse(doc._liked_by) : [];
};

creqit.ui.toggle_like = function ($btn, doctype, name, callback) {
	const add = $btn.hasClass("not-liked") ? "Yes" : "No";
	// disable click
	$btn.css("pointer-events", "none");

	creqit.call({
		method: "creqit.desk.like.toggle_like",
		quiet: true,
		args: {
			doctype: doctype,
			name: name,
		...
		callback: function (r) {
			...
			if (doc) {
				let liked_by = creqit.ui.get_liked_by(doc);

				if (add === "Yes" && !liked_by.includes(creqit.session.user)) {
					liked_by.push(creqit.session.user);
				}

				if (add === "No" && liked_by.includes(creqit.session.user)) {
					liked_by = liked_by.filter((user) => user !== creqit.session.user);
				}
...
creqit.ui.click_toggle_like = function () {
	console.warn("`creqit.ui.click_toggle_like` is deprecated and has no effect.");
};

creqit.ui.setup_like_popover = ($parent, selector) => {
	if (creqit.dom.is_touchscreen()) {
		return;
	}
			...
			content: () => {
				...
				liked_by.forEach((user) => {
					...
					liked_by_list.append(`
						<li data-user=${user}>${creqit.avatar(user, "avatar-xs")}
							<span>${creqit.user.full_name(user)}</span>
						</li>
					`);
				});
				...
				liked_by_list.children("li").click((ev) => {
					...
					creqit.set_route(link_base + user);
				});
