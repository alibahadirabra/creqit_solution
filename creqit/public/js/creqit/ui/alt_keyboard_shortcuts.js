creqit.provide("creqit.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
creqit.ui.keys.shortcut_groups = shortcut_groups;

creqit.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new creqit.ui.keys.AltShortcutGroup());
	}
...
creqit.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (creqit.ui.keys.key_map[e.which] || "").toLowerCase();
...
creqit.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = creqit.ui.keys.standard_shortcuts
			.filter((s) => !s.page)
