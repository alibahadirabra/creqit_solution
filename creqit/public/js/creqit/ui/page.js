 ...
 *
 * @returns {creqit.ui.Page}
 */

/**
 * @typedef {Object} creqit.ui.Page
 */

creqit.ui.make_app_page = function (opts) {
	opts.parent.page = new creqit.ui.Page(opts);
	return opts.parent.page;
};

creqit.ui.pages = {};

creqit.ui.Page = class Page {
	constructor(opts) {
		$.extend(this, opts);
		...
		this.make();
		creqit.ui.pages[creqit.get_route_str()] = this;
	}
	...
	load_lib(callback) {
		creqit.require(this.required_libs, callback);
	}

	add_main_section() {
		$(creqit.render_template("page", {})).appendTo(this.wrapper);
	...
	setup_sidebar_toggle() {
		...
		if (this.disable_sidebar_toggle || !sidebar_wrapper.length) {
			...
			sidebar_toggle.click(() => {
				if (creqit.utils.is_xs() || creqit.utils.is_sm()) {
	...
	add_action_icon(icon, click, css_class = "", tooltip_label) {
		...
		if (!tooltip_label) {
			...
			tooltip_label = creqit.unscrub(icon);
		}
	...
	set_action(btn, opts) {
		...
		creqit.ui.keys.get_shortcut_group(this).add(btn, text_span.length ? text_span : btn);
	}
	...
	add_dropdown_item({
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${creqit.utils.icon(icon)}</span>`;
		}
		...
		if (shortcut) {
			...
			`);
			creqit.ui.keys.add_shortcut(shortcut_obj);
		} else {
			$li = $(`
				<li>
					<a class="grey-link dropdown-item" href="#" onClick="return false;">
		...
		if (standard) {
			...
			if (!this.divider.length) {
				this.divider = $('<li class="dropdown-divider user-action visible-xs"></li>').prependTo(
					parent
				);
			}
	...
	set_title(title, icon = null, strip = true, tab_title = "", tooltip_label = "") {
		...
		this.title = title;
		creqit.utils.set_title(tab_title || title);
		if (icon) {
			title = `${creqit.utils.icon(icon)} ${title}`;
		}
	...
	add_button(label, click, opts) {
		...
		let button = $(`<button
			class="btn ${opts.btn_class || "btn-default"} ${opts.btn_size || "btn-sm"} ellipsis">
				${opts.icon ? creqit.utils.icon(opts.icon) : ""}
				${label}
		</button>`);
	...
	add_dropdown_button(parent, label, click, icon) {
		creqit.ui.toolbar.add_dropdown_button(parent, label, click, icon);
	}
	...
	add_field(df, parent) {
		...
		var f = creqit.ui.form.make_control({
			df: df,
