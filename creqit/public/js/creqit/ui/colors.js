...
creqit.ui.color = {
	get: function (color_name, shade) {
		...
		return creqit.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(creqit.ui.color_map);
		if (color_names.includes(color_name)) {
			return creqit.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(creqit.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(creqit.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in creqit.ui.color_map) {
			const colors = creqit.ui.color_map[key];
...
creqit.ui.color_map = creqit.ui.color.get_color_map();
