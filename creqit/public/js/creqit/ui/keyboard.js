...
creqit.provide("creqit.ui.keys.handlers");

creqit.ui.keys.setup = function () {
	$(window).on("keydown", function (e) {
		var key = creqit.ui.keys.get_key(e);
		if (creqit.ui.keys.handlers[key]) {
			var out = null;
			for (var i = 0, l = creqit.ui.keys.handlers[key].length; i < l; i++) {
				var handler = creqit.ui.keys.handlers[key][i];
...
creqit.ui.keys.standard_shortcuts = standard_shortcuts;
creqit.ui.keys.add_shortcut = ({
	...
	creqit.ui.keys.off(shortcut, page);
	// attach new handler
	creqit.ui.keys.on(shortcut, handler);
...
creqit.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (creqit.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map((shortcut) => {
				let shortcut_label = shortcut.shortcut
					.split("+")
					.map(creqit.utils.to_title_case)
					.join("+");
				if (creqit.utils.is_mac()) {
	...
	let dialog = new creqit.ui.Dialog({
		...
		on_hide() {
			creqit.ui.keys.is_dialog_shown = false;
		},
	});
	...
	dialog.show();
	creqit.ui.keys.is_dialog_shown = true;
};

creqit.ui.keys.get_key = function (e) {
	var keycode = e.keyCode || e.which;
	var key = creqit.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
creqit.ui.keys.on = function (key, handler) {
	if (!creqit.ui.keys.handlers[key]) {
		creqit.ui.keys.handlers[key] = [];
	}
	creqit.ui.keys.handlers[key].push(handler);
};

creqit.ui.keys.off = function (key, page) {
	let handlers = creqit.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	creqit.ui.keys.handlers[key] = handlers.filter((h) => {
...
creqit.ui.keys.add_shortcut({
	...
	action: function (e) {
		...
		creqit.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
creqit.ui.keys.on("escape", function (e) {
	handle_escape_key();
});

creqit.ui.keys.on("esc", function (e) {
	handle_escape_key();
});

creqit.ui.keys.on("enter", function (e) {
	if (window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger("click");
	}
});

creqit.ui.keys.on("ctrl+down", function (e) {
	const grid_row = creqit.ui.form.get_open_grid_form();
...
creqit.ui.keys.on("ctrl+up", function (e) {
	const grid_row = creqit.ui.form.get_open_grid_form();
...
"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter, i) => {
	creqit.ui.keys.key_map[65 + i] = letter;
});
