...
creqit.provide("creqit.web_form_list");

export default class WebFormList {
	constructor(opts) {
		Object.assign(this, opts);
		creqit.web_form_list = this;
	...
	make_filters() {
		...
		creqit
			.call("creqit.website.doctype.web_form.web_form.get_web_form_filters", {
			...
			.then((response) => {
				...
				fields.forEach((field) => {
					...
					let input = creqit.ui.form.make_control({
						df: {
	...
	fetch_data() {
		if (this.condition_json && JSON.parse(this.condition_json)) {
			let filter = creqit.utils.get_filter_from_json(this.condition_json, this.doctype);
			filter = creqit.utils.get_filter_as_json(filter);
			this.filters = Object.assign(this.filters, JSON.parse(filter));
		}

		let args = {
			method: "creqit.www.list.get_list_data",
			args: {
	...
	more() {
		...
		this.fetch_data().then((res) => {
			if (res.message.length === 0) {
				creqit.msgprint(__("No more items to display"));
			}
	...
	make_table_body() {
		if (this.data.length) {
			...
			this.wrapper.empty();
			creqit.has_permission(this.doctype, "", "create", () => {
	...
	append_rows(row_data) {
		...
		row_data.forEach((data_item) => {
			...
			let row = new creqit.ui.WebFormListRow({
				row: $row_element,
				doc: data_item,
	...
	make_actions() {
		...
		creqit.has_permission(this.doctype, "", "delete", () => {
	...
	delete_rows() {
		...
		creqit
			.call({
				type: "POST",
				method: "creqit.website.doctype.web_form.web_form.delete_multiple",
				args: {
					web_form_name: this.web_form_name,
...
creqit.ui.WebFormListRow = class WebFormListRow {
	...
	make_row() {
		...
		this.columns.forEach((field) => {
			let formatter = creqit.form.get_formatter(field.fieldtype);
			let value =
