...
creqit.ready(function () {
	let web_form_doc = creqit.web_form_doc;
	let reference_doc = creqit.reference_doc;
	...
	function show_login_prompt() {
		if (creqit.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new creqit.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(creqit.form.formatters.FileSize($(this).text()));
		});
	}
