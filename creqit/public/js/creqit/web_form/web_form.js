...
creqit.provide("creqit.web_form");
import EventEmitterMixin from "../../creqit/event_emitter";

export default class WebForm extends creqit.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		creqit.web_form = this;
		creqit.web_form.events = {};
		Object.assign(creqit.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		creqit.init_client_script && creqit.init_client_script();
		this.setup_listeners();
		creqit.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = creqit.utils.get_query_params();
		delete values.new;
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${creqit.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _page.find(".creqit-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		creqit.form_dirty = false;

		creqit.call({
			type: "POST",
			method: "creqit.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					creqit.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						creqit.call({
							type: "POST",
							method: "creqit.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			creqit.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {
