...
creqit.provide("creqit.tools");

creqit.tools.downloadify = function (data, roles, title) {
	if (roles && roles.length && !has_common(roles, roles)) {
		creqit.msgprint(
			__("Export not allowed. You need {0} role to export.", [creqit.utils.comma_or(roles)])
		);
		return;
	}

	var filename = title + ".csv";
	var csv_data = creqit.tools.to_csv(data);
...
creqit.markdown = function (txt) {
	if (!creqit.md2html) {
		creqit.md2html = new showdown.Converter({ tables: true });
	}
	...
	return creqit.md2html.makeHtml(txt);
};

creqit.tools.to_csv = function (data) {
	var res = [];
