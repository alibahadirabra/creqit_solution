...
creqit.re_route = { "#login": "" };
creqit.route_titles = {};
creqit.route_flags = {};
creqit.route_history = [];
creqit.view_factory = {};
creqit.view_factories = [];
creqit.route_options = null;
creqit.open_in_new_tab = false;
creqit.route_hooks = {};
...
creqit.router = {
	...
	setup() {
		...
		for (let doctype of creqit.boot.user.can_read) {
			this.routes[this.slug(doctype)] = { doctype: doctype };
		}
		if (creqit.boot.doctype_layouts) {
			for (let doctype_layout of creqit.boot.doctype_layouts) {
	...
	async route() {
		...
		if (!creqit.app) return;

		let sub_path = this.get_sub_path();
		if (creqit.boot.setup_complete) {
			!creqit.re_route["setup-wizard"] && (creqit.re_route["setup-wizard"] = "app");
		} else if (!sub_path.startsWith("setup-wizard")) {
			creqit.re_route["setup-wizard"] && delete creqit.re_route["setup-wizard"];
			creqit.set_route(["setup-wizard"]);
		}
	...
	async convert_to_standard_route(route) {
		...
		if (creqit.workspaces[route[0]]) {
			// public workspace
			route = ["Workspaces", creqit.workspaces[route[0]].name];
		} else if (route[0] == "private") {
			// private workspace
			let private_workspace = route[1] && `${route[1]}-${creqit.user.name.toLowerCase()}`;
			if (!creqit.workspaces[private_workspace]) {
				creqit.msgprint(__("Workspace <b>{0}</b> does not exist", [route[1]]));
				return ["Workspaces"];
			}
			route = ["Workspaces", "private", creqit.workspaces[private_workspace].name];
	...
	set_doctype_route(route) {
		...
		return creqit.model.with_doctype(doctype_route.doctype).then(() => {
			// doctype route
			let meta = creqit.get_meta(doctype_route.doctype);
			...
			} else if (creqit.model.is_single(doctype_route.doctype)) {
	...
	set_history() {
		creqit.route_history.push(this.current_route);
		creqit.ui.hide_open_dialog();
	},
	...
	render_page() {
		...
		const factory = creqit.utils.to_title_case(route[0]);

		if (route[1] && creqit.views[factory + "Factory"]) {
			route[0] = factory;
			// has a view generator, generate!
			if (!creqit.view_factory[factory]) {
				creqit.view_factory[factory] = new creqit.views[factory + "Factory"]();
			}

			creqit.view_factory[factory].show();
		} else {
			// show page
			const route_name = creqit.utils.xss_sanitise(route[0]);
			if (creqit.views.pageview) {
				creqit.views.pageview.show(route_name);
			}
		}
	},

	re_route(sub_path) {
		if (creqit.re_route[sub_path] !== undefined) {
			...
			const re_route_val = this.get_sub_path(creqit.re_route[sub_path]);
			if (re_route_val === this.current_sub_path) {
				window.history.back();
			} else {
				creqit.set_route(re_route_val);
			}
	...
	set_title(sub_path) {
		if (creqit.route_titles[sub_path]) {
			creqit.utils.set_title(creqit.route_titles[sub_path]);
		}
	},

	set_route() {
		// set the route (push state) with given arguments
		// example 1: creqit.set_route('a', 'b', 'c');
		// example 2: creqit.set_route(['a', 'b', 'c']);
		// example 3: creqit.set_route('a/b/c');
		...
		return new Promise((resolve) => {
			...
			sub_path += creqit.route_hash || "";
			creqit.route_hash = null;
			if (creqit.open_in_new_tab) {
				localStorage["route_options"] = JSON.stringify(creqit.route_options);
				window.open(sub_path, "_blank");
				creqit.open_in_new_tab = false;
			} else {
				try {
					const route_options = creqit.route_options || {};
	...
	make_url(params) {
		...
		let private_home = `home-${creqit.user.name.toLowerCase()}`;
		let default_workspace = creqit.router.slug(creqit.boot.user.default_workspace?.name || "");

		let workspace =
			creqit.workspaces[default_workspace] ||
			creqit.workspaces[private_home] ||
			creqit.workspaces["home"] ||
			Object.values(creqit.workspace_map).find((w) => w.app === creqit.current_app) ||
			Object.values(creqit.workspaces)[0];

		if (workspace) {
			return (
				"/app/" + (workspace.public ? "" : "private/") + creqit.router.slug(workspace.name)
			);
		}
	...
	push_state(path, query_params = "") {
		if (window.location.pathname !== path || window.location.search !== query_params) {
			...
			const method = creqit.route_flags.replace_route ? "replaceState" : "pushState";
	...
	set_route_options_from_url() {
		// set query parameters as creqit.route_options
		let query_string = window.location.search;

		if (!creqit.route_options) {
			creqit.route_options = {};
		}

		if (localStorage.getItem("route_options")) {
			creqit.route_options = JSON.parse(localStorage.getItem("route_options"));
...
creqit.get_route = () => creqit.router.current_route;
creqit.get_route_str = () => creqit.router.current_route.join("/");
creqit.set_route = function () {
	return creqit.router.set_route.apply(creqit.router, arguments);
};

creqit.get_prev_route = function () {
	if (creqit.route_history && creqit.route_history.length > 1) {
		return creqit.route_history[creqit.route_history.length - 2];
	} else {
		return [];
	}
};

creqit.set_re_route = function () {
	var tmp = creqit.router.get_sub_path();
	creqit.set_route.apply(null, arguments);
	creqit.re_route[tmp] = creqit.router.get_sub_path();
};

creqit.has_route_options = function () {
	return Boolean(Object.keys(creqit.route_options || {}).length);
};

creqit.utils.make_event_emitter(creqit.router);
