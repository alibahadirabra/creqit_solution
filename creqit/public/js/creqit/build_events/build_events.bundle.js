...
creqit.realtime.on("build_event", (data) => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						creqit.assets._executed = creqit.assets._executed.filter(
		...
		// update assets json
		creqit.call("creqit.sessions.get_boot_assets_json").then((r) => {
			if (r.message) {
				creqit.boot.assets_json = r.message;

				if (creqit.hot_update) {
					creqit.hot_update.forEach((callback) => {
						callback();
					});
				}
			}
