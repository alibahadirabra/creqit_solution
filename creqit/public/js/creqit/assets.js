...
// depends on creqit.versions to manage versioning

creqit.require = function (items, callback) {
	if (typeof items === "string") {
		items = [items];
	}
	items = items.map((item) => creqit.assets.bundled_asset(item));

	return new Promise((resolve) => {
		creqit.assets.execute(items, () => {
			resolve();
			callback && callback();
		});
	});
...
class AssetManager {
	...
	init_local_storage() {
		...
		if (creqit.boot) localStorage.metadata_version = creqit.boot.metadata_version;
	}
	...
	execute(items, callback) {
		...
		const version_string =
			creqit.boot.developer_mode || window.dev_server ? Date.now() : window._version_number;
...
creqit.assets = new AssetManager();
