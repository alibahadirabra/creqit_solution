creqit.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new creqit.Driver({
			className: "creqit-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				creqit.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!creqit.boot.user.onboarding_status[this.tour.name] &&
								(creqit.boot.user.onboarding_status[this.tour.name] = {});
							creqit.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								creqit.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							creqit.call({
								method: "creqit.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(creqit.boot.user.onboarding_status),
				...
				if ($input) creqit.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await creqit.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: creqit.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
creqit.ui.init_onboarding_tour = () => {
	...
	typeof creqit.boot.onboarding_tours == "undefined" && creqit.boot.onboarding_tours == [];
	typeof creqit.boot.user.onboarding_status == "undefined" &&
		creqit.boot.user.onboarding_status == {};
	let route = creqit.router.current_route;
	if (route?.[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		creqit.boot.onboarding_tours &&
			creqit.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (creqit.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (creqit.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(creqit.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == creqit.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == creqit.ui.next_form_tour);
	if (current_tour) {
		...
		creqit.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (creqit.ui.currentTourInstance?.driver) {
		creqit.ui.currentTourInstance.driver_steps = [];
		creqit.ui.currentTourInstance.driver.reset(true);
		creqit.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (creqit.ui.currentTourInstance = new creqit.ui.OnboardingTour());
