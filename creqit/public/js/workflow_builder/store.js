...
export const useStore = defineStore("workflow-builder-store", () => {
	...
	async function fetch() {
		await creqit.model.clear_doc("Workflow", workflow_name.value);
		await creqit.model.with_doc("Workflow", workflow_name.value);

		workflow_doc.value = creqit.get_doc("Workflow", workflow_name.value);
		await creqit.model.with_doctype(workflow_doc.value.document_type);

		if (!workflowfields.value.length) {
			await creqit.model.with_doctype("Workflow");
			workflowfields.value = creqit.get_meta("Workflow").fields;
		}

		if (!statefields.value.length) {
			await creqit.model.with_doctype("Workflow Document State");
			statefields.value = creqit.get_meta("Workflow Document State").fields;
		}

		if (!transitionfields.value.length) {
			await creqit.model.with_doctype("Workflow Transition");
			transitionfields.value = creqit.get_meta("Workflow Transition").fields;
		}

		if (!workflow_doc_fields.value.length) {
			let doc_type = workflow_doc.value.document_type;
			await creqit.model.with_doctype(doc_type);
			workflow_doc_fields.value = creqit.meta
				.get_docfields(doc_type, null, {
					fieldtype: ["not in", creqit.model.no_value_type],
				})
				.sort((a, b) => {
					if (a.label && b.label) {
	...
	async function save_changes() {
		...
		try {
			...
			await creqit.call("creqit.client.save", { doc });
			creqit.toast(__("Workflow updated successfully"));
			fetch();
		} catch (e) {
			console.error(e);
		} finally {
	...
	function setup_breadcrumbs() {
		...
		`;
		creqit.breadcrumbs.clear();
		creqit.breadcrumbs.$breadcrumbs.append(breadcrumbs);
	}
