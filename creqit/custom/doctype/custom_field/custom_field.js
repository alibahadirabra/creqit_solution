...
creqit.ui.form.on("Custom Field", {
	setup: function (frm) {
	...
	dt: function (frm) {
		...
		return creqit.call({
			method: "creqit.custom.doctype.custom_field.custom_field.get_fields_label",
	...
	label: function (frm) {
		if (frm.doc.label && creqit.utils.has_special_chars(frm.doc.label)) {
			...
			frm.add_custom_button(__("Rename Fieldname"), () => {
				creqit.prompt(
					...
					function (data) {
						creqit
							.xcall(
								"creqit.custom.doctype.custom_field.custom_field.rename_fieldname",
								{
...
creqit.utils.has_special_chars = function (t) {
	var iChars = "!@#$%^&*()+=-[]\\';,./{}|\":<>?";
