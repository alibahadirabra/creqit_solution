...
import creqit
from creqit.custom.doctype.custom_field.custom_field import (
	create_custom_field,
...
class TestCustomField(IntegrationTestCase):
	def test_create_custom_fields(self):
		...
		self.assertTrue(creqit.db.exists("Custom Field", "Address-_test_custom_field_1"))
		self.assertTrue(creqit.db.exists("Custom Field", "Address-_test_custom_field_2"))
		self.assertTrue(creqit.db.exists("Custom Field", "Contact-_test_custom_field_2"))
	...
	def test_custom_field_sorting(self):
		try:
			...
			fields = creqit.get_meta("ToDo", cached=False).fields
	...
	def test_custom_section_and_column_breaks_ordering(self):
		...
		updated_meta = creqit.get_meta("Test Custom Breaks Ordering", cached=False)
	...
	def test_custom_field_renaming(self):
		def gen_fieldname():
			return "test_" + creqit.generate_hash()

		field = create_custom_field("ToDo", {"label": gen_fieldname()}, is_system_generated=False)
		old = field.fieldname
		new = gen_fieldname()
		data = creqit.generate_hash()
		doc = creqit.get_doc({"doctype": "ToDo", old: data, "description": "Something"}).insert()

		rename_fieldname(field.name, new)
		field.reload()
		self.assertEqual(field.fieldname, new)

		doc = creqit.get_doc("ToDo", doc.name)  # doc.reload doesn't clear old fields.
