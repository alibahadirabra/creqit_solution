...
from creqit.model import core_doctypes_list
from creqit.model.docfield import supports_translation
from creqit.model.document import Document
from creqit.query_builder.functions import IfNull
from creqit.utils import cstr, random_string
...
class CustomField(Document):
		...
		if not self.fieldname:
			...
			if not label:
				if self.fieldtype in ["Section Break", "Column Break", "Tab Break"]:
					...
					creqit.throw(_("Label is mandatory"))
	...
	def validate(self):
		...
		from creqit.core.doctype.doctype.doctype import check_fieldname_conflicts
		...
		if not self.fieldname:
			creqit.throw(_("Fieldname not set for Custom Field"))
	...
	def on_update(self):
		...
		if not creqit.flags.in_create_custom_fields:
			creqit.clear_cache(doctype=self.dt)
			creqit.db.updatedb(self.dt)

	def on_trash(self):
		# check if Admin owned field
		if self.owner == "Administrator" and creqit.session.user != "Administrator":
			creqit.throw(
				_(
					"Custom Field {0} is created by the Administrator and can only be deleted through the Administrator account."
				).format(creqit.bold(self.label))
			)
		...
		doctype_layouts = creqit.get_all("DocType Layout", filters={"document_type": self.dt}, pluck="name")
	...
	def validate_insert_after(self, meta):
		if not meta.get_field(self.insert_after):
			creqit.throw(
				...
				creqit.DoesNotExistError,
			)

		if self.fieldname == self.insert_after:
			creqit.throw(_("Insert After cannot be set as {0}").format(meta.get_label(self.insert_after)))
...
def get_fields_label(doctype=None):
	...
	if doctype in core_doctypes_list:
		return creqit.msgprint(_("Custom Fields cannot be added to core DocTypes."))

	if meta.custom:
		return creqit.msgprint(_("Custom Fields can only be added to a standard DocType."))

	return [
		{"value": df.fieldname or "", "label": _(df.label, context=df.parent) if df.label else ""}
		for df in creqit.get_meta(doctype).get("fields")
	]


def create_custom_field_if_values_exist(doctype, df):
	df = creqit._dict(df)
	if df.fieldname in creqit.db.get_table_columns(doctype) and creqit.db.count(
...
def create_custom_field(doctype, df, ignore_validate=False, is_system_generated=True):
	...
	if not df.fieldname and df.label:
		df.fieldname = creqit.scrub(df.label)
	if not creqit.db.get_value("Custom Field", {"dt": doctype, "fieldname": df.fieldname}):
...
def create_custom_fields(custom_fields: dict, ignore_validate=False, update=True):
	...
	try:
		creqit.flags.in_create_custom_fields = True
		doctypes_to_update = set()

		if creqit.flags.in_setup_wizard:
		...
		for doctypes, fields in custom_fields.items():
			...
			for doctype in doctypes:
				...
				for df in fields:
					field = creqit.db.get_value("Custom Field", {"dt": doctype, "fieldname": df["fieldname"]})
					if not field:
						try:
							df = df.copy()
						...
						except creqit.exceptions.DuplicateEntryError:
							pass
		...
		creqit.flags.in_create_custom_fields = False
...
def rename_fieldname(custom_field: str, fieldname: str):
	...
	field: CustomField = creqit.get_doc("Custom Field", custom_field)
	...
	if field.is_system_generated:
		creqit.throw(_("System Generated Fields can not be renamed"))
	if creqit.db.has_column(parent_doctype, fieldname):
		creqit.throw(_("Can not rename as column {0} is already present on DocType.").format(fieldname))
	if old_fieldname == new_fieldname:
		creqit.msgprint(_("Old and new fieldnames are same."), alert=True)
		return

	if creqit.db.has_column(field.dt, old_fieldname):
		creqit.db.rename_column(parent_doctype, old_fieldname, new_fieldname)
	...
	creqit.msgprint(_("Custom field renamed to {0} successfully.").format(fieldname), alert=True)
	creqit.db.commit()
	creqit.clear_cache()
