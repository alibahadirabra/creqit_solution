...
creqit.provide("creqit.customize_form");

creqit.ui.form.on("Customize Form", {
	setup: function (frm) {
		...
		frm.set_query("doc_type", function () {
			return {
				filters: [
					["DocType", "issingle", "=", 0],
					["DocType", "custom", "=", 0],
					["DocType", "name", "not in", creqit.model.core_doctypes_list],
					["DocType", "restrict_to_domain", "in", creqit.boot.active_domains],
				],
			};
		});
	...
	refresh: function (frm) {
		...
		if (frm.doc.doc_type) {
			creqit.model.with_doctype(frm.doc.doc_type).then(() => {
				frm.page.set_title(__("Customize Form - {0}", [__(frm.doc.doc_type)]));
				creqit.customize_form.set_primary_action(frm);

				frm.add_custom_button(
					__("Go to {0} List", [__(frm.doc.doc_type)]),
					function () {
						creqit.set_route("List", frm.doc.doc_type);
					},
					__("Actions")
				);
				...
				frm.add_custom_button(
					__("Set Permissions"),
					function () {
						creqit.set_route("permission-manager", frm.doc.doc_type);
					},
					__("Actions")
				);
				...
				frm.add_custom_button(
					...
					function () {
						creqit.customize_form.confirm(__("Remove all customizations?"), frm);
					},
					__("Actions")
				);
				...
				frm.toggle_display(
					["queue_in_background"],
					creqit.get_meta(frm.doc.doc_type).is_submittable || 0
				);
	...
	set_default_doc_type(frm) {
		let doc_type;
		if (creqit.route_options && creqit.route_options.doc_type) {
			doc_type = creqit.route_options.doc_type;
				...
				return frm.call({
					...
					callback: function (r) {
						if (!r.exc) {
							...
							creqit.customize_form.clear_locals_and_refresh(frm);
						}
					},
				});
			}
	...
	async trim_table(frm) {
		let dropped_columns = await creqit.xcall(
			"creqit.custom.doctype.customize_form.customize_form.get_orphaned_columns",
			{ doctype: frm.doc.doc_type }
		);

		if (!dropped_columns?.length) {
			creqit.toast(__("This doctype has no orphan fields to trim"));
			return;
		}
		let msg = __(
			...
			frm.add_custom_button(
				...
				function () {
					creqit.prompt(
						...
						function (data) {
							creqit.call({
								method: "creqit.modules.utils.export_customizations",
								args: {
									doctype: frm.doc.doc_type,
									module: data.module,
	...
	setup_sort_order(frm) {
		...
		if (frm.doc.doc_type) {
			var fields = $.map(frm.doc.fields, function (df) {
				return creqit.model.is_value_type(df.fieldtype) ? df.fieldname : null;
			});
	...
	setup_default_views(frm) {
		creqit.model.set_default_views_for_doctype(frm.doc.doc_type, frm);
	},
});

// can't delete standard fields
creqit.ui.form.on("Customize Form Field", {
	before_fields_remove: function (frm, doctype, name) {
		const row = creqit.get_doc(doctype, name);
	...
	fields_add: function (frm, cdt, cdn) {
		var f = creqit.model.get_doc(cdt, cdn);
...
creqit.ui.form.on("DocType Link", {
	before_links_remove: function (frm, doctype, name) {
		let row = creqit.get_doc(doctype, name);
		...
		if (!(row.custom || row.__islocal)) {
			creqit.msgprint(__("Cannot delete standard link. You can hide it if you want"));
			throw "cannot delete standard link";
		}
	},
	links_add: function (frm, cdt, cdn) {
		let f = creqit.model.get_doc(cdt, cdn);
		f.custom = 1;
	},
...
creqit.ui.form.on("DocType Action", {
	before_actions_remove: function (frm, doctype, name) {
		...
		if (!(row.custom || row.__islocal)) {
			creqit.msgprint(__("Cannot delete standard action. You can hide it if you want"));
...
creqit.ui.form.on("DocType State", {
	before_states_remove: function (frm, doctype, name) {
		...
		if (!(row.custom || row.__islocal)) {
			creqit.msgprint(__("Cannot delete standard document state."));
...
creqit.customize_form.save_customization = function (frm) {
	if (frm.doc.doc_type) {
		return frm.call({
			doc: frm.doc,
			freeze: true,
...
creqit.customize_form.update_fields_from_form_builder = function (frm) {
...
creqit.customize_form.set_primary_action = function (frm) {
...
creqit.customize_form.confirm = function (msg, frm) {
	...
	creqit.customize_form.confirm.dialog = d;
	d.show();
};

creqit.customize_form.clear_locals_and_refresh = function (frm) {
	delete frm.doc.__unsaved;
	// clear doctype from locals
	creqit.model.clear_doc("DocType", frm.doc.doc_type);
	delete creqit.meta.docfield_copy[frm.doc.doc_type];
	frm.refresh();
};

function render_form_builder(frm) {
	if (creqit.form_builder && creqit.form_builder.doctype === frm.doc.doc_type) {
	...
	if (creqit.form_builder) {
		...
		creqit.form_builder.doctype = frm.doc.doc_type;
		creqit.form_builder.customize = true;
