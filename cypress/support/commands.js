			...
			return cy
				.request({
					...
					headers: {
						...
						"X-creqit-CSRF-Token": csrf_token,
					},
				})
				.then((res) => {
...
Cypress.Commands.add("create_records", (doc) => {
	return cy
		.call("creqit.tests.ui_test_helpers.create_if_not_exists", { doc: JSON.stringify(doc) })
		.then((r) => r.message);
});

Cypress.Commands.add("set_value", (doctype, name, obj) => {
	return cy.call("creqit.client.set_value", {
		doctype,
		name,
		fieldname: obj,
	});
...
Cypress.Commands.add(
	...
	(tablefieldname, row_idx, fieldname, fieldtype = "Data") => {
		let selector = `.creqit-control[data-fieldname="${tablefieldname}"]`;
...
Cypress.Commands.add("dialog", (opts) => {
	return cy
		.window({ log: false })
		.its("creqit", { log: false })
		.then((creqit) => {
			Cypress.log({
				name: "dialog",
				displayName: "dialog",
				message: "creqit.ui.Dialog",
				consoleProps: () => {
					return {
						options: opts,
						dialog: d,
			...
			var d = new creqit.ui.Dialog(opts);
			d.show();
			return d;
		});
});
...
Cypress.Commands.add("save", () => {
	cy.intercept("/api/method/creqit.desk.form.save.savedocs").as("save_call");
...
Cypress.Commands.add("add_role", (user, role) => {
	cy.window()
		.its("creqit")
		.then((creqit) => {
			const session_user = creqit.session.user;
...
const add_remove_role = (action, user, role, session_user) => {
	...
	cy.call("creqit.tests.ui_test_helpers.add_remove_role", {
		action: action,
		user: user,
		role: role,
	});
...
Cypress.Commands.add("select_listview_row_checkbox", (row_no) => {
	cy.get(".creqit-list .select-like > .list-row-checkbox").eq(row_no).click();
});
