context("Workspace Blocks", () => {
	before(() => {
		...
		return cy
			...
			.its("creqit")
			.then((creqit) => {
				return creqit.xcall("creqit.tests.ui_test_helpers.setup_workflow");
			});
	});
	...
	it("Create Test Page", () => {
		cy.intercept({
			method: "POST",
			url: "api/method/creqit.desk.doctype.workspace.workspace.new_page",
		}).as("new_page");
	...
	it.skip("Quick List Block", () => {
		...
		cy.intercept({
			method: "GET",
			url: "api/method/creqit.desk.form.load.getdoctype?**",
		}).as("get_doctype");
		...
		cy.intercept({
			method: "POST",
			url: "api/method/creqit.desk.reportview.get",
		}).as("refresh-list");
