	...
	it("check value changes", () => {
		...
		TEST_CASES.forEach((test_case) => {
			cy.window()
				.its("creqit")
				.then((creqit) => {
					creqit.boot.sysdefaults.currency = test_case.currency;
					creqit.boot.sysdefaults.currency_precision = test_case.default_precision ?? 2;
					creqit.boot.sysdefaults.number_format = test_case.number_format ?? "#,###.##";
				});
