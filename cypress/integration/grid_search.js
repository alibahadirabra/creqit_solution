...
context("Grid Search", () => {
	before(() => {
		...
		return cy
			...
			.then((creqit) => {
				return creqit.xcall(
					"creqit.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("creqit")
			.then((creqit) => {
				creqit.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.creqit-control[data-fieldname="child_table_1"]').as("table");
