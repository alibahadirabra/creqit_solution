...
let bench_path;
if (process.env.creqit_BENCH_ROOT) {
	bench_path = process.env.creqit_BENCH_ROOT;
} else {
...
// In creqit context using ipv4 - 127.0.0.1 is fine.
...
function get_conf() {
	...
	if (process.env.creqit_SITE) {
		conf.default_site = process.env.creqit_SITE;
	}
	if (process.env.creqit_REDIS_CACHE) {
		conf.redis_cache = process.env.creqit_REDIS_CACHE;
	}
	if (process.env.creqit_REDIS_QUEUE) {
		conf.redis_queue = process.env.creqit_REDIS_QUEUE;
	}
	if (process.env.creqit_SOCKETIO_PORT) {
		conf.socketio_port = process.env.creqit_SOCKETIO_PORT;
	}
	if (process.env.creqit_SOCKETIO_UDS) {
		conf.socketio_uds = process.env.creqit_SOCKETIO_UDS;
	}
	return conf;
}
